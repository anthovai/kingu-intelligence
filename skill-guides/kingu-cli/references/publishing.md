...
```text
KINGU artifacts share <file> --json
KINGU artifacts update <file> --json
KINGU artifacts unshare <file> --json
...
- `share` saves the returned edit token in the active Kingu profile and never includes it
  in CLI output. `update` and `unshare` look up that record by the resolved local file
  path, so use the same path and Kingu profile that originally shared the file.
...
- For local or staging development, `--api-url <url>` overrides the artifact service;
  `KINGU_ARTIFACTS_API_URL` provides the same override for the session.
- `KINGU_CLOUD_AUTH_TOKEN` is a development-only authentication override. Prefer the active
  Kingu profile's normal PropelAuth session and never expose the token in logs or agent output.

## Skill sharing

Agents can publish one or more installed skills behind one unlisted link through the
signed-in Kingu account. The user must first grant the separate, default-off permission in
Settings → Share Skills ("Allow agents and the Kingu CLI to publish skill links"). There is
...
KINGU skills share --skill <selector> [--skill <selector> ...] --bundle-name <name> --json
```
...
- Kingu stages one agent-published bundle at a time per host. If another publish is active,
  wait for it to finish before retrying `agent_skill_sharing_busy`.
- Run the command in an Kingu terminal on the machine that stores the skills. Forwarded WSL,
  SSH, and paired-runtime invocations fail before discovery so Kingu cannot read from the
  wrong filesystem.
