...
Orchestration is Kingu's structured coordination layer. It records who owns work,
...
| The user asks to hand off ownership or start another agent/worktree without supervision                                                        | Handoff owner           | Use `kingu-cli`; create no Run, Task, or Dispatch and do not monitor completion |
...
| No live preamble and no explicit supervision                                                                                                   | Ordinary terminal agent | Do not emit lifecycle messages; use `kingu-cli` for terminal/worktree work      |

Model or effort selection does not make a handoff supervised. Never substitute a
non-Kingu subagent tool when Kingu orchestration provenance was requested.
...
- Use the executable you used to run `skills get` for the entire run. In the
  examples below, replace `KINGU` with it; do not create a shell variable or run
  `KINGU` literally. If it fails, report that exact error instead of switching.
...
3. Read coordinator follow-ups at each natural checkpoint — before starting a
   ...
   `KINGU orchestration check --terminal <your_handle> --json`.
...
```text
KINGU status --json
KINGU orchestration run-create --objective "<objective>" --json
KINGU orchestration worker-start --spec "<worker A task>" --worktree current --agent codex --json
KINGU orchestration worker-start --spec "<worker B task>" --worktree current --agent claude --json
KINGU orchestration check --wait --types "worker_done,escalation,question" --timeout-ms 900000 --json
```
...
omit it inside the coordinator's own Kingu terminal. It returns the bound Run's
...
```text
KINGU orchestration reply --id <message_id> --body "<answer>" --json
KINGU orchestration worker-release --dispatch <dispatch_id> --json
KINGU orchestration check --ack <delivery_id> --wait --types "worker_done,escalation,question" --timeout-ms 900000 --json
```
...
`KINGU orchestration worker-list --include-remote --json` (defaults to the bound
...
below, run `KINGU skills get orchestration --reference references/<file>.md` and
read only that document; `--references` lists the names. If the CLI rejects
`--reference`, run `KINGU skills get orchestration --full` once instead: it
