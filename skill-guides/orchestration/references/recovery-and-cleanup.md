...
```text
KINGU orchestration worker-list --run <run_id> --json
...
Continue with its top-level cursor, which is pinned to that source. If Kingu
...
operation identity so Kingu can replay, join, or recover it instead of starting a
duplicate. Ask what happened first:

```text
KINGU orchestration request-show --request <request_id> --json
```
...
Kingu restarted before recording its outcome; replay the original command with
...
```text
KINGU orchestration worker-start --task <task_id> --retry-of <dispatch_id> --worktree <explicit_placement> --agent <agent> --json
```
...
```text
KINGU orchestration worker-stop --dispatch <dispatch_id> --json
KINGU orchestration worker-abandon --dispatch <dispatch_id> --json
```
...
```text
KINGU orchestration worker-retain --dispatch <dispatch_id> --json
