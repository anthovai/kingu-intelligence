...
Inside the lifecycle scripts the placeholder does not apply: `kingu serve` written there runs on
the remote machine's own binary.

## Autonomy envelope

Without asking again you may read the repo and its `kingu.yaml`, detect provider CLIs and their
login state, scaffold and edit files under `scripts/kingu-vm/`, and run `KINGU vm recipe doctor`
...
done. Never create an Kingu workspace except for the step-10 test the user asked for. Do not create
...
In **Kingu-server** mode `create` runs `kingu serve` in the environment and emits a `pairingCode`. In
**SSH** mode `create` runs no server and emits a `connection.type:"ssh"` block Kingu dials into.
Settle this first; it changes the `create` output and half the templates.

Keep Kingu's checkout behavior unchanged by default: omit `checkoutMode`, emit schema version 1, and
let Kingu create a linked worktree. Use `checkoutMode: provisioned-root` only when the user
...
1. **Inspect the repo** for an existing `environmentRecipes` entry, `scripts/kingu-vm/`, a state
...
2. **Interview the user up front.** Gather these choices and confirm them back before scaffolding
   ...
   - **Connection mode:** an Kingu server or SSH, as above. Settle it first.
   ...
   - **Provider:** Vercel Sandbox, Fly, Modal, an existing SSH host, and so on. For a non-obvious
     ...
     Kingu's SSH mode needs the former.
...
7. **Wire the recipe** so `kingu.yaml` points create, suspend, resume, and destroy at the scripts.
   ...
   any branch; the picker needs `kingu.yaml` on the primary branch.
...
- Trap errors and remove the half-built environment, so a crash does not leave a paid resource
  running.
- **Never snapshot a machine on which the Kingu runtime has already run.** The first `kingu serve`
  ...
  (`kingu-devices.json`, `kingu-e2ee-keypair.json`), `agent-session-authority.key`, and the build
  ...
  dedicated to Kingu runtime data, never `/`, the home directory, or an ancestor of home. Refuse
  empty or relative paths. Remove only that verified directory, not an unchecked environment value.
  That matches Kingu's Linux precedence for custom and default paths; deleting a named file list
  drifts as Kingu adds state.
...
Section 3's rule still applies: if you ran `kingu serve` on this machine to smoke-test it, delete
...
A repo-local JSON file such as `scripts/kingu-vm/<provider>-state.json` threads non-secret values
...
Scaffold under `scripts/kingu-vm/`. These are shapes; fill in the provider's real commands. **Every
...
You run this by hand, not via `kingu.yaml`, after exporting the first-run inputs state does not have
...
# name = kingu-${KINGU_RECIPE_ID}-${KINGU_VM_INSTANCE_ID} (sanitized, length-capped)
...
# 3. Kingu-server mode only: remote exec starting kingu serve and reading the recipe JSON it writes
...
#!/usr/bin/env bash
set -euo pipefail
payload="$(cat)"                       # Kingu passes lifecycle JSON on stdin
...
# destroy: provider remove "$resource_id"   (or set destroy: none in kingu.yaml)
```
...
Define recipes in `kingu.yaml`:

```yaml
environmentRecipes:
  - id: cloud-sandbox
    name: Cloud Sandbox
    create: ./scripts/kingu-vm/cloud-sandbox-create.sh
    suspend: ./scripts/kingu-vm/cloud-sandbox-suspend.sh
    resume: ./scripts/kingu-vm/cloud-sandbox-resume.sh
    destroy: ./scripts/kingu-vm/cloud-sandbox-destroy.sh
```
...
The base result, which is what Kingu-server mode prints:

```json
{
  "schemaVersion": 1,
  "pairingCode": "kingu-pairing-code-or-url",
...
- **`kingu serve --recipe-json` output** is this same object without `userData`. Merge your own
...
- **SSH mode** replaces `pairingCode` and `projectRoot` with a `connection` block whose `type` is
  `"ssh"`, and does not run `kingu serve`. The exact target shape is in `references/ssh-host.md`.
- **Provisioned root** applies only to direct SSH and only when the user explicitly asked for it. Add
  `checkoutMode: provisioned-root` to the recipe, require `KINGU_RECIPE_RESULT_SCHEMA_VERSION=2`, and
...
### The `kingu serve` invocation

Inside the environment, in Kingu-server mode, run exactly this. These flags are verified; do not
improvise them.
...
In an environment built from source, run it as `pnpm exec kingu-dev serve …` from the repo root;
`kingu-dev` is the in-repo entrypoint. Plain `kingu serve …` is the same command when the built CLI is
...
`KINGU vm recipe doctor <recipe-id> --repo-path <repo> --json` validates static wiring only; it boots
...
run `KINGU skills get kingu-per-workspace-env --reference references/<file>.md` and read only that
document; `--references` lists the names. Read the reference at the gate, not before. If the CLI
rejects `--reference`, run `KINGU skills get kingu-per-workspace-env --full` once instead: it returns
...
| The recipe connects over SSH instead of starting `kingu serve`, including provisioned root | `references/ssh-host.md`        |
