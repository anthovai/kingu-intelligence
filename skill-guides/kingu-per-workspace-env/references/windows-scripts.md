...
`.sh` will not execute there. Either require WSL or Git Bash and point `kingu.yaml` at a launcher such
as `bash ./scripts/kingu-vm/<name>.sh` through a `.cmd` file, or scaffold PowerShell equivalents.
...
# Kingu-server mode: @{ schemaVersion=1; pairingCode=$pairingCode; projectRoot=$projectRoot; userData=@{...} }
