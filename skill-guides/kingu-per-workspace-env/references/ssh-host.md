...
Load this when the recipe connects over SSH instead of starting `kingu serve`, and when the user has
explicitly asked for `checkoutMode: provisioned-root`.

SSH mode is a different shape, not the Kingu-server templates relabeled. `create` runs no
`kingu serve` and emits no `pairingCode`. Kingu connects over its SSH relay, brings up the git and
filesystem providers, and imports the repo. The script only readies the host and prints the SSH
details Kingu dials.

## The result shape

Kingu rejects anything else. Required fields only; add optionals from the next section as the
network needs them.
...
These describe how the user's desktop reaches the box; there is no `kingu serve` URL in SSH mode.
...
- `relayGracePeriodSeconds` bounds how long Kingu keeps the SSH relay alive after the workspace
...
# 1. ensure the repo is present and at the right commit on the host (NO kingu serve here).
...
# 2. print the SSH connection block (NO pairingCode, NO kingu serve). host/port/username tell Kingu's
...
and resume. Kingu still disconnects and reconnects its own SSH relay on sleep, wake, and delete, which
...
`KINGU_RECIPE_RESULT_SCHEMA_VERSION`, `KINGU_REPO_URL`, `KINGU_REPO_REF`, `KINGU_REPO_REF_HEAD`, and
`KINGU_REPO_BRANCH`. Use `KINGU_REPO_REF` to fetch the selected source, but create `KINGU_REPO_BRANCH`
at the exact `KINGU_REPO_REF_HEAD` commit, because resolving the symbolic ref again can race with an
upstream update. `KINGU_REPO_URL` and `KINGU_REPO_REF` are a matched fetch pair, and the URL is the
remote Kingu resolved the base ref against, which is not necessarily named `origin` on the desktop.
Fetch from the URL the pair supplies:

```bash
[ -n "${KINGU_REPO_REF_HEAD:-}" ] || { echo "missing pinned source commit" >&2; exit 1; }
git fetch "$KINGU_REPO_URL" "$KINGU_REPO_REF"
git cat-file -e "${KINGU_REPO_REF_HEAD}^{commit}"
git checkout -B "$KINGU_REPO_BRANCH" "$KINGU_REPO_REF_HEAD"
```
