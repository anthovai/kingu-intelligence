...
This is the Kingu-server connection mode: the recipe emits a pairing URL. If the user chose SSH in
...
verdict="$(vercel sandbox exec "$auth" "${vercel_args[@]}" --timeout 30s \
  -- bash -lc 'if codex login status >/dev/null 2>&1; then echo KINGU_AGENT_LOGGED_IN; else echo KINGU_AGENT_LOGGED_OUT; fi')"
case "$verdict" in
  *KINGU_AGENT_LOGGED_IN*) ;;
...
recipe_id="${KINGU_RECIPE_ID:-vercel-sandbox}"
recipe_id="${recipe_id//./-}"  # Vercel names forbid dots.
instance_id="${KINGU_VM_INSTANCE_ID:-$(date +%s)}"
max_recipe_id_length=$((128 - ${#instance_id} - 6))  # Preserve the unique instance suffix.
[ "$max_recipe_id_length" -gt 0 ] || { echo "KINGU_VM_INSTANCE_ID is too long for a Vercel sandbox name" >&2; exit 1; }
...
vercel sandbox exec "$name" "${vercel_args[@]}" --timeout 20m \
  --env "GH_TOKEN=$gh_token" --env "KINGU_PROJECT_ROOT=$project_root" \
  --env "KINGU_REPO_URL=$repo_url" --env "KINGU_REPO_REF=$repo_ref" \
  -- bash -lc 'set -euo pipefail; cd "$KINGU_PROJECT_ROOT"; \
    ...
    git fetch origin "$KINGU_REPO_REF"; \
    git checkout -B "$KINGU_REPO_REF" FETCH_HEAD; \
    rm -f /tmp/askpass.sh; \
    c="$(git rev-parse HEAD)"; [ -f .kingu-built ] && [ "$(cat .kingu-built)" = "$c" ] || { \
      ...
      printf "%s" "$c" > .kingu-built; }' >&2

# 3. (remote) start kingu serve in the background, writing recipe JSON to a file; poll until it parses
recipe_json="$(vercel sandbox exec "$name" "${vercel_args[@]}" --timeout 60s \
  --env "KINGU_PORT=$port" --env "KINGU_PROJECT_ROOT=$project_root" --env "KINGU_PAIRING_ADDRESS=$pairing_ws" \
  -- bash -lc 'set -euo pipefail; cd "$KINGU_PROJECT_ROOT"; rm -f /tmp/kingu-recipe.json /tmp/kingu-serve.log; \
    nohup pnpm exec kingu-dev serve --port "$KINGU_PORT" --project-root "$KINGU_PROJECT_ROOT" \
      --pairing-address "$KINGU_PAIRING_ADDRESS" --recipe-json >/tmp/kingu-recipe.json 2>/tmp/kingu-serve.log </dev/null & \
    pid=$!; for _ in $(seq 1 80); do \
      node -e "JSON.parse(require(\"node:fs\").readFileSync(\"/tmp/kingu-recipe.json\",\"utf8\"))" >/dev/null 2>&1 && { cat /tmp/kingu-recipe.json; exit 0; }; \
      kill -0 "$pid" 2>/dev/null || { cat /tmp/kingu-serve.log >&2; exit 1; }; sleep 0.25; \
    done; cat /tmp/kingu-serve.log >&2; echo "serve recipe JSON timed out" >&2; exit 1')"
