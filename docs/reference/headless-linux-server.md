...
Use this guide when you want to run `kingu serve` on a Linux machine without a
desktop session, such as an Ubuntu VPS or a remote build box.

`kingu serve` starts the Kingu runtime without opening the desktop window. On
Linux, the packaged AppImage still needs the libraries that Electron expects at
startup. Current Kingu builds start Xvfb automatically for `kingu serve` when no
`DISPLAY` is set, but Xvfb must be installed first. A separate D-Bus session is
not required. When `DISPLAY` is set, Kingu uses that display instead of starting
...
whose lock names a dead process is refused rather than replaced, and `kingu serve`
exits — unset `DISPLAY` to let Kingu start its own Xvfb. A socket published with
...
`kingu serve` then fails before Electron starts:
...
```bash
sudo mkdir -p /opt/kingu
sudo curl -L https://github.com/anthovai/kingu-intelligence/releases/latest/download/kingu-linux.AppImage \
  -o /opt/kingu/kingu-linux.AppImage
sudo chmod +x /opt/kingu/kingu-linux.AppImage
```
...
```bash
cd /opt/kingu
sudo ./kingu-linux.AppImage --appimage-extract
sudo chmod -R a+rX /opt/kingu/squashfs-root
/opt/kingu/squashfs-root/AppRun serve --port 6768
```
...
extract-and-run wrapper can print extracted paths before Kingu starts, so
...
```bash
LIBGL_ALWAYS_SOFTWARE=1 /opt/kingu/kingu-linux.AppImage serve --port 6768
```
...
```bash
LIBGL_ALWAYS_SOFTWARE=1 /opt/kingu/kingu-linux.AppImage serve \
  --port 6768 \
  --pairing-address 100.64.1.20
```

`--pairing-address` is only the address advertised to clients. It does not
change the listener bind address. Kingu binds its WebSocket listener, then
...
proxy URL such as `https://kingu.example.com/runtime` (`http(s)` is normalized
...
Pairing URL: kingu://pair?code=...
```

For supervisors, request the versioned single-line JSON contract:

```bash
/opt/kingu/kingu-linux.AppImage serve --port 6768 \
...
```bash
sudo useradd --system --create-home --shell /usr/sbin/nologin kingu
sudo chown root:root /opt/kingu /opt/kingu/kingu-linux.AppImage
sudo chmod 755 /opt/kingu /opt/kingu/kingu-linux.AppImage
...
`kingu` service user cannot read or traverse the extracted tree and the unit fails
at startup. `chmod 755 /opt/kingu` alone does not reach into it.

For most hosts, one `kingu serve` service is enough because Kingu starts Xvfb on
display `:99` when no display exists:

```ini
# /etc/systemd/system/kingu-serve.service
[Unit]
Description=Kingu runtime server
...
Type=simple
User=kingu
WorkingDirectory=/home/kingu
Environment=LIBGL_ALWAYS_SOFTWARE=1
ExecStart=/opt/kingu/kingu-linux.AppImage serve --port 6768 --pairing-address 100.64.1.20
...
`KillMode=mixed` sends the graceful stop signal only to Kingu's main process,
...
process itself, never a grace window for the cgroup's remains. This lets Kingu
...
own transient `kingu-daemon-<launch-nonce>.scope` unit rather than in
`kingu-serve.service`'s cgroup. A `systemctl stop` or `restart` of this unit
...
With `User=kingu` and no interactive login there is none by default, so enable
lingering once:

```bash
sudo loginctl enable-linger kingu
```
...
`kingu-serve.service`'s cgroup, and is then killed when the stop completes:
...
payload: a `kingu-daemon-*.scope` value means isolated, `null` means the
unscoped fallback.
...
subprocesses; without it, Kingu is PID 1 and those children accumulate as
...
`sudo systemctl reset-failed kingu-serve.service` first to clear it — the
...
20.04, Kingu's oldest supported base, ships systemd 245.

Enable the service:

```bash
sudo systemctl daemon-reload
sudo systemctl enable --now kingu-serve.service
sudo journalctl -u kingu-serve.service -f
```
...
```bash
sudo journalctl -u kingu-serve.service -o cat \
  | jq -Rrc 'fromjson? | select(.type == "kingu_server_ready" and .schemaVersion == 1)'
```
...
separate service and set `DISPLAY=:99` for Kingu.

```ini
# /etc/systemd/system/kingu-xvfb.service
[Unit]
Description=Virtual X display for Kingu
After=network-online.target
Wants=network-online.target
...
Then add the display dependency to the Kingu service:

```ini
# /etc/systemd/system/kingu-serve.service
[Unit]
Description=Kingu runtime server
After=network-online.target kingu-xvfb.service
Wants=network-online.target kingu-xvfb.service
StartLimitIntervalSec=300
StartLimitBurst=5
...
```bash
sudo systemctl daemon-reload
sudo systemctl enable --now kingu-xvfb.service kingu-serve.service
```

## CLI Install Note

The registered Linux CLI command is `kingu-ide`, not `kingu`, to avoid shadowing
the GNOME Kingu screen reader. Desktop-managed terminals receive a
terminal-scoped bare-`kingu` shim. A packaged headless `kingu serve` also makes a
best-effort dispatcher at `$HOME/.local/bin/kingu` for the service user's own
shell, so the Claude Teams launcher can resolve its bare command; it does not
replace another user's `kingu`. From an ordinary shell outside that service
user's managed environment, substitute `kingu-ide` for `kingu` in commands below.

On a headless host, you do not need to open the desktop UI just to run the
server. Invoke the AppImage directly:

```bash
/opt/kingu/kingu-linux.AppImage serve --help
```
...
```bash
/opt/kingu/kingu-linux.AppImage --no-sandbox serve --port 6768
```
...
The Linux CLI is named `kingu-ide`, not `kingu`, so it never shadows the GNOME
Kingu screen reader at `/usr/bin/kingu`. The `.deb` and `.rpm` packages put
`kingu-ide` on `PATH` themselves at install time; with the AppImage it arrives
as `~/.local/bin/kingu-ide` when the CLI is registered.

A packaged `kingu serve` start also writes a bare `kingu` into `~/.local/bin`
that execs the same launcher, which is why the skills commands below can be
typed as `kingu`. It writes it while starting, so it is never the command that
starts the server — the first launch is `kingu-ide serve`, or the AppImage
...
a file Kingu does not own already holds that name (ownership is a marker on the
...
If you later install the desktop CLI from Kingu settings, use that CLI for normal
shell workflows. Keep the AppImage path in systemd so service restarts do not
depend on an interactive shell profile.

## Upgrade

`kingu serve` never updates itself. In headless mode Kingu wires up no auto-updater
...
- **State lives in the service user's home, not next to the binary.** Persisted
  data is under `/home/kingu/.config/` (Kingu uses both an `kingu` and an `Kingu`
  directory there), fully independent of `/opt/kingu/kingu-linux.AppImage`.
...
- **New builds migrate old state on load.** Kingu loads older `kingu-data.json`
...
`kingu-daemon-*.scope`, as reported by `health.terminalDaemon.cgroupUnit`. The
...
`kingu-daemon-*.scope` on that host.
...
Replace both `kingu` and `/home/kingu` with the service account and home used by
your unit; for an extracted deployment, use its absolute `resources/bin/kingu-ide`
...
and the stop; Kingu does not yet provide an atomic census-and-stop fence.

Rolling back is the case that needs care — see [Roll back](#roll-back).

### Record the version you deploy

The bundled CLI launcher prints the Kingu build with `kingu-ide --version`. For an
extracted deployment, that launcher is
`squashfs-root/resources/bin/kingu-ide`; deb/rpm installs and CLI registration put
it on `PATH`. Do not use `kingu-linux.AppImage --version` for this audit because
...
next to the binary. The steps below keep that record in `/opt/kingu/VERSION`.

### Upgrade steps

Never download straight onto `/opt/kingu/kingu-linux.AppImage`. The AppImage is
...
```bash
sudo chown root:root /opt/kingu
sudo chmod 755 /opt/kingu
sudo test ! -L /opt/kingu/kingu-linux.AppImage
sudo chown root:root /opt/kingu/kingu-linux.AppImage
sudo chmod 755 /opt/kingu/kingu-linux.AppImage
# Clear predictable staging names left by an older attempt after locking the directory
sudo rm -f /opt/kingu/kingu-linux.AppImage.new /opt/kingu/VERSION.new \
  /opt/kingu/kingu-linux.AppImage.recovering /opt/kingu/VERSION.recovering
sudo du -sh /home/kingu/.config
df -h /opt/kingu /home/kingu
```

`/opt/kingu` needs room for the compressed Kingu profile archive, the staged
build, and the rollback binary. A rollback extracts the old profile and preserves
the post-upgrade Kingu profile directories, so `/home` needs room for both copies.
...
KINGU_VERSION=v1.4.147

# Select the release asset on the server where Kingu runs
case "$(uname -m)" in
  x86_64)
    KINGU_ASSET=kingu-linux.AppImage
    KINGU_FILE_MACHINE=x86-64
    ;;
  aarch64 | arm64)
    KINGU_ASSET=kingu-linux-arm64.AppImage
    KINGU_FILE_MACHINE='ARM aarch64'
    ;;
  *)
...
KINGU_ROLLBACK_NEW=
KINGU_ROLLBACK=
KINGU_SERVICE_STOPPED=0
KINGU_BINARY_PROMOTED=0
recover_failed_upgrade() {
  ...
  if ((exit_status != 0)) && [[ -n "$KINGU_ROLLBACK_NEW" ]] && \
    sudo test -d "$KINGU_ROLLBACK_NEW"; then
    sudo rm -rf -- "$KINGU_ROLLBACK_NEW"
  fi
  if ((exit_status != 0 && KINGU_SERVICE_STOPPED)); then
    recovery_ok=1
    if ((KINGU_BINARY_PROMOTED)); then
      if ! sudo cp -a "$KINGU_ROLLBACK/kingu-linux.AppImage" \
        /opt/kingu/kingu-linux.AppImage.recovering || \
        ! sudo mv -f /opt/kingu/kingu-linux.AppImage.recovering \
          /opt/kingu/kingu-linux.AppImage; then
        recovery_ok=0
      fi
      if sudo test -f "$KINGU_ROLLBACK/VERSION"; then
        if ! sudo cp -a "$KINGU_ROLLBACK/VERSION" /opt/kingu/VERSION.recovering || \
          ! sudo mv -f /opt/kingu/VERSION.recovering /opt/kingu/VERSION; then
          recovery_ok=0
        fi
      elif ! sudo rm -f /opt/kingu/VERSION; then
        recovery_ok=0
      fi
    fi
    sudo rm -f /opt/kingu/kingu-linux.AppImage.recovering \
      /opt/kingu/VERSION.recovering
    if ((recovery_ok)); then
      # A tripped StartLimitBurst refuses a plain start
      sudo systemctl reset-failed kingu-serve.service || true
      sudo systemctl start kingu-serve.service || true
    else
...
sudo curl -fL --retry 3 "https://github.com/anthovai/kingu-intelligence/releases/download/${KINGU_VERSION}/${KINGU_ASSET}" \
  -o /opt/kingu/kingu-linux.AppImage.new
sudo chown root:root /opt/kingu/kingu-linux.AppImage.new
sudo chmod 755 /opt/kingu/kingu-linux.AppImage.new

# Both checks must match; either grep stops this fail-fast block otherwise
KINGU_FILE_INFO=$(LC_ALL=C file /opt/kingu/kingu-linux.AppImage.new)
grep 'ELF .* executable' <<<"$KINGU_FILE_INFO"
grep -F "$KINGU_FILE_MACHINE" <<<"$KINGU_FILE_INFO"

# 2. Assemble the prior binary and version in a root-only rollback bundle
KINGU_ROLLBACK_BASE=/opt/kingu/kingu-rollback-$(date +%F-%H%M%S-%N)
KINGU_ROLLBACK_NEW=${KINGU_ROLLBACK_BASE}.new
KINGU_ROLLBACK=${KINGU_ROLLBACK_BASE}.ready
sudo install -d -m 700 "$KINGU_ROLLBACK_NEW"
sudo cp -a /opt/kingu/kingu-linux.AppImage "$KINGU_ROLLBACK_NEW/kingu-linux.AppImage"
if sudo test -f /opt/kingu/VERSION; then
  sudo cp -a /opt/kingu/VERSION "$KINGU_ROLLBACK_NEW/VERSION"
fi

# Stage the new version record before the stop window
printf '%s\n' "$KINGU_VERSION" | sudo tee /opt/kingu/VERSION.new >/dev/null
sudo chown root:root /opt/kingu/VERSION.new
sudo chmod 644 /opt/kingu/VERSION.new

# 3. Stop the server so the profile backup is consistent
KINGU_SERVICE_STOPPED=1
sudo systemctl stop kingu-serve.service

# Add only Kingu-owned profile directories, then publish the complete bundle
KINGU_PROFILE_DIRS=()
for profile_dir in kingu Kingu; do
  if sudo test -L "/home/kingu/.config/$profile_dir"; then
    echo "Refusing symlinked Kingu profile: /home/kingu/.config/$profile_dir" >&2
    exit 1
  fi
  if sudo test -d "/home/kingu/.config/$profile_dir"; then
    if [[ "$profile_dir" == Kingu ]] && \
      sudo test /home/kingu/.config/kingu -ef /home/kingu/.config/Kingu; then
      continue
    fi
    KINGU_PROFILE_DIRS+=("$profile_dir")
  fi
done
if ((${#KINGU_PROFILE_DIRS[@]} == 0)); then
  echo 'No Kingu profile directory found under /home/kingu/.config' >&2
  exit 1
fi
sudo tar czf "$KINGU_ROLLBACK_NEW/profile.tgz" \
  -C /home/kingu/.config "${KINGU_PROFILE_DIRS[@]}"
sudo chmod 600 "$KINGU_ROLLBACK_NEW/profile.tgz"
sudo mv "$KINGU_ROLLBACK_NEW" "$KINGU_ROLLBACK"

# 4. Atomically replace the binary and version record, then start
KINGU_BINARY_PROMOTED=1
sudo mv -f /opt/kingu/kingu-linux.AppImage.new /opt/kingu/kingu-linux.AppImage
sudo mv -f /opt/kingu/VERSION.new /opt/kingu/VERSION
# Clears a start-limit hit left by the version being replaced
sudo systemctl reset-failed kingu-serve.service
sudo systemctl start kingu-serve.service
KINGU_SERVICE_STOPPED=0
trap - EXIT
```

The profile archive created in step 3 captures both Kingu profile directory names
when present without rewinding unrelated tools under `/home/kingu/.config`. The
...
`kingu-serve.service` needs restarting — leave `kingu-xvfb.service` running.
...
A healthy start prints one `Kingu server ready` block with the actual bound and
...
```bash
shopt -s nullglob
KINGU_ROLLBACK_SETS=(/opt/kingu/kingu-rollback-*.ready)
((${#KINGU_ROLLBACK_SETS[@]} > 0))
KINGU_ROLLBACK=${KINGU_ROLLBACK_SETS[${#KINGU_ROLLBACK_SETS[@]} - 1]}
printf 'Removing rollback bundle: %s\n' "$KINGU_ROLLBACK"
sudo test -d "$KINGU_ROLLBACK"
sudo rm -rf -- "$KINGU_ROLLBACK"
```
...
rewrite `kingu-data.json` in the current schema. If an older build then writes
that file, it can discard fields it does not recognize. The rolling
`kingu-data.json.bak.*` files are corruption-recovery snapshots, not a dedicated
...
((${#KINGU_ROLLBACK_SETS[@]} > 0))
KINGU_ROLLBACK=${KINGU_ROLLBACK_SETS[${#KINGU_ROLLBACK_SETS[@]} - 1]}
sudo test -f "$KINGU_ROLLBACK/kingu-linux.AppImage"
sudo tar tzf "$KINGU_ROLLBACK/profile.tgz" >/dev/null

# Extract and validate the old profile while the current server stays online
sudo test ! -L /home
KINGU_HOME_OWNER=$(sudo stat -c %u /home)
KINGU_HOME_MODE=$(sudo stat -c %a /home)
if [[ "$KINGU_HOME_OWNER" != 0 ]] || ((8#$KINGU_HOME_MODE & 0022)) || \
  sudo -u kingu test -w /home; then
  echo 'Refusing rollback because /home is not root-controlled' >&2
  exit 1
fi
KINGU_RESTORE=$(sudo mktemp -d /home/.kingu-restore.XXXXXX)
KINGU_SERVICE_STOPPED=0
KINGU_MOVED_CURRENT_DIRS=()
KINGU_INSTALLED_RESTORE_DIRS=()
KINGU_CURRENT_BINARY_MOVED=0
KINGU_CURRENT_VERSION_MOVED=0
KINGU_VERSION_REPLACEMENT_STARTED=0
KINGU_POST_UPGRADE=
KINGU_ROLLBACK_BINARY_STAGED=
KINGU_ROLLBACK_VERSION_STAGED=
KINGU_ROLLBACK_HAS_VERSION=0
restart_after_rollback_error() {
  ...
  if ((exit_status != 0 && KINGU_SERVICE_STOPPED)); then
    recovery_ok=1
    if ((${#KINGU_INSTALLED_RESTORE_DIRS[@]})); then
      for profile_dir in "${KINGU_INSTALLED_RESTORE_DIRS[@]}"; do
        if sudo test -d "/home/kingu/.config/$profile_dir"; then
          if ! sudo mv "/home/kingu/.config/$profile_dir" \
            "$KINGU_RESTORE/$profile_dir.failed"; then
            recovery_ok=0
          fi
        fi
      done
    fi
    if ((${#KINGU_MOVED_CURRENT_DIRS[@]})); then
      for profile_dir in "${KINGU_MOVED_CURRENT_DIRS[@]}"; do
        if sudo test -d "$KINGU_POST_UPGRADE/$profile_dir"; then
          if ! sudo mv "$KINGU_POST_UPGRADE/$profile_dir" /home/kingu/.config/; then
            recovery_ok=0
          fi
        elif ! sudo test -d "/home/kingu/.config/$profile_dir"; then
          recovery_ok=0
        fi
      done
    fi
    if [[ -n "$KINGU_POST_UPGRADE" ]]; then
      sudo rmdir "$KINGU_POST_UPGRADE" 2>/dev/null || true
    fi
    if ((KINGU_CURRENT_BINARY_MOVED)); then
      if sudo test -f "$KINGU_CURRENT_BINARY"; then
        if ! sudo mv -f "$KINGU_CURRENT_BINARY" /opt/kingu/kingu-linux.AppImage; then
          recovery_ok=0
        fi
      elif ! sudo test -f /opt/kingu/kingu-linux.AppImage; then
        recovery_ok=0
      fi
    fi
    if ((KINGU_CURRENT_VERSION_MOVED)); then
      if sudo test -f "$KINGU_CURRENT_VERSION"; then
        if ! sudo mv -f "$KINGU_CURRENT_VERSION" /opt/kingu/VERSION; then
          recovery_ok=0
        fi
      elif ! sudo test -f /opt/kingu/VERSION; then
        recovery_ok=0
      fi
    elif ((KINGU_VERSION_REPLACEMENT_STARTED)); then
      if ! sudo rm -f /opt/kingu/VERSION; then
        recovery_ok=0
      fi
    fi
    if ((recovery_ok)); then
  ...
  if [[ -n "$KINGU_ROLLBACK_BINARY_STAGED" ]]; then
    sudo rm -f -- "$KINGU_ROLLBACK_BINARY_STAGED"
  fi
  if [[ -n "$KINGU_ROLLBACK_VERSION_STAGED" ]]; then
    sudo rm -f -- "$KINGU_ROLLBACK_VERSION_STAGED"
  fi
  sudo rm -rf -- "$KINGU_RESTORE"
  exit "$exit_status"
}
trap restart_after_rollback_error EXIT

if [[ "$(sudo stat -c %d "$KINGU_RESTORE")" != \
  "$(sudo stat -c %d /home/kingu/.config)" ]]; then
  echo 'Refusing rollback because staging and the Kingu profile are on different filesystems' >&2
  exit 1
fi
sudo tar xzf "$KINGU_ROLLBACK/profile.tgz" -C "$KINGU_RESTORE"
KINGU_RESTORE_DIRS=()
for profile_dir in kingu Kingu; do
  if sudo test -L "$KINGU_RESTORE/$profile_dir"; then
    echo "Rollback bundle contains a symlinked profile: $profile_dir" >&2
    exit 1
  fi
  if sudo test -d "$KINGU_RESTORE/$profile_dir"; then
    if [[ "$profile_dir" == Kingu ]] && \
      sudo test "$KINGU_RESTORE/kingu" -ef "$KINGU_RESTORE/Kingu"; then
      continue
    fi
    KINGU_RESTORE_DIRS+=("$profile_dir")
  fi
done
if ((${#KINGU_RESTORE_DIRS[@]} == 0)); then
  echo "Rollback bundle has no Kingu profile directories: $KINGU_ROLLBACK" >&2
  exit 1
fi
for profile_dir in "${KINGU_RESTORE_DIRS[@]}"; do
  sudo chown -R kingu:kingu "$KINGU_RESTORE/$profile_dir"
done

KINGU_ROLLBACK_STAMP=$(date +%F-%H%M%S-%N)
KINGU_ROLLBACK_BINARY_STAGED=/opt/kingu/kingu-linux.AppImage.rollback-staged-$KINGU_ROLLBACK_STAMP
sudo cp -a "$KINGU_ROLLBACK/kingu-linux.AppImage" "$KINGU_ROLLBACK_BINARY_STAGED"
if sudo test -f "$KINGU_ROLLBACK/VERSION"; then
  KINGU_ROLLBACK_HAS_VERSION=1
  KINGU_ROLLBACK_VERSION_STAGED=/opt/kingu/VERSION.rollback-staged-$KINGU_ROLLBACK_STAMP
  sudo cp -a "$KINGU_ROLLBACK/VERSION" "$KINGU_ROLLBACK_VERSION_STAGED"
fi

KINGU_SERVICE_STOPPED=1
sudo systemctl stop kingu-serve.service

# Preserve and replace only Kingu-owned profile directories
KINGU_CURRENT_DIRS=()
for profile_dir in kingu Kingu; do
  if sudo test -L "/home/kingu/.config/$profile_dir"; then
    echo "Refusing symlinked Kingu profile: /home/kingu/.config/$profile_dir" >&2
  ...
  if sudo test -d "/home/kingu/.config/$profile_dir"; then
    ...
    KINGU_CURRENT_DIRS+=("$profile_dir")
  fi
done
KINGU_POST_UPGRADE=/home/kingu/.config/kingu-rollback-$KINGU_ROLLBACK_STAMP
sudo install -d -o kingu -g kingu -m 700 "$KINGU_POST_UPGRADE"
if ((${#KINGU_CURRENT_DIRS[@]})); then
  for profile_dir in "${KINGU_CURRENT_DIRS[@]}"; do
    KINGU_MOVED_CURRENT_DIRS+=("$profile_dir")
    sudo mv "/home/kingu/.config/$profile_dir" "$KINGU_POST_UPGRADE/"
  done
fi
for profile_dir in "${KINGU_RESTORE_DIRS[@]}"; do
  KINGU_INSTALLED_RESTORE_DIRS+=("$profile_dir")
  sudo mv "$KINGU_RESTORE/$profile_dir" /home/kingu/.config/
done

KINGU_CURRENT_BINARY=/opt/kingu/kingu-linux.AppImage.rollback-current-$KINGU_ROLLBACK_STAMP
KINGU_CURRENT_BINARY_MOVED=1
sudo mv /opt/kingu/kingu-linux.AppImage "$KINGU_CURRENT_BINARY"
sudo mv -f "$KINGU_ROLLBACK_BINARY_STAGED" /opt/kingu/kingu-linux.AppImage

KINGU_CURRENT_VERSION=/opt/kingu/VERSION.rollback-current-$KINGU_ROLLBACK_STAMP
if sudo test -f /opt/kingu/VERSION; then
  KINGU_CURRENT_VERSION_MOVED=1
  sudo mv /opt/kingu/VERSION "$KINGU_CURRENT_VERSION"
fi
KINGU_VERSION_REPLACEMENT_STARTED=1
if ((KINGU_ROLLBACK_HAS_VERSION)); then
  sudo mv -f "$KINGU_ROLLBACK_VERSION_STAGED" /opt/kingu/VERSION
else
  sudo rm -f /opt/kingu/VERSION
fi
...
the newer `kingu-data.json` in place, where an older build can discard state it
does not understand. Keep the pre-upgrade backup until the new version is proven
on your host. The `kingu-rollback-*` directory inside `.config` is also retained
deliberately. The post-upgrade binary and version record are retained in
`/opt/kingu` with the same `rollback-current-<timestamp>` suffix. Inspect these
artifacts and remove them according to your retention policy after the rollback
is resolved.

## Installing Agent Skills Without A Desktop

Kingu's agent skills (CLI usage, orchestration, computer use, etc.) are normally
installed from Kingu Settings, which pre-fills an `npx skills add ... --global`
command in a terminal for you to run. A headless host has no Settings UI, so
use `kingu skills install` instead:

```bash
kingu skills install                                      # list installable skills
kingu skills install --skill kingu-cli --skill orchestration # install globally (default)
kingu skills install --skill kingu-cli --local              # install into the current project only
kingu skills install --all                                 # install every bundled skill
kingu skills install --all --dry-run                       # print the npx command without running it
```
...
host; it does not need a running Kingu runtime.
...
the choice Kingu makes it explicitly: it passes an `--agent` list built from the
...
kingu skills install --skill kingu-cli --agent universal
```

If Kingu detects no agent at all, `kingu skills install` stops and asks for
`--agent` rather than guessing.

To refresh already-installed skills, `kingu skills update` mirrors the same
...
```bash
kingu skills update --all                                  # update every bundled skill globally
kingu skills update --skill kingu-cli --dry-run             # print the npx command without running it
```

`kingu skills update` only refreshes skills that are already installed — it exits
...
Both commands install onto the machine that runs them. In an Kingu SSH workspace
or the WSL bridge the `kingu` shim forwards commands to the Kingu host, so they
...
- Chromium sandbox errors: confirm the service is running as the non-root
  `kingu` user and that `/opt/kingu` is readable by that user, including
  `/opt/kingu/squashfs-root` if you extracted the AppImage.
...
- Journal shows `Another Kingu instance is already running for this userData
profile` and the unit exits `3`: another process already owns the profile, so
  ...
  owner with `systemctl status kingu-serve` and `pgrep -af kingu`. Stop it (or
  keep it and leave the unit down), then run
  `sudo systemctl reset-failed kingu-serve && sudo systemctl start kingu-serve` —
  ...
  AppImage mounts, list them with `findmnt -rn -t fuse.kingu-linux.AppImage` and
...
- Service crash-loops right after an upgrade: use [Roll back](#roll-back) with
  ...
  `sudo systemctl reset-failed kingu-serve.service` first.
- Diagnosing other missing libraries: extract the AppImage without launching it
  with `./kingu-linux.AppImage --appimage-extract`, then run
  `ldd squashfs-root/kingu-ide` to list any shared libraries the host is missing.
  The Electron binary is `kingu-ide`, not `kingu`; `ldd` on a path that does not
