...
`kingud` is the Kingu runtime served from plain Node. This is the contract between it and
...
A deployment is **kingud** plus **the terminal daemon**.

|            | kingud                            | terminal daemon                       |
| ---------- | -------------------------------- | ------------------------------------- |
| Started by | the supervisor                   | kingud, detached                       |
| Owns       | RPC, git, worktrees, persistence | every local PTY                       |
| Lifetime   | one supervised run               | detached from kingud, not its service  |
| Endpoint   | `ws://<bind>:<port>`             | `<data-root>/daemon/daemon-v<N>.sock` |

kingud detaches the daemon and calls `disconnectDaemon()`, never `shutdownDaemon()`. The
built-in remote deployment path stops only the recorded kingud PID, so the daemon and its PTYs
...
Process detachment is not service isolation. A daemon that kingud launches directly, and every
...
Service-restart survival therefore requires a separately supervised cgroup, and kingud now asks
for one: on Linux it launches the daemon through `systemd-run --user --scope`, which places the
daemon and its PTYs in their own transient `kingu-daemon-<launch-nonce>.scope` unit under the
...
`kingu serve`'s wide default, and a startup where some device has connected before — and an
...
Under the shipping design a client reaches a remote kingud over an SSH local port-forward, so
loopback is the correct default and the pairing credential travels over SSH.

## Data root and the instance lock

The data root is `$KINGU_USER_DATA`, else `$XDG_DATA_HOME/Kingu`, else `~/.kingu`.

Before the profile index or the store is touched, kingud takes `<data-root>/kingud.lock`.
It refuses to start when:

| Code                                   | Meaning                                                       |
| -------------------------------------- | ------------------------------------------------------------- |
| `kingud_data_root_wrong_owner`          | the root is owned by another uid (POSIX)                      |
| `kingud_data_root_shared`               | the root is group/world accessible and could not be tightened |
| `kingud_instance_lock_held`             | another live kingud owns this root                             |
| `kingud_instance_lock_foreign_identity` | the lock belongs to a different identity                      |
| `kingud_data_root_unusable`             | the root cannot be created, stat'd or written                 |

A root that is merely too permissive and that we own is tightened to `0700` rather than
refused — kingud stores credentials there unsealed (no OS keyring on this host), so the goal
...
`health.terminalDaemon.cgroupUnit` names an `kingu-daemon-*.scope` on that host.

Before a cgroup-wide stop, obtain a fresh `kingu-ide terminal list --json` result using the same OS
...
`sudo -Hu kingu /home/kingu/.local/bin/kingu-ide terminal list --json`). Replace both `kingu` and
`/home/kingu` with the service account and home used by the unit; an extracted deployment may use
its absolute `resources/bin/kingu-ide` launcher instead. A safe empty census is untruncated, has an explicit `hostScope`, covers every
...
not admit new work after the census. Kingu does not yet provide an atomic census-and-stop fence.

### Who supervises kingud

An external supervisor (systemd, launchd, a process manager). kingud conforms to it:

- **Readiness.** One JSON line on stdout (`--json`), `type: "kingu_server_ready"`, published
...
- **Shutdown.** `SIGTERM` or `SIGINT` starts a graceful stop. A **second** signal exits
  ...
  the one teardown that skips the daemon handoff. kingud also imposes its own 15s deadline
...
- **Logs.** kingud writes human-readable diagnostics to **stderr** and its readiness contract
  ...
  `KINGU_DIAGNOSTICS_DISABLED=1`). Rotation of that file is not implemented — see
  [What is not covered](#what-is-not-covered).

### kingud supervising the daemon
...
- **Launch.** On Linux, through `systemd-run --user --scope` so the daemon gets its own
  ...
  `kingud.js` with its own PID record, token and socket under `<data-root>/daemon`.
...
- **Crash-loop containment.** At most **5 launches per 60s rolling window** per kingud run;
  ...
  recovers without restarting kingud. An operator-initiated daemon restart clears it — that
  is the deliberate "try again".
- **No macOS login-session watch.** That watch retires the daemon when the spawning GUI login
  session dies. An kingud daemon must survive its SSH session ending.
- **Shutdown.** kingud never stops the daemon. A daemon that was never adopted retires itself
  after its adoption window; an adopted one stays resident (see Decommissioning).

### Decommissioning

After a PID-scoped stop, an adopted daemon stays resident so the next kingud can reattach. A
...
above, stop kingud, then stop the daemon named by `health.terminalDaemon.pid`.
...
```
buildHash    sha256 (16 hex) of the running kingud bundle — build identity that a version
             string cannot give, so a rollback that did not replace the file is visible
buildVersion KINGU_VERSION
...
terminalDaemon:
  ...
  buildVersion       the build the LIVE daemon was forked from (may legitimately predate
                     this kingud after an update — reporting kingud's version for both would
                     hide exactly that)
...
processes: kingud drives it, the daemon performs it, the verdict crosses the socket.
...
because those terminals die with kingud. A daemon that answered and then failed its spawn
...
- **A continuous health endpoint.** `health` is published once, in the readiness payload. A
  ...
  `collectKingudHealth()`; that surface does not exist yet.
- **Supervision of an unscoped fallback daemon.** When the durable `systemd-run --user --scope`
  launch is unavailable (see [above](#two-long-lived-processes-not-one)) kingud and its daemon
