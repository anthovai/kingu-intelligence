...
Process detachment is not service isolation. A daemon forked by kingud, and every PTY it owns,
...
- **Launch.** Forked detached from `daemon-entry.js` beside `kingud.js`, with its own PID
...
After a PID-scoped stop, an adopted daemon stays resident so the next kingud can reattach.
A combined-unit systemd stop kills it instead. To retire a process-scoped deployment, apply
the census rule above, stop kingud, then stop the daemon named by `health.terminalDaemon.pid`.
...
- **Systemd-isolated daemon supervision.** kingud and its daemon currently share one service
