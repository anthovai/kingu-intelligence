...
- CLI: `kingu search` calls both over the runtime RPC, against the host that
  ...
  In an Kingu SSH terminal, the forwarded CLI defaults to the controlling Kingu
...
runs in the scanner child; kingud and the SSH relay register their own in-process
