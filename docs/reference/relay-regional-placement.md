...
Kingu selects a Relay region in the Electron main process before requesting a new assignment. The
director publishes an allowlisted region catalog containing only HTTPS cell subdomains of that
director. Kingu discards one warm-up `/health` request per probe origin — a cold request pays TCP and
...
be measured, Kingu sends no hint rather than selecting the sole survivor. Sending no hint is not
...
After a control socket registers, Kingu probes the cell it actually landed on, once per cell URL per
...
For deterministic local diagnostics, set `KINGU_RELAY_REGION_OVERRIDE` to `us-central1` or
`asia-east2` before launching Kingu. The override is not an end-user setting and is not written to
