...
On native Windows, Kingu writes the `kingu.yaml` setup script (and the issue command) to a generated
...
- Two people on the same repo got different interpreters for the same `kingu.yaml`, so no project
...
A `#!` line only takes effect when Kingu can actually launch bash from the configured terminal — the
