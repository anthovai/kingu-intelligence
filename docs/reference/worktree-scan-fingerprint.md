...
(`KinguRuntimeService.listRepoWorktreesForResolution`). It keeps the existing
...
- `computeResolvedWorktrees` fans out over **every** registered repo
  (`src/main/runtime/kingu-runtime.ts`), calling
...
The in-Kingu mutation surface is already event-driven: create, remove, rename,
...
worktree changes made **outside** Kingu (`git worktree add/remove/move/prune`,
...
| Change                                                                | Before            | After             |
| --------------------------------------------------------------------- | ----------------- | ----------------- |
| Kingu-initiated create/remove/rename/sparse/repo edit                  | immediate (event) | immediate (event) |
...
changes Kingu does not make itself.
...
Git runtime is still probed, because it is not WSL-routed from Kingu's point of
