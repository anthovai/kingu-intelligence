...
against Kingu for exactly this reason.
...
- **The interactive terminal.** `src/main/daemon/pty-subprocess/native-pty-spawn.ts`
  calls `pty.spawn` directly, so typing `codex` in an Kingu terminal is
  completely unaffected.
- **Kingu's own hook wrappers** (`codex-hook.cmd` and friends). These are batch
  files Kingu writes, matching none of the generator shapes, so they keep the
...
Set **`KINGU_DISABLE_CMD_SHIM_RESOLUTION`** to any non-empty value in the
...
exporting it before launching Kingu disables resolution process-wide.
