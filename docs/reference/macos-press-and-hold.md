...
eligible launch, Kingu writes:

```sh
defaults write com.anthovai.kingu ApplePressAndHoldEnabled -bool false
```

The write is scoped to Kingu's packaged bundle domain. Bare Electron development bundles and
...
Kingu checks for an explicit domain value before writing. Either `true` or `false` is treated as a
...
Set the preference explicitly, then restart Kingu:

```sh
defaults write com.anthovai.kingu ApplePressAndHoldEnabled -bool true
```

After Kingu has recorded its one-time decision, deleting the key also restores the macOS default
without Kingu recreating it:

```sh
defaults delete com.anthovai.kingu ApplePressAndHoldEnabled
```

Development and prerelease channels may use a channel-suffixed Kingu bundle identifier; use that
