...
Kingu's code editor is Monaco — the same editor VS Code uses — with a few Kingu-specific tweaks.
...
Syntax highlighting ships for the languages Monaco supports out of the box. Kingu is intentionally editor-first, not IDE-first — run type-checkers and linters in a terminal pane.
