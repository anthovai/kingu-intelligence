...
description: Quit Kingu, reopen it, and pick up exactly where you left off — worktrees, splits, scrollback, focused tab.
---

import { Callout } from '@/components/docs/prose'

When you close Kingu, the next launch rehydrates the whole workspace: every open worktree, every terminal split, the scrollback in each pane, and the tab you had focused. You shouldn't have to remember which agents you had running on which branches — that's Kingu's job.
...
- **Running agent processes** — agents you had running keep running across an Kingu quit. A background daemon owns the PTYs, so closing the app window doesn't kill Claude Code, Codex, or any other agent CLI mid-task. On next launch, Kingu warm-reattaches to the same processes.
- **Terminal scrollback** — the buffer in each terminal, including output produced while Kingu was closed.
- **Focused worktree and tab** — Kingu opens to the same view you closed.
...
A daemon crash while Kingu is closed has the same effect for any sessions it was holding, but the layout and scrollback still restore on next launch.
...
- **Auto-updater relaunch** — Kingu restarts to install an update; agents are unaffected.
- **App crash** — if Kingu itself crashes, the daemon keeps your sessions alive for warm reattach on next launch.
...
<Callout title="Starting fresh">
  ...
  session" mode — Kingu always restores. Closed worktrees stay closed.
</Callout>
