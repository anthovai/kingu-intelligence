...
description: Diagnose rate limits, auth failures, and other GitHub CLI errors in Kingu PR checks, Tasks, and Source Control.
---

Kingu talks to GitHub through the **GitHub CLI (`gh`)** on your machine (or remote Kingu host). When PR status, checks, issues, or Tasks fail to refresh, the cause is almost always GitHub auth, permissions, or API rate limits — not a broken PR panel by itself.
...
| ----------------------------------------------------------------- | ---------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| “GitHub is rate-limiting requests” / “rate limit exceeded (core)” | GitHub REST (core) quota exhausted for your user           | Wait for reset; stop extra `gh` / agent / Kingu usage; check [Settings → Git → GitHub API Budget](/docs/settings) |
...
| “GitHub CLI is unavailable”                                       | `gh` missing from PATH Kingu uses                           | Install `gh` and restart Kingu                                                                                    |

## Rate limits (most common)

GitHub gives each **authenticated user** a shared hourly budget. **Every tool on that account shares it**: Kingu, `gh` in terminals, Claude/Codex/Grok agents that call `gh`, CI scripts, browser extensions, and other apps.

### Buckets Kingu cares about
...
When a primary bucket is exhausted, GitHub returns HTTP **403** with a message like `API rate limit exceeded`. Kingu classifies that as rate-limited, keeps the last known PR status when it can, and **stops spawning more `gh` calls** for a short window so a single limit doesn’t turn into a storm of failures.
...
- Many Kingu windows or electron-dev builds open at once (each may refresh PRs / Tasks)
...
2. **Reduce concurrent GitHub clients** — quit extra Kingu instances and pause bulk `gh` automations.
3. Avoid hammering refresh on the PR panel while limited; Kingu is already backing off.
...
Kingu inherits whatever `gh` uses on that host.
...
`gh auth status` may show the token as invalid. Run `gh auth login` (or `gh auth refresh`) and restart Kingu so it picks up the new credentials.
...
GitHub auth is **per host**. Logging in on your laptop does not log in `gh` on a remote machine. SSH into the host and run `gh auth login` there, or use Kingu’s remote-server GitHub budget view for that environment.
...
If Kingu reports the GitHub CLI is unavailable:

1. Install [`gh`](https://cli.github.com/)
2. Confirm `which gh` works in a normal terminal
3. Fully quit and reopen Kingu (so PATH matches)
4. On Windows, install for the same environment Kingu launches (WSL vs native)

## How Kingu behaves when GitHub fails
...
- **Circuit breaker**: After a primary rate-limit 403, Kingu briefly refuses new `gh` spawns for that bucket (`core`, `search`, or `graphql`) so the app stays responsive and doesn’t dig a deeper hole.

## Check GitHub API Budget in Kingu
...
- On remote Kingu servers, use the remote advanced budget view for the **server-owned** `gh` identity (local Settings only shows the desktop client)

## Still stuck?

1. Reproduce once in a terminal with `gh pr view` or `gh api user` from the same machine/user Kingu uses.
...
- [GitHub Issues](https://github.com/anthovai/kingu-intelligence/issues)
...
- [Troubleshooting & FAQ](/docs/troubleshooting) — general Kingu issues
