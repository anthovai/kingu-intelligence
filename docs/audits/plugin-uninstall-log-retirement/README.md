...
```sh
KINGU_BACKGROUND_LAUNCH=1 KINGU_PLUGIN_LOG_VARIANT=before pnpm exec vitest run --config docs/audits/plugin-uninstall-log-retirement/phase.config.mjs
KINGU_BACKGROUND_LAUNCH=1 KINGU_PLUGIN_LOG_VARIANT=fixed pnpm exec vitest run --config docs/audits/plugin-uninstall-log-retirement/phase.config.mjs
KINGU_BACKGROUND_LAUNCH=1 node docs/audits/plugin-uninstall-log-retirement/loader-controls.cjs
```

For Electron's installed Node runtime, use the installed Electron executable with `ELECTRON_RUN_AS_NODE=1 KINGU_BACKGROUND_LAUNCH=1` and `node_modules/vitest/vitest.mjs` followed by the same `run --config` arguments. `KINGU_PLUGIN_LOG_OUTPUT` and `KINGU_PLUGIN_LOG_LOADER_OUTPUT` select alternate report paths. `before.config.mjs` runs the six permanent retention cases against baseline and is expected to fail those retention assertions.
