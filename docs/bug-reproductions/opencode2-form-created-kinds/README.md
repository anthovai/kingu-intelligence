...
OpenCode 2 has one form primitive and several producers. Kingu's setup bridge mapped every
`form.created` to `question.asked`, which is Kingu's un-evictable "the pane owner must answer
...
So Kingu blocks on every session-owned form and drops only the non-session sentinel. Upstream notes
in `form.ts:122-129` that `"global"` is temporary and elicitations will get real session ids; when
that lands the exclusion stops matching and Kingu starts blocking on them correctly.
...
not the `{ messageID, callID }` that Kingu's `clearQuestionForToolPart` matches on, and OpenCode 2
...
they produce no server event of any kind and cannot be the thing Kingu saw.
