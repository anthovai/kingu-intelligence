# Kingu could not resolve OpenCode 2 session lineage (#22371 follow-up)

Kingu's OpenCode plugin resolves a session's ancestry so a subagent's work rolls up to the pane
...
Captured against the shipped `opencode v2.0.12` binary on macOS: Kingu's real generated plugin in
`~/.config/opencode/plugins/kingu-opencode2-status.js`, real `KINGU_PANE_KEY` / hook-token env, a
logging hook server on a loopback port, and `opencode2 --standalone` driven in a real PTY.

## The SDK shape Kingu was given
...
KINGU_PANE_KEY=tab:leaf KINGU_OPENCODE_AGENT=opencode2 \
KINGU_AGENT_HOOK_PORT=<port> KINGU_AGENT_HOOK_TOKEN=<token> \
  opencode2 --standalone --prompt '<delegate something to one subagent>'
```

Unset `KINGU_AGENT_HOOK_ENDPOINT` so the plugin reads the port/token from env instead of a live
Kingu's endpoint file.

One trap: OpenCode 2 loads both `kingu-opencode-status.js` and `kingu-opencode2-status.js` from
`~/.config/opencode/plugins/`, and both declare the same plugin `id` (`kingu-opencode-status`).
