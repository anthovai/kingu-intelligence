...
public sealed class KinguRenderTestCounter {
    public int FindAll;
}

public sealed class KinguRenderTestBounds {
...
public sealed class KinguRenderTestControlType {
    public string ProgrammaticName = "";
}

public sealed class KinguRenderTestCurrent {
    public string AutomationId = "";
    public KinguRenderTestBounds BoundingRectangle = new KinguRenderTestBounds();
    public string ClassName = "fake";
    public KinguRenderTestControlType ControlType = new KinguRenderTestControlType();
...
public sealed class KinguRenderTestPatternCurrent {
    public bool IsSelected;
    public string Value = "";
}

public sealed class KinguRenderTestPattern {
    public KinguRenderTestPatternCurrent Current = new KinguRenderTestPatternCurrent();
}

public sealed class KinguRenderTestCollection : IEnumerable {
    private readonly KinguRenderTestElement[] values;

    public KinguRenderTestCollection(KinguRenderTestElement[] values) {
        this.values = values;
    }
    ...
    public KinguRenderTestElement Item(int index) {
        return values[index];
    }
...
public sealed class KinguRenderTestElement {
    public readonly List<KinguRenderTestElement> Children =
        new List<KinguRenderTestElement>();
    public KinguRenderTestCounter Counter = new KinguRenderTestCounter();
    public readonly KinguRenderTestCurrent Current = new KinguRenderTestCurrent();
    public bool FailFindAll;
    public int RuntimeIdValue;
    public string ValueText = "";

    public KinguRenderTestCollection FindAll(object scope, object condition) {
        Counter.FindAll++;
        if (FailFindAll) {
            throw new System.InvalidOperationException("defunct node");
        }
        return new KinguRenderTestCollection(Children.ToArray());
    }

    public KinguRenderTestPattern GetCurrentPattern(object pattern) {
        KinguRenderTestPattern result = new KinguRenderTestPattern();
...
function New-TestCounter {
    New-Object -TypeName KinguRenderTestCounter
}
...
function New-TestElement {
    ...
    $element = New-Object -TypeName KinguRenderTestElement
...
$operationPath = Join-Path ([IO.Path]::GetTempPath()) ("kingu-runtime-render-test-" + [guid]::NewGuid() + ".json")
try {
    ...
    $tree = Render-KinguTree $root $null
