...
internal static class KinguCliLauncher
{
    private static int Main(string[] args)
    {
        try
        {
            string launcherDirectory = Path.GetDirectoryName(typeof(KinguCliLauncher).Assembly.Location);
            ...
            string electronPath = Path.Combine(appDirectory, "Kingu.exe");
            ...
            {
                Console.Error.WriteLine("Unable to locate Kingu.exe next to \"{0}\"", resourcesDirectory);
                return 1;
            }

            if (!File.Exists(cliPath))
            {
                Console.Error.WriteLine("Unable to locate the Kingu CLI entrypoint at \"{0}\"", cliPath);
                return 1;
            }
            ...
            MoveEnvironmentVariable("NODE_OPTIONS", "KINGU_NODE_OPTIONS");
            MoveEnvironmentVariable("NODE_REPL_EXTERNAL_MODULE", "KINGU_NODE_REPL_EXTERNAL_MODULE");
            Environment.SetEnvironmentVariable("ELECTRON_RUN_AS_NODE", "1");
            Environment.SetEnvironmentVariable("KINGU_WINDOWS_PACKAGED_CLI_LAUNCHER", "1");
            string requestedCliCommand = Environment.GetEnvironmentVariable("KINGU_CLI_COMMAND");
            Environment.SetEnvironmentVariable(
                "KINGU_CLI_COMMAND",
                requestedCliCommand == "kingu-ide" ? "kingu-ide" : "kingu"
            );
        ...
        {
            Console.Error.WriteLine("Unable to start the Kingu CLI: {0}", error.Message);
            return 1;
        }
    }
