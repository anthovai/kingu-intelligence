# Kingu Relay operations runbook
...
```sh
export PROJECT_ID=onkingu-cloud-staging
export REGION=us-central1
export DIRECTOR_ORIGIN=https://relay-staging.onkingu.dev
export DEPLOY_SERVICE_ACCOUNT=kingu-cloud-staging-gha-deploy@onkingu-cloud-staging.iam.gserviceaccount.com
...
`KINGU_RELAY_CELL_CONNECTION_HARD_CAP=600` and
`KINGU_RELAY_CELL_CONNECTION_UNOBSERVED_BOUND=<load-proven bound>` use the hard
...
`kingu_relay_idle_rehome_target_deferred` with the cause, and the director moves
...
`KINGU_RELAY_REGION_CORRECTION_COHORT_PERCENT` (integer 0–100, default **0**).
...
`kingu_relay_region_correction_outcomes` reports attempts by source/target,
registration/completion/abort state, oldest open age and target
reservation units every five minutes. `kingu_relay_region_comparison` samples a
