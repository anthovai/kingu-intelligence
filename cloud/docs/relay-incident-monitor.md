...
audience `https://relay.onkingu.dev/v1/admin/drain`, and it needs read access to the monitored GCP
...
`KINGU_RELAY_ADMIN_ID_TOKEN`. The token must remain valid for the whole run and is never persisted.
...
| --- | ---: |
| Kingu Relay: far-cell phone accept latency | per cell, median 30-second `clientAcceptTotalMsP95` over 15 minutes above 2,000 ms with at least 20 completed accepts |
| Kingu Relay: cell control round trip | per cell, median `controlRttMsP50` over one hour above 150 ms with at least 500 samples |
| Kingu Relay: region hint skew | fleet-wide, asia-east2 share of hinted requests over one hour more than 2x and more than 15 points above its share of actual placements, with at least 500 hinted requests |
...
stall the fleet. `Kingu Relay: cell table lock held over 1 second` fires on any
30-second runtime sample from a cell whose `cellInventoryHoldMsMax` is at least
1,000 ms, labelled with the cell's `cell_id`. `Kingu Relay: lock timeout burst`
...
They go to `Kingu Relay: director cell table lock held over 1 second`, which has
...
- Recalibrated the relay pool freezes from 30 waiters / 1,000 ms to
  ...
  `kingu_relay_runtime_metrics` (`databasePoolWaitersMax`,
...
- Recalibrated the PostgreSQL-retry freeze from 20 to 300 per five minutes
  ...
  `jsonPayload.event="kingu_relay_postgres_transaction_retry"` in production
  ...
  own `kingu_relay_postgres_retries` metric read 1,510 for that window; see the
  2026-09-04 entry).
- Recalibrated the PostgreSQL-retry freeze from 300 to 2,000 per five minutes
  ...
  cells, summed per five minutes from the `kingu_relay_postgres_retries`
...
- Recalibrated the exhausted-PostgreSQL-retry freeze from 0 to 300 per five
  ...
  succeeding slowly, and `kingu_relay_postgres_transaction_exhausted` became
