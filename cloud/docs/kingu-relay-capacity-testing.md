...
1. Set `KINGU_RELAY_LOAD_ACCESS_TOKEN` and pass `--auth-origin`. Every control and refresh then uses the real auth-plane relay-token exchange.
...
node dev/scripts/load-relay-controls.mjs \
  --director-origin https://relay-staging.onkingu.dev \
  --auth-origin https://auth-staging.onkingu.dev \
  --signing-key-file <(gcloud secrets versions access latest \
    --secret=kingu-cloud-auth-signing-key \
...
```sh
KINGU_RELAY_LOAD_ACCESS_TOKEN="$ACCESS_TOKEN" pnpm load:relay:controls -- \
  --director-origin https://relay-staging.onkingu.dev \
  --auth-origin https://auth-staging.onkingu.dev \
  --controls 800 \
  --ramp-seconds 210 \
...
KINGU_RELAY_ADMIN_ID_TOKEN="$(gcloud auth print-identity-token \
  --impersonate-service-account="${CAPACITY_SA}" \
  --project=onkingu-cloud-staging \
  --include-email \
  --audiences=https://relay-staging.onkingu.dev/v1/admin/drain)" \
node dev/scripts/load-relay-controls.mjs \
  --director-origin https://relay-staging.onkingu.dev \
  --auth-origin https://auth-staging.onkingu.dev \
  --signing-key-file <(gcloud secrets versions access latest \
