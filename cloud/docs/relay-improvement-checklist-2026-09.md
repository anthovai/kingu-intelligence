...
- Roll 2 relay image `4916ed67` (anthovai/kingu-intelligence #18959 + #18722 + #18720 flag unset): director since 2026-09-06 01:02Z, all 19 general cells by 16:29Z. Control lease 6 h ± 30 min, accept abandonment, per-cell inventory locks, pool `statement_timeout`. Record: findings doc, "Roll 2" section.
- Auth instance cap 20 + dead-family audit fix (kingu-cloud #474) as revision `kingu-cloud-auth-00031-tox`.
- Dynamic NAT ports in both regions (anthovai/kingu-intelligence #18693). Zero drops and zero proxy dial errors since.
...
- Refresh rotation grace window (kingu-cloud #478). Startup adds one nullable column (brief exclusive lock on `refresh_tokens`).
- Pruning job code (kingu-cloud #476) is in the image; the job itself is Terraform-disabled until 1.2.
...
3. kingu-cloud #477 private IP (2.1): merge arms an instance restart and a one-way door. Recommendation: hold.
...
- Deploy script strips `KINGU_CLOUD_REFRESH_TOKEN_TTL_DAYS` on every release (3.1 first item).
...
### 1.2 Enable pruning
- [x] `auth_token_pruner_image` = digest of `kingu-cloud-auth-00031-tox` (`343a0915…`; it contains the entrypoint). kingu-cloud #479 merged.
- [x] `auth_token_pruner_enabled = true`, `auth_token_pruner_max_rows_per_run = 20000` for the first day (kingu-cloud #479).
...
### 2.1 Private IP (PRs open: kingu-cloud #477 foundation, anthovai/kingu-intelligence #18720 relay flag)
...
- [ ] Production: announce a window; same steps; verify `kingu_relay_runtime_metrics` controls recover to pre-cutover count.
...
### 3.1 Refresh rotation grace window (kingu-cloud #478 merged 2026-09-04; deploy pending owner go)
- [ ] Fix the deploy-script env strip for `KINGU_CLOUD_REFRESH_TOKEN_TTL_DAYS` (pre-existing; found by #478).
...
- [x] Deploy via `deploy-auth-production` (candidate → smoke → promote). Deployed 2026-09-04 23:15Z as `kingu-cloud-auth-00035-gos`, cap 20 kept, 0 5xx; `successor_material` column present; sealed successors being written. (candidate → smoke → promote).

### 3.2 / 4.3 Desktop (merged anthovai/kingu-intelligence #18719; ships next desktop release; relay side of 4.3 deployed in Roll 2)
...
### 4.1 Lock contention (partial: anthovai/kingu-intelligence #18722 deployed in Roll 2, 2026-09-06)
...
- [ ] Measure with `kingu_relay_runtime_metrics` region counters before/after.

### 5.x Observability
- [x] **Relay-root runtime-metric drift**: resolved by dropping the `region` label to match live state (anthovai/kingu-intelligence #18734). Applied 2026-09-04 23:11Z: 8 never-applied `control_*` renewal metrics + the incident dashboard created, 0 destroyed, 21 live metrics untouched.
...
- [x] 5.4 One dashboard (applied 2026-09-04 23:11Z): `kingu_relay_cloud_sql_wal_checkpoint`, NAT drops, `kingu_auth_refresh_401`, summed `controls`.
