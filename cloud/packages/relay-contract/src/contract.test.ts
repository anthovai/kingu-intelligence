...
describe('relay protocol contract', () => {
  ...
  it('accepts moves only from the configured director at a strictly newer epoch', () => {
    ...
    const base = {
      configuredDirectorOrigin: 'https://relay.onkingu.dev',
      currentAssignmentEpoch: 3,
      move
    }
    expect(isTrustedNewerMove({ ...base, sourceOrigin: 'https://relay.onkingu.dev' })).toBe(true)
    expect(isTrustedNewerMove({ ...base, sourceOrigin: move.cellUrl })).toBe(false)
    expect(
      isTrustedNewerMove({
        ...base,
        sourceOrigin: 'https://relay.onkingu.dev',
  ...
  it('locks the complete host key-possession transcript', () => {
    const transcript = buildHostProofTranscript({
      relayOrigin: 'https://relay.onkingu.dev',
...
describe('pending connection details capability', () => {
  ...
  it('pins the header and token the desktop mirrors by hand', () => {
    ...
    expect(RELAY_HOST_CAPABILITIES_HEADER).toBe('x-kingu-host-capabilities')
