...
const ARGV = [
  ...
  '--cell-host', 'c28.relay.onkingu.dev',
  '--project-id', 'onkingu-cloud',
  '--director-service', 'kingu-cloud-relay',
...
test('binds every gcloud input to a pinned pattern and to one cell', () => {
  ...
  assert.throws(() => parseShadowGateArguments(ARGV.with(5, 'Onkingu Cloud')))
  assert.throws(() => parseShadowGateArguments(ARGV.with(7, 'kingu cloud relay')))
  // Host and cell id must name the same cell, or the serving check reads a neighbour.
  assert.throws(() => parseShadowGateArguments(ARGV.with(3, 'c29.relay.onkingu.dev')))
...
function metricSamples({ cellId, from, count, payload = {}, intervalMs = 30_000 }) {
  return Array.from({ length: count }, (_, index) => ({
    matches: ['kingu_relay_runtime_metrics', `jsonPayload.cellId="${cellId}"`],
...
function productionLikeEntries() {
  return [
    {
      matches: ['listening on https://c28.relay.onkingu.dev'],
