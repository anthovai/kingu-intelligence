...
test('binds selector operations and director configuration to reviewed implementations', () => {
  ...
  assert.match(workflow, /kingu-cloud-relay-regional-placement-enabled/)
...
test('requires immutable staged evidence and a timed production canary before expansion', () => {
  ...
  assert.match(workflow, /pnpm --filter @kingu-cloud\/relay-contract build/)
  assert.ok(
  ...
  assert.match(workflow, /kingu_relay_runtime_metrics/)
...
test('creates staging evidence only after the bounded launch-path load and rollback', () => {
  ...
  assert.match(stagingProof, /pnpm --filter @kingu-cloud\/relay-contract build/)
  ...
  assert.match(stagingProof, /--capacity-cell-origin https:\/\/c4\.relay-staging\.onkingu\.dev/)
...
test('keeps the production US-first switch in durable Secret Manager state', () => {
  ...
  assert.match(directorWorkflow, /kingu-cloud-relay-regional-placement-enabled/)
