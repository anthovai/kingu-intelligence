...
test('proves unhinted US-first placement and sticky Asia preservation', async () => {
  const oldClientPeer = peer('https://c3.relay-staging.onkingu.dev')
  const stickyPeer = peer('https://c4.relay-staging.onkingu.dev')
  let retryDelayMs = 0
  assert.deepEqual(await proveRelayLoadRegionBehavior({
    oldClientPeer,
    stickyPeer,
    asiaOrigin: 'https://c4.relay-staging.onkingu.dev',
...
test('rejects Asia placement for an unhinted client or a moved sticky assignment', async () => {
  await assert.rejects(proveRelayLoadRegionBehavior({
    oldClientPeer: peer('https://c4.relay-staging.onkingu.dev'),
    stickyPeer: peer('https://c4.relay-staging.onkingu.dev'),
    asiaOrigin: 'https://c4.relay-staging.onkingu.dev',
    scheduleAssignmentRetry: (resolve) => resolve()
  }), /US-first/)
  await assert.rejects(proveRelayLoadRegionBehavior({
    oldClientPeer: peer('https://c3.relay-staging.onkingu.dev'),
    stickyPeer: peer('https://c4.relay-staging.onkingu.dev', 'https://c3.relay-staging.onkingu.dev'),
    asiaOrigin: 'https://c4.relay-staging.onkingu.dev',
    scheduleAssignmentRetry: (resolve) => resolve()
