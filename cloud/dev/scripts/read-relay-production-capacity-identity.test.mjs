...
test('reads absent, exact, and foreign literal capacity identities', () => {
  ...
  assert.equal(
    readProductionCapacityIdentity(revision([
      { name: 'KINGU_RELAY_CAPACITY_SERVICE_ACCOUNT', value: 'capacity@example.test' }
    ])),
    'capacity@example.test'
  )
  assert.equal(
    readProductionCapacityIdentity(revision([
      { name: 'KINGU_RELAY_CAPACITY_SERVICE_ACCOUNT', value: 'foreign@example.test' }
    ])),
...
test('rejects malformed or duplicate capacity identity entries', () => {
  for (const entry of [
    { name: 'KINGU_RELAY_CAPACITY_SERVICE_ACCOUNT', value: null },
    { name: 'KINGU_RELAY_CAPACITY_SERVICE_ACCOUNT', value: '' },
    { name: 'KINGU_RELAY_CAPACITY_SERVICE_ACCOUNT', valueSource: { secretKeyRef: {} } }
  ]) {
    assert.throws(
  ...
  assert.throws(
    () => readProductionCapacityIdentity(revision([
      { name: 'KINGU_RELAY_CAPACITY_SERVICE_ACCOUNT', value: 'one@example.test' },
      { name: 'KINGU_RELAY_CAPACITY_SERVICE_ACCOUNT', value: 'two@example.test' }
    ])),
