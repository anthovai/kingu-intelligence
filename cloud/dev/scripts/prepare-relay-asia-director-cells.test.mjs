...
test('appends C30 after the configured launch cells without touching them', () => {
  ...
  assert.deepEqual(result[3], {
    id: 'production-gce-c30', url: 'https://c30.relay.onkingu.dev', capacityRequests: 6_000,
...
test('checks C30 against its own digest, not the launch cells\' digest', () => {
  ...
  const c30 = {
    ...
    image: `us-central1-docker.pkg.dev/onkingu-cloud/kingu-cloud/relay@${c30Digest}`
  }
  const launch = [27, 28, 29].map((ordinal) => ({
    id: `production-gce-c${ordinal}`, url: `https://c${ordinal}.relay.onkingu.dev`,
