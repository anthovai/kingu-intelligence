...
const DIRECTOR_IDENTITY = 'relay-director@onkingu-cloud.iam.gserviceaccount.com'
const CAPACITY_IDENTITY = 'kingu-cloud-gha-cap@onkingu-cloud.iam.gserviceaccount.com'
const AUDIENCE = 'https://relay.onkingu.dev/v1/admin/host-drain'
const ROLLBACK_IMAGE = `us-central1-docker.pkg.dev/p/kingu-cloud/relay@sha256:${'d'.repeat(64)}`
const TARGET_IMAGE = `us-central1-docker.pkg.dev/p/kingu-cloud/relay@sha256:${'e'.repeat(64)}`
...
describe('same-cap roll scripts accept every same-cap cell', () => {
    ...
    for (const cellId of SAME_CAP_CELLS) {
      for (const mode of ['isolate', 'drain', 'activate']) {
        assert.deepEqual(parseProductionCapacityCellArguments([
          ...
          '--cell-origin', `https://${hostname(cellId)}.relay.onkingu.dev`,
          '--cell-id', cellId,
          '--approved-cells', 'same-cap',
          '--mode', mode
        ]), {
          directorOrigin: 'https://relay.onkingu.dev',
          cellOrigin: `https://${hostname(cellId)}.relay.onkingu.dev`,
          cellId,
          mode,
          paceWindowMs: 0
        })
  ...
  it('rolls a template stale enough to predate the pinned capacity identity', () => {
    ...
    assert.throws(
      () => validateCapacityPlan(stale, {
        ...config,
        capacityServiceAccount: 'kingu-cloud-gha-other@onkingu-cloud.iam.gserviceaccount.com'
      }),
      /reviewed image and capacity/
    )
  ...
  it('classifies every rollback stage from the real block', () => {
    const repository = 'us-central1-docker.pkg.dev/onkingu-cloud/kingu-cloud/relay'
