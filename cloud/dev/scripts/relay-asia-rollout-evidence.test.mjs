...
function canaryInput(overrides = {}, cellId = 'production-gce-c27') {
  ...
  Object.assign(load, {
    ...
    assignedCellOrigins: [`https://${cellId.split('-').at(-1)}.relay.onkingu.dev`]
  })
  return {
...
test('rejects staging evidence from a non-canonical repository', () => {
  assert.throws(() => buildStagingEvidence(stagingInput({ repository: 'fork/kingu-cloud' })), /repository/)
})
...
test('rejects a C27 canary whose control was placed on another cell', () => {
  for (const origins of [
    ['https://c28.relay.onkingu.dev'],
    ['https://c27.relay.onkingu.dev', 'https://c28.relay.onkingu.dev'],
    [],
    undefined
  ]) {
    const input = canaryInput()
...
test('rejects a C30 canary that C30 did not serve', () => {
  ...
  onLaunchCell.loadReport.assignedCellOrigins = ['https://c27.relay.onkingu.dev']
