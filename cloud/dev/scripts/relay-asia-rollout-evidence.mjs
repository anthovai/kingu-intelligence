...
const PRODUCTION_CANARIES = {
  [C27]: { kind: 'production-c27-canary', origin: 'https://c27.relay.onkingu.dev' },
  'production-gce-c30': { kind: 'production-c30-canary', origin: 'https://c30.relay.onkingu.dev' }
}
...
function assertLoadReports(reports, expected) {
  ...
  for (const report of reports) {
    ...
    if (expected.slowReaders > 0 && ownsReaderPressure && (
      queue.length !== 1 ||
      queue[0]?.origin !== 'https://c4.relay-staging.onkingu.dev' ||
