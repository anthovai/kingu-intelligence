...
const input = { dataDir: '/tmp/kingu-relay-boot', databaseUrl: 'postgres://relay@localhost/relay' }
...
describe('relay boot database open', () => {
  it('waits out a cold proxy instead of failing the boot', async () => {
    ...
    expect(loggedEvents(warn)).toEqual([
      'kingu_relay_boot_database_retry',
      'kingu_relay_boot_database_retry',
      'kingu_relay_boot_database_recovered'
    ])
  ...
  it('fails the boot immediately when the database rejects the relay', async () => {
    ...
    expect(loggedEvents(warn)).toEqual(['kingu_relay_boot_database_failed'])
    expect(JSON.parse(String(warn.mock.calls[0]?.[0]))).toMatchObject({
      attempts: 1,
      retryable: false,
      code: 'unknown'
  ...
  it('gives up once the retry budget is spent', async () => {
    ...
    expect(events.at(-1)).toBe('kingu_relay_boot_database_failed')
    expect(events.filter((event) => event === 'kingu_relay_boot_database_retry')).toHaveLength(
