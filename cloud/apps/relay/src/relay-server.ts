...
function guardSocketErrors(socket: WebSocket, kind: string): void {
  socket.on('error', (error) => {
    console.warn(`[kingu-relay] ${kind} socket error: ${error.message}`)
  })
}
...
export function createRelayServer(
  ...
  const awaitFirstFrame = (
    ...
    socket.once('message', (raw, binary) => {
      ...
      void callback(raw).catch((error: unknown) => {
        console.warn(
          '[kingu-relay] first frame handler failed',
  ...
  server.on('upgrade', (request, socket, head) => {
    ...
    void verifyRelayToken(bearer).then((identity) => {
      ...
      console.warn(
        `[kingu-relay] control upgrade failed: ${error instanceof Error ? error.message : 'unknown'}`
      )
      socket.destroy()
    })
  })
