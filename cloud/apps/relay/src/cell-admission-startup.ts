...
export async function reconcileCellAdmissionAtStartup(
  ...
  for (let attempt = 1; attempt <= STARTUP_RECONCILE_ATTEMPTS; attempt += 1) {
    try {
      ...
      if (attempt > 1) {
        console.warn(
          JSON.stringify({ event: 'kingu_relay_startup_reconcile_recovered', attempts: attempt })
        )
      }
      return
    } catch (error) {
      ...
      if (
        ...
        if (isRelayDatabaseTransientError(error)) {
          console.warn(
            JSON.stringify({ event: 'kingu_relay_startup_reconcile_exhausted', attempts: attempt })
          )
        }
        throw error
      }
