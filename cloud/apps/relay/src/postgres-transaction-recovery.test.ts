...
const databaseUrl = postgresTestDatabaseUrl(process.env.KINGU_RELAY_TEST_POSTGRES_URL)
...
describePostgres('PostgreSQL transaction recovery', () => {
  ...
  it('retries an entire deadlock victim transaction on a fresh attempt', async () => {
    ...
    expect(warn).not.toHaveBeenCalledWith(
      expect.stringContaining('"event":"kingu_relay_postgres_transaction_exhausted"')
    )
    warn.mockRestore()
  }, 10_000)
  ...
  it('defers assignment around a released-cell activity transaction', async () => {
    ...
    expect(warn).not.toHaveBeenCalledWith(
      expect.stringContaining('kingu_relay_postgres_transaction_retry')
    )
    expect(warn).not.toHaveBeenCalledWith(
      expect.stringContaining('kingu_relay_postgres_transaction_exhausted')
    )
