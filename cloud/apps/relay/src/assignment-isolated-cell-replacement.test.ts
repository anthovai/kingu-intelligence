...
// (cloud/docs/kingu-relay-operations.md:229-233) that evacuation targets, Asia
...
describe('re-placing a host off a cell isolated for a roll', () => {
  ...
  it('never re-places across a region boundary, and says so', async () => {
    ...
    expect(jsonEvents(warn, 'kingu_relay_sticky_replacement_deferred')).toEqual([
      {
        event: 'kingu_relay_sticky_replacement_deferred',
        reason: 'no_same_region_headroom',
        cellId: first.cellId,
        region: 'us-central1'
      }
    ])
    expect(jsonEvents(warn, 'kingu_relay_sticky_replaced_off_isolated_cell')).toEqual([])
  })
  ...
  it('emits nothing when the placement rolls back after the decision', async () => {
    ...
    const moved = await store.assign(IDENTITY, 'us-central1')
    expect(moved.cellId).not.toBe(first.cellId)
    expect(jsonEvents(warn, 'kingu_relay_sticky_replaced_off_isolated_cell')).toHaveLength(1)
  })
  ...
  it('logs one event naming both cells, the admission state and the region', async () => {
    ...
    expect(jsonEvents(warn, 'kingu_relay_sticky_replaced_off_isolated_cell')).toEqual([
      {
        event: 'kingu_relay_sticky_replaced_off_isolated_cell',
