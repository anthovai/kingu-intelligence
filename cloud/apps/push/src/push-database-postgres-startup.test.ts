...
describe('PostgreSQL push gateway startup', () => {
  ...
  const socketUrl = `postgresql://push:${encodeURIComponent(socketPassword)}@/kingu_push?host=/cloudsql/test:region:instance`
  ...
  it('applies the schema on an untimed pool that is gone before the serving pool opens', async () => {
    const database = await openPushDatabase({
      databaseUrl: 'postgresql://push@localhost:55440/kingu_push',
      dataDir: '/unused',
      poolMax: 2,
      applicationName: 'kingu-push'
    })
    ...
    expect(fakes.configs[0]).toMatchObject({
      application_name: 'kingu-push/schema',
      lock_timeout: 1_000,
