...
import { applyPostgresSchema } from '@kingu-cloud/postgres-schema'
  ...
  async transaction<T>(operation: (transaction: PushDatabase) => Promise<T>): Promise<T> {
    for (let attempt = 1; attempt <= POSTGRES_TRANSACTION_ATTEMPTS; attempt++) {
      ...
      try {
        ...
        console.warn(
          JSON.stringify({
            event: 'kingu_push_postgres_transaction_retry',
...
export function absorbPostgresIdleClientErrors(pool: Pick<pg.Pool, 'on'>): void {
  pool.on('error', () => {
    ...
    console.warn('[kingu-push] idle PostgreSQL client failed')
  })
}
...
export async function openPushDatabase(input: {
  ...
  if (input.databaseUrl) {
    ...
    const sqlite = new DatabaseSync(join(input.dataDir, 'kingu-push.sqlite'), {
