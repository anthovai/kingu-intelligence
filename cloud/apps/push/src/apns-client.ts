import type { ApnsEnvironment } from '@kingu-cloud/push-contract'
    ...
    try {
      response = await this.options.transport({
        ...
        headers: {
          ...
          'apns-push-type': delivery.kingu.kind === 'dismiss' ? 'background' : 'alert',
          'apns-priority': delivery.kingu.kind === 'dismiss' ? '5' : '10',
          'apns-expiration': String(expiration),
          ...(delivery.kingu.kind === 'dismiss' ? {} : { 'apns-collapse-id': delivery.collapseId })
        },
