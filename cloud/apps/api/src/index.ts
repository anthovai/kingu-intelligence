...
const server = serve({ fetch: createApp(config, { artifacts, skills }).fetch, port: config.port, hostname: '0.0.0.0' }, (info) => {
...
for (const signal of ['SIGINT', 'SIGTERM'] as const) {
  process.on(signal, () => {
    server.close()
    void Promise.all([artifacts.close(), skills.close()]).finally(() => process.exit(0))
  })
}
