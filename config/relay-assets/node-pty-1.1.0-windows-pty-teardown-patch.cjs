 ...
 *     unpatched node-pty;
 *   - the kingud prebuild matrix has no win32 entry (`MATRIX_SLOTS`,
 *     `config/scripts/build-kingud-prebuilds.mjs`), so no Windows binary is ever compiled from
...
function patchNodePtyWindowsTeardown(relayDir = process.cwd()) {
  for (const target of PATCH_TARGETS) {
    ...
    const temporaryPath = `${inspected.filePath}.kingu-patch-${process.pid}`
