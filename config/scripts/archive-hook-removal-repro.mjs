 ...
 * and drives `kingu worktree rm` end to end against real git worktrees on disk.
 */
...
function ok(args) {
  ...
  if (parsed.ok === false) {
    throw new Error(
      `kingu ${args.join(' ')} failed: ${parsed.error?.code} ${parsed.error?.message}`
    )
  }
...
/** A throwaway git repo with one commit; optionally an kingu.yaml archive hook. */
function seedGitRepo(label, withHook, githubSlug) {
  ...
  git(dir, 'init', '-b', 'main')
  git(dir, 'config', 'user.email', 'verify@kingu.test')
...
/** Filesystem + git truth about a worktree, read directly rather than through Kingu. */
...
async function main() {
  ...
  try {
    ...
    out('\n$ kingu worktree rm --worktree <id> --run-hooks     (human output)')
    ...
    out(
      '\n$ kingu worktree rm --worktree <id> --force --run-hooks --json   (--force must NOT waive)'
    )
    ...
    check('Kingu still resolves the worktree', shown?.id === wt1.id)
    check(
    ...
    out(`\n$ kingu worktree rm --worktree <folder workspace> --force --run-hooks`)
