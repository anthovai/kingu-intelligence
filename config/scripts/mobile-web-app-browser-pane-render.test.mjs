...
async function openPane({ grants }) {
  ...
  await page.addInitScript(() => {
    globalThis.__kinguRenderCheckCsp = []
    document.addEventListener('securitypolicyviolation', (event) => {
      globalThis.__kinguRenderCheckCsp.push({
  ...
  return {
    ...
    consoleErrors,
    foreignRequests,
    csp: () => page.evaluate(() => globalThis.__kinguRenderCheckCsp)
  }
}
...
function emitFrame(page, { b64, frameSeq, width, height, pageScaleFactor = 1 }) {
  return page.evaluate(
    ({ b64, frameSeq, width, height, pageScaleFactor, source }) => {
      const subscription = globalThis.__kinguRenderCheckSubscribes.at(-1)
      if (!subscription) {
        return 'no-subscription'
      }
      return globalThis.__kinguRenderCheckEmitBinary(subscription.id, {
        b64,
        format: 'jpeg',
        frameSeq,
        metadata: {
...
describePane('the browser pane in a page', () => {
  ...
  it('files no CSP violation at all, through load and first paint', async () => {
    ...
    try {
      await view.page.waitForFunction(() => globalThis.__kinguRenderCheckSubscribes.length > 0)
  ...
  it('subscribes over the binary lane and paints the frame it is handed', async () => {
    ...
    try {
      await view.page.waitForFunction(() => globalThis.__kinguRenderCheckSubscribes.length > 0)
      const subscribes = await view.page.evaluate(() => globalThis.__kinguRenderCheckSubscribes)
  ...
  it('drops an over-cap frame, keeps the stream, and paints the next one', async () => {
    ...
    try {
      ...
      expect(await view.page.evaluate(() => globalThis.__kinguRenderCheckDroppedFrames)).toEqual([
        2
      ])
      expect(await view.page.evaluate(() => globalThis.__kinguRenderCheckSubscribes.length)).toBe(1)
      expect(await view.csp()).toEqual([])
      expect(view.consoleErrors).toEqual([])
    } finally {
      await view.context.close()
  ...
  it('asks for no binary lane at all when the shell withholds the grant', async () => {
    ...
    try {
      await view.page.waitForFunction(() =>
        document.body.innerText.includes('Update the Kingu app to stream browser tabs here.')
      )
      expect(await view.page.evaluate(() => globalThis.__kinguRenderCheckSubscribes)).toEqual([])
      expect(view.consoleErrors).toEqual([])
      expect(await view.csp()).toEqual([])
      expect(view.foreignRequests).toEqual([])
    } finally {
  ...
  it('streams past the unacked window because the page acks, and drops nothing', async () => {
    ...
    try {
      ...
      const acks = await view.page.evaluate(() => globalThis.__kinguRenderCheckAcks)
  ...
  it('charges a JSON event to the window too, so one ack does not drive the ledger negative', async () => {
    ...
    try {
      ...
      const ledger = await view.page.evaluate((protocolVersion) => {
        const id = globalThis.__kinguRenderCheckSubscribes[0].id
        globalThis.__kinguRenderCheckEmitEvent(id, { type: 'probe', payload: 'x'.repeat(4096) })
        const charged = globalThis.__kinguRenderCheckWindow(id)
        globalThis.kinguBridge.postMessage(
          JSON.stringify({ v: protocolVersion, type: 'ack', id, seq: charged.frames })
        )
        return { charged, settled: globalThis.__kinguRenderCheckWindow(id) }
      }, bridgeVersion)
  ...
  it('issues one mouseClick with the geometry the native pane would send', async () => {
    ...
    try {
      ...
      await view.page.waitForFunction(() => globalThis.__kinguRenderCheckRequests.length > 0)

      const requests = await view.page.evaluate(() => globalThis.__kinguRenderCheckRequests)
