...
// Why: under a Git Bash setup runner, Kingu exports KINGU_WORKTREE_PATH in MSYS form (/c/...), which
...
export function runInternalDevSetup({
  ...
  const setupPath = env.KINGU_INTERNAL_DEV_SETUP?.trim()
  ...
  spawnOptionalSetup(spawn, setupPath, env.KINGU_WORKTREE_PATH || cwd, platform, env)
