...
// installed; @kingu/windows-registry is a workspace link and present everywhere.
...
describe('Electron runtime package contract', () => {
  ...
  it('keeps the native Windows registry addon optional and platform-gated', () => {
    ...
    expect(packageJson.optionalDependencies['@kingu/windows-registry']).toBe('workspace:*')
    ...
    expect(pnpmWorkspace.allowBuilds['@kingu/windows-registry']).toBe(false)
    ...
    expect(rebuildScript).toContain("'@kingu/windows-registry'")
    expect(ensureScript).toContain("process.platform === 'win32'")
    expect(ensureScript).toContain("'@kingu/windows-registry'")
    if (windowsAddonsInstalled) {
      expect(packageTargets.win32).toEqual(
        expect.arrayContaining([
          expect.objectContaining({ to: join('node_modules', '@kingu', 'windows-registry') }),
  ...
  it('guards release publishing before electron-builder runs', () => {
    ...
    expect(macReleaseCommand).toContain(' && KINGU_MAC_RELEASE=1 ')
    ...
    expect(releaseCommands.get('linux-arm64')).toContain('KINGU_LINUX_ARM64_RELEASE=1')
  ...
  it('runs the macOS release build in an isolated Blacksmith workflow', () => {
    ...
    expect(publishStep.with.command).toContain('KINGU_MAC_RELEASE=1')
  ...
  it('bumps separate Homebrew casks for stable and RC desktop tags', () => {
    ...
    expect(resolveCaskStep.run).toContain('token="kingu@rc"')
    expect(resolveCaskStep.run).toContain('token="kingu"')
  ...
  it('keeps terminal scale perf wired to the report budget gate', () => {
    ...
    const manualProfileKnobs = [
      ['KINGU_TERMINAL_PERF_FRAME_COUNT', 'frame_count', 'KINGU_E2E_OPENCODE_FRAME_COUNT'],
      [
        'KINGU_TERMINAL_PERF_FRAME_INTERVAL_MS',
        'frame_interval_ms',
        'KINGU_E2E_OPENCODE_FRAME_INTERVAL_MS'
      ],
      [
        'KINGU_TERMINAL_PERF_PRESSURE_OUTPUT_CHARS',
        'pressure_output_chars',
        'KINGU_E2E_OPENCODE_PRESSURE_OUTPUT_CHARS'
      ],
      ['KINGU_TERMINAL_PERF_SCALE_PANES', 'scale_panes', 'KINGU_E2E_OPENCODE_SCALE_PANES'],
      [
        'KINGU_TERMINAL_PERF_SCALE_CROSS_WORKSPACE_PANES',
        'scale_cross_workspace_panes',
        'KINGU_E2E_OPENCODE_SCALE_CROSS_WORKSPACE_PANES'
      ],
      [
        'KINGU_TERMINAL_PERF_SCALE_PRESSURE_PANES',
        'scale_pressure_panes',
        'KINGU_E2E_OPENCODE_SCALE_PRESSURE_PANES'
      ],
      [
        'KINGU_TERMINAL_PERF_SCALE_HIDDEN_PRESSURE_PANES',
    ...
    expect(uploadStep.with.path).toBe('${{ env.KINGU_E2E_TERMINAL_PERF_REPORT_PATH }}')
  })
  ...
  it('keeps platform golden regressions in the manual and release workflows', () => {
    ...
    expect(releaseEvidenceJob.steps.map((step) => step.run ?? '')).toContain(
      'xvfb-run --auto-servernum env SKIP_BUILD=1 KINGU_E2E_FORWARD_APP_LOGS=1 pnpm run test:e2e:terminal-rendering-release-evidence'
    )
  })
})
