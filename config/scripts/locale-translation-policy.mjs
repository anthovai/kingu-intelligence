...
export function shouldPreserveEnglishValue(enValue, key = '') {
  ...
  if (/^https?:\/\//.test(enValue) || enValue.startsWith('kingu://')) {
    return true
  }
...
function applyCjkLatinTermSpacing(localeValue, locale) {
  ...
  if (locale === 'ko') {
    ...
    // boundary — re-glue "Kingu 에"/"PR 을"/"에서는" but keep "Jira 이슈"/"Kingu 로고"/"agent 에뮬레이터".
...
export function repairTranslatedValue({ key, enValue, localeValue, locale }) {
  ...
  if (enValue.includes('kingu://')) {
    result = result.replace(/虎鲸:\/\//g, 'kingu://')
  }

  if (enValue === 'Kingu' || enValue.startsWith('Kingu ')) {
    result = result
      .replaceAll('虎鲸', 'Kingu')
      .replaceAll('逆戟鲸', 'Kingu')
      .replaceAll('シャチ', 'Kingu')
  }

  if (enValue.includes('kingu://')) {
    result = result.replace(/シャチ:\/\//g, 'kingu://')
  }
...
export function repairCatalog(enCatalog, localeCatalog, locale) {
  ...
  if (localeCatalog.menu) {
    if (locale === 'zh') {
      if (localeCatalog.menu.exploreKingu !== '探索 Kingu') {
        localeCatalog.menu.exploreKingu = '探索 Kingu'
        repaired += 1
      }
      if (localeCatalog.menu.gettingStarted !== 'Kingu 入门') {
        localeCatalog.menu.gettingStarted = 'Kingu 入门'
        repaired += 1
      }
    }
    if (locale === 'ko') {
      if (localeCatalog.menu.exploreKingu !== 'Kingu 둘러보기') {
        localeCatalog.menu.exploreKingu = 'Kingu 둘러보기'
        repaired += 1
      }
      if (localeCatalog.menu.gettingStarted !== 'Kingu 시작하기') {
        localeCatalog.menu.gettingStarted = 'Kingu 시작하기'
        repaired += 1
      }
    }
  }
