...
const BRANCH_ENV_KEYS = [
  'KINGU_DEV_DOCK_TITLE',
  'KINGU_DEV_BRANCH',
  'KINGU_DEV_INSTANCE_LABEL',
  'KINGU_DEV_WORKTREE_NAME'
] as const
...
function patchesUnder(dockTitle: string, branch: string) {
  ...
  Object.assign(process.env, {
    KINGU_DEV_DOCK_TITLE: dockTitle,
    KINGU_DEV_BRANCH: branch,
    KINGU_DEV_INSTANCE_LABEL: branch,
    KINGU_DEV_WORKTREE_NAME: branch
  })
  try {
...
describe('dev-electron-bundle-identity', () => {
  it('leaves process.env untouched, including its object identity', () => {
    ...
    patchesUnder('Kingu: some-branch', 'some-branch')
  ...
  it('keeps the bundle display name in step with the name safeStorage keys off', () => {
    // Two independently hardcoded 'Kingu Dev' strings: this one names the bundle (notifications,
  ...
  it('produces byte-identical patches on two different branches', () => {
    ...
    expect(patchesUnder('Kingu: fix-login-crash', 'fix-login-crash')).toEqual(
      patchesUnder('Kingu: perf-2', 'perf-2')
    )
    expect(patchesUnder('Kingu: dev', 'main')).toEqual(
      patchesUnder('Kingu: some-worktree @ feature/x', 'feature/x')
    )
  })

  it('leaks no branch, worktree, or title text into any patched value', () => {
    const branch = 'fix-login-crash'
    const worktree = 'Kingu-safe-storage-lock'
    for (const patch of patchesUnder(`Kingu: ${branch}`, branch)) {
