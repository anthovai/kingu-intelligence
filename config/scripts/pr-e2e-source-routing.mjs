...
export const PR_E2E_SOURCE_ROUTES = [
  ...
  },
  {
    ...
    matches: (file) =>
      /^(?:src\/main\/ephemeral-vm-(?:runtime-(?:service|provisioning-persistence)|failed-start-cleanup)|src\/shared\/(?:ephemeral-vm-runtime-(?:store|feature-store|rollback-projection|runtimes)|ephemeral-vm-recipes|kingu-yaml-hook-types))\.ts$/.test(
        file
      )
  },
  {
  ...
  },
  {
    ...
    // Input.imeSetComposition and so prove Kingu's handling without an input method existing.
  ...
  },
  {
    ...
    // snapshot the client culls rows against. kingu-runtime.ts is included despite its churn: it
    ...
    matches: (file) =>
      isProductSource(file) &&
      /^src\/.*(?:[Cc]lient-?[Hh]osted|browser-host-(?:lease|page|client-page)|browser-client-(?:host|page)|runtime-browser-(?:client-)?page|session-tabs-sync|host-session-snapshot-authority|kingu-runtime(?:-browser)?\.ts|\/runtime-(?:status|types)\.ts)/.test(
        file
      )
  }
]
