...
const binaryPath = path.join(packagePath, '.build', 'release', 'kingu-computer-use-macos')
const appPath = path.join(packagePath, '.build', 'release', 'Kingu Computer Use.app')
const appExecutablePath = path.join(appPath, 'Contents', 'MacOS', 'kingu-computer-use-macos')
...
const bundleId = process.env.KINGU_COMPUTER_MACOS_BUNDLE_ID ?? 'com.anthovai.kingu.computer-use'
const displayName = 'Kingu Computer Use'
...
function buildUniversalBinary() {
  const builtBinaries = universalTriples.map((triple) => {
    ...
    return path.join(packagePath, '.build', triple, 'release', 'kingu-computer-use-macos')
  })
...
function codesignArgs(identity, targetPath) {
  ...
  if (process.env.KINGU_MAC_RELEASE === '1' && identity !== '-') {
...
function resolveSigningIdentity() {
  const explicitIdentity = process.env.KINGU_COMPUTER_MACOS_SIGN_IDENTITY ?? process.env.CSC_NAME
  if (explicitIdentity) {
  ...
  if (process.env.KINGU_MAC_RELEASE !== '1' && developmentMatch) {
...
<dict>
  ...
  <string>kingu-computer-use-macos</string>
  ...
  <string>Kingu Computer Use needs Accessibility permission to read and interact with app interfaces when you ask Kingu to use apps.</string>
  <key>NSScreenCaptureUsageDescription</key>
  <string>Kingu Computer Use needs Screen Recording permission to capture app windows when you ask Kingu to inspect your screen.</string>
</dict>
</plist>
`
}
