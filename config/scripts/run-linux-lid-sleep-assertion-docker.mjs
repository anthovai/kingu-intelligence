...
const container = options.get('--container') ?? `kingu-inhibitor-oracle-${suffix}`
const image = `kingu-inhibitor-oracle:${suffix}`
const docker =
  process.env.KINGU_DOCKER ??
...
const platform =
  process.env.KINGU_DOCKER_PLATFORM ?? (process.arch === 'arm64' ? 'linux/arm64' : 'linux/amd64')
const temp = mkdtempSync(join(tmpdir(), 'kingu-inhibitor-oracle-'))
const remote = `/tmp/kingu-inhibitor-oracle-${suffix}`
