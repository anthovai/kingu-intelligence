...
function Harness() {
  ...
  useEffect(() => {
    globalThis.__kinguMermaidSet = setState
...
function installJitRecorder() {
  ...
  const record = (kind, source) => {
    ...
    globalThis.__kinguJit.push({ kind, source: String(source).slice(0, 60), stack })
  }
 ...
 *
 * `__kinguJit` being non-empty is the precondition: an attribution filter over a list nothing ever
 * wrote to answers "none from the page" for a recorder that was never installed.
 */
async function pageJitCalls(page) {
  const all = await page.evaluate(() => globalThis.__kinguJit)
...
async function show(page, source, contains = null) {
  await page.evaluate(
    (next) => globalThis.__kinguMermaidSet({ mounted: true, source: next }),
    source
  )
...
async function unmount(page) {
  await page.evaluate(() => globalThis.__kinguMermaidSet({ mounted: false, source: '' }))
...
async function readNativeDocument(browser, file) {
  ...
  try {
    await page.addInitScript(() => {
      globalThis.__kinguNativePosts = []
      globalThis.ReactNativeWebView = {
        postMessage: (message) => globalThis.__kinguNativePosts.push(String(message))
      }
    })
    await page.goto(`${origin}/${file}`, { waitUntil: 'load' })
    await page.waitForFunction(() => globalThis.__kinguNativePosts.length > 0, null, {
      timeout: 120_000
    })
    return await page.evaluate(() => {
      const svg = document.querySelector('#c svg')
      return {
        posts: globalThis.__kinguNativePosts,
