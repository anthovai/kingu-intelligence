...
if (!appImageArg) {
  fail('Usage: run-headless-serve-shutdown-docker.mjs --appimage /path/to/kingu.AppImage')
}
...
const image = `kingu-headless-serve-shutdown:${suffix}`
const artifactVolume = `kingu-headless-serve-shutdown-${suffix}`
...
try {
  ...
  docker([
    ...
    [
      ...
      'timeout --kill-after=5s 15s /input/kingu.AppImage --appimage-help > /artifacts/appimage-help.log 2>&1',
      'cd /artifacts',
      'timeout --kill-after=10s 120s /input/kingu.AppImage --appimage-extract > /artifacts/appimage-extract.log 2>&1',
      'mv squashfs-root root',
      launcherExecOverlay
        ? "sed -i 's/^ELECTRON_RUN_AS_NODE=1 /export ELECTRON_RUN_AS_NODE=1\\nexec /' /artifacts/root/resources/bin/kingu-ide"
        : ':',
    ...
    for (const signal of ['INT', 'TERM']) {
      const result = docker(
        [
          ...
          '256m',
          '--name',
          `kingu-headless-serve-shutdown-${entrypoint}-${signal.toLowerCase()}-${suffix}`,
          '-e',
          `KINGU_SIGNAL_TARGET=${signalTarget}`,
          '-e',
          `KINGU_TEST_ENTRYPOINT=${entrypoint}`,
          '-e',
          `KINGU_INT_DELIVERY=${intDelivery}`,
          '-v',
          `${appImage}:/input/kingu.AppImage:ro`,
          '-v',
