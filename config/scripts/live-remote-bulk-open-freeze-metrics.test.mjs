...
describe('live-remote-bulk-open-freeze-metrics', () => {
  ...
  it('builds worktree selectors from id/path', () => {
    expect(
      worktreeSelector({ id: 'repo::C:/Users/neil/kingu/kingu', path: 'C:/Users/neil/kingu/kingu' })
    ).toBe('id:repo::C:/Users/neil/kingu/kingu')
  ...
  it('rejects invalid numeric environment values', () => {
    process.env.KINGU_FREEZE_TEST_NUMBER = 'not-a-number'
    expect(() => readFreezeNumberEnv('KINGU_FREEZE_TEST_NUMBER', 5)).toThrow(
      'Invalid KINGU_FREEZE_TEST_NUMBER'
    )
    delete process.env.KINGU_FREEZE_TEST_NUMBER
    expect(readFreezeNumberEnv('KINGU_FREEZE_TEST_NUMBER', 5)).toBe(5)
  })
