...
beforeAll(async () => {
  ...
  scratch = await mkdtemp(join(tmpdir(), 'kingu-mobile-web-app-drawer-'))
...
function notifyNames() {
  return globalThis.__kinguRenderCheckNotifies.map((frame) => frame.name)
}
  ...
  it('closes the sheet instead of leaving the screen', async () => {
    ...
    try {
      ...
      const claimed = await page.waitForFunction(
        (name) =>
          globalThis.__kinguRenderCheckNotifies.find(
