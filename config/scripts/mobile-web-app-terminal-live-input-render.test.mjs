...
beforeAll(async () => {
  ...
  scratch = await mkdtemp(join(tmpdir(), 'kingu-mobile-web-live-input-'))
...
async function openProbe() {
  ...
  await page.waitForFunction(() => document.documentElement.dataset.kinguWebEntry === 'mounted', {
    timeout: 60_000,
    polling: 250
  })
  await page.waitForFunction(
    () =>
      globalThis.__kinguLiveInputProbe !== undefined ||
      (globalThis.__kinguRenderCheckFaults ?? []).length > 0,
    { timeout: 60_000, polling: 100 }
  )
  const faults = await page.evaluate(() => globalThis.__kinguRenderCheckFaults ?? [])
...
async function typeIntoField(page, text) {
  await page.evaluate((typed) => globalThis.__kinguLiveInputProbe.type(typed), text)
  await page.waitForFunction(
...
describeRender(
  ...
  () => {
    ...
    it('writes the field itself, so a drifted value is cleared without a render', async () => {
      ...
      await page.evaluate(() => globalThis.__kinguLiveInputProbe.clear())
    ...
    it('clears the field when Enter submits, the way it does natively', async () => {
      ...
      await page.waitForFunction(
        () => (globalThis.__kinguLiveInputProbe.sent() ?? []).includes('\r'),
        undefined,
        { timeout: 30_000, polling: 100 }
      )
      ...
      expect(await page.evaluate(() => globalThis.__kinguLiveInputProbe.sent())).toEqual([
        'l',
        's',
        '\r'
      ])
    ...
    it("clears the field when the key bar's Enter chip ends the line", async () => {
      ...
      await page.evaluate(() => globalThis.__kinguLiveInputProbe.accessory({ bytes: '\r' }))
    ...
    it('edits the field from the accessory bar and mirrors the erase to the terminal', async () => {
      ...
      const result = await page.evaluate(() =>
        globalThis.__kinguLiveInputProbe.accessory({ bytes: '\u007f', localEdit: 'backspace' })
      )
    ...
    describe('in buffered mode, where the field holds the draft until Enter', () => {
      ...
      it('sends the draft and empties the field on a plain Enter', async () => {
        ...
        expect(await page.evaluate(() => globalThis.__kinguLiveInputProbe.bufferedSent())).toEqual([
          'ls -la'
        ])
        expect(errors).toEqual([])
        await page.close()
      ...
      it('sends the draft when Enter arrives with the keyboard composition still open', async () => {
        ...
        await page.waitForFunction(
          () => (globalThis.__kinguLiveInputProbe.bufferedSent() ?? []).length > 0,
          undefined,
          { timeout: 30_000, polling: 100 }
        )
        expect(await page.evaluate(() => globalThis.__kinguLiveInputProbe.bufferedSent())).toEqual([
      ...
      it("leaves the draft alone when the key bar's Enter chip fires", async () => {
        ...
        const { errors, page } = await openProbe()
        await page.evaluate(() => globalThis.__kinguLiveInputProbe.setLiveInputEnabled(false))
        await page.focus(`#${BUFFERED_FIELD_ID}`)
        await page.keyboard.type('ls -la')

        const result = await page.evaluate(() =>
          globalThis.__kinguLiveInputProbe.accessory({ bytes: '\r' })
        )

        expect(result).toEqual({ kind: 'allow-raw' })
        expect(await page.evaluate(() => globalThis.__kinguLiveInputProbe.sent())).toEqual(['\r'])
        expect(await page.evaluate(() => globalThis.__kinguLiveInputProbe.bufferedSent())).toEqual(
          []
        )
