...
const CORPUS_REPLIES = {
  ...
  // "Update Kingu on your computer" over the route, which is what the screen showed first. The
...
beforeAll(async () => {
  ...
  scratch = await mkdtemp(join(tmpdir(), 'kingu-mobile-web-app-agent-history-'))
...
function readNotifies(page) {
  return page.evaluate(() => globalThis.__kinguRenderCheckNotifies ?? [])
}
...
describeRender('the agent-history route in a real browser', () => {
  ...
  it('mounts the route, whose panel no unit test renders for real', async () => {
    ...
    const session = await opened.page.evaluate(
      () => document.documentElement.dataset.kinguWebSessionId ?? null
    )
