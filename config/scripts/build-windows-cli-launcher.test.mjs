...
describe('Windows CLI launcher', () => {
  it('reuses a compiled launcher that is at least as new as the C# source', () => {
    const root = mkdtempSync(join(tmpdir(), 'kingu-cli-launcher-reuse-'))
    try {
      const sourcePath = join(root, 'KinguCliLauncher.cs')
      const outputPath = join(root, '.build', 'kingu.exe')
  ...
  itCrossHost('fails closed when the Windows launcher cannot be compiled on this host', () => {
    const outputRoot = mkdtempSync(join(tmpdir(), 'kingu cross-host launcher '))
    try {
  ...
  itCrossHost('never materializes the child environment block from ProcessStartInfo', () => {
    ...
    // dictionary that throws when the inherited block holds PATH and Path (anthovai/kingu-intelligence#12046).
    const source = readFileSync(
      join(projectRoot, 'native', 'windows-cli-launcher', 'KinguCliLauncher.cs'),
      'utf8'
    )
  ...
  itWindows('preserves a multiline argument from PowerShell through the native launcher', () => {
    const appRoot = mkdtempSync(join(tmpdir(), 'kingu cli launcher '))
    try {
      const resourcesPath = join(appRoot, 'resources')
      const launcherPath = join(resourcesPath, 'bin', 'kingu.exe')
      const cliPath = join(resourcesPath, 'app.asar.unpacked', 'out', 'cli', 'index.js')
      mkdirSync(join(resourcesPath, 'bin'), { recursive: true })
      mkdirSync(dirname(cliPath), { recursive: true })
      copyFileSync(process.execPath, join(appRoot, 'Kingu.exe'))
      writeFileSync(
        cliPath,
  ...
  kinguNodeOptions: process.env.KINGU_NODE_OPTIONS ?? null
}))\n`,
        'utf8'
      )
      ...
      const powershell = spawnSync(
        ...
        [
          ...
          '-NonInteractive',
          '-Command',
          '& $env:KINGU_TEST_LAUNCHER orchestration send --body $env:KINGU_TEST_BODY --json'
        ],
        {
          encoding: 'utf8',
          env: {
            ...
            KINGU_TEST_BODY: body,
            KINGU_TEST_LAUNCHER: launcherPath
          }
        }
      )
  ...
  itWindows('survives an inherited environment block containing PATH and Path', () => {
    const appRoot = mkdtempSync(join(tmpdir(), 'kingu duplicate path launcher '))
    try {
      const resourcesPath = join(appRoot, 'resources')
      const launcherPath = join(resourcesPath, 'bin', 'kingu.exe')
      const cliPath = join(resourcesPath, 'app.asar.unpacked', 'out', 'cli', 'index.js')
      ...
      writeFileSync(
        cliPath,
        `require('node:fs').writeFileSync(process.env.KINGU_TEST_OUTPUT, JSON.stringify({
  electronRunAsNode: process.env.ELECTRON_RUN_AS_NODE,
