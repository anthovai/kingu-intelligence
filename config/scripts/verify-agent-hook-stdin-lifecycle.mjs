...
function withoutKinguEnvironment(extra = {}) {
  return {
    ...Object.fromEntries(Object.entries(process.env).filter(([key]) => !key.startsWith('KINGU_'))),
    ...extra
  }
}
...
function readGeneratedScripts(home, minMtime) {
  const hooksDir = join(home, '.kingu', 'agent-hooks')
...
async function verifyNoOpWrites(scripts, home, payload) {
  const commandCodeBin = mkdtempSync(join(tmpdir(), 'kingu-hook-command-code-bin-'))
  ...
  try {
    ...
    const result = await runShell(
      ...
      withoutKinguEnvironment({
        ...
        HOME: home,
        KINGU_AGENT_HOOK_ENDPOINT: '',
        KINGU_AGENT_HOOK_PORT: String(address.port),
        KINGU_AGENT_HOOK_TOKEN: 'electron-verification-token',
        KINGU_PANE_KEY: 'electron-verification-pane'
      })
    )
    ...
    for (const script of scripts) {
      ...
      const result = await runShell(
        ...
        withoutKinguEnvironment({
          ...
          KINGU_AGENT_HOOK_TOKEN: 'electron-verification-token',
          KINGU_PANE_KEY: 'electron-verification-pane',
          KINGU_TAB_ID: 'electron-verification-tab',
          KINGU_WORKTREE_ID: 'electron-verification-worktree',
          KINGU_AGENT_HOOK_ENV: 'test',
          KINGU_AGENT_HOOK_VERSION: '1',
          KINGU_ANTIGRAVITY_EVENT: 'PostInvocation',
          KINGU_COPILOT_HOOK_EVENT: 'PostToolUse'
        })
      )
      ...
      if (request.headers['x-kingu-agent-hook-token'] !== 'electron-verification-token') {
...
async function verifyInstalledLauncher(home, payload) {
  ...
  if (
    !command ||
    !command.includes('"${HOME-}/.kingu/agent-hooks/claude-hook.sh"') ||
    !command.includes('] && [ -r ') ||
    !command.includes('else { command -p cat')
  ) {
    throw new Error('Electron did not install the guarded Claude launcher')
  }
  const scratch = mkdtempSync(join(tmpdir(), 'kingu-hook-launcher-'))
  try {
    const missingResult = await runShell(
      command,
      payload,
      withoutKinguEnvironment({ HOME: scratch })
    )
    assertSuccessfulWrite(missingResult, 'installed missing-script launcher')

    const failingPath = join(scratch, '.kingu', 'agent-hooks', 'claude-hook.sh')
    mkdirSync(join(scratch, '.kingu', 'agent-hooks'), { recursive: true })
