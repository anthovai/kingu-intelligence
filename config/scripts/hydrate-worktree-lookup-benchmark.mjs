...
const ITERATIONS = Number(process.env.KINGU_HYDRATE_BENCH_ITERATIONS ?? '60')
const WARMUP = Number(process.env.KINGU_HYDRATE_BENCH_WARMUP ?? '10')
const ROUNDS = 6

for (const [name, value] of [
  ['KINGU_HYDRATE_BENCH_ITERATIONS', ITERATIONS],
  ['KINGU_HYDRATE_BENCH_WARMUP', WARMUP]
]) {
...
// Row shapes are synthetic, sized against a real kingu-data.json on a heavy machine
