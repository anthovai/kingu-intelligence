#!/usr/bin/env node
// Builds the kingu-notification-status helper binary.
//
...
const bundleId = readArg('--bundle-id') ?? 'com.anthovai.kingu'
...
const workDir = path.join(tmpdir(), `kingu-notification-status-${process.pid}`)
...
try {
  ...
  const builtBinaries = triples.map((triple) => {
    const output = path.join(workDir, `kingu-notification-status-${triple}`)
    execFileSync(
      'swiftc',
      [
        '-O',
...
<dict>
  ...
  <string>kingu-notification-status</string>
</dict>
</plist>
`
}
