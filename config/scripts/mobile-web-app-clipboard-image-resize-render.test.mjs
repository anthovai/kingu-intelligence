...
globalThis.__kinguResizeCheck = async ({ width, height, maxBase64Length, attempts }) => {
...
globalThis.__kinguResizeRefusal = async (source) => {
  try {
...
async function openPage() {
  ...
  await page.addInitScript(() => {
    ...
    document.addEventListener('securitypolicyviolation', (event) => {
      globalThis.__kinguCspViolations.push(event.violatedDirective)
    })
  })
...
describeResize(
  ...
  () => {
    it('brings a noise PNG under the upload chunk, and says what it weighed', async () => {
      ...
      try {
        const measured = await page.evaluate((args) => globalThis.__kinguResizeCheck(args), {
          ...FIXTURE,
    ...
    it('rejects a source the browser cannot decode rather than waiting on a load', async () => {
      ...
      try {
        ...
        const message = await page.evaluate(
          (source) => globalThis.__kinguResizeRefusal(source),
          'bm90LWFuLWltYWdl'
        )
