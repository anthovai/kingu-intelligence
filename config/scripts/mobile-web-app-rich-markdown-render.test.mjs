...
function Harness() {
  ...
  const editor = (key, withHandle, content, onChanged) =>
    createElement(MobileRichMarkdownEditor, {
      ...
      onChange: onChanged,
      onOpenLink: (url) => globalThis.__kinguEditor.links.push(url),
      onKeyboardInsetChange: (bottom) => globalThis.__kinguEditor.insets.push(bottom)
    })
      ...
      state.mounted
        ? createElement(
            ...
            editor('first-' + state.generation, true, state.content, (next) => {
              globalThis.__kinguEditor.changes.push(next)
      ...
      state.both
        ? createElement(
            ...
            editor('second', false, state.secondContent, (next) => {
              globalThis.__kinguEditor.secondChanges.push(next)
...
function installBridgeObjectRecorder() {
  globalThis.__kinguBridgeReads = []
  const bridge = {
    postMessage: (message) => globalThis.__kinguBridgeReads.push(`post ${String(message)}`)
  }
  Object.defineProperty(globalThis, 'ReactNativeWebView', {
    configurable: true,
    get: () => {
      globalThis.__kinguBridgeReads.push('read')
      return bridge
    }
  })
}
...
async function setContent(page, markdown, html) {
  await page.evaluate((next) => globalThis.__kinguEditor.set({ content: next }), markdown)
      ...
      describe(engine.name, () => {
        ...
        it('mounts the document, reports ready through the seam and never touches the bridge', async () => {
          ...
          try {
            ...
            expect(await page.evaluate(() => globalThis.__kinguBridgeReads)).toEqual([])
            expect(await page.evaluate(() => globalThis.__kinguCspViolations)).toEqual([])
            ...
            expect(await page.evaluate(() => globalThis.__kinguEditor.insets)).toEqual([])
          } finally {
            await page.close()
          }
        }, 600_000)
          ...
          try {
            ...
            const variable = await page.evaluate(() => ({
              onRoot: getComputedStyle(document.documentElement)
                ...
                .trim(),
              onHost: getComputedStyle(document.querySelector('.kingu-rich-markdown-document-host'))
                .getPropertyValue('--editor-surface')
                .trim()
            }))
        ...
        it('reports a typed bullet list as a list, and renders that markdown back as one', async () => {
          ...
          try {
            ...
            await select(page, 'all')
            await page.evaluate(() => {
              globalThis.__kinguEditor.changes.length = 0
            })
            ...
            await page.waitForFunction(() => globalThis.__kinguEditor.changes.length > 0, null, {
              timeout: 15_000
            })
            ...
            expect(await page.evaluate(() => globalThis.__kinguEditor.changes.at(-1))).toBe(
              `- ${body}`
            )
        ...
        it('reports one change per checkbox tap and one per inline code', async () => {
          ...
          try {
            ...
            await page.waitForFunction(() => globalThis.__kinguEditor.changes.length > 0)
            // One tap raises click, input and change, and each of the three used to report.
            expect(await page.evaluate(() => globalThis.__kinguEditor.changes)).toHaveLength(1)
            expect(await page.evaluate(() => globalThis.__kinguEditor.changes[0])).toContain('[x]')
        ...
        it('opens a link through the host rather than navigating the page', async () => {
          ...
          try {
            ...
            await page.waitForFunction(() => globalThis.__kinguEditor.links.length > 0)
            expect(await page.evaluate(() => globalThis.__kinguEditor.links)).toEqual([
              'https://example.com/a'
            ])
        ...
        it('leaves no listener, timer or frame of the first mount in the second', async () => {
          const { page, consoleErrors } = await openPage(browser)
          const listeners = () => page.evaluate(() => globalThis.__kinguListeners.snapshot())
          try {
            ...
            await page.evaluate(() => globalThis.__kinguEditor.set({ mounted: false }))
            ...
            await page.evaluate(() => {
              ...
              globalThis.__kinguEditor.set({ mounted: true, generation: 1 })
            })
            await page.waitForSelector('#first-surface #editor')
            ...
            expect(await page.evaluate(() => globalThis.__kinguScheduler.leaked)).toEqual([])
            expect(await page.evaluate(() => globalThis.__kinguCspViolations)).toEqual([])
            expect(consoleErrors).toEqual([])
            // The sheet stays in the head across mounts, and there is one of it.
            expect(
              await page.evaluate(
                () => document.querySelectorAll('#kingu-rich-markdown-document-style').length
              )
            ).toBe(1)
          } finally {
            await page.close()
        ...
        it('gives two editors on one page their own surfaces', async () => {
          ...
          try {
            ...
            await page.evaluate(() =>
              globalThis.__kinguEditor.set({ both: true, secondContent: 'second only' })
            )
            ...
            await page.waitForFunction(() =>
              globalThis.__kinguEditor.secondChanges.includes('typed in the second')
            )
            // The edit reached the second editor's own host, and the first document neither
            // reported it nor lost its content.
            expect(await page.evaluate(() => globalThis.__kinguEditor.changes)).not.toContain(
              'typed in the second'
            )
            expect(await readSurface(page, '#first-surface')).toContain('first only')
            expect(consoleErrors).toEqual([])
