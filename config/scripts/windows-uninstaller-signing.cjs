// Why this exists: the NSIS uninstaller is the one Kingu binary SignPath never
...
//
// Kingu does not sign during electron-builder — SignPath signs afterwards, behind
...
function signWindowsUninstallerViaSignPath(configuration) {
  const paths = {
    ...
    exportPath: process.env.KINGU_WIN_UNINSTALLER_EXPORT_PATH || undefined,
    signedPath: process.env.KINGU_WIN_UNINSTALLER_SIGNED_PATH || undefined
  }
