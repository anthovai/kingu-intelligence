...
function run(entry, argv, env) {
  ...
  if (result.error) {
    if (result.error.code === 'ETIMEDOUT') {
      throw new Error(
        `kingu ${argv.join(' ')} did not exit within ${RUN_TIMEOUT_MS} ms — it reached a blocking command`
      )
    }
    throw result.error
  }
...
function buildCases(isolatedUserData) {
  const isolated = { KINGU_USER_DATA_PATH: isolatedUserData }
  const cases = [
    ...
    [['no-such-command'], {}],
    [['no-such-command'], { KINGU_PAIRING_CODE: 'garbage' }],
    [['wrktree', 'list'], {}],
    [['agent-context'], {}],
    [['agent-context', '--json'], {}],
    ...
    [['status', '--pairing-code', 'kingu://pair?code=zzzz'], isolated],
    ...
    [['status', '--json'], { ...isolated, KINGU_PAIRING_CODE: 'not-a-pairing-code' }],
    [['status', '--json'], { ...isolated, KINGU_REMOTE_PAIRING: 'not-a-pairing-code' }],
    [['status', '--json'], { ...isolated, KINGU_ENVIRONMENT: 'no-such-environment' }],
    ...
    // KINGU_USER_DATA_PATH. They are deliberately NOT invoked here. The
    ...
    [['environment', 'list', '--json'], { ...isolated, KINGU_ENVIRONMENT: 'no-such-environment' }],
    [['environment', 'list', '--json'], { ...isolated, KINGU_PAIRING_CODE: 'not-a-pairing-code' }],
    [['agent-context', '--json'], { ...isolated, KINGU_PAIRING_CODE: 'not-a-pairing-code' }],
...
//
//   1. FOREGROUND — `kingu serve` runs Kingu until Ctrl+C and `kingu open` /
...
//   2. MUTATING — writes outside KINGU_USER_DATA_PATH (`agent hooks off` parks
...
const UNSAFE_TOKENS = new Map([
  ...
  ...MUTATING_TOKENS.map((token) => [token, 'can write outside KINGU_USER_DATA_PATH'])
])
