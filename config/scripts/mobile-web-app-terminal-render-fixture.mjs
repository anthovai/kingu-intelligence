 ...
 * to fail — no Chromium on the machine, an `KINGU_MOBILE_WEB_RENDER_BROWSER` that points nowhere —
...
export async function startTerminalRenderFixture() {
  ...
  const scratch = await mkdtemp(join(tmpdir(), 'kingu-c75-terminal-render-'))
  let browser = null
  let served = null
  try {
  ...
  async function openTerminal(options) {
    ...
    await opened.page.waitForFunction(() => globalThis.__kinguTerminalReady === true, {
      timeout: 60_000,
      polling: 100
    })
    return opened
...
export async function openProbeTerminal(page) {
  ...
  await page.evaluate(
    ([cols, rows]) => globalThis.__kinguTerminalProbe.init(cols, rows, ''),
    [COLS, ROWS]
  )
  ...
  await page.evaluate(() => globalThis.__kinguTerminalProbe.awaitReady())
}
