...
export const MAX_KINGU_RPC_OUTPUT_BYTES = 20 * 1024 * 1024

export function appendKinguRpcOutput(output, chunk, bytes, limit = MAX_KINGU_RPC_OUTPUT_BYTES) {
...
export function resolveKinguCliCommand({ env = process.env, platform = process.platform } = {}) {
  if (env.KINGU_CLI_COMMAND?.trim()) {
    return env.KINGU_CLI_COMMAND.trim()
  }
...
export function resolveKinguCliInvocation({
  env = process.env,
  platform = process.platform,
  nodeExecutable = process.execPath
} = {}) {
  const command = resolveKinguCliCommand({ env, platform })
  const commandName = platform === 'win32' ? path.win32.basename(command).toLowerCase() : command
  if (
    platform === 'win32' &&
    env.KINGU_DEV_REPO_ROOT &&
    (commandName === 'kingu-dev' || commandName === 'kingu-dev.cmd')
  ) {
    ...
    return {
      ...
      prefixArgs: [path.win32.join(env.KINGU_DEV_REPO_ROOT, 'out', 'cli', 'index.js')],
      env: {
        ...env,
        KINGU_USER_DATA_PATH:
          env.KINGU_USER_DATA_PATH ?? env.KINGU_DEV_USER_DATA_PATH ?? defaultUserDataPath,
        KINGU_DEV_CLI_INVOCATION: '1',
        KINGU_APP_EXECUTABLE:
          env.KINGU_APP_EXECUTABLE ??
          path.win32.join(
            env.KINGU_DEV_REPO_ROOT,
            'node_modules',
            'electron',
...
export function createKinguRpc({
  envName,
  cliCommand,
  env = process.env,
  platform = process.platform
  ...
  const cliInvocation = cliCommand
    ...
    : resolveKinguCliInvocation({ env, platform })
  const commandLabel = cliCommand ?? resolveKinguCliCommand({ env, platform })
  ...
  function kinguJsonAsync(args, opts = {}) {
    const started = performance.now()
    return new Promise((resolve, reject) => {
      ...
      const append = (stream, chunk) => {
        ...
        const appended = appendKinguRpcOutput(stream, chunk, outputBytes)
  ...
  async function runReconnectRefreshStorm(notes) {
    const started = performance.now()
    const jobs = [
      () => kinguJsonAsync(['status'], { timeoutMs: 90_000 }),
      () => kinguJsonAsync(['worktree', 'list'], { timeoutMs: 120_000 }),
      () => kinguJsonAsync(['terminal', 'list'], { timeoutMs: 120_000 }),
      () => kinguJsonAsync(['status'], { local: true, timeoutMs: 60_000 }),
      () => kinguJsonAsync(['worktree', 'list'], { timeoutMs: 120_000 }),
      () => kinguJsonAsync(['terminal', 'list'], { timeoutMs: 120_000 })
    ]
  ...
  async function runRestartProxy(notes) {
    const started = performance.now()
    try {
      const opened = await kinguJsonAsync(['open'], { local: true, timeoutMs: 120_000 })
      notes.push(`kingu open ms=${opened.elapsedMs.toFixed(0)}`)
    } catch (error) {
      notes.push(`kingu open failed: ${String(error).slice(0, 200)}`)
    }
    const storm = await runReconnectRefreshStorm(notes)
    return { wallMs: performance.now() - started, storm }
  }

  return { kinguJsonSync, kinguJsonAsync, runReconnectRefreshStorm, runRestartProxy }
}
