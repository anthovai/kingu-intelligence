...
async function terminalCspViolations(page) {
  const seen = await page.evaluate(() => globalThis.__kinguCspViolations)
...
describeRender(
  ...
  () => {
    it('records what the page refuses before any terminal is on it', async () => {
      ...
      await page.waitForFunction(() => globalThis.__kinguTerminalControlMounted === true, {
        timeout: 60_000,
        polling: 100
      })
      controlCspViolations = await page.evaluate(() => globalThis.__kinguCspViolations)
    ...
    it('opens xterm under the shipped policy and paints a dense stream into its buffer', async () => {
      ...
      const applied = await page.evaluate((data) => {
        globalThis.__kinguTerminalProbe.write(data)
        return data.length
      }, stream)
      ...
      await page.evaluate(() => globalThis.__kinguTerminalProbe.selectAll())
      ...
      await page.waitForFunction(() => typeof globalThis.__kinguTerminalCopied === 'string', {
        timeout: 30_000,
        polling: 100
      })
      const copied = await page.evaluate(() => globalThis.__kinguTerminalCopied)
      console.log(
        '[c7.5][stream]',
      ...
      expect(await page.evaluate(() => globalThis.__kinguTerminalEngineErrors)).toEqual([])
    ...
    it('leaves the page its own window.onerror across mount and dispose', async () => {
      ...
      expect(await page.evaluate(() => window.onerror === globalThis.__kinguSentinel)).toBe(true)
      await openProbeTerminal(page)
      expect(await page.evaluate(() => window.onerror === globalThis.__kinguSentinel)).toBe(true)
      ...
      await page.evaluate(() => {
        setTimeout(() => {
          throw new Error('kingu-terminal-render-uncaught')
        }, 0)
      })
      const sawIt = (entries) =>
        entries.some((entry) => entry.includes('kingu-terminal-render-uncaught'))
      await page.waitForFunction(
        () =>
          globalThis.__kinguTerminalEngineErrors.some((entry) =>
            entry.includes('kingu-terminal-render-uncaught')
          ),
        { timeout: 30_000, polling: 100 }
      )
      expect(sawIt(await page.evaluate(() => globalThis.__kinguSentinelCalls))).toBe(true)

      // Dispose takes the terminal's listener off and leaves the page's handler where it was.
      await page.evaluate(() => globalThis.__kinguTerminalProbe.setMounted(false))
      await page.locator('#terminal-container').waitFor({ state: 'detached', timeout: 30_000 })
      expect(await page.evaluate(() => window.onerror === globalThis.__kinguSentinel)).toBe(true)
      const before = await page.evaluate(() => {
        setTimeout(() => {
          throw new Error('kingu-terminal-render-after-dispose')
        }, 0)
        return globalThis.__kinguTerminalEngineErrors.length
      })
      await page.waitForFunction(
        () =>
          globalThis.__kinguSentinelCalls.some((entry) =>
            entry.includes('kingu-terminal-render-after-dispose')
          ),
        { timeout: 30_000, polling: 100 }
      )
      // The page's handler saw it and the terminal's did not, which is what dispose has to mean.
      expect(await page.evaluate(() => globalThis.__kinguTerminalEngineErrors.length)).toBe(before)
      await page.close()
    }, 300_000)
    ...
    async function listenersWithNoTerminal(page) {
      await page.evaluate(() => globalThis.__kinguTerminalProbe.setMounted(false))
      await page.locator('#terminal-container').waitFor({ state: 'detached', timeout: 30_000 })
      return page.evaluate(() => globalThis.__kinguListeners.snapshot())
    }
    ...
    async function assertLiveTerminal(page, label) {
      ...
      const marker = `kingu-remount-${label}`
      await page.evaluate((thrown) => {
        globalThis.__kinguTerminalEngineErrors = []
        setTimeout(() => {
          throw new Error(thrown)
        }, 0)
      }, marker)
      await page.waitForFunction(
        (thrown) => globalThis.__kinguTerminalEngineErrors.some((entry) => entry.includes(thrown)),
        marker,
        { timeout: 30_000, polling: 100 }
      )
    }
    ...
    it('is a live terminal again after an unmount and a remount', async () => {
      ...
      await page.locator('#terminal-container').waitFor({ state: 'detached', timeout: 30_000 })
      await page.evaluate(() => {
        globalThis.__kinguTerminalReady = false
        globalThis.__kinguTerminalProbe.setMounted(true)
      })
      await page.waitForFunction(() => globalThis.__kinguTerminalReady === true, {
        timeout: 60_000,
        polling: 100
      })
      await openProbeTerminal(page)
    ...
    it('is a live terminal again after the user reloads a failed one', async () => {
      ...
      await page.evaluate(() => {
        setTimeout(() => {
          throw new Error('kingu-terminal-render-fatal')
        }, 0)
      })
    ...
    it('leaves the page the listeners it found, across a mount and a dispose', async () => {
      ...
      await openProbeTerminal(page)
      const whileLive = await page.evaluate(() => globalThis.__kinguListeners.snapshot())
    ...
    it('still reports runtime errors after a first mount spent the non-fatal budget', async () => {
      ...
      await page.evaluate(() => {
        for (let index = 0; index < 6; index++) {
          setTimeout(() => {
            throw new Error(`kingu-budget-burn-${String(index)}`)
          }, 0)
        }
      })
      await page.waitForFunction(
        () =>
          globalThis.__kinguTerminalEngineErrors.filter((entry) =>
            entry.includes('kingu-budget-burn')
          ).length >= 5,
        { timeout: 30_000, polling: 100 }
      )
      ...
      await page.evaluate(() => {
        globalThis.__kinguTerminalEngineErrors = []
        setTimeout(() => {
          throw new Error('kingu-second-mount-error')
        }, 0)
      })
      await page.waitForFunction(
        () =>
          globalThis.__kinguTerminalEngineErrors.some((entry) =>
            entry.includes('kingu-second-mount-error')
          ),
        { timeout: 30_000, polling: 100 }
      )
      await page.close()
    ...
    it('cancels the timers it armed, so none of the first mount fires into the second', async () => {
      ...
      await page.evaluate(() => {
        globalThis.__kinguScheduler.watching = true
        ...
        globalThis.__kinguTerminalProbe.setMounted(false)
      })
      await page.locator('#terminal-container').waitFor({ state: 'detached', timeout: 30_000 })
      await page.evaluate(() => {
        globalThis.__kinguTerminalReady = false
      ...
      const scheduler = await page.evaluate(() => globalThis.__kinguScheduler)
    ...
    it('takes back the frames it is owed, not only the timers', async () => {
      ...
      await page.evaluate((chunk) => {
        const state = globalThis.__kinguScheduler
        state.disposed = null
        state.watching = true
        ...
        const host = document.querySelector('.kingu-terminal-document-host')
        const observer = new MutationObserver(() => {
          if (state.disposed !== null || host.classList.contains('kingu-terminal-document-host')) {
            return
          }
          state.disposed = {
        ...
        requestAnimationFrame(pulse)
        globalThis.setTimeout(() => globalThis.__kinguTerminalProbe.setMounted(false), 200)
      }, documentChunk)
      await page.locator('#terminal-container').waitFor({ state: 'detached', timeout: 30_000 })
      await page.evaluate(() => {
        globalThis.__kinguTerminalReady = false
    ...
    it('measures a fit through the handle and records what beforeinput reports', async () => {
      ...
      const fit = await page.evaluate(() => globalThis.__kinguTerminalProbe.measure())
      ...
      await page.waitForFunction(() => globalThis.__kinguTerminalBeforeInput.length >= 2, {
        timeout: 30_000,
        polling: 100
      })
      const beforeInput = await page.evaluate(() => globalThis.__kinguTerminalBeforeInput)
