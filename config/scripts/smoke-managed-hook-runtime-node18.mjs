...
const home = await mkdtemp(join(tmpdir(), 'kingu-managed-hook-node18-'))
...
try {
  ...
  assert.match(codexHooks, /\.kingu\/agent-hooks\/codex-hook\.sh/)
  assert.match(claudeSettings, /\.kingu\/agent-hooks\/claude-hook\.sh/)
  await access(join(home, '.kingu', 'agent-hooks', 'codex-hook.sh'), constants.X_OK)
  await access(join(home, '.kingu', 'agent-hooks', 'claude-hook.sh'), constants.X_OK)
} finally {
