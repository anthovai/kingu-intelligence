...
export const JA_UNIFIED_VALUES = {
  ...
  'Show Kingu Mobile Button': 'Kingu Mobile ボタンを表示',
  Paste: '貼り付け',
  ...
  'Show the Kingu Mobile button at the top of the left sidebar.':
    '左サイドバー上部に Kingu Mobile ボタンを表示します。',
  'Enable Kingu CLI': 'Kingu CLI を有効化',
  'Registered the Kingu CLI in PATH.': 'Kingu CLI を PATH に登録しました。',
  ...
  'Learn more': '詳しく見る',
  'Gitea credentials are configured but could not authenticate. Check the token, API base URL, and repository permissions, then restart Kingu if environment variables changed.':
    'Gitea の認証情報は設定されていますが、認証できませんでした。トークン、API ベース URL、リポジトリの権限を確認し、環境変数を変更した場合は Kingu を再起動してください。',
  'Azure DevOps credentials are configured but could not authenticate. Check the token, API base URL, and repository permissions, then restart Kingu if environment variables changed.':
    'Azure DevOps の認証情報は設定されていますが、認証できませんでした。トークン、API ベース URL、リポジトリの権限を確認し、環境変数を変更した場合は Kingu を再起動してください。',
  'Bitbucket credentials are configured but could not authenticate. Check the token and repository permissions, then restart Kingu if environment variables changed.':
    'Bitbucket の認証情報は設定されていますが、認証できませんでした。トークンとリポジトリの権限を確認し、環境変数を変更した場合は Kingu を再起動してください。',
  ...
  Plugins: 'プラグイン',
  'Kingu will continue to work, but commands such as kingu status, kingu terminal, and orchestration are unavailable for this session.\n\n{{guidance}}\n\nCause: {{cause}}':
    'Kingu は引き続き使用できますが、このセッションでは kingu status や kingu terminal などのコマンドとオーケストレーションを利用できません。\n\n{{guidance}}\n\n原因: {{cause}}'
}
