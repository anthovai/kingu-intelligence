...
const CANDIDATE_COUNT = Number(process.env.KINGU_PENDING_MARKER_BENCH_CANDIDATES ?? '64')
const TOTAL_TICKS = Number(process.env.KINGU_PENDING_MARKER_BENCH_TICKS ?? '900')
const STEADY_TICKS = Number(process.env.KINGU_PENDING_MARKER_BENCH_STEADY_TICKS ?? '300')

for (const [name, value] of [
  ['KINGU_PENDING_MARKER_BENCH_CANDIDATES', CANDIDATE_COUNT],
  ['KINGU_PENDING_MARKER_BENCH_TICKS', TOTAL_TICKS],
  ['KINGU_PENDING_MARKER_BENCH_STEADY_TICKS', STEADY_TICKS]
]) {
...
if (STEADY_TICKS >= TOTAL_TICKS) {
  throw new Error('KINGU_PENDING_MARKER_BENCH_STEADY_TICKS must be smaller than total ticks')
}

const root = await mkdtemp(join(os.tmpdir(), 'kingu-pending-marker-bench-'))
