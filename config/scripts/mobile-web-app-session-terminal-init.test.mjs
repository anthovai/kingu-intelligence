...
const SCROLLBACK_MARKER = 'kingu-terminal-init-marker'
...
beforeAll(async () => {
  ...
  scratch = await mkdtemp(join(tmpdir(), 'kingu-mobile-web-app-terminal-init-'))
...
async function openSessionWithTerminalTab() {
  ...
  await page.evaluate(
    ([id, event]) => globalThis.__kinguRenderCheckEmitEvent(id, event),
...
async function waitForSubscribe(page, method) {
  try {
    await page.waitForFunction(
      (name) => (globalThis.__kinguRenderCheckSubscribes ?? []).some((one) => one.method === name),
      method,
    ...
    const opened = await page.evaluate(() => globalThis.__kinguRenderCheckSubscribes ?? [])
    throw new Error(
  ...
  const found = await page.evaluate(
    (name) => (globalThis.__kinguRenderCheckSubscribes ?? []).find((one) => one.method === name),
    method
  )
  return found
}
