...
const tempRoot = await mkdtemp(join(tmpdir(), 'kingu-claude-spawn-'))
...
const fixtureDir = join(tempRoot, 'Profile space & ^ (paren) %KINGU_PATH_TRAP% !bang! 한글')
...
const fixtureEnv = {
  ...process.env,
  CLAUDE_CONFIG_DIR: join(fixtureDir, 'config space & ^ (paren) %KINGU_ENV_LITERAL% !bang! 한글'),
  KINGU_ARG_TRAP: 'EXPANDED_ARG',
  KINGU_PATH_TRAP: 'EXPANDED_PATH',
  KINGU_FIXTURE_CAPTURE: capturePath,
  KINGU_FIXTURE_PIDS: pidPath,
  KINGU_FIXTURE_NODE: process.execPath
}
const reportedEnv = {
  ...fixtureEnv,
  CLAUDE_CONFIG_DIR: join(reportedDir, 'config with spaces 한글'),
  KINGU_FIXTURE_CAPTURE: reportedCapturePath,
  KINGU_FIXTURE_PIDS: reportedPidPath
}
...
const fixtureSource =
  ...
  `  writeFileSync(process.env.KINGU_FIXTURE_PIDS, JSON.stringify({ child: process.pid, grandchild: grandchild.pid }))\n` +
  `  setInterval(() => {}, 1000)\n` +
  `} else {\n` +
  `  writeFileSync(process.env.KINGU_FIXTURE_CAPTURE, JSON.stringify({ argv: process.argv.slice(2), configDir: process.env.CLAUDE_CONFIG_DIR }))\n` +
  `}\n`
const shimSource = '@echo off\r\n"%KINGU_FIXTURE_NODE%" "%~dp0capture-child.cjs" %*\r\n'
...
try {
  ...
  for (const [name, segment] of Object.entries({
    ...
    percent: 'profile%KINGU_PATH_TRAP%',
    bang: 'profile!name',
    ...
    const env = {
      ...
      KINGU_FIXTURE_CAPTURE: captureFile
    }
