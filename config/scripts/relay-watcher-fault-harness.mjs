...
async function main() {
  ...
  let relay
  try {
    tempRoot = await mkdtemp(join(tmpdir(), 'kingu-relay-watcher-fault-'))
    ...
    const socketPath =
      process.platform === 'win32'
        ? `\\\\.\\pipe\\kingu-relay-watcher-fault-${process.pid}-${Date.now()}`
    ...
    daemon = spawn(
      ...
      ],
      {
        ...
        env: { ...process.env, KINGU_WATCHER_CHILD_PID_FILE: pidFile },
    ...
    const beforePtyMarker = `KINGU_PTY_BEFORE_${Date.now()}`
    ...
    const afterPtyMarker = `KINGU_PTY_AFTER_${Date.now()}`
    startIndex = relay.messageCount()
