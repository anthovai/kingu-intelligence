...
function parseEndpoint(contents) {
  const port = Number(/KINGU_AGENT_HOOK_PORT=['"]?(\d+)/.exec(contents)?.[1])
  const token = /KINGU_AGENT_HOOK_TOKEN=['"]?([^'"\r\n]+)/.exec(contents)?.[1]
...
async function writeEndpoint(distro, path, endpoint) {
  const contents = [
    `KINGU_AGENT_HOOK_PORT=${endpoint.port}`,
    `KINGU_AGENT_HOOK_TOKEN=${endpoint.token}`,
    'KINGU_AGENT_HOOK_ENV=benchmark',
...
async function invokeHook(distro, scriptPath, endpointPath) {
  ...
  const result = await run(
    ...
    wslArgs(distro, [
      ...
      `KINGU_PANE_KEY=${PANE_KEY}`,
      'KINGU_TAB_ID=wsl-relay-bench',
      'KINGU_WORKTREE_ID=wsl-relay-bench',
      '/bin/sh',
      scriptPath
    ]),
...
async function main() {
  ...
  if (nodeVersion.status !== 0 || Number(nodeVersion.stdout.trim().split('.')[0]) < 18) {
    throw new Error(`WSL distro '${distro}' needs Node.js 18 or newer to run Kingu's relay`)
  }
  ...
  let staller = null
  try {
    userDataDir = mkdtempSync(join(tmpdir(), 'kingu-wsl-relay-bench-'))
    ...
    const benchmarkRoot = `${guestHome}/.kingu-wsl/benchmarks/${instanceKey}`
    const scriptPath = `${benchmarkRoot}/.kingu/agent-hooks/codex-hook.sh`
    const endpointPath = `${guestHome}/.kingu-wsl/agent-hooks/instance-${instanceKey}/endpoint.env`
    cleanupPaths = [benchmarkRoot, `${guestHome}/.kingu-wsl/agent-hooks/instance-${instanceKey}`]
    if (cleanupPaths.some((cleanupPath) => !cleanupPath.startsWith(`${guestHome}/.kingu-wsl/`))) {
    ...
    const survivingSessionId = `kingu-wsl-relay-bench-${instanceKey}`
    ...
    const createManager = async (token) => {
      ...
      return new WslHookRelayManager({
        ...
        hookCoordsEnv: () => ({
          KINGU_AGENT_HOOK_PORT: String(preferredPort),
