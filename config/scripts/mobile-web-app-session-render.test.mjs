...
beforeAll(async () => {
  ...
  scratch = await mkdtemp(join(tmpdir(), 'kingu-mobile-web-app-session-'))
...
describeRender(
  ...
  () => {
    ...
    it('asks the origin for no icon, which the shell has none to answer with', async () => {
      ...
      // `KINGU_MOBILE_WEB_RENDER_BROWSER` is what CI resolves, and that is where this bites.
    ...
    it('writes no storage key it was never handed, on a mount that read the host status', async () => {
      ...
      // `kingu:host-app-version:v1:<hostId>` — a key no page route reads and `page-storage-keys.ts`
