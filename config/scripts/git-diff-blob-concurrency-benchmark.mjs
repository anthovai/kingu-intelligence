...
const ITERATIONS = Number(process.env.KINGU_DIFF_BLOB_BENCH_ITERATIONS ?? '10')
const WARMUP = Number(process.env.KINGU_DIFF_BLOB_BENCH_WARMUP ?? '3')

for (const [name, value] of [
  ['KINGU_DIFF_BLOB_BENCH_ITERATIONS', ITERATIONS],
  ['KINGU_DIFF_BLOB_BENCH_WARMUP', WARMUP]
]) {
