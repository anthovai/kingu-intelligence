 ...
 * `@kingu/expo-two-way-audio` web stub lands in the closure along with `expo-keep-awake` — which is
 * what dictation on the page used to be: a module answering denied microphone permission, and a
 * wake lock that did nothing.
 *
 * Eight vendored modules re-enter that way, five from `@kingu/expo-two-way-audio` and three from
...
const NATIVE_AUDIO_MODULES = ['@kingu/expo-two-way-audio', 'expo-keep-awake']
 ...
 * against a mutated tree. Five from `@kingu/expo-two-way-audio` (its module, `core`, `events`,
