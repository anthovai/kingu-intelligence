...
function readAppDirArg(argv) {
  ...
  if (process.platform === 'darwin') {
    return 'dist/mac-arm64/Kingu.app'
  }
...
function getPackagedCliPath(appDir) {
  if (process.platform === 'darwin' || appDir.endsWith('.app')) {
    return join(appDir, 'Contents', 'Resources', 'bin', 'kingu')
  }
  if (process.platform === 'win32') {
    return join(appDir, 'resources', 'bin', 'kingu.exe')
  }
  return join(appDir, 'resources', 'bin', 'kingu-ide')
}

const appDir = resolve(readAppDirArg(process.argv.slice(2)))
const tempRoot = await mkdtemp(join(tmpdir(), 'kingu-packaged-cli-smoke-'))
...
try {
  ...
  assert(list.topics.some((topic) => topic.name === 'kingu-cli'))
  assert.match((await run(['skills', 'get', 'kingu-cli'])).stdout, /name: kingu-cli/)
  ...
  const update = JSON.parse(
    (await run(['skills', 'update', '--skill', 'kingu-cli', '--dry-run', '--json'])).stdout
  )
...
// Why: on Windows the launcher above spawns the copied Kingu.exe (and its crashpad/utility children)
