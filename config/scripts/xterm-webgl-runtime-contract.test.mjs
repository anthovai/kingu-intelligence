...
describe('vendored xterm WebGL runtime contract', () => {
  it('keeps shared WebGL atlas invalidation per renderer', () => {
    // Kingu panes with the same font share one atlas, so a page merge or a clear in one
    ...
    // Kingu patches in. Assert it on the resolved dependency rather than on the patch.
  ...
  it('keeps the Kingu-only WebGL hunks in the generated patch', () => {
