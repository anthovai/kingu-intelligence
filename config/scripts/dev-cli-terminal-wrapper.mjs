...
export function prepareDevCliTerminalWrappers({
  ...
  if (platform === 'win32') {
    const wrapperContent = `@echo off\r\nset "KINGU_USER_DATA_PATH=${escapeWindowsBatchValue(userDataPath)}"\r\nset "KINGU_DEV_CLI_INVOCATION=1"\r\nset "KINGU_APP_EXECUTABLE=${escapeWindowsBatchValue(electronExecutable)}"\r\nset "KINGU_APP_EXECUTABLE_NEEDS_APP_ROOT=1"\r\nnode "${escapeWindowsBatchValue(cliPath)}" %*\r\n`
    for (const targetDir of [binDir, userDataBinDir]) {
      for (const commandName of ['kingu-dev.cmd', 'kingu.cmd']) {
        writeFileSync(path.join(targetDir, commandName), wrapperContent, 'utf8')
      }
    }
  } else {
    const wrapperContent = `#!/usr/bin/env bash\nexport KINGU_USER_DATA_PATH=${JSON.stringify(userDataPath)}\nexport KINGU_DEV_CLI_INVOCATION=1\nexport KINGU_APP_EXECUTABLE=${JSON.stringify(electronExecutable)}\nexport KINGU_APP_EXECUTABLE_NEEDS_APP_ROOT=1\nexec node ${JSON.stringify(cliPath)} "$@"\n`
    for (const targetDir of [binDir, userDataBinDir]) {
      for (const commandName of ['kingu-dev', 'kingu']) {
