...
describe('Windows signing workflow contract', () => {
  ...
  it('verifies Windows inner binary signatures fail-open before publishing', () => {
    ...
    expect(steps[innerVerifyIndex].env.KINGU_WINDOWS_INNER_SIGNATURE_REQUIRED).toBe('false')
...
// Every link is asserted here the way Kingu.exe and conpty_console_list.node are.
describe('Windows NSIS uninstaller signing', () => {
  ...
  const EXPORT_ENV = 'KINGU_WIN_UNINSTALLER_EXPORT_PATH'
  const SIGNED_ENV = 'KINGU_WIN_UNINSTALLER_SIGNED_PATH'
  ...
  it('exports the uninstaller from the first Windows build', () => {
    ...
    expect(build.env[EXPORT_ENV]).toContain('kingu-uninstaller.exe')
  })
  ...
  it('stages the uninstaller into the same request as the inner binaries', () => {
    ...
    expect(stage.run).toContain('uninstaller-signing\\unsigned\\kingu-uninstaller.exe')
    expect(stage.run).toContain('uninstaller\\kingu-uninstaller.exe')
  ...
  it('keeps the uninstaller out of the inner-binary copy-back list', () => {
    ...
    expect(restoreInner.run).not.toContain('kingu-uninstaller.exe')
  })
  ...
  it('re-injects the signed uninstaller into the rebuilt installer', () => {
    ...
    expect(restore.run).toContain('kingu-uninstaller.exe')
  ...
  it('reports the embedded uninstaller in the inner-binary evidence gate', () => {
    ...
    expect(gate.run).toContain('not signed by SignPath Foundation: Uninstall Kingu.exe')
    // The uninstaller must not join the 7z payload loop, which cannot see it.
    expect(gate.run).not.toContain("$targets += 'Uninstall Kingu.exe'")
  })
  ...
  it('rehearses the uninstaller leg end to end', () => {
    ...
    expect(pack.env[EXPORT_ENV]).toContain('kingu-uninstaller.exe')
    expect(names).toContain('Restore signed uninstaller for the installer rebuild')
    expect(rebuild.env[SIGNED_ENV]).toContain('kingu-uninstaller.exe')
    ...
    expect(verify.run).toContain('shipped: Uninstall Kingu.exe')
  ...
  it('never lets an unreliable extract fail the rehearsal', () => {
    ...
    expect(verify.run).toContain("Get-Process -Name 'kingu-terminal-daemon'")
  })
  ...
  it('confines the advisory escape hatch to elevate.exe', () => {
    ...
    for (const call of ['relayed: kingu-uninstaller.exe', 'shipped: Uninstall Kingu.exe']) {
