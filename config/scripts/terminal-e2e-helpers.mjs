#!/usr/bin/env node
/**
 * Terminal E2E helpers for agent-browser + CDP testing against a running Kingu
 ...
 *
 * Usage:
 *   import { KinguTerminal } from './terminal-e2e-helpers.mjs'
 *
 *   const term = new KinguTerminal(9444)       // CDP port
 *   await term.connect()
...
export class KinguTerminal {
  /** @param {number} cdpPort — the --remote-debugging-port used when launching Kingu */
  constructor(cdpPort) {
    this.port = cdpPort
  }
   ...
   */
  probePtyIdWithScreenshot(maxId = 10, screenshotPath = tempScreenshotPath('kingu-pty-probe.png')) {
    for (let i = 1; i <= maxId; i++) {
  ...
  /**
   * Take a screenshot of the Kingu window.
   *
   * @param {string} path — output file path
   * @returns {string} the screenshot path
   */
  screenshot(path = tempScreenshotPath('kingu-terminal.png')) {
    ab(this.port, ['screenshot', path])
    return path
  }

  /**
   * Open a new terminal tab in Kingu.
   * @returns {void}
   */
  newTerminal() {
  ...
  readLang(ptyId) {
    ...
    return this.screenshot(tempScreenshotPath('kingu-lang-check.png'))
  }
}
...
if (process.argv[1]?.endsWith('terminal-e2e-helpers.mjs')) {
  ...
  const term = new KinguTerminal(port)
  console.log('Connecting to Kingu on CDP port', port, '...')
  term.connect()
