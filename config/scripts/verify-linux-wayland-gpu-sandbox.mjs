...
function createGitRepo() {
  const repoDir = mkdtempSync(path.join(tmpdir(), 'kingu-wayland-gpu-repo-'))
...
async function runValidation(mode) {
  ...
  const userDataPath = mkdtempSync(path.join(tmpdir(), 'kingu-wayland-gpu-userdata-'))
  ...
  try {
      ...
      () =>
        electron.launch({
          ...
          env: {
            ...
            KINGU_DEV_USER_DATA_PATH: userDataPath,
            HOME: isolatedHome,
    ...
    app.process().stderr?.on('data', (chunk) => {
      ...
      if (process.env.KINGU_WAYLAND_GPU_VERBOSE === '1') {
