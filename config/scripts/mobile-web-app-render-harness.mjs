...
export async function readShellDocumentHeaders() {
  const source = await readFile(
    join(
      projectDir,
      'mobile/modules/kingu-mobile-web-shell/android/src/main/java/expo/modules/kingumobilewebshell/MobileWebShellResponseHeaders.kt'
    ),
    'utf8'
  )
 ...
 * have negotiated, and every notify the page posts is kept whole in `__kinguRenderCheckNotifies`,
...
export function installShellDouble({
  ...
  globalThis.__kinguRenderCheckFaults = []
  ...
  globalThis.__kinguRenderCheckNotifies = []
  ...
  globalThis.__kinguRenderCheckRequests = []
  ...
  globalThis.__kinguRenderCheckSubscribes = []
  ...
  globalThis.__kinguRenderCheckDroppedFrames = []
  ...
  globalThis.__kinguRenderCheckAcks = []
  ...
  globalThis.__kinguRenderCheckSendBack = () => {
    if (backFrame === null) {
  ...
  globalThis.__kinguRenderCheckResendInit = (patch) => {
  ...
  const channel = {
    postMessage: (json) => {
      ...
      if (frame.type === 'notify') {
        globalThis.__kinguRenderCheckNotifies.push(frame)
        if (frame.name === faultGrant) {
          globalThis.__kinguRenderCheckFaults.push(frame.error.message)
        }
        return
      }
      ...
      if (frame.type === 'subscribe' && streams.includes(frame.method)) {
        globalThis.__kinguRenderCheckSubscribes.push({
          id: frame.id,
          method: frame.method,
          params: frame.params,
      ...
      if (frame.type === 'ack') {
        ...
        if (stream) {
          ...
          globalThis.__kinguRenderCheckAcks.push(frame.seq)
        }
        return
      }
      ...
      if (frame.type === 'request') {
        globalThis.__kinguRenderCheckRequests.push({ method: frame.method, params: frame.params })
      }
   ...
   */
  globalThis.__kinguRenderCheckEmitBinary = (id, binary) => {
    ...
    if (!carries) {
      globalThis.__kinguRenderCheckDroppedFrames.push(binary.frameSeq)
      return 'dropped'
    }
    stream.seq = seq
   ...
   */
  globalThis.__kinguRenderCheckEmitEvent = (id, payload) => {
    const stream = openStreams.get(id)
    if (!stream) {
      return 'no-stream'
    }
  ...
  globalThis.__kinguRenderCheckWindow = (id) => {
    const stream = openStreams.get(id)
    return stream === undefined
      ? null
      : { frames: stream.unacked.length, unackedBytes: stream.unackedBytes }
  }
  globalThis.kinguBridge = channel
}
...
export async function waitForRecordedRequests(
  ...
  for (;;) {
    const requests = await page.evaluate(() => globalThis.__kinguRenderCheckRequests ?? [])
...
export function installPageErrorSentinel() {
  globalThis.__kinguSentinelCalls = []
  const sentinel = (message) => {
    globalThis.__kinguSentinelCalls.push(String(message))
    return false
  }
  globalThis.__kinguSentinel = sentinel
...
export function installSchedulerRecorder() {
  globalThis.__kinguScheduler = { watching: false, scheduled: [], leaked: [] }
...
export function installCspViolationRecorder() {
  globalThis.__kinguCspViolations = []
  document.addEventListener('securitypolicyviolation', (event) => {
    globalThis.__kinguCspViolations.push(
...
export async function terminalStyleReach(page) {
  return await page.evaluate(() => {
    const sheet = [...document.styleSheets].find(
      (one) => one.ownerNode?.id === 'kingu-terminal-document-style'
    )
    if (!sheet) {
