 ...
 *   restart-proxy                — idle, then kingu open + status/list storm + open
 *                                  (does NOT kill the desktop; proxies restore work)
 *
 * Usage:
 *   KINGU_FREEZE_ENV=paired-remote KINGU_FREEZE_SCENARIO=idle-backlog-open \
...
import { createKinguRpc } from './live-remote-freeze-rpc.mjs'
...
const envName = process.env.KINGU_FREEZE_ENV || 'paired-remote'
const scenario = process.env.KINGU_FREEZE_SCENARIO || 'idle-backlog-open'
const createCount = Math.max(0, readFreezeNumberEnv('KINGU_FREEZE_CREATE', 0))
const openCount = Math.max(2, readFreezeNumberEnv('KINGU_FREEZE_OPEN_COUNT', 20))
const idleMs = Math.max(0, readFreezeNumberEnv('KINGU_FREEZE_IDLE_MS', 45_000))
const paceMs = Math.max(0, readFreezeNumberEnv('KINGU_FREEZE_PACE_MS', 250))
const paceJitterMs = Math.max(0, readFreezeNumberEnv('KINGU_FREEZE_PACE_JITTER_MS', 150))
const createWorktreeSpan = Math.max(1, readFreezeNumberEnv('KINGU_FREEZE_CREATE_WT_SPAN', 12))
const softMs = readFreezeNumberEnv('KINGU_FREEZE_SOFT_MS', DEFAULT_SOFT_MS)
const hardMs = readFreezeNumberEnv('KINGU_FREEZE_HARD_MS', DEFAULT_HARD_MS)
/** Concurrent opens during lockup-storm (wake refresh overlaps fan-out). */
const stormParallel = Math.max(1, readFreezeNumberEnv('KINGU_FREEZE_STORM_PARALLEL', 16))
/** Kill a switch if it exceeds this — counts toward permanent lockup. */
const opTimeoutMs = Math.max(10_000, readFreezeNumberEnv('KINGU_FREEZE_OP_TIMEOUT_MS', 60_000))
const permanentTimeoutMs = Math.max(
  15_000,
  readFreezeNumberEnv('KINGU_FREEZE_PERMANENT_MS', 60_000)
)
const foreverWindowMs = Math.max(
  10_000,
  readFreezeNumberEnv('KINGU_FREEZE_FOREVER_WINDOW_MS', DEFAULT_FOREVER_WINDOW_MS)
)
const statusSlowMs = Math.max(
  5_000,
  readFreezeNumberEnv('KINGU_FREEZE_STATUS_SLOW_MS', DEFAULT_STATUS_SLOW_MS)
)
const watchdogIntervalMs = Math.max(
  500,
  readFreezeNumberEnv('KINGU_FREEZE_WATCHDOG_INTERVAL_MS', 1500)
)
const scratchDir = process.env.KINGU_FREEZE_SCRATCH || ''

function sleep(ms) {
  return new Promise((resolve) => setTimeout(resolve, ms))
}

const rpc = createKinguRpc({ envName })
const { kinguJsonSync, kinguJsonAsync, runReconnectRefreshStorm, runRestartProxy } = rpc
...
function sampleKinguIfPossible() {
  if (process.platform !== 'darwin') {
    return null
  }
  try {
    const status = kinguJsonSync(['status'], { local: true }).result
    const pid = status?.app?.pid
    if (!pid) {
      return null
    }
    const out = path.join(reportDir, `kingu-sample-realistic-${Date.now()}.txt`)
...
function listLiveTerminalHandles() {
  const listed = kinguJsonSync(['terminal', 'list'])
...
async function main() {
  if (!REALISTIC_SCENARIOS.includes(scenario)) {
    throw new Error(
      `Unknown KINGU_FREEZE_SCENARIO=${scenario}. Expected one of: ${REALISTIC_SCENARIOS.join(', ')}`
    )
  }
  ...
  const local = kinguJsonSync(['status'], { local: true })
  const remote = kinguJsonSync(['status'])
  notes.push(
  ...
  const worktrees = kinguJsonSync(['worktree', 'list']).result
  ...
  if (createCount > 0) {
    ...
    await mapPool(
      ...
      async (i) => {
        ...
        try {
          const createdTerm = await kinguJsonAsync(
            [
              'terminal',
              'create',
              '--worktree',
  ...
  try {
    ...
    const parked = await kinguJsonAsync(['terminal', 'switch', '--terminal', parkHandle], {
      timeoutMs: 60_000
    })
  ...
  if (scenario === 'idle-backlog-reconnect-open' || scenario === 'lockup-storm') {
    ...
    console.log('[realistic-freeze] restart proxy: kingu open + refresh storm (no process kill)')
    ...
    for (let offset = 0; offset < openList.length; offset += stormParallel) {
      ...
      const batchResults = await Promise.all(
        batch.map(async (handle, batchIndex) => {
          ...
          try {
            const sw = await kinguJsonAsync(['terminal', 'switch', '--terminal', handle], {
  ...
  try {
    const statusProbe = await kinguJsonAsync(['status'], {
      local: true,
  ...
  try {
    const mem = await kinguJsonAsync(['diagnostics', 'memory'], {
      local: true,
      timeoutMs: permanentTimeoutMs
    })
  ...
  if (signals.softFreeze || signals.hardFreeze || fullApp.foreverUiLockupObserved) {
    samplePath = sampleKinguIfPossible()
    if (samplePath) {
