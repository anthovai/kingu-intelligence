...
const agentSessionToken = '--kingu-repro-agent-session'
...
try {
  ...
  execFileSync(
    ...
    [
      ...
      projectPath,
      '-c',
      'user.name=Kingu Repro',
      '-c',
      'user.email=kingu-repro@example.invalid',
      'commit',
      '--allow-empty',
      '-m',
  ...
  writeFileSync(
    path.join(profilePath, 'kingu-data.json'),
    JSON.stringify({
...
async function startServer(port) {
  ...
  const env = {
    ...
    KINGU_DEV_USER_DATA_PATH: profilePath,
    KINGU_USER_DATA_PATH: profilePath,
    KINGU_REPRO_SPAWN_MARKER: spawnMarkerPath,
    KINGU_REPRO_EXIT_TRIGGER: exitTriggerPath,
    KINGU_REPRO_INPUT_MARKER: inputMarkerPath,
    KINGU_REPRO_AGENT_SESSION_TOKEN: agentSessionToken,
  ...
  return await new Promise((resolve, reject) => {
    ...
    lines.on('line', (line) => {
      try {
        ...
        if (parsed.type === 'kingu_server_ready' && parsed.pairing?.url) {
