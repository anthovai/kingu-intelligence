...
if (!appImageArg) {
  fail('Usage: run-headless-linux-pairing-docker.mjs --appimage /path/to/kingu.AppImage')
}
...
function extractAppImage(image) {
  ...
  docker([
    ...
    'bash',
    '-v',
    `${appImage}:/input/kingu.AppImage:ro`,
    '-v',
    `${artifactVolume}:/artifacts`,
    image,
    '-lc',
    'cp /input/kingu.AppImage /artifacts/kingu.AppImage && chmod +x /artifacts/kingu.AppImage && cd /artifacts && ./kingu.AppImage --appimage-extract >/dev/null && chmod -R a+rX /artifacts/squashfs-root'
  ])
}
...
async function validateStartupMatrix() {
  ...
  const publicEntry = await startAndWait({
    ...
    mode: 'json',
    address: '127.0.0.1',
    appPath: '/artifacts/kingu.AppImage',
...
async function validateAuthenticatedPairing() {
  const server = await startAndWait({
    ...
    launch: 'direct',
    mode: 'json',
    address: 'ws://kingu-pairing-server:6768/runtime?route=runtime',
    port: '6768',
    networkAlias: 'kingu-pairing-server'
  })
  ...
  assert(
    ...
    'paired server did not report its Kingu app version'
  )
  assert(
...
async function validateUnreachableOffer() {
  const server = await startAndWait({
    ...
    launch: 'direct',
    mode: 'json',
    address: 'kingu-pairing-server:6769',
    port: '6768',
    networkAlias: 'kingu-pairing-server'
  })
  const payload = readyJson(server.stdout)
...
async function startAndWait({
  ...
  const name = `kingu-pairing-${suffix}-${containers.size}`
  const args = [
    'run',
    '-d',
    '--init',
    ...
    '-e',
    'KINGU_KEEP_RUNNING=1',
    '-e',
    'LIBGL_ALWAYS_SOFTWARE=1',
    '-e',
    `KINGU_READY_JSON=${mode === 'json' ? '1' : '0'}`,
    '-e',
    `KINGU_PAIRING_ADDRESS=${address}`,
    '-e',
    `KINGU_SERVE_PORT=${port}`,
    '-e',
    `KINGU_TEST_APPIMAGE=${appPath}`,
    ...(noPairing ? ['-e', 'KINGU_NO_PAIRING=1'] : []),
    '-v',
...
function hasCompleteReadyContract(stdout) {
  if (
    stdout.includes('Kingu server ready\n') &&
...
function validateReady(logs, mode, expectedHost, options = {}) {
  if (mode === 'human') {
    assert(
      (logs.match(/^Kingu server ready$/gm) ?? []).length === 1,
    ...
    assert(logs.includes('Pairing URL: kingu://pair?code='), 'human pairing URL is missing')
    return
  }
...
function readyJsonObjects(logs) {
  const marker = '{"type":"kingu_server_ready"'
  return logs
    .split(/\r?\n/)
    .map((line) => {
    ...
    .filter((value) => value?.type === 'kingu_server_ready')
}
