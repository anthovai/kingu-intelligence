...
const ROUNDS = Number(process.env.KINGU_CLI_DEFER_BENCH_ROUNDS ?? '30')
const WARMUP = Number(process.env.KINGU_CLI_DEFER_BENCH_WARMUP ?? '3')

for (const [name, value] of [
  ['KINGU_CLI_DEFER_BENCH_ROUNDS', ROUNDS],
  ['KINGU_CLI_DEFER_BENCH_WARMUP', WARMUP]
]) {
...
if (ROUNDS % 2 !== 0) {
  ...
  throw new Error(`KINGU_CLI_DEFER_BENCH_ROUNDS must be even, received ${ROUNDS}`)
}
...
try {
  ...
  const isolated = { KINGU_USER_DATA_PATH: userDataPath }
  /** @type {Array<[string, string[], Record<string, string>]>} */
  const cases = [
    ['kingu --help', ['--help'], {}],
    ['kingu help worktree', ['help', 'worktree'], {}],
    ['kingu (no args)', [], {}],
    ['unknown command', ['no-such-command'], {}],
    ['unknown flag', ['worktree', 'list', '--nope'], {}],
    ['kingu agent-context --json', ['agent-context', '--json'], {}],
    ['kingu status --json', ['status', '--json'], isolated],
    ['kingu worktree list --json', ['worktree', 'list', '--json'], isolated]
  ]
