...
/** A real loadable addon, so the OS holds the same lock a running Kingu holds. */
...
/**
 * Stage a stale addon and keep it loaded, exactly as a running Kingu does.
 *
...
// a real EPERM, and the classifier that turns that into "close running Kingu"
    ...
    async () => {
      ...
      try {
        ...
        expect(result.stderr).toContain(
          'Close running Kingu/Electron/dev processes for this worktree'
        )
