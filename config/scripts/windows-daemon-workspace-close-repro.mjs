 ...
 * graceful-then-immediate kill pair emitted when Kingu closes a workspace.
...
const iterations = Number(process.env.KINGU_WINDOWS_DAEMON_CLOSE_ITERATIONS ?? 25)
...
async function main() {
  ...
  const scratch = mkdtempSync(join(tmpdir(), 'kingu-windows-daemon-close-'))
  const socketPath = `\\\\.\\pipe\\kingu-daemon-close-${process.pid}-${randomUUID()}`
  ...
  const child = fork(
    ...
    ['--socket', socketPath, '--token', tokenPath, '--log-file', daemonLogPath],
    {
      ...
      windowsHide: true,
      env: { ...process.env, KINGU_USER_DATA_PATH: scratch }
    }
  )
