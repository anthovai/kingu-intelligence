    ...
    const testProcess = spawn(
          ...
          ],
      {
        ...
        env: {
          ...
          ...(nestedWayland
            ? {
                KINGU_E2E_IME_INJECTOR: 'nested',
                KINGU_E2E_NESTED_FOCUS_CMD: path.join(
                  projectDir,
                  'config/scripts/focus-nested-wayland-terminal.sh'
                ),
                KINGU_E2E_EXTRA_APP_ARGS:
            ...
            : {}),
          KINGU_E2E_FORWARD_APP_LOGS: '1',
          KINGU_E2E_NATIVE_IBUS_HANGUL: '1',
...
async function runOuter() {
  ...
  const evidenceDir = mkdtempSync(path.join(os.tmpdir(), 'kingu-terminal-ime-e2e-'))
  ...
  const sessionProcess = spawn(
    ...
    ],
    {
      ...
      env: {
        ...
        ...(nestedWayland
          ? {
              WAYLAND_DISPLAY: 'wayland-kingu-ime',
