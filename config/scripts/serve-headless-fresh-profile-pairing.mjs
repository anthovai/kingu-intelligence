...
const kinguDevScript = path.join(scriptDir, 'kingu-dev.mjs')
const ensureNativeRuntimeScript = path.join(scriptDir, 'ensure-native-runtime.mjs')
const fixedProfileDir = process.env.KINGU_HEADLESS_PAIRING_PROFILE_DIR
...
const profileDir =
  fixedProfileDir ?? mkdtempSync(path.join(tmpdir(), 'kingu-headless-pairing-profile-'))
...
delete childEnv.KINGU_CODEX_HOME
Object.assign(childEnv, {
  // Why: a fresh temporary Kingu profile must not make the default Codex lane
  // read or mutate the developer profile during a pairing smoke test.
  KINGU_DEV_USER_DATA_PATH: profileDir,
  HOME: isolatedHome,
...
console.error(
  `[headless-pairing] starting: kingu-dev serve --json${formatForwardedArgs(serveArgs)}`
)

child = spawn(process.execPath, [kinguDevScript, 'serve', '--json', ...serveArgs], {
  cwd: repoRoot,
...
/**
 * Parses wrapper flags and forwards everything else to `kingu serve`.
 */
 ...
 */
function printHelp() {
  console.log(`Usage: node config/scripts/serve-headless-fresh-profile-pairing.mjs [--keep] [kingu serve flags]

Starts kingu-dev serve --json with a fresh isolated userData profile, ensures Electron's dev runtime is usable, and prints the pairing URL.
...
Environment:
  KINGU_HEADLESS_PAIRING_ADDRESS=<host|host:port|ws://...>  Override the auto pairing address.
  KINGU_HEADLESS_PAIRING_PROFILE_DIR=/path/to/profile       Use a fixed profile directory.
`)
}
...
function resolveDefaultPairingAddress() {
  const configured = process.env.KINGU_HEADLESS_PAIRING_ADDRESS?.trim()
  if (configured) {
    return configured
  }
...
function printReadyLine(line) {
  ...
  if (!payload || payload.type !== 'kingu_server_ready') {
    return false
  }
  console.log(`Kingu server ready: ${payload.boundEndpoint ?? 'websocket unavailable'}`)
...
function stopChild(signal) {
  ...
  if (process.platform === 'win32' && child.pid) {
    // Why: child.kill() only targets kingu-dev on Windows; taskkill walks the
  ...
  if (process.platform !== 'win32' && child.pid) {
    // Why: kingu-dev synchronously owns the CLI child, which owns Electron; kill
...
function cleanupProfile() {
  ...
  if (!existsSync(profileDir) || !profileDir.includes('kingu-headless-pairing-profile-')) {
