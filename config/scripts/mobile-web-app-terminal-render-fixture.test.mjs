...
const SCRATCH_PREFIX = 'kingu-c75-terminal-render-'
...
beforeAll(async () => {
  ...
  temporaryRoot = await mkdtemp(join(tmpdir(), 'kingu-c75-fixture-rollback-'))
...
describeFixture('the terminal render fixture', () => {
  it('takes back the server and the scratch tree when the browser will not start', async () => {
    ...
    const realBrowser = process.env.KINGU_MOBILE_WEB_RENDER_BROWSER
    process.env.KINGU_MOBILE_WEB_RENDER_BROWSER = join(temporaryRoot, 'kingu-c75-no-such-browser')
    try {
      ...
      if (realBrowser === undefined) {
        delete process.env.KINGU_MOBILE_WEB_RENDER_BROWSER
      } else {
        process.env.KINGU_MOBILE_WEB_RENDER_BROWSER = realBrowser
      }
    }
