...
async function waitForRoute({ page, errors, uncaught }, route, awaitText) {
  ...
  const cause = await race(
    page.waitForFunction(() => document.documentElement.dataset.kinguWebEntry === 'mounted', {
      timeout: 30_000,
      polling: 250
    })
  )
  ...
  for (const fault of await page.evaluate(() => globalThis.__kinguRenderCheckFaults ?? [])) {
...
async function render(route, awaitText, { shellRoute = { pathname: route }, ...shell } = {}) {
  ...
  const session = await opened.page.evaluate(() => ({
    sessionId: document.documentElement.dataset.kinguWebSessionId ?? null,
    buildId: document.documentElement.dataset.kinguWebBuildId ?? null
  }))
...
const paintReports = (page, name) =>
  page.evaluate(
    (paint) =>
      (globalThis.__kinguRenderCheckNotifies ?? []).filter((frame) => frame.name === paint).length,
    name
  )
...
describeRender('the Route A page in a real browser', () => {
  ...
  // Both files routes reach KinguMobileWebShellView from their native file, whose module calls
  ...
  it('says to update the app when the shell that opened it named no screen', async () => {
    ...
    expect(errors).toEqual([])
    expect(text).toContain('Update Kingu to open this workspace')
  ...
  it('tells the shell when a route chunk throws, rather than sitting on a blank page', async () => {
    ...
    try {
      ...
      const reported = await opened.page
        .waitForFunction(
          () => {
            const faults = globalThis.__kinguRenderCheckFaults ?? []
  ...
  it('reports its frame from the route screen, not from the router shell above it', async () => {
    ...
    try {
      ...
      await opened.page.waitForFunction(
        (name) =>
          (globalThis.__kinguRenderCheckNotifies ?? []).filter((frame) => frame.name === name)
            .length > 0,
        paintName,
        { timeout: 30_000, polling: 250 }
      )
  ...
  it('refuses a target the shell will not take, rather than opening it in the page', async () => {
    ...
    const notifies = await page.evaluate(() => globalThis.__kinguRenderCheckNotifies ?? [])
    ...
    expect(await page.evaluate(() => globalThis.__kinguRenderCheckFaults ?? [])).toEqual([])
    expect(errors).toEqual([])
    await page.close()
  }, 60_000)
...
describeRender('the stack the page Back button rests on', () => {
  ...
  it('carries a handoff the shell granted across the bridge from a real tap', async () => {
    ...
    const notifies = await opened.page.evaluate(() => globalThis.__kinguRenderCheckNotifies ?? [])
    expect(notifies.filter((frame) => frame.name === 'navigate')).toEqual([
