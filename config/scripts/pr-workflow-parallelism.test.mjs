...
describe('PR workflow parallelism', () => {
  ...
  it('reuses native preparation after the dependency action gate', () => {
    ...
    expect(packageStep.env.KINGU_REUSE_PREPARED_NATIVE_RUNTIME).toBe('1')
  })
  ...
  it('keeps verify as the aggregate required check', () => {
    ...
    // KINGU_BROWSER_EXECUTABLE, so it only guards anything if verify actually reads it.
    expect(verifyStep.env.KINGUD_BROWSER).toBe('${{ needs.kingud_browser.result }}')
    expect(verifyStep.run).toContain('"$KINGUD_BROWSER"')
