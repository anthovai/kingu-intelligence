...
```sh
KINGU_BACKGROUND_LAUNCH=1 node config/scripts/run-linux-lid-sleep-assertion-docker.mjs --baseline <git-ref>
```
...
`KINGU_DOCKER` and `KINGU_DOCKER_PLATFORM` override the executable and architecture.
