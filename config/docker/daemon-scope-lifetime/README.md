...
```sh
KINGU_BACKGROUND_LAUNCH=1 node config/scripts/run-daemon-scope-lifetime-docker.mjs --baseline <git-ref>
```
...
`KINGU_DOCKER` overrides the Docker executable. `KINGU_DOCKER_PLATFORM` overrides the Linux architecture.
