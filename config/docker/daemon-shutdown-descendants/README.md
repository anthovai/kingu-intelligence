...
```sh
KINGU_BACKGROUND_LAUNCH=1 node config/scripts/run-daemon-shutdown-descendants-docker.mjs
```
...
`KINGU_DOCKER` overrides the Docker executable. `KINGU_DOCKER_PLATFORM` overrides the Linux architecture. The test creates a temporary image and removes it afterward.
