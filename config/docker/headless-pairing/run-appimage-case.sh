...
appimage=${KINGU_TEST_APPIMAGE:-/artifacts/squashfs-root/AppRun}
timeout_seconds=${KINGU_STARTUP_TIMEOUT_SECONDS:-12}
pairing_address=${KINGU_PAIRING_ADDRESS:-127.0.0.1}
port=${KINGU_SERVE_PORT:-0}
state_dir="/tmp/kingu-${case_name}"

if ((EUID == 0)); then
  mkdir -p "$state_dir/config" "$state_dir/cache"
  chown -R kingu:kingu "$state_dir"
...
app_args=("${launcher[@]}")
if [[ ${KINGU_TEST_NO_SANDBOX:-1} == 1 ]]; then
  app_args+=(--no-sandbox)
fi
app_args+=(serve --port "$port" --pairing-address "$pairing_address")
if [[ ${KINGU_READY_JSON:-0} == 1 ]]; then
  app_args+=(--json)
fi
if [[ ${KINGU_NO_PAIRING:-0} == 1 ]]; then
  app_args+=(--no-pairing)
fi
...
if [[ $is_appimage == 0 ]]; then
  export APPDIR=${KINGU_TEST_APPDIR:-"$(dirname "$appimage")"}
fi

if [[ ${KINGU_KEEP_RUNNING:-0} == 1 ]]; then
  exec "${command[@]}"
fi
