...
([#8512](https://github.com/anthovai/kingu-intelligence/pull/8512)) merged; PR B not yet
implemented
...
Reduce the recurring engineering and human work required to localize Kingu
...
Use gettext PO as Kingu's canonical bilingual translation source, under a
constrained profile:

- `msgctxt` — the stable Kingu message ID;
...
dependency. Kingu must not require network access or a translation service to
...
Kingu will support only the subset needed for its message model. PR B must
define and validate that subset rather than accepting arbitrary PO documents.

Each supported entry must carry:

- one stable Kingu message ID (`msgctxt`), unique within the file;
...
Plurals use Kingu's existing per-key convention: each i18next plural-suffixed
...
- XML is the format most likely to let a hand-edited community PR fail on a
  ...
  external editors produce whole-file diffs; Kingu's translation contributions
  arrive overwhelmingly as direct catalog PRs.

If a translation provider requires XLIFF, Kingu should add a deterministic
...
translator context, or review state. Meeting Kingu's requirements would require
...
own platform but are not a suitable authority for Kingu's macOS, Linux,
...
not become Kingu's sole source of truth or a runtime dependency. Selecting one
...
to establish a format decision. Kingu therefore chooses based on its own
