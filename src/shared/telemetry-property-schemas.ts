...
// Small set: only failures Kingu's PTY-typed-command launch can observe (`binary_not_found` = shell ENOENT, `paste_readiness_timeout`, `unknown`).
// Provider-side errors (auth/rate-limit/network) happen inside the agent CLI subprocess and are invisible to Kingu. See telemetry-plan.md §Defer per-incident error fields.
...
// Whitelist of settings emittable on `settings_changed`. `kingu_channel` (build-time, not user-togglable) is absent.
