...
export function formatCommandHelp(spec: CommandSpec): string {
  const lines = [`kingu ${spec.path.join(' ')}`, '', `Usage: ${spec.usage}`, '', spec.summary]
...
export function formatGroupHelp(specs: CommandSpec[], groupPath: string[]): string {
  ...
  const lines = [`kingu ${group}`, '', `Usage: kingu ${group} <command> [options]`, '', 'Commands:']
  ...
  lines.push('', `Run \`kingu ${group} <command> --help\` for command-specific usage.`)
  return lines.join('\n')
}
