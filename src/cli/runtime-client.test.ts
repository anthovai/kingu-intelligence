...
import { launchKinguApp } from './runtime/launch'

vi.mock('./runtime/launch', () => ({
  launchKinguApp: vi.fn()
}))
...
afterEach(async () => {
  vi.mocked(launchKinguApp).mockClear()
...
function writeMetadata(
  ...
  writeFileSync(
    join(userDataPath, 'kingu-runtime.json'),
    JSON.stringify({
      runtimeId: 'runtime-1',
      pid,
      transports: [
...
describe.skipIf(process.platform === 'win32')('RuntimeClient', () => {
  it('adds an opaque durable request ID only to orchestration mutations', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'kingu-runtime-client-'))
    ...
    const priorLaunchToken = process.env.KINGU_AGENT_LAUNCH_TOKEN
    process.env.KINGU_AGENT_LAUNCH_TOKEN = 'launch-secret'
    ...
    try {
      ...
      if (priorLaunchToken === undefined) {
        delete process.env.KINGU_AGENT_LAUNCH_TOKEN
      } else {
        process.env.KINGU_AGENT_LAUNCH_TOKEN = priorLaunchToken
      }
    }
  ...
  it('openKingu activates the app even when a desktop runtime is already reachable', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'kingu-runtime-client-'))
    const endpoint = join(userDataPath, 'runtime.sock')
    const server = createServer((socket) => {
      sockets.add(socket)
    ...
    const client = new RuntimeClient(userDataPath, 100)
    const status = await client.openKingu(100)

    expect(status.result.runtime.state).toBe('ready')
    expect(status.result.runtime.reachable).toBe(true)
    expect(launchKinguApp).toHaveBeenCalledOnce()
  })

  it('openKingu waits for a reachable headless runtime to expose a desktop window', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'kingu-runtime-client-'))
    const endpoint = join(userDataPath, 'runtime.sock')
    ...
    const client = new RuntimeClient(userDataPath, 100)
    const status = await client.openKingu(1_000)
  ...
  it('openKingu fails explicitly when the serve owner cannot promote safely', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'kingu-runtime-client-'))
    const endpoint = join(userDataPath, 'runtime.sock')
    const server = createServer((socket) => {
      sockets.add(socket)
    ...
    await expect(client.openKingu(100)).rejects.toMatchObject({
      code: 'desktop_activation_blocked'
    })
    // A blocked runtime can't promote, so we bail before spawning the app.
    expect(launchKinguApp).not.toHaveBeenCalled()
  })
  ...
  it('preserves a dropped read-only orchestration failure exactly', async () => {
    ...
    expect((failure as RuntimeClientError).message).toBe(
      'The Kingu runtime closed the connection before responding. Restart Kingu and try again.'
    )
