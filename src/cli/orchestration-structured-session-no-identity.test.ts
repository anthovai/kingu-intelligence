 ...
 *
 * Non-worker structured sessions get no `KINGU_TERMINAL_HANDLE`, so `orchestration check` fell
...
import { KINGU_STRUCTURED_SESSION_ENV } from '../shared/structured-session-marker'
...
const originalMarker = process.env[KINGU_STRUCTURED_SESSION_ENV]
const originalHandle = process.env.KINGU_TERMINAL_HANDLE
...
describe('a structured chat session with no orchestration identity', () => {
  beforeEach(() => {
    ...
    process.env[KINGU_STRUCTURED_SESSION_ENV] = '1'
  ...
  afterEach(() => {
    if (originalMarker === undefined) {
      delete process.env[KINGU_STRUCTURED_SESSION_ENV]
    } else {
      process.env[KINGU_STRUCTURED_SESSION_ENV] = originalMarker
    }
    if (originalHandle === undefined) {
      delete process.env.KINGU_TERMINAL_HANDLE
    } else {
      process.env.KINGU_TERMINAL_HANDLE = originalHandle
    }
  })
