...
// Why: guides write examples as `kingu ...`, `kingu-dev ...`, or the `KINGU` placeholder.
const CLI_INVOCATION = /(?:^|[\s`(])(?:kingu|kingu-dev|kingu-ide|KINGU)\s+([^\n`]*)/g
...
function collectGuideInvocations(): GuideInvocation[] {
  ...
  for (const guide of BUNDLED_SKILL_GUIDES) {
    for (const match of guide.fullMarkdown.matchAll(CLI_INVOCATION)) {
      ...
      // Why: a quoted flag value belongs to the nested program (`--command 'codex --model ...'`), not to kingu.
      const kinguArgs = invocation.replace(/'[^']*'|"[^"]*"/g, ' ')
      const flags = [...kinguArgs.matchAll(/(?:^|[\s[(])--([a-z][a-z0-9-]*)/g)].map(
        (flag) => flag[1]
      )
