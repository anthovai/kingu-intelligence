...
export const CORE_COMMAND_SPECS: CommandSpec[] = [
  {
    path: ['open'],
    summary: 'Launch Kingu and wait for the runtime to be reachable',
    usage: 'kingu open [--json]',
    allowedFlags: [...GLOBAL_FLAGS],
    examples: ['kingu open', 'kingu open --json']
  },
  ...SERVE_COMMAND_SPECS,
  {
    path: ['status'],
    summary: 'Show app/runtime/graph readiness',
    usage: 'kingu status [--json]',
    allowedFlags: [...GLOBAL_FLAGS],
    examples: ['kingu status', 'kingu status --json']
  },
  {
    path: ['claude-teams'],
    argumentMode: 'passthrough',
    summary: 'Start Claude Code Agent Teams in the current Kingu terminal',
    usage: 'kingu claude-teams [claude args...]',
    allowedFlags: [...GLOBAL_FLAGS],
    notes: [
      'Passes all following arguments through to Claude Code after enabling Agent Teams native panes.',
      'Must be run from inside an Kingu terminal. Starts Claude Code Agent Teams in the current pane and opens teammates as native Kingu splits.'
    ],
    examples: ['kingu claude-teams', 'kingu claude-teams --resume <session-id>']
  },
  {
    path: ['repo', 'list'],
    summary: 'List repos registered in Kingu',
    usage: 'kingu repo list [--json]',
    allowedFlags: [...GLOBAL_FLAGS]
  },
  {
    path: ['repo', 'add'],
    summary: 'Add a project to Kingu by filesystem path',
    usage: 'kingu repo add --path <path> [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'path']
  },
  {
    path: ['repo', 'show'],
    summary: 'Show one registered repo',
    usage: 'kingu repo show --repo <selector> [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'repo']
  },
  {
    path: ['repo', 'set-base-ref'],
    summary: "Set the repo's default base ref for future worktrees",
    usage: 'kingu repo set-base-ref --repo <selector> --ref <ref> [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'repo', 'ref']
  },
  {
    path: ['repo', 'search-refs'],
    summary: 'Search branch/tag refs within a repo',
    usage: 'kingu repo search-refs --repo <selector> --query <text> [--limit <n>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'repo', 'query', 'limit']
  },
  {
    path: ['worktree', 'list'],
    summary: 'List Kingu-managed worktrees',
    usage: 'kingu worktree list [--repo <selector>] [--limit <n>] [--json]',
  ...
  },
  {
    ...
    usage: 'kingu worktree show --worktree <selector> [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'worktree']
  },
  {
    path: ['worktree', 'current'],
    summary: 'Show the Kingu-managed worktree for the current directory',
    usage: 'kingu worktree current [--json]',
    allowedFlags: [...GLOBAL_FLAGS],
    notes: [
      'Resolves the current shell directory to a path: selector so agents can target the enclosing Kingu worktree without spelling out $PWD.'
    ],
    examples: ['kingu worktree current', 'kingu worktree current --json']
  },
  {
    path: ['worktree', 'create'],
    summary: 'Create a new Kingu-managed worktree',
    usage:
    ...
    notes: [
      'This creates a new checkout. For a fresh agent in an existing worktree, use `kingu terminal create --worktree active --command "codex"` instead.',
      'By default, Kingu records the new worktree as a child of the caller context when it can infer one from the Kingu terminal or current directory.',
      'If --repo is omitted, Kingu infers the repo from the current Kingu-managed worktree.',
      'Use --project with --host to create on a ready project host setup without spelling the backing repo id.',
      '--host runtime:<environment-id> creates on that paired Kingu server; use the id from `kingu environment list`, not the environment name.',
      'For related work, use the inferred parent or pass --parent-worktree active, folder:<id>, or worktree:<worktreeId> to make the relationship explicit. Worktree ids are the full <repo-id>::<path> values returned by `kingu worktree list --json`.',
      'Use --no-parent when the new worktree should be independent of the current context.',
      '--no-parent only affects Kingu lineage; omit --base-branch to use the repo default base, or pass the default base ref explicitly for independent top-level work.',
      'By default this creates the worktree and its first terminal without switching the active Kingu view.',
    ...
    examples: [
      ...
      'kingu worktree create --repo id:<repoId> --name independent-task --no-parent --json'
    ]
  },
  {
    path: ['worktree', 'set'],
    summary: 'Update Kingu metadata for a worktree',
    usage:
    ...
    examples: [
      'kingu worktree set --worktree active --linear-issue STA-335 --json',
      'kingu worktree set --worktree active --linear-issue null --json'
    ]
  },
  {
    path: ['worktree', 'rm'],
    ...
    destructive: true,
    summary: 'Remove a worktree from Kingu and git',
    usage:
    ...
    notes: [
      'Repo-defined kingu.yaml archive hooks are skipped unless --run-hooks is passed.',
      ...
      'For Git worktrees, removal also attempts to delete the checked-out local branch, with or without --force. Kingu retains branches it knows predated the worktree and any branch whose changes it cannot prove are already merged.'
    ]
  },
  {
    path: ['worktree', 'ps'],
    summary: 'Show a compact orchestration summary across worktrees',
    usage: 'kingu worktree ps [--limit <n>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'limit'],
    notes: [...WORKTREE_LISTING_SCOPE_NOTES]
  },
  {
    path: ['terminal', 'list'],
    summary: 'List live Kingu-managed terminals',
    usage:
  ...
  },
  {
    ...
    usage: 'kingu terminal show [--terminal <handle>] [--json]',
  ...
  },
  {
    ...
    usage:
      'kingu terminal read [--terminal <handle>] [--cursor <n>] [--limit <n>] [--screen] [--json]',
    ...
    examples: [
      ...
      'kingu terminal read --terminal term_abc123 --cursor 42 --limit 1000 --json',
      'kingu terminal read --terminal term_abc123 --screen --json'
    ]
  },
  ...
  },
  {
    ...
    usage: 'kingu terminal stop --worktree <selector> [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'worktree'],
    notes: [
  ...
  },
  {
    ...
    notes: [
      ...
      'A host that cannot apply --shell refuses the create rather than quietly spawning its default shell: macOS and Linux execution hosts spawn the login shell, terminals routed over SSH resolve their shell on the SSH host, a --shell that contradicts the project execution runtime (WSL vs Windows host) is refused, and an Kingu host older than --shell is refused by the CLI.'
    ],
    examples: [
      'kingu terminal create --json',
      'kingu terminal create --worktree active --command "codex" --json',
      'kingu terminal create --worktree path:/projects/myapp --title "RUNNER" --command "opencode"',
      'kingu terminal create --worktree path:/projects/myapp --command "opencode" --focus',
      'kingu terminal create --worktree path:C:/src/app --shell cmd.exe --json'
    ]
  },
  {
    ...
    usage: 'kingu terminal switch [--terminal <handle>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'terminal'],
    examples: ['kingu terminal switch --terminal term_abc123']
  },
  TERMINAL_CLOSE_COMMAND_SPEC,
  {
    path: ['terminal', 'rename'],
    summary: 'Set or clear the title of a terminal tab',
    usage: 'kingu terminal rename [--terminal <handle>] [--title <text>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'terminal', 'title'],
    notes: ['Omit --title or pass an empty string to reset to the auto-generated title.'],
    examples: [
      'kingu terminal rename --terminal term_abc123 --title "RUNNER"',
      'kingu terminal rename --terminal term_abc123 --json'
    ]
  },
  {
    path: ['terminal', 'split'],
    ...
    usage:
      'kingu terminal split [--terminal <handle>] [--direction horizontal|vertical] [--command <text>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'terminal', 'direction', 'command'],
    examples: [
      'kingu terminal split --terminal term_abc123 --direction horizontal --json',
      'kingu terminal split --terminal term_abc123 --command "codex"'
    ]
  }
]
