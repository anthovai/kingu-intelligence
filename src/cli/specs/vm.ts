...
export const VM_COMMAND_SPECS: CommandSpec[] = [
  {
    ...
    usage:
      'kingu vm recipe doctor <recipe-id> [--repo-path <path>] [--provision|--connect] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'recipe-id', 'repo-path', 'provision', 'connect'],
    positionalArgs: ['recipe-id'],
    notes: [
      'Reads environmentRecipes from kingu.yaml in the repo path, validates the selected recipe, and reports agent-friendly checks.',
    ...
    examples: [
      'kingu vm recipe doctor cloud-sandbox',
      'kingu vm recipe doctor cloud-sandbox --repo-path /path/to/repo --json',
      'kingu vm recipe doctor cloud-sandbox --provision --json'
    ]
  }
]
