...
export const SKILL_COMMAND_SPECS: CommandSpec[] = [
  {
    ...
    usage: 'kingu skills installed [--json]',
    ...
    notes: [
      ...
      'Use an exact ID or an unambiguous name with `kingu skills share --skill <selector>`.'
    ]
  },
  {
    path: ['skills', 'share'],
    ...
    usage:
      'kingu skills share --skill <selector> [--skill <selector> ...] --bundle-name <name> ' +
    ...
    notes: [
      ...
      'The bundle name may be human-readable; Kingu converts it to a portable lowercase package name.',
      'Selectors are exact discovery IDs or unambiguous names from `kingu skills installed`.',
    ...
    examples: [
      'kingu skills share --skill frontend --bundle-name "Frontend Skills"',
      'kingu skills share --skill frontend --skill testing --bundle-name "Team Toolkit" --json'
    ]
  },
  {
    path: ['skills', 'list'],
    summary: 'List version-matched skill guides bundled with this Kingu CLI',
    usage: 'kingu skills list [--json]',
    allowedFlags: [...GLOBAL_FLAGS],
    notes: [
      'Reads bundled guide metadata locally without contacting the Kingu runtime.',
      'With --json, prints a topics array of canonical names and one-line descriptions.',
      'Use `kingu skills get <name>` for the compact guide, `--full` for its full reference package, or `kingu skills install` to install skills.'
    ]
  },
  {
    path: ['skills', 'get'],
    ...
    usage: 'kingu skills get <topic> [--full | --reference <name>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'topic', 'full', 'reference', 'references'],
    positionalArgs: ['topic'],
    notes: [
      'Reads bundled guide content locally without contacting the Kingu runtime.',
  ...
  {
    path: ['skills', 'install'],
    summary: 'Install bundled Kingu skills via the community skills CLI',
    usage:
      'kingu skills install [--skill <name>]... [--all] [--agent <name>[,<name>]] ' +
      '[--local] [--dry-run] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'skill', 'all', 'agent', 'local', 'dry-run'],
    notes: [
      'Reads the bundled skill registry locally without contacting the Kingu runtime.',
      'Resolves to the same `npx skills add <repo> --skill <name> ...` command used by ' +
        'Kingu Settings, plus the non-interactive flags an unattended host needs ' +
      ...
      'Targets the coding agents Kingu detects on this host, plus the shared ' +
      ...
      'Use --agent <name>[,<name>...] to choose targets yourself, or --agent universal ' +
        'for the shared directory alone. Required when Kingu detects no agent.',
    ...
    examples: [
      'kingu skills install',
      'kingu skills install --skill kingu-cli --skill orchestration',
      'kingu skills install --skill kingu-cli --local',
  ...
  {
    path: ['skills', 'update'],
    summary: 'Update already-installed Kingu skills via the community skills CLI',
    usage: 'kingu skills update [--skill <name>]... [--all] [--local] [--dry-run] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'skill', 'all', 'local', 'dry-run'],
    notes: [
      'Reads the bundled skill registry locally without contacting the Kingu runtime.',
      'Resolves to the same `npx skills update <names...>` command used by Kingu Settings, ' +
      ...
      'Only refreshes skills that are already installed; use `kingu skills install` first.',
      'Use --dry-run to print the resolved command without running it.',
    ...
    examples: [
      'kingu skills update',
      'kingu skills update --skill kingu-cli --skill orchestration',
      'kingu skills update --skill kingu-cli --local',
      'kingu skills update --all --dry-run'
    ]
  }
]
