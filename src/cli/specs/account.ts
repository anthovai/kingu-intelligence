...
export const ACCOUNT_COMMAND_SPECS: CommandSpec[] = [
  {
    path: ['account', 'add'],
    summary: 'Add a managed Claude or Codex account by signing in on this Kingu host',
    usage: 'kingu account add [--agent claude|codex] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'agent'],
    notes: [
      'Runs the agent login (`claude login` / `codex login`) in this terminal, then registers the account with the local Kingu runtime.',
      ...
      '--agent defaults to claude. Requires the Kingu runtime to be running on this machine.'
    ],
    examples: ['kingu account add', 'kingu account add --agent codex']
  },
  {
    path: ['account', 'list'],
    summary: 'List managed Claude and Codex accounts on this Kingu host',
    usage: 'kingu account list [--json]',
