...
import { readKinguCliVersion } from './cli-version'
...
// Why: the SSH relay bridge executes this CLI on the Kingu host while the
// caller's shell cwd lives on the remote machine (which cannot be chdir'd
// into). KINGU_CLI_CWD carries that remote cwd so cwd-based selectors like
// `--worktree active` resolve against the caller's directory.
function resolveInvocationCwd(): string {
  const override = process.env.KINGU_CLI_CWD
...
export async function main(
  ...
  if (argv.length === 1 && (argv[0] === '--version' || argv[0] === '-v')) {
    const version = readKinguCliVersion()
    if (!version) {
      process.stderr.write('Could not determine the Kingu version for this build.\n')
      process.exitCode = 1
      return
    }
  ...
  try {
    ...
    // lookup so users do not get misleading "Kingu is not running" failures for
    ...
    // Why: only the explicit flag is asserted eagerly. An ambient KINGU_ENVIRONMENT is background
    ...
    // An ambient KINGU_ENVIRONMENT is checked for disagreement too — silently
      ...
      : await resolveHostFlagEnvironmentId(parsed.flags, {
          ...
          environmentSelector:
            typeof environmentSelector === 'string'
              ...
              : process.env.KINGU_ENVIRONMENT
                ? { value: process.env.KINGU_ENVIRONMENT, label: 'KINGU_ENVIRONMENT' }
                : null
        })
    ...
    // KINGU_PAIRING_CODE / KINGU_ENVIRONMENT env-var fallback for commands
...
async function runClaudeTeams(argv: string[], cwd: string): Promise<void> {
  try {
    // Why: everything after `kingu claude-teams` belongs to Claude Code, not
    // Kingu's own flag parser, so new Claude flags work without Kingu changes.
...
async function runAgentTeamsTmuxShim(argv: string[]): Promise<void> {
  try {
    ...
    const response = await client.call<{
      ...
      {
        teamId: process.env.KINGU_AGENT_TEAMS_TEAM_ID,
        token: process.env.KINGU_AGENT_TEAMS_TOKEN,
