...
describe('kingu cli worktree awareness', () => {
  ...
  it('formats group orchestration sends in text mode', async () => {
    process.env.KINGU_TERMINAL_HANDLE = 'term_sender'
  ...
  it('passes dev mode to injected orchestration dispatches', async () => {
    process.env.KINGU_TERMINAL_HANDLE = 'term_sender'
    process.env.KINGU_USER_DATA_PATH = '/tmp/kingu-dev'
    callMock.mockResolvedValueOnce({
      id: 'req_dispatch',
      ok: true,
      result: {
  ...
  it('passes dev mode from an explicit dev CLI marker with a custom profile path', async () => {
    process.env.KINGU_TERMINAL_HANDLE = 'term_sender'
    process.env.KINGU_USER_DATA_PATH = '/tmp/federation-acceptance-profile'
