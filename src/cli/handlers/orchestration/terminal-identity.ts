...
export function throwNoActiveSenderTerminal(): never {
  ...
  // KINGU_TERMINAL_HANDLE is a different case — that caller HAS an identity, so it keeps the advice
  // to re-run under a live one.
  if (isStructuredSessionWithoutIdentity() && !process.env.KINGU_TERMINAL_HANDLE) {
  ...
  throw new RuntimeClientError(
    ...
    'Could not determine the sender terminal for this orchestration command. ' +
      ...
      'or run the command inside a live Kingu terminal with KINGU_TERMINAL_HANDLE set.'
  )
}
