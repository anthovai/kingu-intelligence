...
describe('kingu emulator CLI handlers', () => {
  ...
  it('uses the folder workspace exported by the current Kingu terminal', async () => {
    process.env.KINGU_WORKSPACE_ID = 'folder:folder-1'
    delete process.env.KINGU_WORKTREE_ID
  ...
  it('uses the current git worktree exported by the Kingu terminal', async () => {
    process.env.KINGU_WORKSPACE_ID = 'folder:stale-parent'
    process.env.KINGU_WORKTREE_ID = 'repo-1::/repo/project '
    callMock.mockResolvedValue(
      okFixture('req_attach', {
        attached: true,
        info: { deviceUdid: 'device-1', streamUrl: 'scrcpy://device-1' }
