...
afterEach(() => {
  ...
  if (originalPaneKey === undefined) {
    ...
    process.env.KINGU_PANE_KEY = originalPaneKey
  }
  if (originalStructuredSession === undefined) {
    delete process.env.KINGU_STRUCTURED_SESSION
  } else {
    process.env.KINGU_STRUCTURED_SESSION = originalStructuredSession
  }
})
...
describe('orchestration send structured payload flags', () => {
  ...
  it('sends lifecycle messages from KINGU_TERMINAL_HANDLE without a liveness probe', async () => {
    process.env.KINGU_TERMINAL_HANDLE = 'term_worker_env'
  ...
  it.each(['worker_done', 'heartbeat'] as const)(
    ...
    async (type) => {
      process.env.KINGU_TERMINAL_HANDLE = 'term_worker_env'
      process.env.KINGU_PANE_KEY = 'tab_worker:leaf_worker'
  ...
  it('passes KINGU_PANE_KEY as the sender pane identity', async () => {
    process.env.KINGU_TERMINAL_HANDLE = 'term_worker_env'
    process.env.KINGU_PANE_KEY = 'tab_worker:leaf_worker'
  ...
  it('refuses a structured session without naming a handle it could pass', async () => {
    process.env.KINGU_STRUCTURED_SESSION = '1'
...
describe('orchestration timeout flag validation', () => {
  ...
  it('passes a parsed check timeout and peek mode into the RPC payload', async () => {
    process.env.KINGU_TERMINAL_HANDLE = 'term_worker'
    ...
    expect(callMock).toHaveBeenCalledWith('orchestration.check', {
      ...
      types: undefined,
      format: undefined,
      compatibilityCliCommand: expect.stringMatching(/^kingu(?:-ide)?$/),
      run: undefined,
      ack: undefined,
      wait: true,
      timeoutMs: 250
