...
// and serveKinguApp binding are referenced by the module under test.
...
describe('kingu claude-teams CLI handler', () => {
  ...
  beforeEach(() => {
    ...
    previousPaneKey = process.env.KINGU_PANE_KEY
    previousExitCode = process.exitCode
    // The `kingu` launcher runs Kingu's Electron binary as Node, so the CLI process
    // itself carries ELECTRON_RUN_AS_NODE=1. Reproduce that inherited flag here.
    process.env.ELECTRON_RUN_AS_NODE = '1'
    process.env.KINGU_PANE_KEY = 'tab-1:leaf-1'
  })
  ...
  afterEach(() => {
    ...
    if (previousPaneKey === undefined) {
      ...
      process.env.KINGU_PANE_KEY = previousPaneKey
    }
  ...
  it.skipIf(isWindows)(
    ...
    async () => {
      const previousMarker = process.env.KINGU_TEST_MARKER
      process.env.KINGU_TEST_MARKER = 'keep-me'
      try {
        await runClaudeTeams()
      } finally {
        if (previousMarker === undefined) {
          delete process.env.KINGU_TEST_MARKER
        } else {
          process.env.KINGU_TEST_MARKER = previousMarker
        }
      }

      const spawnEnv = spawnMock.mock.calls.at(-1)?.[2].env as SpawnEnv
      expect(spawnEnv.KINGU_TEST_MARKER).toBe('keep-me')
