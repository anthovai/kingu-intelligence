...
const originalPackagedLauncher = process.env.KINGU_WINDOWS_PACKAGED_CLI_LAUNCHER
...
describe('packaged Windows legacy ask protocol', () => {
  beforeEach(() => {
    callMock.mockReset()
    process.env.KINGU_WINDOWS_PACKAGED_CLI_LAUNCHER = '1'
  ...
  afterEach(() => {
    ...
    restoreEnv('KINGU_CLI_COMMAND', originalCliCommand)
    restoreEnv('KINGU_WINDOWS_PACKAGED_CLI_LAUNCHER', originalPackagedLauncher)
    restoreEnv('KINGU_TERMINAL_HANDLE', originalTerminalHandle)
  })

  it.each(['kingu', 'kingu-ide'] as const)(
    'commits with the %s launcher and exits 75 before resume',
    async (command) => {
      process.env.KINGU_CLI_COMMAND = command
  ...
  it('resumes the committed question without another exit-75 handoff', async () => {
    process.env.KINGU_CLI_COMMAND = 'kingu'
    callMock.mockResolvedValue({
      result: {
        answer: 'yes',
        messageId: 'msg_question',
    ...
    expect(callMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        question: undefined,
        resume: 'msg_question',
        compatibilityWindowsCommand: 'kingu'
      }),
      expect.any(Object)
    )
