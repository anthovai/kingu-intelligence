...
export const ENVIRONMENT_HANDLERS: Record<string, CommandHandler> = {
    ...
    if (typeof requestedName === 'string') {
      ...
      if (current.result.machineName === undefined) {
        throw new RuntimeClientError(
          ...
          'This Kingu runtime does not support machine names. Update Kingu on that host and try again.'
        )
      }
  ...
  // Why: an agent told "run it on <name>" had nowhere to look. `kingu environment list` showed
  ...
  'host list': async ({ client, flags, json }) => {
    rejectLocalPairingStoreRetargeting(
      flags,
      '`kingu host list`. It answers from this machine\u2019s own pairing store, so a routed answer would name servers paired with a different machine.',
      'Run `kingu host list` on that machine to see the SSH targets registered there.'
    )
    ...
    // it slow and offline-fragile. `kingu host name --environment <name>` asks one server directly.
  ...
  'environment list': async ({ flags, json }) => {
    rejectLocalPairingStoreRetargeting(
      flags,
      '`kingu environment list`. Paired servers are stored on this machine, so there is no other host to ask.',
      'Run `kingu environment list` on that machine to see the servers paired with it.'
    )
