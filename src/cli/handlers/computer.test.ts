...
describe('kingu computer observation CLI handlers', () => {
  beforeEach(() => {
    vi.restoreAllMocks()
    callMock.mockReset()
  ...
  it('prints targeted permission setup command help', async () => {
    ...
    expect(output).toContain(
      'kingu computer permissions [--id <accessibility|screenshots>] [--json]'
    )
  ...
  it('prints provider capabilities without resolving a worktree', async () => {
    queueFixtures(
      ...
      okFixture('req_capabilities', {
        platform: 'darwin',
        provider: 'kingu-computer-use-macos',
    ...
    expect(vi.mocked(console.log).mock.calls[0][0]).toContain('kingu-computer-use-macos')
  })
  ...
  it('opens computer permission setup without resolving a worktree', async () => {
    queueFixtures(
      ...
      okFixture('req_permissions', {
        platform: 'darwin',
        helperAppPath: '/Applications/Kingu Computer Use.app',
    ...
    const output = vi.mocked(console.log).mock.calls[0][0]
    expect(output).toContain('Opened Kingu Computer Use permission setup')
    expect(output).toContain('/Applications/Kingu Computer Use.app')
  })
