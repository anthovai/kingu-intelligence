...
function rejectForwardedSkillFilesystem(ctx: HandlerContext, command: string): void {
  if (!process.env.KINGU_CLI_CWD && !ctx.client.isRemote) {
    return
  }
  throw new RuntimeClientError(
    'invalid_environment',
    `kingu skills ${command} must run on the machine whose installed skills you want to use. Run the command from an Kingu terminal on that machine.`
  )
}
...
function requireCloudOperation<T>(operation: SkillCloudOperation<T>): T {
  ...
  if (operation.status === 'reconnect-required') {
    throw new RuntimeClientError('authentication_required', 'Sign in to Kingu and try again.')
  }
...
async function callShare(
  ...
  try {
    ...
    if (error instanceof RuntimeRpcFailureError && error.code === 'method_not_found') {
      throw new RuntimeClientError(
        'update_required',
        'The connected Kingu runtime does not support agent skill sharing yet. Update Kingu on that machine and try again.'
      )
    }
    throw error
  }
...
export const SKILL_SHARING_HANDLERS: Record<string, CommandHandler> = {
  ...
  'skills share': async (ctx) => {
    ...
    if (skillSelectors.length === 0) {
      throw new RuntimeClientError(
        'invalid_argument',
        'Select at least one installed skill with --skill. Run `kingu skills installed` to list them.'
      )
    }
