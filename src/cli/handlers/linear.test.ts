...
vi.mock('../runtime-client', async () => {
  class RuntimeClient {
    ...
    constructor(
      ...
      remotePairingCode = process.env.KINGU_PAIRING_CODE ?? null,
...
describe('kingu linear CLI handlers', () => {
  ...
  beforeEach(() => {
    ...
    // Why: these tests can run inside an Kingu-managed terminal, which exports
  ...
  it('passes verified current-context hints without resolving cwd for remote runtimes', async () => {
    process.env.KINGU_TERMINAL_HANDLE = 'term_123'
    process.env.KINGU_WORKTREE_ID = 'repo::/srv/app'
    process.env.KINGU_PAIRING_CODE = 'kingu://pair?payload=bad'
    queueFixtures(callMock, okFixture('req_linear', issueResult()))
