...
vi.mock('./launch', () => ({ launchKinguApp: vi.fn() }))
...
const RESTART_OR_ABSENT_ADVICE =
  /Restart Kingu and try again|Kingu is not running|Run 'kingu open' first/
...
describe('runtime access denied', () => {
  it.each(['EPERM', 'EACCES'])('classifies a %s connect without restart advice', async (code) => {
    ...
    expect(human).toContain(
      `Permission denied connecting to Kingu (${code}). Kingu may be running normally`
    )
    expect(human).toContain("Next step: Do not restart Kingu or run 'kingu open'")
  ...
  it('names the Codex sandbox only when CODEX_SANDBOX is set', async () => {
    ...
    expect(human).toContain('The Codex sandbox blocked this command from connecting to Kingu')
  ...
  // Why: a dead Kingu leaves its socket behind, and the sandbox denies it before ECONNREFUSED.
  it('gives not-running advice when the denied endpoint belongs to a dead pid', async () => {
    mockKill('ESRCH')
    const human = formatCliError(await deniedRequest('EPERM'))

    expect(human).toContain("Kingu is not running. Run 'kingu open' first.")
  ...
  it('does not launch or poll Kingu when the initial status is denied', async () => {
    const pending = new RuntimeClient('/test', 1000, null, null).openKingu()
    failConnect('EPERM')
