  ...
  try {
    ...
    if (!metadata || !findTransport(metadata, 'unix', 'named-pipe') || !metadata.authToken) {
      throw new RuntimeClientError(
        'runtime_unavailable',
        `Kingu runtime metadata is incomplete at ${metadataPath}`
      )
    }
    return metadata
  } catch (error) {
    ...
    throw new RuntimeClientError(
      'runtime_unavailable',
      `Could not read Kingu runtime metadata at ${metadataPath}. Start the Kingu app first.`
    )
  }
}
...
export function getDefaultUserDataPath(
  ...
  // Why: in dev mode (and for parallel Kingu instances), the Electron app writes
  // runtime metadata to a separate userData directory (e.g. `kingu-dev`) to avoid
  ...
  if (process.env.KINGU_USER_DATA_PATH) {
    return process.env.KINGU_USER_DATA_PATH
  }
  if (platform === 'darwin') {
    return join(homeDir, 'Library', 'Application Support', 'kingu')
  }
  if (platform === 'win32') {
    ...
    if (!appData) {
      throw new RuntimeClientError(
        'runtime_unavailable',
        'APPDATA is not set, so the Kingu runtime metadata path cannot be resolved.'
      )
    }
    return join(appData, 'kingu')
  }
  ...
  return join(process.env.XDG_CONFIG_HOME || join(homeDir, '.config'), 'kingu')
}
