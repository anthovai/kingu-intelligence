...
export function attachUnverifiedTerminalPromptRecovery(error: unknown): RuntimeClientError {
  ...
  return attachUnknownTerminalPromptRecovery(
    normalized,
    'Kingu cannot prove whether the prompt was delivered by the prompt-delivery-capable runtime from the preflight',
    [
      INSPECT_STEP,
      'A different Kingu runtime answered than the one whose prompt-delivery support was verified; confirm which runtime serves this host before sending again.'
    ]
  )
}
