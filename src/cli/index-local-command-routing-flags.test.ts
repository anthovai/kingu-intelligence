...
describe('runtime-selector flags on locally pinned CLI commands', () => {
  ...
  it('routes `host name` through an ambient KINGU_ENVIRONMENT, unlike the pinned `host list`', async () => {
    ...
    process.env.KINGU_ENVIRONMENT = 'm4air'
    pairRuntimeEnvironment(listEnvironmentsMock, 'env-m4air', 'm4air')
    fakeMachineNameRuntime('M4 Air')
    const logSpy = vi.spyOn(console, 'log').mockImplementation(() => {})
    runtimeClientConstructorMock.mockClear()
  ...
  it('reports an unreachable runtime as an error instead of inventing a name', async () => {
    ...
    callMock.mockRejectedValue(
      new RuntimeClientError('runtime_unavailable', 'Kingu is not running')
    )
    const logSpy = vi.spyOn(console, 'log').mockImplementation(() => {})
  ...
  it('rejects `host list --environment` instead of answering with a half-routed listing', async () => {
    ...
    expect(printed.error.message).toContain('`--environment` does not retarget `kingu host list`')
    expect(process.exitCode).toBe(1)
  ...
  it('rejects `environment list --environment` rather than repeating the local answer', async () => {
    ...
    expect(printed.error.message).toContain(
      '`--environment` does not retarget `kingu environment list`'
    )
    process.exitCode = 0
  })
  ...
  it('rejects `--pairing-code` on both listings for the same reason', async () => {
    ...
    await main(['host', 'list', '--pairing-code', 'kingu://pair?code=x', '--json'], '/tmp/repo')
    await main(
      ['environment', 'list', '--pairing-code', 'kingu://pair?code=x', '--json'],
      '/tmp/repo'
    )
  ...
  it('keeps `host list` local when KINGU_ENVIRONMENT is set ambiently', async () => {
