...
vi.mock('./runtime-client', async () => {
  ...
  return {
    ...
    serveKinguApp: vi.fn(),
    getDefaultUserDataPath: vi.fn(() => '/tmp/kingu-user-data')
  }
})
...
describe('kingu skills CLI', () => {
  ...
  it('shows leaf, group, and root help for skills', async () => {
    ...
    expect(String(logSpy.mock.calls[1]?.[0])).toContain(
      'install            Install bundled Kingu skills'
    )
    expect(String(logSpy.mock.calls[1]?.[0])).toContain(
      'update             Update already-installed Kingu skills'
    )
  ...
  it('lists installable skills when no --skill/--all is given', async () => {
    ...
    expect(stdoutText(stdoutSpy)).toBe(
      [
        ...
        '  zeta',
        '',
        'Usage: kingu skills install --skill <name> [--skill <name> ...]',
        '   or: kingu skills install --all',
        ''
      ].join('\n')
    )
      ...
      JSON.stringify(
        {
          ...
          error: {
            ...
            message:
              "kingu skills install --json only supports --dry-run. Real installs stream npx's " +
  ...
  it('prints the resolved install command without running it for --dry-run', async () => {
    ...
    expect(stdoutText(stdoutSpy)).toBe(
      'npx --yes skills add https://github.com/anthovai/kingu-intelligence --skill alpha --global --agent claude-code --agent universal -y\n\n' +
  ...
  it('drops --global for --local in the dry-run command and JSON', async () => {
    ...
    expect(stdoutText(stdoutSpy)).toBe(
      'npx --yes skills add https://github.com/anthovai/kingu-intelligence --skill alpha --agent claude-code --agent universal -y\n\n' +
        'Rerun without --dry-run to install now.\n'
    )
  ...
  it('lists updatable skills when no --skill/--all is given', async () => {
    ...
    expect(stdoutText(stdoutSpy)).toBe(
      [
        ...
        '  zeta',
        '',
        'Usage: kingu skills update --skill <name> [--skill <name> ...]',
        '   or: kingu skills update --all',
        ''
      ].join('\n')
    )
    expect(spawnMock).not.toHaveBeenCalled()
  ...
  it('refuses a real run when the shell forwards kingu to the Kingu host', async () => {
    vi.stubEnv('KINGU_CLI_CWD', '/home/alice/wt')
    const errorSpy = vi.spyOn(console, 'error').mockImplementation(() => {})

    await main(['skills', 'install', '--skill', 'alpha'], '/tmp/repo')

    // Why: the SSH relay and WSL bridge run argv on the Kingu host, so a real
  ...
  it('puts the resolved npx directory on the child PATH', async () => {
    ...
    const npxBin = mkdtempSync(join(tmpdir(), 'kingu-npx-'))
  ...
  it('leaves PATH untouched when no node ships beside the resolved npx', async () => {
    ...
    const npxBin = mkdtempSync(join(tmpdir(), 'kingu-npx-bare-'))
  ...
  it('refuses to install when Kingu detects no agent, instead of targeting them all', async () => {
  ...
  it('maps detected agents onto the skills CLI namespace, not Kingu ids', async () => {
    const stdoutSpy = vi.spyOn(process.stdout, 'write').mockImplementation(() => true)
    ...
    // Kingu's `claude` is `claude-code` and its `rovo` is `rovodev`.
  ...
  it('reports the command it is about to run on stderr', async () => {
    ...
    expect(stderrSpy).toHaveBeenCalledWith(
      'Running: npx --yes skills add https://github.com/anthovai/kingu-intelligence --skill alpha --global --agent claude-code --agent universal -y\n'
    )
  })
      ...
      JSON.stringify(
        {
          ...
          error: {
            code: 'invalid_argument',
            message:
              "kingu skills update --json only supports --dry-run. Real updates stream npx's " +
              "own output, which isn't JSON."
          },
          _meta: { runtimeId: null }
        },
