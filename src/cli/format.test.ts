...
afterEach(() => {
  vi.restoreAllMocks()
  delete process.env.KINGU_COMPUTER_SCREENSHOT_TMPDIR
...
describe('formatCliError', () => {
  it('prints structured computer-use startup recovery steps', () => {
    const error = new RuntimeRpcFailureError({
      ...
      error: {
        ...
        data: {
          nextSteps: [
            'Run `kingu computer list-apps --json` and retry with the exact app name or bundle ID.',
            'If the target is a website or web app such as Gmail, choose the desktop browser app/window that contains it; `kingu computer` app selectors refer to desktop apps, not website names.',
            'Do not retry the same `kingu computer ... --app <web app>` command unchanged.',
            'If the desired browser is not listed, open or focus that browser first, then retry `kingu computer list-apps --json` and `kingu computer list-windows --app <browser> --json`.'
          ]
        }
      },
    ...
    expect(output).toContain('Next step: Run `kingu computer list-apps --json`')
    expect(output).toContain('desktop browser app/window')
    expect(output).toContain('--app <web app>')
    expect(output).not.toContain('kingu goto')
  })
  ...
  it('preserves orchestration migration recovery in human and JSON errors', () => {
    const error = new RuntimeRpcFailureError({
      ...
      error: {
        ...
        data: {
          ...
          nextSteps: ['Using this same Kingu CLI executable, run: skills get orchestration --full']
        }
      },
      _meta: { runtimeId: 'runtime-1' }
    })
    ...
    expect(formatCliError(error)).toContain(
      'Next step: Using this same Kingu CLI executable, run: skills get orchestration --full'
    )
...
describe('formatAutomationShow', () => {
  ...
  it('shows explicit run context before the legacy repo id', () => {
    ...
    expect(output).toContain('runProjectId: github:anthovai/kingu-intelligence')
    ...
    expect(output).toContain('runPath: /srv/kingu')
...
describe('formatComputerAction', () => {
  it('includes routed worktree and explicit window target in the suggested follow-up command', () => {
    ...
    expect(output).toContain(
      `Use \`kingu computer get-app-state --app ${quoteCliCommandArgument('Text Editor')} --worktree id:repo::/tmp/repo --window-id 99\``
    )
  ...
  it('preserves explicit window-index targeting in the suggested follow-up command', () => {
    ...
    expect(output).toContain(
      'Use `kingu computer get-app-state --app com.apple.finder --session manual --window-index 1`'
    )
  })
  ...
  it('surfaces action screenshot failures in pretty output', () => {
    ...
    expect(output).toContain(
      'Use `kingu computer get-app-state --app com.apple.finder --window-id 42`'
    )
  ...
  it('drops stale requested window selectors after a window-changed fallback', () => {
    ...
    expect(output).toContain(
      'Use `kingu computer get-app-state --app com.apple.finder --session manual --window-id 42`'
    )
  ...
  it('uses snapshot window index for follow-up commands when the provider has no stable window id', () => {
    ...
    expect(output).toContain(
      `Use \`kingu computer get-app-state --app ${quoteCliCommandArgument('Linux Browser')} --window-index 2\``
    )
...
describe('printResult computer screenshots', () => {
  ...
  it('removes expired screenshot temp files when cleanup is due', () => {
    testScreenshotDir = mkdtempSync(join(tmpdir(), 'kingu-format-test-'))
    process.env.KINGU_COMPUTER_SCREENSHOT_TMPDIR = testScreenshotDir
  ...
  it('keeps inline screenshot data when temp export fails', () => {
    testScreenshotDir = join(tmpdir(), `kingu-format-blocked-${Date.now()}`)
