...
describe('command aliases dispatch to the canonical handler', () => {
  ...
  it('keeps `agent-context` local when remote environment variables are set', async () => {
    vi.stubEnv('KINGU_PAIRING_CODE', 'pairing-code')
    vi.stubEnv('KINGU_ENVIRONMENT', 'stale-environment')
    try {
      await main(['agent-context', '--json'], '/tmp/repo')
...
describe('artifact runtime routing', () => {
  ...
  it('uses the desktop runtime despite remote-selection environment fallbacks', async () => {
    vi.stubEnv('KINGU_ENVIRONMENT', 'remote-environment')
    vi.stubEnv('KINGU_PAIRING_CODE', 'remote-pairing-code')
...
describe('unknown command surfaces a suggestion', () => {
  ...
  it('prints did-you-mean for a near-miss command and exits non-zero', async () => {
    ...
    expect(stderr).toContain('kingu worktree')
  })
  ...
  it('names the offending --worktree value and the valid forms on selector_not_found', async () => {
    ...
    expect(process.exitCode).toBe(1)
    const stderr = errorSpy.mock.calls.map((call) => String(call[0])).join('\n')
    expect(stderr).toContain('No Kingu workspace matched the worktree selector "repo-1"')
...
describe('unknown help command surfaces a suggestion', () => {
  it.each([
    ...
    expect(process.exitCode).toBe(1)
    expect(logSpy.mock.calls.flat().join('\n')).toContain('Did you mean: kingu worktree')
    logSpy.mockRestore()
    process.exitCode = 0
  })
})
  ...
  it.each([
    ...
    async (_, path, commands) => {
      ...
      try {
        ...
        expect(output).toContain(`kingu ${path.join(' ')}`)
...
describe('kingu root help', () => {
  ...
  it('advertises host-local account management', async () => {
    ...
    expect(logSpy.mock.calls.flat().join('\n')).toContain(
      'account add               Add a managed Claude or Codex account on this Kingu host'
    )
    expect(logSpy.mock.calls.flat().join('\n')).toContain(
      'account list              List managed Claude and Codex accounts on this Kingu host'
    )
    logSpy.mockRestore()
  })
  ...
  it('advertises computer-use capabilities discovery', async () => {
    ...
    expect(logSpy.mock.calls[0][0]).toContain(
      'orchestration worker-start Start a supervised worker locally or on a connected Kingu server'
    )
    expect(logSpy.mock.calls[0][0]).toContain(
  ...
  it('progressively discloses Linear commands', async () => {
    ...
    expect(groupHelp).toContain('kingu linear')
    ...
    expect(issueHelp).toContain('kingu linear issue [<id>]')
    ...
    expect(searchHelp).toContain('kingu linear search <query>')
  ...
  it('hides removed parent-workspace help and scopes create parent selectors', async () => {
    ...
    expect(createHelp).not.toContain('active Kingu workspace')
    ...
    expect(createHelp).toContain(
      '--no-parent only affects Kingu lineage; omit --base-branch to use the repo default base'
    )
