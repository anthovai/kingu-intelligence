...
export async function startHeadlessPairingRuntime({
  ...
  const runDir = mkdtempSync(path.join(os.tmpdir(), 'kingu-mobile-run.'))
  ...
  const child = spawn(
    ...
    ['serve', '--mobile-pairing', '--pairing-address', pairingAddress, '--json'],
    {
      ...
      env: {
        ...process.env,
        KINGU_E2E_USER_DATA_DIR: userData,
        KINGU_E2E_HOME_DIR: homeDir,
        HOME: homeDir,
        USERPROFILE: homeDir
      },
...
export async function registerWorktreeForPairingRuntime(runtime, worktree, tools) {
  ...
  await tools.kingu(['repo', 'add', '--path', worktree, '--json'], {
    cwd: worktree,
    env: runtime.env,
    timeout: 60000
  })
...
async function waitForPairingRuntime({ child, userData, pairingAddress, logSuccess }) {
  ...
  const runtimeResult = (pairingUrl) => ({
    ...
    env: {
      ...process.env,
      KINGU_USER_DATA_PATH: userData,
      // Why: `kingu-dev` derives its own profile and ignores KINGU_USER_DATA_PATH, so
      // without this an KINGU_CLI=kingu-dev run would address the dev profile instead
      // of this disposable runtime. Plain `kingu` ignores it.
      KINGU_DEV_USER_DATA_PATH: userData
    },
    stop
  })
