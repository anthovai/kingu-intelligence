...
#
# Everything is driven with xcrun simctl rather than `kingu emulator`, because the
...
#     > /tmp/kingu-emulator-boot.log 2>&1 &
#
...
LOG="${3:-/tmp/kingu-emulator-boot.log}"
BUNDLE_ID=com.stably.kingu.mobile
# Long enough for the tiered backoff to reach its 30s/60s tail.
ESCALATE_SECONDS=200

APP=$(pgrep -f "CoreSimulator.*Kingu.app/Kingu" | head -1)
...
T0=$(date +%s)
step "[5] user returns to Kingu  <-- t0, desktop still down"
