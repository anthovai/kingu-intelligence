...
// Provenance: the last two resume windows of a real diagnostics export (Kingu
...
const HOST_OFFLINE_STEP =
  'Check the desktop is awake, Kingu is running, and it is signed in to Kingu Cloud.'
...
describe('diagnoseConnection on the other relay close codes', () => {
  ...
  it('leaves a director outage to the director branch', () => {
    ...
    expect(diagnose([appResumed(), lanOpening(), director]).likelyCause).toBe(
      'Relay service was temporarily unavailable and asked Kingu to retry in 30s.'
    )
  })
})
