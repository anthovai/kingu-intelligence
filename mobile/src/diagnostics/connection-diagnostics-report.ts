...
export function buildConnectionDiagnosticsReport(args: {
  ...
  const lines: string[] = []
  lines.push('Kingu Mobile connection diagnostics')
  lines.push(`Generated: ${new Date(now).toISOString()}`)
  lines.push(`App: Kingu Mobile ${args.appVersion} · ${args.platform}`)
  const desktopAppVersion = normalizeHostAppVersion(args.desktopAppVersion)
  lines.push(`Host Kingu version: ${desktopAppVersion ?? 'unknown'}`)
