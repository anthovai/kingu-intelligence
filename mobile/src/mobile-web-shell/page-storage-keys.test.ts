...
describe('the keys a page may read and write', () => {
  it('takes the ones the list keeps', () => {
    expect(isPageStorageKey('kingu:last-visited-worktree')).toBe(true)
    expect(isPageStorageKey('kingu:pins:host-1')).toBe(true)
    expect(isPageStorageKey('kingu:terminalTextScale')).toBe(true)
    expect(isPageStorageKey('kingu:nativeChatTabs:host-1:wt-1')).toBe(true)
  })

  it('refuses the rest of the namespace, including the flag that turns this on', () => {
    // Everything the app stores lives under `kingu:`, so a page that could write any of it could
    // turn the hybrid shell on for a build that never offered it.
    for (const key of [
      'kingu:mobileWebShellEnabled',
      'kingu:remotePushHostRegistrations',
      'kingu:pushServiceNotificationsEnabled',
      'kingu:home-snapshot:v1',
      'kingu:hosts'
    ]) {
  ...
  it('refuses the bare prefix, which names no host', () => {
    expect(isPageStorageKey('kingu:pins:')).toBe(false)
    expect(isPageStorageKey('kingu:nativeChatTabs:')).toBe(false)
  })

  it('refuses a key that only starts like an allowlisted one', () => {
    expect(isPageStorageKey('kingu:last-visited-worktree:other')).toBe(false)
    expect(isPageStorageKey('not-kingu:pins:host-1')).toBe(false)
  })

  it('refuses a key past the cap, whatever it starts with', () => {
    expect(isPageStorageKey(`kingu:pins:${'h'.repeat(PAGE_STORAGE_MAX_KEY_CHARS)}`)).toBe(false)
  })
  ...
  it('names what the shell reads out of the app store for a route with no workspace', () => {
    ...
    expect(keys).toEqual([...PAGE_STORAGE_EXACT_KEYS, 'kingu:pins:host-1'])
  ...
  it("adds the session route's two workspace-scoped keys, and only for it", () => {
    ...
    ).toEqual(['kingu:nativeChatTabs:host-1:wt-1', 'kingu:terminalLiveInputDisabled:host-1:wt-1'])
...
describe('the allowlist narrowed to one session', () => {
  ...
  it("refuses another host's pinned list, which the shape check alone admits", () => {
    expect(isPageStorageKey('kingu:pins:host-2')).toBe(true)
    expect(isPageStorageKeyForRoute('kingu:pins:host-2', 'host-1', SESSION_ROUTE)).toBe(false)
  })
  ...
  it("refuses another workspace's chat tabs, which the shape check alone admits", () => {
    ...
    expect(isPageStorageKey('kingu:nativeChatTabs:host-1:wt-2')).toBe(true)
    expect(
      isPageStorageKeyForRoute('kingu:nativeChatTabs:host-1:wt-2', 'host-1', SESSION_ROUTE)
    ).toBe(false)
    expect(
      isPageStorageKeyForRoute('kingu:nativeChatTabs:host-1:wt-1', 'host-1', SESSION_ROUTE)
    ).toBe(true)
  })

  it('refuses a workspace key on a route that names no workspace', () => {
    expect(isPageStorageKeyForRoute('kingu:nativeChatTabs:host-1:wt-1', 'host-1', HOST_ROUTE)).toBe(
      false
    )
  })
})
...
describe('what init may carry', () => {
  it('keeps a value at the bound and drops the one above it, naming what it dropped', () => {
    const held = {
      'kingu:terminalTextScale': '1',
      'kingu:mobileStructuredSendOperations:v1': 'x'.repeat(PAGE_STORAGE_MAX_VALUE_CHARS + 1)
    }
    const { entries, dropped } = pageStorageEntriesForInit(held)
    expect(Object.keys(entries)).toEqual(['kingu:terminalTextScale'])
    expect(dropped).toEqual(['kingu:mobileStructuredSendOperations:v1'])
  })

  it('keeps a value of exactly the bound, so the drop above discriminates', () => {
    const at = { 'kingu:custom-accessory-keys': 'x'.repeat(PAGE_STORAGE_MAX_VALUE_CHARS) }
