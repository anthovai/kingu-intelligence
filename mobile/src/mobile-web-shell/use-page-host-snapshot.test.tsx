...
const PINS = 'kingu:pins:host-1'
const LAST_VISITED = 'kingu:last-visited-worktree'
const CHAT_TABS = 'kingu:nativeChatTabs:host-1:wt-1'
...
describe('what the shell puts on every init', () => {
  ...
  it('never carries another host key, whatever the store holds', async () => {
    ...
    doubles.store.set('kingu:pins:host-2', '["theirs"]')
    const mounted = await mount()
    await act(async () => {
      mounted.view().refreshStorage()
    })
    ...
    await act(async () => {
      mounted.view().writeStorage('kingu:pins:host-2', '["theirs"]')
    })
    expect(mounted.view().readStorage().storage).toEqual({ [PINS]: '["mine"]' })
  })
  ...
  it("carries the session route's own workspace, and never the workspace beside it", async () => {
    ...
    doubles.store.set('kingu:nativeChatTabs:host-1:wt-2', '{"tab-9":"chat"}')
    const mounted = await mount()
    await act(async () => {
      mounted.view().refreshStorage()
    })
    ...
    await act(async () => {
      mounted.view().writeStorage('kingu:nativeChatTabs:host-1:wt-2', '{}')
    })
    expect(mounted.view().readStorage().storage).toEqual({ [CHAT_TABS]: '{"tab-1":"chat"}' })
  })
