 ...
 *
 * An allowlist and not a passthrough. Everything the app keeps under `kingu:` is in one namespace —
...
export const PAGE_STORAGE_EXACT_KEYS = [
  ...
  'kingu:terminal-accessory-layout',
  /** The user's own accessory keys, which `CustomKeyModal` writes. */
  'kingu:custom-accessory-keys',
  /** Whether a supported agent session opens on the terminal or the native chat. */
  'kingu:defaultSessionView',
  /** The durable send journal: which `agentSession.send` operation ids are still unsettled. */
  'kingu:mobileStructuredSendOperations:v1',
  /** The terminal's text scale, which pinch-to-zoom writes. */
  'kingu:terminalTextScale',
  /** Whether the terminal's command inputs offer autocorrect. */
  'kingu:terminalAutocompleteEnabled',
  /** Whether a terminal link opens in Kingu's browser or the phone's. */
  'kingu:terminalLinkOpenMode'
] as const

export const PAGE_STORAGE_KEY_PREFIXES = [
  /** `kingu:pins:<hostId>`: the pinned worktrees of the host whose list the page is showing. */
  'kingu:pins:',
  /** `kingu:nativeChatTabs:<hostId>:<worktreeId>`: which tabs of one workspace show the chat. */
  'kingu:nativeChatTabs:',
  /** `kingu:terminalLiveInputDisabled:<hostId>:<worktreeId>`: the handles typing goes around. */
  'kingu:terminalLiveInputDisabled:'
] as const
...
export function pageStorageKeysForRoute(hostId: string, routePathname: string): string[] {
  ...
  const scoped =
    workspace === null || workspace.hostId !== hostId
      ? []
      : [
          workspaceScopedKey('kingu:nativeChatTabs:', hostId, workspace.worktreeId),
          workspaceScopedKey('kingu:terminalLiveInputDisabled:', hostId, workspace.worktreeId)
        ]
  return [...PAGE_STORAGE_EXACT_KEYS, `kingu:pins:${hostId}`, ...scoped].filter(isPageStorageKey)
}

/**
 * The allowlist narrowed to one session, which is the one every write is actually held to.
 *
 * `isPageStorageKey` answers for the shape, so `kingu:pins:<any host>` passes it; a page opened for
