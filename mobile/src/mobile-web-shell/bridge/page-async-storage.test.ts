...
describe('what init primed', () => {
  it('answers a read without waiting on the shell', async () => {
    publish({ 'kingu:pins:host-1': '["wt-1"]' })
    ...
    await expect(pageAsyncStorage.getItem('kingu:pins:host-1')).resolves.toBe('["wt-1"]')
    await expect(pageAsyncStorage.getItem('kingu:pins:host-2')).resolves.toBeNull()
  })

  it('reads several keys at once, which is how the list asks', async () => {
    publish({ 'kingu:pins:host-1': '["wt-1"]' })
    await expect(
      pageAsyncStorage.multiGet(['kingu:pins:host-1', 'kingu:last-visited-worktree'])
    ).resolves.toEqual([
      ['kingu:pins:host-1', '["wt-1"]'],
      ['kingu:last-visited-worktree', null]
    ])
  })

  it('replaces what the last page held rather than adding to it', async () => {
    publish({ 'kingu:pins:host-1': '["wt-1"]' })
    publish({ 'kingu:pins:host-2': '["wt-2"]' })
    await expect(pageAsyncStorage.getItem('kingu:pins:host-1')).resolves.toBeNull()
  })
})

describe('a write the page makes', () => {
  it('goes to the app, and is readable here at once', async () => {
    await pageAsyncStorage.setItem('kingu:pins:host-1', '["wt-1","wt-2"]')
    expect(writes).toEqual([{ key: 'kingu:pins:host-1', value: '["wt-1","wt-2"]' }])
    await expect(pageAsyncStorage.getItem('kingu:pins:host-1')).resolves.toBe('["wt-1","wt-2"]')
  })

  it('removes by writing null, which is what the app then deletes', async () => {
    publish({ 'kingu:pins:host-1': '["wt-1"]' })
    await pageAsyncStorage.removeItem('kingu:pins:host-1')
    expect(writes).toEqual([{ key: 'kingu:pins:host-1', value: null }])
    await expect(pageAsyncStorage.getItem('kingu:pins:host-1')).resolves.toBeNull()
  })
  ...
  it('is dropped, and not kept, for a key outside the allowlist', async () => {
    ...
    await expect(
      pageAsyncStorage.setItem('kingu:mobileWebShellEnabled', 'true')
    ).resolves.toBeUndefined()
    expect(writes).toEqual([])
    await expect(pageAsyncStorage.getItem('kingu:mobileWebShellEnabled')).resolves.toBeNull()
  })
  ...
  it('carries each pair of a multi-write up to the first it cannot, and no further', async () => {
    ...
    expect(writes).toEqual([{ key: 'kingu:pins:host-1', value: '["wt-1"]' }])
  })
...
describe('what the page will not keep', () => {
  it("drops another host's pinned list, so a later read cannot answer with it", async () => {
    publish({ 'kingu:pins:host-1': '["mine"]' })
    await expect(
      pageAsyncStorage.setItem('kingu:pins:host-2', '["theirs"]')
    ).resolves.toBeUndefined()
    ...
    expect(writes).toEqual([])
    expect(await pageAsyncStorage.getItem('kingu:pins:host-2')).toBeNull()
  })

  it("drops another workspace's chat tabs on the session route it was not opened for", async () => {
    publish()
    await expect(
      pageAsyncStorage.setItem('kingu:nativeChatTabs:host-1:wt-2', '{}')
    ).resolves.toBeUndefined()
    expect(writes).toEqual([])
    // And the one it was opened for goes through, so the drop above is about the workspace.
    await pageAsyncStorage.setItem('kingu:nativeChatTabs:host-1:wt-1', '{}')
    expect(writes).toEqual([{ key: 'kingu:nativeChatTabs:host-1:wt-1', value: '{}' }])
  })
  ...
  it('refuses a value over the envelope bound rather than caching what the wire will drop', async () => {
    ...
    const refusal = await refusalOf(
      pageAsyncStorage.setItem('kingu:mobileStructuredSendOperations:v1', oversized)
    )
    ...
    expect(refusal.message).toContain('kingu:mobileStructuredSendOperations:v1')
    expect(refusal.message).toContain(String(PAGE_STORAGE_MAX_VALUE_CHARS))
    expect(writes).toEqual([])
    expect(await pageAsyncStorage.getItem('kingu:mobileStructuredSendOperations:v1')).toBeNull()
  })
  ...
  it('rejects a batch of two oversize pairs once, leaving no rejection nobody holds', async () => {
    ...
    const refusal = await refusalOf(
      pageAsyncStorage.multiSet([
        ['kingu:mobileStructuredSendOperations:v1', oversized],
        ['kingu:custom-accessory-keys', oversized]
      ])
    )
    ...
    expect(refusal.key).toBe('kingu:mobileStructuredSendOperations:v1')
    expect(writes).toEqual([])
  })
  ...
  it('still keeps a value exactly at the bound, so the refusal above discriminates', async () => {
    ...
    await pageAsyncStorage.setItem('kingu:last-visited-worktree', atBound)
    expect(writes).toEqual([{ key: 'kingu:last-visited-worktree', value: atBound }])
    expect(await pageAsyncStorage.getItem('kingu:last-visited-worktree')).toBe(atBound)
  })
  ...
  it('drops a write it may not make rather than rejecting, because nobody catches one', async () => {
    ...
    publish()
    await expect(
      pageAsyncStorage.setItem('kingu:notificationDeliveryPreferences', '{}')
    ).resolves.toBeUndefined()
    expect(writes).toEqual([])
    expect(await pageAsyncStorage.getItem('kingu:notificationDeliveryPreferences')).toBeNull()
  })

  it('drops a removal it may not make, for the same reason', async () => {
    publish()
    await expect(pageAsyncStorage.removeItem('kingu:pins:host-2')).resolves.toBeUndefined()
    expect(writes).toEqual([])
  })

  it('drops a write the shell would not take, rather than rejecting', async () => {
    granted = false
    await expect(pageAsyncStorage.setItem('kingu:pins:host-1', '["wt-1"]')).resolves.toBeUndefined()
    await expect(pageAsyncStorage.getItem('kingu:pins:host-1')).resolves.toBeNull()
  })
})
