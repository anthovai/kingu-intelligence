...
describe('the notify guard before init', () => {
  it('answers false for the grant-gated notifies and posts nothing', () => {
    ...
    expect(page.client.notifyStorageWrite('kingu:last-visited-worktree', 'value')).toBe(false)
...
describe('externalLink', () => {
  ...
  it('posts an allowed URL under its own grant', () => {
    const page = granted()
    expect(page.client.notifyExternalLink('https://github.com/anthovai/kingu-intelligence')).toBe(
      true
    )
    expect(page.frames().at(-1)).toEqual({
      v: BRIDGE_PROTOCOL_VERSION,
