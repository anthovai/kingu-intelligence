...
const STORAGE = { 'kingu:pins:host-1': '["wt-1"]' }
...
describe('the props one render passed', () => {
  ...
  it('routes each of them into the latest render, without rebuilding the host', async () => {
    ...
    for (const json of [
      ...
      clientFrame({ type: 'notify', name: 'storage', key: 'kingu:pins:host-1', value: '["wt-2"]' }),
      clientFrame({
        type: 'notify',
        name: BRIDGE_FAULT_GRANT,
        error: { category: 'Error', message: 'the route threw', isRpcDeliveryUnknown: false }
    ...
    expect({
      ...
      haptics: ['selection'],
      backPops: 1,
      storageWrites: [{ key: 'kingu:pins:host-1', value: '["wt-2"]' }],
