...
vi.mock('../components/KinguLogo', () => ({ KinguLogo: 'Logo' }))
...
let stopSync: () => void
const records = () => JSON.parse(mocks.storage.get('kingu:remotePushHostRegistrations') ?? '{}')
...
function expectChoiceComplete(entry: string) {
  expect(mocks.storage.get('kingu:pushServiceNotificationsEnabled')).toBe('true')
  if (entry === 'settings') {
...
it.each(['true', 'false'])(
  ...
  async (legacy) => {
    mocks.storage.set('kingu:pushNotificationsEnabled', legacy)
...
it('remembers Not now without registering and does not ask again', async () => {
  mocks.storage.set('kingu:pushNotificationsEnabled', 'true')
  const client = await connectedHost()
  await act(async () => {
  ...
  await drain()
  await expect(shouldPresentNotificationOptIn()).resolves.toBe(false)
  expect(mocks.storage.get('kingu:pushServiceNotificationsEnabled')).toBe('false')
  expect(getDevicePushToken).not.toHaveBeenCalled()
  expect(
...
it('exposes a failed consent write without scheduling or changing durable consent', async () => {
  ...
  await drain()
  expect(mocks.storage.has('kingu:pushServiceNotificationsEnabled')).toBe(false)
  expect(client.sendRequest).not.toHaveBeenCalled()
})
