...
// Same-named repos on different hosts (a local "kingu" and a runtime "kingu")
...
describe('every worktree stays visible regardless of repo.list contents', () => {
  it('shows worktrees whose repoId is missing from repo.list', () => {
    ...
    const worktrees = [
      worktree('kingu-local', 'a'),
      worktree('kingu-local', 'b'),
      worktree('kingu-runtime', 'c')
    ]
    const repoIdsByName = new Map([['kingu-runtime', 'kingu-runtime']])
  ...
  it('shows every worktree when the same repo name maps to two host-specific ids', () => {
    // The exact trigger: a local "kingu" and a runtime "kingu". repo.list returns
    // both under the same displayName; a name-keyed map keeps only the last id.
    const worktrees = [
      worktree('kingu-local', 'local-1', 'kingu'),
      worktree('kingu-local', 'local-2', 'kingu'),
      worktree('kingu-runtime', 'runtime-1', 'kingu')
    ]
    // Name-keyed map collapses "kingu" to a single id, as the host screen builds it.
    const repoIdsByName = new Map([['kingu', 'kingu-runtime']])
