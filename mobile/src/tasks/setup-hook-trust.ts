...
export function isSetupHookTrusted(
  trust: PersistedTrustedKinguHooks,
  repoId: string,
  contentHash: string
): boolean {
...
export function trustedKinguHooksWithSetupApproval(args: {
  trust: PersistedTrustedKinguHooks
  repoId: string
  contentHash: string
  alwaysTrust: boolean
  approvedAt?: number
}): PersistedTrustedKinguHooks {
...
export async function persistSetupHookTrustApproval(args: {
  ...
  contentHash: string
  alwaysTrust: boolean
}): Promise<PersistedTrustedKinguHooks> {
  const next = trustedKinguHooksWithSetupApproval(args)
  taskUiStateWrite.interpret(
    await taskUiStateWrite.request(args.client, { trustedKinguHooks: next })
  )
  return next
}
