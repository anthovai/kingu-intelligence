...
describe('the lock a microphone holds', () => {
  it('takes the tag once and gives it back once', async () => {
    ...
    const lock = createMicrophoneScreenLock(device, 'kingu-mic')
    lock.hold()
    lock.release()
    await flush()
    expect(calls).toEqual(['+kingu-mic', '-kingu-mic'])
  })
  ...
  it('asks the device nothing for a hold it already has', async () => {
    ...
    lock.hold()
    await flush()
    expect(calls).toEqual(['+kingu-mic'])
  })
  ...
  it('keeps working after a call the device refused', async () => {
    ...
    lock.hold()
    await flush()
    expect(calls).toEqual(['+kingu-mic', '-kingu-mic', '+kingu-mic'])
  })
})
