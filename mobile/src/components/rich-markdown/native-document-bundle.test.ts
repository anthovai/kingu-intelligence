...
function evaluateBundle(options: { prompt?: string | null } = {}) {
  ...
  const handle = window.__kinguRichMarkdown!
  evaluated.push(() => {
    Reflect.deleteProperty(globalThis, '__kinguRichMarkdown')
  })
...
describe('the bundled rich Markdown editor document', () => {
  ...
  it('takes markdown through the injected handle and gives back the source it was given', () => {
    ...
    inject(
      `window.__kinguRichMarkdown.setMarkdown(${escapeInjectedJavaScriptString(markdown)}, 7);`
    )
