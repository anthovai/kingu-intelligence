...
private let networkBlockIdentifier = "dev.kingu.mobile-web-shell.network-block-v1"
...
/// Installs `window.kinguBridge`, the whole page-facing surface: `postMessage(json)` and an
...
private let bridgeInstaller = """
  (function(){
  var sink=window.webkit.messageHandlers.kinguBridge;
  var handler=null;
  var bridge={};
  Object.defineProperty(bridge,'postMessage',{value:function(json){
  if(typeof json!=='string'){throw new TypeError('kinguBridge.postMessage expects a string')}
  ...
  Object.defineProperty(globalThis,'kinguBridge',{value:bridge,configurable:false,writable:false,enumerable:true});
  })();
  """
...
private let bridgeDeliver = """
  globalThis.kinguBridge.__deliver(m)
  """
...
private final class MobileWebShellSchemeHandler: NSObject, WKURLSchemeHandler {
  ...
  private let readQueue = DispatchQueue(label: "dev.kingu.mobile-web-shell.read")
...
private final class MobileWebShellBridgeReceiver: NSObject, WKScriptMessageHandler {
  weak var view: KinguMobileWebShellView?
...
final class KinguMobileWebShellView: ExpoView, WKNavigationDelegate, WKUIDelegate {
