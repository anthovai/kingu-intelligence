...
const DEFAULT_EXECUTABLE = '/Applications/Kingu.app/Contents/MacOS/Kingu'
const DEFAULT_PROFILE = path.join(os.homedir(), 'Library', 'Application Support', 'kingu')
const URL = 'https://example.com/kingu-terminal-garble-repro'
...
const runRoot = mkdtempSync(path.join(os.tmpdir(), 'kingu-terminal-garble-'))
const userDataDir = path.join(runRoot, 'user-data')
const evidenceDir = path.join(runRoot, 'evidence')
mkdirSync(userDataDir, { recursive: true })
mkdirSync(evidenceDir, { recursive: true })
function sanitizeProfile() {
  const sourceDataPath = path.join(sourceProfile, 'kingu-data.json')
  ...
  writeFileSync(path.join(userDataDir, 'kingu-data.json'), serialized)
...
try {
  ...
  app = await electron.launch({
    ...
    env: {
      ...
      ...(mainPath ? { NODE_ENV: 'development', KINGU_E2E_HEADFUL: '1' } : {})
    }
  })
