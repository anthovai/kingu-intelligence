...
const scratch = await mkdtemp(join(tmpdir(), 'kingu-omp-root-reader-'))
...
try {
  ...
  const { readNativeChatTranscript } = await import(
    source(kinguRoot, 'src/main/native-chat/transcript-reader.ts')
  )
  const { resolveSessionFilePath } = await import(
    source(kinguRoot, 'src/main/native-chat/session-file-resolver.ts')
  )
  const results = []
