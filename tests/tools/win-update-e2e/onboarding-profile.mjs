// Persistence seed for a fresh packaged Kingu profile: dismisses onboarding and
...
// kingu-data.json` BEFORE launch dismisses it. The app derives the Projects list
...
//
// IMPORTANT: seed only BEFORE the first launch. The app rewrites kingu-data.json
 ...
 * for it. A real checkout (init + one commit) is required — Kingu treats a
...
export function createSeededRepo(dir) {
  ...
  git('init', '-b', 'main')
  git('config', 'user.email', 'win-update-e2e@kingu.test')
...
export function seedFreshProfile(userDataDir, profile) {
  ...
  writeFileSync(path.join(userDataDir, 'kingu-data.json'), `${JSON.stringify(profile, null, 2)}\n`)
}
