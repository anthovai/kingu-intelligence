...
/**
 * Read-only discovery of the machine's current Kingu install registry state.
...
export function discoverInstallRegistryState() {
  const ps = [
    ...
    `  if ($dn -eq 'Kingu' -or ($il -and $il -match '\\\\Programs\\\\kingu')) {`,
    ...
    `  if ($il -and ($sn -eq 'Kingu' -or $il -match 'kingu')) {`,
...
/** The two directories NSIS writes Kingu shortcuts into: Start Menu + Desktop. */
...
/** Read-only: list Kingu *.lnk shortcuts under the given dirs (recursive). */
export function discoverKinguShortcuts(dirs = shortcutDirs()) {
  const found = []
  for (const dir of dirs) {
    collectKinguLnks(dir, found)
  }
  return found
}

function collectKinguLnks(dir, out) {
  if (!existsSync(dir)) {
    return
  }
  let entries = []
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      collectKinguLnks(full, out)
    } else if (entry.isFile() && /\.lnk$/i.test(entry.name) && /kingu/i.test(entry.name)) {
      out.push(full)
    }
  }
}
...
/**
 * Snapshot the shared install registry keys + Kingu shortcuts before an isolated
...
export function backupInstallState(runDir) {
  ...
  let i = 0
  for (const lnk of discoverKinguShortcuts(dirs)) {
 ...
 * Pre-existing shortcuts are copied back; test-created Kingu shortcuts are removed.
...
function restoreShortcuts(manifest, result) {
  ...
  for (const lnk of discoverKinguShortcuts(manifest.shortcutDirs)) {
...
function printMismatchWarning(manifest, expected, actual) {
  ...
  console.error('!! Your REAL Kingu install pointer may be hijacked to the test directory.')
  console.error('!! The next real Kingu update could install into the test location.')
...
if (process.argv[1] && path.resolve(process.argv[1]) === import.meta.filename) {
  ...
  console.log('[registry-shortcut-backup] Kingu shortcuts that would be backed up:')
  console.log(JSON.stringify(discoverKinguShortcuts(), null, 2))
}
