// Drive the installed, packaged Kingu app with Playwright's Electron driver.
//
...
/**
 * Launch the installed Kingu.exe. Pointing userDataDir at a harness-owned temp
 * dir isolates this run's daemon (its socket/token path becomes unique), so
 * daemon lookups never collide with other Kingu installs/daemons on the box.
 * Pass `seedProfile` (a buildFreshProfile object) to write kingu-data.json
...
export async function launchInstalledApp({
  ...
  const {
    ...
    KINGU_CODEX_HOME: _kinguCodexHome,
    ...cleanEnv
  } = process.env
  void _drop
  void _codexHome
  void _kinguCodexHome
...
const CLI_FEATURE_TIP_TITLE = 'Let agents drive Kingu with the Kingu CLI'
