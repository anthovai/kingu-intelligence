...
//
// Given two Kingu Windows installers (version N and N+1), proves what happens to
...
async function main() {
  ...
  const canary = `KINGU-E2E-CANARY-${runId}`
  const runDir = mkdtempSync(path.join(tmpdir(), `kingu-win-update-e2e-${runId}-`))
...
function resolveScopedDaemon(userDataDir) {
  ...
  return {
    ...
    // install-dir Kingu.exe — the key survival signal.
...
async function teardown({
  ...
  if (hadPreexistingInstall) {
    console.log(
      '\n*** NOTE: an Kingu install existed before this run. It was OVERWRITTEN and the\n' +
