#!/usr/bin/env node
/**
 * Kingu main-thread jank / subprocess-churn benchmark (issue #7576).
 *
 ...
 * launched with KINGU_MAIN_THREAD_DIAGNOSTICS=1 so the main process emits one
 ...
 *     [--exe <path-to-packaged-Kingu>] [--headless] [--no-log-stream]
 *
 ...
 * the shape Kingu worktrees commonly have, which forces the effective-upstream
...
function ensureFixture(fixtureDir) {
  ...
  writeFileSync(join(fixtureDir, 'kingu-data.json'), JSON.stringify(state, null, 2))
  return repoPath
}
...
async function main() {
  ...
  const env = {
    ...
    KINGU_MAIN_THREAD_DIAGNOSTICS: '1',
    KINGU_E2E_USER_DATA_DIR: fixtureDir,
    HOME: isolatedHome,
