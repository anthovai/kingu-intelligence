#!/usr/bin/env node
/**
 * A/B benchmark for worktree deletion against real Kingu dev instances.
 *
 * Usage:
...
function createFixture(instanceLabel) {
  const root = mkdtempSync(path.join(os.tmpdir(), `kingu-delete-bench-${instanceLabel}-`))
  ...
  run('git', ['config', 'user.email', 'worktree-delete-bench@kingu.invalid'], repoPath)
  run('git', ['config', 'user.name', 'Kingu Worktree Delete Bench'], repoPath)
  writeFileSync(path.join(repoPath, 'README.md'), '# Kingu worktree deletion benchmark\n')
...
function launchDevInstance({ label, repoRoot }, fixture, port) {
  ...
  const env = {
    ...process.env,
    KINGU_DEV_USER_DATA_PATH: fixture.userDataPath,
...
async function connectToKingu(instance) {
  ...
  while (Date.now() < deadline) {
    ...
    try {
      ...
      try {
        ...
        // findKinguPage timeout drops a live browser handle and leaks a connection per retry.
        const page = await findKinguPage(browser)
        return { browser, page }
      } catch (error) {
...
async function findKinguPage(browser) {
  ...
  throw new Error('Kingu renderer with window.__store was not found')
}
...
async function verifyRestart(instanceConfig, fixture, port, repoId) {
  ...
  let browser = null
  try {
    const connection = await connectToKingu(relaunched)
...
async function benchmarkInstance(instanceConfig, index, options) {
  ...
  try {
    ...
    const { browser: connectedBrowser, page } = await connectToKingu(instance)
