...
function createLocalRepoFixture() {
  const baseDir = mkdtempSync(path.join(os.tmpdir(), 'kingu-termperf-'))
  const repoPath = path.join(baseDir, 'repo')
  mkdirSync(repoPath, { recursive: true })
  git(repoPath, 'init', '--initial-branch=main')
  git(repoPath, 'config', 'user.email', 'bench@kingu.local')
  git(repoPath, 'config', 'user.name', 'Kingu Bench')
...
async function main() {
  ...
  try {
    ...
    if (launched) {
      ...
      // Main-process phase attribution (requires KINGU_PTY_SPAWN_TIMING=1 in the
