...
async function callKingu(cli, args, options = {}) {
...
function buildLongSpec(sizeKb, marker) {
  ...
  const header = [
    `KINGU_LONG_PROMPT_REPRO_START ${marker}`,
  ...
  let index = 0
  while (
    Buffer.byteLength(`${lines.join('\n')}\nKINGU_LONG_PROMPT_REPRO_END ${marker}`, 'utf8') <
    targetBytes
  ) {
    lines.push(
  ...
  lines.push(`KINGU_LONG_PROMPT_REPRO_END ${marker}`)
  return lines.join('\n')
}
...
async function tryCloseTerminal(cli, handle, cwd) {
  ...
  try {
    await callKingu(cli, ['terminal', 'close', '--terminal', handle], { cwd })
  } catch {
...
async function parentMain() {
  const cli = argValue('cli', process.env.KINGU_REPRO_CLI ?? 'kingu')
  ...
  const tempDir = path.join(
    tmpdir(),
    `kingu-orchestration-long-prompt-${process.pid}-${Date.now()}`
  )
  await mkdir(tempDir, { recursive: true })
  const workerReportPath = path.join(tempDir, 'worker-report.json')

  console.log(`runtime: ${cli} status`)
  await callKingu(cli, ['status'], { cwd })

  const coordinator = await callKingu(
    cli,
    [
      'terminal',
      'create',
  ...
  try {
    ...
    const dispatch = await callKingu(
      cli,
      [
        'orchestration',
        'dispatch',
...
async function fakeWorkerMain() {
  ...
  const finish = async (reason) => {
    ...
    process.stdout.write(
      `\nKINGU_REPRO_REPORT ${report.contractOk ? 'ok' : 'failed'} ${reportPath}\n`
    )
...
The parent mode requires a running Kingu runtime and creates temporary Kingu
terminals. The fake worker records whether orchestration dispatch delivered a
long prompt in a safe agent-input contract.

Options:
  --cli <path>         Kingu CLI command (default: KINGU_REPRO_CLI or kingu)
