...
1. Launch an isolated Kingu development instance with CDP using the Electron skill.
2. Open one terminal in a git worktree or folder workspace. Start Pi with Kingu's
   generated status extension and this additional extension:

   ```sh
   pi --offline --no-session -e /absolute/path/to/kingu/tests/tools/pi-ui-prompt-extension.mjs
   ```

   If launching Pi directly through `node` or disabling extension discovery, explicitly
   load Kingu's generated `kingu-agent-status.ts` with another `-e` argument.

3. Leave Pi at its input editor, then run from the Kingu repository:
...
For manual verification, run `/kingu-modal select`, `/kingu-modal confirm`,
`/kingu-modal input`, `/kingu-modal editor`, or `/kingu-modal custom` inside Pi.
