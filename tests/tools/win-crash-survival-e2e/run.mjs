...
//
// GitHub #7742: on Windows, when Kingu's main/renderer process crashed, open
...
const SENTINEL_ENV = 'KINGU_CRASH_SENTINEL'
...
async function main() {
  ...
  // clear platform message, not a confusing "no Kingu.exe found" default-resolution
  // failure.
  ...
  const canary = `KINGU-CRASH-SENTINEL-${runId}`
  const runDir = mkdtempSync(path.join(tmpdir(), `kingu-win-crash-e2e-${runId}-`))
  ...
  const diagDir = process.env.KINGU_E2E_DIAG_DIR || path.join(runDir, 'diag')
  let passed = false
  try {
 ...
 * daemons on the machine (including the developer's live Kingu) are ignored.
 ...
 * touches any other Kingu on the box (a live user instance uses a different
