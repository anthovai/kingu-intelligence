...
// installed, packaged Kingu.exe. So its only source is `--exe-path` (defaulting
...
Proves that force-killing ONLY Kingu's main process (a real crash, no tree-kill)
...
Usage:
  node tests/tools/win-crash-survival-e2e/run.mjs --expect <profile> [--exe-path <Kingu.exe>] [options]
...
Options:
  --exe-path <path>        Installed Kingu.exe to drive (default: the per-user
                           install under %LOCALAPPDATA%\\Programs\\Kingu). The
...
export function parseArgs(argv) {
  ...
  const opts = {
    ...
    // "no Kingu.exe found" default-resolution error.
    exePath:
...
function validate(opts, exePathFlagPresent, argv) {
  ...
  if (exePathFlagPresent && takeValue(argv, '--exe-path') === undefined) {
    ...
    errors.push(
      'No installed Kingu.exe found under %LOCALAPPDATA%\\Programs — pass --exe-path <Kingu.exe>'
    )
