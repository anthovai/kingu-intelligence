...
1. **Desktop Kingu running** (`kingu status --json`).
...
kingu worktree list --environment <name> --json | head
kingu terminal list --environment <name> --json | head
```
...
KINGU_FREEZE_ENV=paired-remote \
KINGU_FREEZE_SCENARIO=idle-backlog-open \
KINGU_FREEZE_CREATE=8 \
KINGU_FREEZE_IDLE_MS=45000 \
KINGU_FREEZE_OPEN_COUNT=24 \
pnpm run repro:live-remote-realistic-freeze

# Wake-like: idle + reconnect metadata storm + open  ← hard freeze in lab
KINGU_FREEZE_ENV=paired-remote \
KINGU_FREEZE_SCENARIO=idle-backlog-reconnect-open \
KINGU_FREEZE_CREATE=10 \
KINGU_FREEZE_IDLE_MS=60000 \
KINGU_FREEZE_OPEN_COUNT=40 \
pnpm run repro:live-remote-realistic-freeze

# Restart-proxy: idle + kingu open + refresh storm + open (does not kill desktop)
KINGU_FREEZE_ENV=paired-remote \
KINGU_FREEZE_SCENARIO=restart-proxy \
KINGU_FREEZE_CREATE=0 \
KINGU_FREEZE_IDLE_MS=20000 \
KINGU_FREEZE_OPEN_COUNT=30 \
pnpm run repro:live-remote-realistic-freeze
```
...
| `restart-proxy`               | `kingu open` + refresh storm + open (post-restart discovery; no process kill)                  |
| `lockup-storm`                | Idle + flood + reconnect + **concurrent** open fan-out + **mid-storm `kingu status` watchdog** |

### Realistic knobs

| Variable                          | Default             | Meaning                                                           |
| --------------------------------- | ------------------- | ----------------------------------------------------------------- |
| `KINGU_FREEZE_ENV`                 | `paired-remote`     | Paired remote environment name                                    |
| `KINGU_FREEZE_SCENARIO`            | `idle-backlog-open` | See table above                                                   |
| `KINGU_FREEZE_CREATE`              | `0`                 | New flood terminals; mutation requires an explicit positive value |
| `KINGU_FREEZE_IDLE_MS`             | `45000`             | Time “away” while floods run                                      |
| `KINGU_FREEZE_OPEN_COUNT`          | `20`                | Sessions to open after return                                     |
| `KINGU_FREEZE_PACE_MS`             | `250`               | Base delay between opens (human pace)                             |
| `KINGU_FREEZE_PACE_JITTER_MS`      | `150`               | Random extra delay                                                |
| `KINGU_FREEZE_SOFT_MS` / `HARD_MS` | 2000 / 5000         | Thresholds                                                        |
...
| **lockup-storm** (parallel open + overlap refresh) | 12–16  | 45–60s | 64–80 @ p20–32 | **27–35s** batches; some `Terminal reveal timed out` | **HARD stalls + reveal timeouts; app still answers `kingu status` ~150ms** |
...
| Post-storm `kingu status`    | **~113ms**                          |
| Force Quit required         | **no**                              |

Bar for full-app freeze in the harness: continuous **≥30s** window where `kingu status` hangs/fails or stays ≥15s slow (`evaluateFullAppFreeze` / `foreverUiLockupObserved`).
...
KINGU_FREEZE_ENV=paired-remote KINGU_FREEZE_SCENARIO=lockup-storm \
  KINGU_FREEZE_CREATE=12 KINGU_FREEZE_IDLE_MS=30000 KINGU_FREEZE_OPEN_COUNT=80 \
  KINGU_FREEZE_STORM_PARALLEL=28 KINGU_FREEZE_FOREVER_WINDOW_MS=30000 \
  pnpm run repro:live-remote-realistic-freeze
...
KINGU_FREEZE_ENV=paired-remote \
KINGU_FREEZE_CREATE=0 \
KINGU_FREEZE_SWITCH_PASSES=3 \
KINGU_FREEZE_PARALLEL=16 \
...
| Layer    | Module                                                                             |
| -------- | ---------------------------------------------------------------------------------- |
| Runtime  | `TerminalFocusNavigationCoalescer` via `KinguRuntimeService.focusTerminal`          |
...
- Both harnesses default to `KINGU_FREEZE_CREATE=0`. A positive value creates persistent, high-output remote terminals; use it only on an isolated target you can clean up.
- `restart-proxy` does **not** kill Kingu; it runs `kingu open` + refresh RPCs only.
- Manual capture if UI fully freezes: `sample Kingu 5 -file ~/Desktop/kingu-freeze-sample.txt`

The scripts honor `KINGU_CLI_COMMAND`, then use `kingu-dev` in a dev runtime, `kingu-ide` on Linux, and `kingu` elsewhere.

PowerShell equivalent for the first example:

```powershell
$env:KINGU_FREEZE_ENV = 'paired-remote'
$env:KINGU_FREEZE_SCENARIO = 'idle-backlog-open'
$env:KINGU_FREEZE_CREATE = '8'
$env:KINGU_FREEZE_IDLE_MS = '45000'
$env:KINGU_FREEZE_OPEN_COUNT = '24'
pnpm run repro:live-remote-realistic-freeze
```
