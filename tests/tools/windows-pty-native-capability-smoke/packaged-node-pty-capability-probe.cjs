...
const EVIDENCE_PREFIX = 'KINGU_NODE_PTY_CAPABILITY_EVIDENCE='
...
function buildGrandchildLaunch(channel, fixtureToken) {
  return {
    ...
    args: [
      path.join(__dirname, 'real-kingu-detached-launcher.vbs'),
      process.execPath,
      __filename,
      '--grandchild-member',
      channel,
...
async function exercise(resourcesDir, fixtureExecutable) {
  ...
  const channel = `\\\\.\\pipe\\kingu-pty-native-capability-${fixtureToken}`
...
async function main() {
  ...
  if (mode === '--exit-contract-fixture') {
    process.stdout.write('KINGU_ONE_SHOT_EVIDENCE=flushed\n')
    setInterval(() => {}, 60_000)
    return
  }
