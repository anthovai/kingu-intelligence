...
test('middle-clicking a busy tab prompts, and cancelling keeps the tab', async ({ kinguPage }) => {
  test.setTimeout(120_000)
  const busyTabId = await startBusyTerminal(kinguPage)
  const busyTab = kinguPage.locator(`${SORTABLE_TAB}[data-tab-id="${busyTabId}"]`).first()
  const tabsBefore = await kinguPage.locator(SORTABLE_TAB).count()

  await busyTab.click({ button: 'middle' })

  await expect(closeDialogTitle(kinguPage)).toBeVisible({ timeout: 15_000 })
  await kinguPage.getByRole('button', { name: /^Cancel$/ }).click()
  await expect(closeDialogTitle(kinguPage)).toBeHidden()
  await expect(busyTab).toBeVisible()
  expect(await kinguPage.locator(SORTABLE_TAB).count()).toBe(tabsBefore)
})

test('confirming the X-button prompt closes the busy tab', async ({ kinguPage }) => {
  test.setTimeout(120_000)
  const busyTabId = await startBusyTerminal(kinguPage)
  const busyTab = kinguPage.locator(`${SORTABLE_TAB}[data-tab-id="${busyTabId}"]`).first()
  ...
  await expect(closeDialogTitle(kinguPage)).toBeVisible({ timeout: 15_000 })
  await kinguPage.getByRole('button', { name: /^Stop and Close$/ }).click()

  await expect(busyTab).toHaveCount(0, { timeout: 15_000 })
  await expect(closeDialogTitle(kinguPage)).toBeHidden()
})

test('Cmd+W on a busy single-pane tab raises exactly one dialog', async ({ kinguPage }) => {
  test.setTimeout(120_000)
  const busyTabId = await startBusyTerminal(kinguPage)
  const busyTab = kinguPage.locator(`${SORTABLE_TAB}[data-tab-id="${busyTabId}"]`).first()

  await focusActiveTerminalInput(kinguPage)
  await kinguPage.keyboard.press(process.platform === 'darwin' ? 'Meta+w' : 'Control+w')
  await expect(closeDialogTitle(kinguPage)).toBeVisible({ timeout: 15_000 })
  await kinguPage.getByRole('button', { name: /^Stop and Close$/ }).click()
