...
test('restores the Markdown viewport when an image gains height after a tab switch', async ({
  ...
  const directory = '.kingu-e2e-markdown-scroll'
  ...
  await openMarkdownFixture(kinguPage, context, otherPath)
  ...
  await expect
    ...
    .toBe(1500)
  const viewport = kinguPage.locator('.rich-markdown-editor-shell .overflow-auto')
  ...
  const pendingImage = await kinguPage.addStyleTag({
