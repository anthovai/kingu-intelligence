...
test('installs on a headed desktop runtime without a local fallback', async ({
  ...
  const folderRoot = mkdtempSync(join(tmpdir(), 'kingu-paired-skill-folder-'))
  let client: PairedElectronClient | null = null
  try {
    const hostHome = await electronApp.evaluate(({ app }) => app.getPath('home'))
    const worktreeId = await activeWorktreeId(kinguPage)
    const folderWorkspaceId = await createHostFolderWorkspace(kinguPage, folderRoot)
...
function cloudClientEnvironment(): Record<string, string> {
  ...
  return {
    KINGU_ARTIFACTS_API_URL: origin,
    KINGU_CLOUD_API_URL: origin,
    KINGU_CLOUD_CLIENT_ID: 'skills-e2e-client',
    KINGU_CLOUD_DEV_AUTH: '1',
    KINGU_CLOUD_ALLOW_PLAINTEXT_SESSION: '1',
    KINGU_SKILL_PACKAGE_DOWNLOAD_ORIGINS: origin
  }
}

async function connectCloud(page: Page): Promise<void> {
  const auth = await page.evaluate(() => window.api.kinguProfiles.connectCurrent())
