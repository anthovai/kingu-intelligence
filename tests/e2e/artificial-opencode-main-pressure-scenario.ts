...
>({
  ...
  await deps.waitForSessionReady(kinguPage)
  await deps.waitForActiveWorktree(kinguPage)
  const panes = await deps.ensureActiveWorktreePaneLoad(kinguPage, backgroundPaneCount + 1)
  const [typingPane, ...loadPanes] = panes
  await deps.focusPane(kinguPage, typingPane.paneKey)

  const runId = randomUUID()
  const scrollRunId = randomUUID()
  const typingScriptPath = path.join(testRepoPath, `.kingu-opencode-pressure-typing-${runId}.mjs`)
  const pressureScriptPath = path.join(testRepoPath, `.kingu-opencode-pressure-load-${runId}.mjs`)
  await seedActiveTerminalScrollback(kinguPage, typingPane.ptyId, scrollRunId)
  ...
  try {
    ...
    const pressureBeforeTyping = await deps.waitForMainPtyPressureBacklog(kinguPage)
    ...
    deps.annotateTypingMeasurement(
      ...
      panes.length,
      measurement,
      await deps.readTerminalPtyOutputDebug(kinguPage),
      scheduler,
      mainPressure,
      ackGate
    )
...
>({
  ...
  const scrollMeasurement = await measureActiveTerminalWheelScroll(kinguPage)
  const mainPressureAfterScroll = await deps.readMainPtyPressureDebug(kinguPage)
  const ackGateAfterScroll = await deps.readTerminalAckGateDebug(kinguPage)
  annotateScrollMeasurement(
    testInfo,
  ...
  await scrollActiveTerminalToBottom(kinguPage)
}
