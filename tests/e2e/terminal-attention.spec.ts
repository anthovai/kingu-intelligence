...
test.describe('Terminal attention', () => {
  ...
  test('background terminal attention marks a tab unread and clears on focus', async ({
    ...
    const secondTabId = await createTerminalTab(kinguPage)
    await waitForActiveTerminalManager(kinguPage, 30_000)
    ...
    await expect
      .poll(async () => (await getUnreadTerminalTabIds(kinguPage)).includes(secondTabId), {
        timeout: 10_000,
        message: 'Background tab did not become unread after BEL'
      })
      .toBe(true)

    const secondTabBell = kinguPage
      .locator(
  ...
  test('a BEL on the focused tab raises, then clears on click', async ({ kinguPage }) => {
    await waitForSessionReady(kinguPage)
    await waitForActiveWorktree(kinguPage)
    await ensureTerminalVisible(kinguPage)
    await waitForActiveTerminalManager(kinguPage, 30_000)

    const activeTabId = await getActiveTabId(kinguPage)
    if (!activeTabId) {
      throw new Error('Expected an active terminal tab')
    }
    const activePtyId = await waitForActivePanePtyId(kinguPage)
    await installRendererTitleLog(kinguPage)
    ...
    expect((await getUnreadTerminalTabIds(kinguPage)).includes(activeTabId)).toBe(true)
    const activeTabBell = kinguPage
      .locator(
    ...
    await expect
      .poll(async () => (await getUnreadTerminalTabIds(kinguPage)).includes(activeTabId), {
        timeout: 5_000,
  ...
  test('a BEL on the focused tab raises, then clears on plain Escape', async ({ kinguPage }) => {
    await waitForSessionReady(kinguPage)
    await waitForActiveWorktree(kinguPage)
    await ensureTerminalVisible(kinguPage)
    await waitForActiveTerminalManager(kinguPage, 30_000)
    ...
    const activePaneKey = await getActivePaneKey(kinguPage, activeTabId)
    const activePtyId = await waitForActivePanePtyId(kinguPage)
    await installRendererTitleLog(kinguPage)
    ...
    await kinguPage.evaluate((paneKey) => {
    ...
    await expect
      .poll(async () => (await getUnreadTerminalPaneKeys(kinguPage)).includes(activePaneKey), {
        timeout: 5_000,
    ...
    await focusActiveXterm(kinguPage, activeTabId)
  ...
  // after restarting Kingu with a Claude Code session open, clicking between
  ...
  test('mode bits replayed into xterm do not leak focus escapes to the shell', async ({
    ...
    try {
      ...
      // shell-startup BELs.
      const emittedFromXterm = await kinguPage.evaluate(
        () =>
