...
import { _electron as electron, type ElectronApplication } from '@anthovai/playwright-test'
import { test, expect, forwardElectronProcessLogs } from './helpers/kingu-app'
import { TEST_REPO_PATH_FILE } from './global-setup'
import { getE2ECompletedOnboardingProfile } from './helpers/e2e-completed-onboarding-profile'
import { getKinguElectronLaunchArgs } from './helpers/electron-launch-args'
...
function createHeadlessLaunchIsolation(userDataDir: string): ElectronHomeIsolation {
  ...
  return createElectronHomeIsolation({
    ...
    launchEnv: {
      ...
      KINGU_E2E_ENFORCE_SINGLE_INSTANCE_LOCK: '1'
    },
    extraEnv: {},
    userDataDir
  })
...
function readPersistedPromotionBinding(
  ...
  try {
    const persisted = JSON.parse(
      readFileSync(
        path.join(userDataDir, 'profiles', DEFAULT_LOCAL_KINGU_PROFILE_ID, 'kingu-data.json'),
        'utf8'
      )
    ) as {
      workspaceSession?: {
...
{}, testInfo) => {
  ...
  const userDataDir = mkdtempSync(path.join(os.tmpdir(), 'kingu-e2e-serve-promotion-'))
  ...
  try {
    serveApp = await electron.launch({
      args: [...getKinguElectronLaunchArgs(mainPath, false), '--serve', '--serve-no-pairing'],
      env
    })
    ...
    const forwardAppLogs = process.env.KINGU_E2E_FORWARD_APP_LOGS === '1'
    activatingProcess = spawn(electronPath, getKinguElectronLaunchArgs(mainPath, false), {
      env,
