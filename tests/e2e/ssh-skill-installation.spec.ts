...
const REMOTE_FOLDER = '/tmp/kingu-skill-folder-workspace'
...
test.use({
  ...
  kinguAppExtraEnv: async ({}, provideEnv) => {
    if (!cloud) {
      throw new Error('Skill cloud fixture unavailable')
    }
    await provideEnv({
      KINGU_ARTIFACTS_API_URL: cloud.origin,
      KINGU_CLOUD_API_URL: cloud.origin,
      KINGU_CLOUD_CLIENT_ID: 'skills-e2e-client',
      KINGU_CLOUD_DEV_AUTH: '1',
      KINGU_CLOUD_ALLOW_PLAINTEXT_SESSION: '1',
      KINGU_SKILL_PACKAGE_DOWNLOAD_ORIGINS: cloud.origin
    })
  }
})
...
test.describe('SSH skill installation', () => {
  ...
  test('installs and removes global, Git-worktree, and folder copies through the real relay', async ({
    ...
    try {
      ...
      const auth = await kinguPage.evaluate(() => window.api.kinguProfiles.connectCurrent())
      ...
      const globalInstalls = await kinguPage.evaluate(
      ...
      await previewUnchanged(kinguPage, globalDestination)
      ...
      const worktreePath = '/tmp/kingu-docker-relay-perf-repo/.agents/skills/remote-e2e-skill'
      await installAndVerify(kinguPage, target, worktreeDestination, worktreePath)
      await removeAndVerify(kinguPage, target, worktreeDestination, worktreePath)

      const folderWorkspaceId = await createRemoteFolderWorkspace(kinguPage, remote.targetId)
      const folderDestination: SkillInstallDestination = {
        scope: 'workspace',
        folderWorkspaceId
      }
      const folderPath = `${REMOTE_FOLDER}/.agents/skills/remote-e2e-skill`
      await installAndVerify(kinguPage, target, folderDestination, folderPath)
      await removeAndVerify(kinguPage, target, folderDestination, folderPath)
