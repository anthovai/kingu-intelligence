...
describe('getKinguElectronLaunchArgs', () => {
  ...
  it.each([
    ...
    (platform, ci, headful, enabled) => {
      ...
      const args = getKinguElectronLaunchArgs(join('kingu', 'out', 'main', 'index.js'), headful)
  ...
  it('launches the package root that owns the compiled main entry', () => {
    const root = join('workspace', 'kingu')
    const mainPath = join(root, 'out', 'main', 'index.js')

    const args = getKinguElectronLaunchArgs(mainPath, true)
    ...
    expect(getKinguElectronLaunchArgs(mainPath, false)).toContain(root)
  })
})
