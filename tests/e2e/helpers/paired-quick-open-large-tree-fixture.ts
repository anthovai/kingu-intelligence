...
export type PairedQuickOpenLargeTreeFixture = {
  ...
  kinguIgnoredTargetPath: string
  root: string
}

export function createPairedQuickOpenLargeTreeFixture(): PairedQuickOpenLargeTreeFixture {
  const root = mkdtempSync(path.join(os.tmpdir(), 'kingu-paired-quick-open-large-tree-'))
  try {
    execFileSync('git', ['init'], { cwd: root, stdio: 'pipe' })
    writeFileSync(path.join(root, '.gitignore'), 'data/\n')
    writeFileSync(path.join(root, '.kinguignore'), 'kingu-ignored/\n')
    ...
    const kinguIgnoredTargetPath = 'kingu-ignored/sta-4354-kinguignore-target.ts'
    mkdirSync(path.join(root, 'kingu-ignored'))
    closeSync(openSync(path.join(root, ...kinguIgnoredTargetPath.split('/')), 'w'))
    truncateSync(
      path.join(root, 'data', 'chunk-000000', `${LONG_STEM}000000.bin`),
      LARGE_TREE_NOMINAL_IGNORED_BYTES
    )

    execFileSync('git', ['add', '.gitignore', '.kinguignore'], { cwd: root, stdio: 'pipe' })
    execFileSync(
      'git',
      [
        '-c',
        'user.name=Kingu E2E',
        '-c',
        'user.email=kingu-e2e@example.invalid',
        'commit',
        '-m',
