...
function createUserDataDir(): string {
  const tempDir = mkdtempSync(path.join(os.tmpdir(), 'kingu-home-isolation-test-'))
  ...
  it('strips ambient home and Codex state before forcing a disposable home', () => {
    ...
    const isolation = createElectronHomeIsolation({
      inheritedEnv: {
        ...
        KINGU_CODEX_HOME: '/real/kingu-codex',
    ...
    expect(isolation.env.KINGU_CODEX_HOME).toBeUndefined()
  ...
  it('rejects generic fixture overlays that could escape the boundary', () => {
    ...
    expect(() =>
      createElectronHomeIsolation({
        inheritedEnv: {},
        launchEnv: {},
        extraEnv: { KINGU_E2E_USER_DATA_DIR: '/unsafe' },
        userDataDir: createUserDataDir(),
        realHome: '/real/home'
      })
    ).toThrow(/kinguAppExtraEnv\.KINGU_E2E_USER_DATA_DIR/)
  })
