...
const TEMP_PREFIX = 'kingu-9749-dg-'
...
function knownKinguUserDataDirs(): string[] {
  if (process.platform === 'darwin') {
    const appSupport = path.join(homedir(), 'Library', 'Application Support')
    return [path.join(appSupport, 'kingu'), path.join(appSupport, 'kingu-dev')]
  }
  if (process.platform === 'win32') {
    const roaming = process.env.APPDATA ?? path.join(homedir(), 'AppData', 'Roaming')
    return [path.join(roaming, 'kingu'), path.join(roaming, 'kingu-dev')]
  }
  const config = process.env.XDG_CONFIG_HOME ?? path.join(homedir(), '.config')
  return [path.join(config, 'kingu'), path.join(config, 'kingu-dev')]
}
...
function assertDisposableRoot(rootDir: string): void {
  ...
  for (const userDataDir of knownKinguUserDataDirs()) {
    if (isEqualToOrInside(rootDir, userDataDir)) {
      throw new Error('Refusing daemon-generation fixture inside real Kingu user data')
    }
  }
}
