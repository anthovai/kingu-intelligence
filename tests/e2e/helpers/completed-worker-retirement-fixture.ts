...
const fakeCliDir = mkdtempSync(path.join(os.tmpdir(), 'kingu-e2e-retired-worker-'))
...
const ledger = process.env.KINGU_E2E_CODEX_LIFECYCLE_LEDGER
...
process.stdin.on('data', (chunk) => {
  ...
  if (input.includes('KINGU_E2E_EXIT_AFTER_DONE')) {
...
export const completedWorkerLaunchEnv = {
  ...
  KINGU_E2E_CODEX_LIFECYCLE_LEDGER: lifecycleLedgerPath
}
...
export function runBuiltKinguCli(
  args: string[],
  options: { userDataDir: string; cwd: string }
): unknown {
  const {
    KINGU_ENVIRONMENT: _environment,
    KINGU_PAIRING_CODE: _pairingCode,
    KINGU_USER_DATA_PATH: _userDataPath,
    ...cleanEnv
  } = process.env
  void _environment
  void _pairingCode
  ...
  const output = execFileSync(
    ...
    {
      cwd: options.cwd,
      env: { ...cleanEnv, KINGU_USER_DATA_PATH: options.userDataDir },
      encoding: 'utf8',
      timeout: 30_000
    }
  )
