...
async function installMockSkillDiscovery(
  ...
  await app.evaluate((electron, initialResult) => {
    ...
    global.__kinguSettingsSkillDiscoveryResult = initialResult
    electron.ipcMain.removeHandler('skills:discover')
    electron.ipcMain.handle('skills:discover', () => {
      const latest = (globalThis as MockSkillDiscoveryGlobal).__kinguSettingsSkillDiscoveryResult
      if (!latest) {
...
async function setMockSkillDiscovery(
  ...
  await app.evaluate((_, nextResult) => {
    ;(globalThis as MockSkillDiscoveryGlobal).__kinguSettingsSkillDiscoveryResult = nextResult
  }, result)
}
...
test.describe('Settings skill detection', () => {
  ...
  test('shows installed only for global orchestration skill installs', async ({
    ...
    await openOrchestrationSettings(kinguPage)
    const section = kinguPage.locator('[data-settings-section="orchestration"]')
    await section.getByRole('button', { name: 'Re-check' }).click()

    await expect(section.getByText('Not installed', { exact: true })).toBeVisible()
    await expect(
      section.getByText('Enables agents to hand off context and coordinate work through Kingu.')
    ).toBeVisible()
