...
test.describe('Windows project runtime smoke', () => {
  test('keeps a Windows-host project and WSL project available side by side', async ({
    ...
    const wsl = await kinguPage.evaluate(async () => ({
    ...
    const wslRepoPath = await createGitRepo('kingu-e2e-project-runtime-', 'wsl-runtime-project')

    const smoke = await kinguPage.evaluate(
    ...
    await openRepoSettings(kinguPage, smoke.hostRepoId)
    const hostSection = kinguPage.locator(`[data-settings-section="repo-${smoke.hostRepoId}"]`)
    ...
    await chooseProjectRuntime(kinguPage, smoke.hostRepoId, 'WSL')
    await expect(
      hostSection.getByText(`This project runs in ${smoke.wslDistro} via WSL.`)
    ).toBeVisible()
    const hostAfterWslUiSwitch = await kinguPage.evaluate((hostRepoId) => {
      const state = window.__store!.getState()
      const hostProject = state.projects.find((project) =>
    ...
    await chooseProjectRuntime(kinguPage, smoke.hostRepoId, 'Windows')
    await expect(hostSection.getByText('This project runs on Windows.')).toBeVisible()
    const hostAfterWindowsUiSwitch = await kinguPage.evaluate((hostRepoId) => {
      const state = window.__store!.getState()
      const hostProject = state.projects.find((project) =>
        project.sourceRepoIds.includes(hostRepoId)
      )
    ...
    await openRepoSettings(kinguPage, smoke.wslRepoId)
    const wslSection = kinguPage.locator(`[data-settings-section="repo-${smoke.wslRepoId}"]`)
    ...
    await kinguPage.evaluate(async (repoId) => {
