...
test.describe('terminal document visibility WebGL recovery', () => {
  test('@headful preserves the WebGL atlas and keeps terminal text painted after document visibility resumes', async ({
    ...
    const webglActive = await forceWebgl(kinguPage)
    test.skip(!webglActive, 'WebGL was not active in this Electron environment')

    await writeStableTerminalContent(kinguPage)
    expect(await patchAtlasCounter(kinguPage)).toBe(true)
    expect(await countPatchedWebglAddons(kinguPage)).toBeGreaterThanOrEqual(2)
    const baseline = await terminalScreenshots(kinguPage)
    ...
    try {
      ...
      await resetAtlasResetCount(kinguPage)
      ...
      if (!browserWindowVisibilityWorked) {
        await resetAtlasResetCount(kinguPage)
        await dispatchDocumentVisibilityCycle(kinguPage)
      }

      await waitForTerminalPaint(kinguPage)
      expect(
        await readAtlasResetCount(kinguPage),
        'ordinary document visibility resume cleared the shared WebGL atlas'
      ).toBe(0)

      const afterResume = await terminalScreenshots(kinguPage)
