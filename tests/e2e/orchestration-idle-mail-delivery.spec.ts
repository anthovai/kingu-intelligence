 ...
 * covered in src/main/runtime/kingu-runtime.test.ts. What lives here is every
...
const POINTER_COMMAND = 'kingu-dev orchestration check'
...
async function setUpMailFixture(
  ...
  const openAgentPane = async (options?: {
    ...
    const { paneKey } = await waitForActivePaneHookDescriptor(kinguPage)
    ...
    await waitForPtyShellEcho(kinguPage, ptyId, 60_000)
    const agent = createMailPaneAgent(options)
    await execInTerminal(kinguPage, ptyId, agent.launchCommand)
    await expect
...
test.describe('orchestration push-on-idle mail delivery', () => {
  test('delivers mail that arrives while the agent is already idle', async ({
    ...
    const { client, userDataDir, openAgentPane } = await setUpMailFixture(kinguPage, electronApp)
  ...
  test('holds mail while the agent is working and releases it on the idle frame', async ({
    ...
    const messageId = await sendMail(client, mailbox, { subject })
    await expectStaysPending(kinguPage, userDataDir, pane, messageId)
  ...
  test('keeps unbound direct mail durable without pointing to an unsafe check', async ({
    ...
    await kinguPage.waitForTimeout(NO_DELIVERY_SETTLE_MS)
  ...
  test('writes and submits the pointer for the active coordinator pane', async ({
    ...
    test.setTimeout(180_000)
    const { client, openAgentPane } = await setUpMailFixture(kinguPage, electronApp)
    const pane = await openAgentPane()
    await driveToLiveIdle(client, pane)
...
test.describe('orchestration delivery to a cold-parked agent', () => {
  ...
  test.use({
    kinguAppExtraEnv: {
      KINGU_E2E_TERMINAL_PARKING_DELAY_MS: String(parkingDelayMs),
      ...
      KINGU_E2E_ORCHESTRATION_POINTER_ENTER_DELAY_MS: '5000'
    }
  })
  ...
  test('keeps one pointer and one idempotent prompt on the same parked PTY', async ({
    ...
    const beforePark = await waitForPaneIdentitySnapshot(kinguPage, 1)
    ...
    const parkDetectedAfterMs = await parkHiddenTabBehindDecoy(kinguPage, worktreeId, tabId, {
      parkDelayMs: parkingDelayMs
    })
    expect(await getActiveTabId(kinguPage)).not.toBe(tabId)
    expect(await kinguPage.locator(`[data-terminal-tab-id=${JSON.stringify(tabId)}]`).count()).toBe(
      0
    )
    ...
    const promptMarker = `KINGU_E2E_PARKED_PROMPT_${randomUUID()}`
    ...
    const promptParams = {
      ...
      client: { id: 'kingu-e2e', type: 'desktop' as const }
    }
    ...
    const afterReveal = await waitForPaneIdentitySnapshot(kinguPage, 1)
    expect(afterReveal.tabId).toBe(tabId)
    expect(afterReveal.panes[0]?.ptyId).toBe(pane.ptyId)
    await expect(
      kinguPage.locator(`[data-terminal-tab-id=${JSON.stringify(tabId)}] .xterm-screen`).first()
    ).toBeVisible()
