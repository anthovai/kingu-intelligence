...
const fakeCliDir = mkdtempSync(path.join(os.tmpdir(), 'kingu-e2e-orchestration-worker-'))
...
appendLedger('KINGU_E2E_SPAWN_LEDGER', { event: 'spawn', startedAt: Date.now() })
...
test('worker-start preserves one live inactive worker across workspace re-entry', async ({
  ...
  const coordinatorTabId = await getActiveTabId(kinguPage)
  ...
  const workerTab = kinguPage.locator(
  ...
  await expect(
    kinguPage.locator(`[data-testid="sortable-tab"][data-tab-id="${coordinatorTabId}"]`)
  ...
  const otherWorktreeId = await switchToOtherWorktree(kinguPage, worktreeId)
  ...
  await expect(workerTab).toBeVisible()
  await expect(
    kinguPage.locator(`[data-testid="sortable-tab"][data-tab-id="${workerTerminal!.tabId}"]`)
  ).toHaveCount(1)
  await expect(
    kinguPage.locator(`[data-testid="sortable-tab"][data-tab-title="${workerTabTitle}"]`)
  ).toHaveCount(1)
  ...
  await expect(kinguPage.locator('body')).not.toContainText('Conversation interrupted')
})
