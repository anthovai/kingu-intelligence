...
test('stale helper cannot take goto or eval away from the real embedded webview', async ({
  ...
  try {
    ...
    const browserTab = await createBrowserTab(kinguPage, worktreeId, server.sourceUrl)

    await expect
      .poll(() => readEmbeddedPage(kinguPage, browserTab.id), { timeout: 10_000 })
      .toMatchObject({ marker: 'source-webview', title: 'Owned source', url: server.sourceUrl })

    const snapshot = await callBrowserRuntime(kinguPage, 'browser.snapshot', {
      page: browserTab.pageId
    })
    expect(snapshot, JSON.stringify(snapshot)).toMatchObject({ ok: true })

    const sessionName = `kingu-tab-${browserTab.pageId}`
    await expect
    ...
    const navigation = await callBrowserRuntime(kinguPage, 'browser.goto', {
      page: browserTab.pageId,
    ...
    await expect(kinguPage.locator(`[data-tab-id="${browserTab.id}"]`)).toContainText(
      'Owned destination'
    )

    const evaluation = await callBrowserRuntime(kinguPage, 'browser.eval', {
      page: browserTab.pageId,
