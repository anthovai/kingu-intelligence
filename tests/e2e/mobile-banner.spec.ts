...
test('mobile subscribe mounts overlay; collapse → chip; Take back dismisses', async ({
  ...
  const overlay = kinguPage.locator('.mobile-driver-banner')
  await expect(overlay).toHaveCount(0)

  // Fire the IPC events main emits when a mobile client subscribes in 'auto'
  // mode (handleMobileSubscribe in src/main/runtime/kingu-runtime.ts). The
  ...
  await expectExpandedOverlayLeavesPaneReadable(kinguPage, ptyId)
  ...
  await captureAttachment(kinguPage, testInfo, 'overlay-loud.png')
  ...
  await expectChipIsCompactInPane(kinguPage, ptyId)

  await captureAttachment(kinguPage, testInfo, 'overlay-collapsed.png')
...
test('held phone-fit state mounts restore overlay without collapse', async ({
  ...
  await captureAttachment(kinguPage, testInfo, 'overlay-held-fit.png')
...
test('restore this terminal refits the active restored pane', async ({
  ...
  await expect(kinguPage.locator('.mobile-driver-banner')).toHaveCount(1, { timeout: 15_000 })
  await expect
    .poll(() => getPaneTerminalCols(kinguPage, ptyId), {
...
test('restore all refits non-focused restored terminal panes', async ({
  ...
  const ptyIds = await waitForVisiblePanePtyIds(kinguPage, 2)
  const focusPtyId = await waitForActivePanePtyId(kinguPage)
  ...
  await expect(kinguPage.locator('.mobile-driver-banner')).toHaveCount(2, { timeout: 15_000 })

  await forcePaneToOneColumn(kinguPage, inactivePtyId)
  await expect
    .poll(() => getPaneTerminalCols(kinguPage, inactivePtyId), {
...
test('restore all recovers a hidden workspace held at narrow terminal geometry', async ({
  ...
  await ensureTerminalVisible(kinguPage)
  await waitForActiveTerminalManager(kinguPage)
  const hiddenWorkspacePtyId = await waitForActivePanePtyId(kinguPage)
  await sendHeldPhoneFitIpc(electronApp, { ptyId: hiddenWorkspacePtyId, cols: 45, rows: 20 })
  await expect(kinguPage.locator('.mobile-driver-banner')).toHaveCount(1, { timeout: 15_000 })

  await forcePaneToOneColumnAndSwitchWorktree(kinguPage, hiddenWorkspacePtyId, secondWorktreeId)
  await expect
  ...
  await expect
    .poll(() => getPaneTerminalCols(kinguPage, hiddenWorkspacePtyId), {
      message: 'test harness should hold workspace 1 in the bad narrow state'
    })
    .toBeLessThanOrEqual(2)
  await ensureTerminalVisible(kinguPage)
  await waitForActiveTerminalManager(kinguPage)
  const activeWorkspacePtyId = await waitForActivePanePtyId(kinguPage)
  await installRestoreTerminalFitAutoRestoreRecorder(electronApp)
  await sendHeldPhoneFitIpc(electronApp, { ptyId: activeWorkspacePtyId, cols: 45, rows: 20 })
  await expect(
    kinguPage.locator(`[data-pty-id="${activeWorkspacePtyId}"] .mobile-driver-banner`)
