...
test.describe('terminal scroll intent keeps following output', () => {
  test('a sub-row wheel-up that never moves the viewport must not stop follow-output', async ({
    ...
    const ptyId = await startStreamingFixturePhase1(kinguPage)
    ...
    await dispatchSubRowWheelUp(kinguPage)
    await kinguPage.waitForTimeout(INTENT_SETTLE_WAIT_MS)

    // Any byte releases the fixture's phase-2 stream.
    await sendToTerminal(kinguPage, ptyId, 'g')
    await waitForMarkerAtBottom(kinguPage, 'STREAM_PHASE2_DONE')
  })

  test('a plain Home keypress delivered to the app must not stop follow-output', async ({
    kinguPage
  }) => {
    await startStreamingFixturePhase1(kinguPage)
    ...
    await dispatchPlainHomeKeydown(kinguPage)
    await waitForMarkerAtBottom(kinguPage, 'STREAM_PHASE2_DONE')
  })

  test('a real wheel pin stays fixed while visible output streams', async ({ kinguPage }) => {
    const ptyId = await startStreamingFixturePhase1(kinguPage)

    await dispatchRealWheel(kinguPage, -240)
    await expect
      .poll(async () => {
        const probe = await probeActiveViewport(kinguPage, 'STREAM_PHASE1_DONE')
        return probe ? probe.baseY - probe.viewportY : 0
      })
      .toBeGreaterThan(1)
    const pinned = await probeActiveViewport(kinguPage, 'STREAM_PHASE1_DONE')
    if (!pinned) {
    ...
    await expect
      .poll(
        async () => {
          const probe = await probeActiveViewport(kinguPage, 'STREAM_PHASE2_DONE')
  ...
  test('typing after a pinned write is queued resumes follow-output', async ({ kinguPage }) => {
    await startStreamingFixturePhase1(kinguPage)
    const { paneKey } = await waitForActivePaneHookDescriptor(kinguPage)
    await waitForTerminalPtyDataInjector(kinguPage, paneKey)

    await dispatchRealWheel(kinguPage, -320)
    await expect
      .poll(async () => {
        const probe = await probeActiveViewport(kinguPage, 'STREAM_PHASE1_DONE')
        return probe ? probe.baseY - probe.viewportY : 0
    ...
    await injectQueuedWriteThenType(kinguPage, paneKey)
    await expect
      .poll(
        async () => {
          const probe = await probeActiveViewport(kinguPage, 'STREAM_PHASE1_DONE')
