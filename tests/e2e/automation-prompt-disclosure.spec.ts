...
test('automation detail keeps short prompts readable and reveals a very long prompt at narrow width', async ({
  ...
  await kinguPage.setViewportSize({ width: 820, height: 700 })
  ...
  await kinguPage.getByRole('button', { name: new RegExp(`^${SHORT_NAME}`) }).click()
  await expect(kinguPage.getByText('Synthetic short prompt.')).toBeVisible()
  await expect(kinguPage.getByRole('button', { name: 'Show more' })).toHaveCount(0)

  await kinguPage.getByRole('button', { name: 'All automations' }).click()
  await kinguPage.getByRole('button', { name: new RegExp(`^${RESIZE_NAME}`) }).click()
  const resizePrompt = kinguPage.getByText(RESIZE_PROMPT)
  const resizeToggle = kinguPage.getByRole('button', { name: 'Show more' })
  await expect(resizeToggle).toBeVisible()
  await resizeToggle.focus()
  await kinguPage.setViewportSize({ width: 1400, height: 700 })
  await expect(resizeToggle).toHaveCount(0)
  await expect(resizePrompt).toBeFocused()

  await kinguPage.setViewportSize({ width: 820, height: 700 })
  await kinguPage.getByRole('button', { name: 'All automations' }).click()
  await kinguPage.getByRole('button', { name: new RegExp(`^${LONG_NAME}`) }).click()

  const prompt = kinguPage.getByText(new RegExp(END_MARKER))
  const showMore = kinguPage.getByRole('button', { name: 'Show more' })
  ...
  await expect(kinguPage.getByRole('button', { name: 'Show less' })).toBeFocused()
