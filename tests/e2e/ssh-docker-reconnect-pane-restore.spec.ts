...
test.describe('SSH reconnect pane restore', () => {
  ...
  test('restores shell scrollback, a full-screen frame, and a usable new tab across a reconnect', async ({
    ...
    try {
      ...
      await execInTerminal(kinguPage, ptyId, 'top -b -n 1 > /dev/null; top')
      await waitForTerminalOutput(kinguPage, 'load average', 30_000, 8000)

      await reconnectDockerSshRelayTarget(kinguPage, remote.targetId)
      await waitForActiveTerminalManager(kinguPage, 60_000)
      await waitForActivePanePtyId(kinguPage, 60_000)

      await waitForTerminalOutput(kinguPage, 'load average', 60_000, 8000)
      const tuiContent = await getTerminalContent(kinguPage, 8000)
      ...
      await openTerminalTabInActiveGroup(kinguPage)
      await waitForActiveTerminalManager(kinguPage, 60_000)
      const freshPtyId = await waitForActivePanePtyId(kinguPage, 60_000)
      ...
      await execInTerminal(kinguPage, freshPtyId, `echo ${freshMarker}`)
      await waitForTerminalOutput(kinguPage, freshMarker, 60_000)

      // And it must be a FRESH shell, not a repaint of the old pane's history.
      const freshContent = await getTerminalContent(kinguPage, 8000)
      ...
      // never printed a prompt for Kingu to read one from.
      await expect
        .poll(
          async () =>
            kinguPage.evaluate(() => {
