...
test.describe('SSH host add/edit modal', () => {
  ...
  test.afterEach(async ({ kinguPage }) => {
    const ids = await listTargetIdsByLabelPrefix(kinguPage, HOST_PREFIX)
    if (ids.length > 0) {
      await removeSshTargets(kinguPage, ids)
    }
  })
  ...
  test('opens add/edit form in a viewport-stable dialog over a long host list', async ({
    ...
    const seeded = await seedSshTargets(kinguPage, 10)
    await openSshHostSettings(kinguPage)

    const sshSection = kinguPage.locator('[data-settings-section="ssh"]')
    ...
    const addDialog = kinguPage.getByRole('dialog', { name: 'Add SSH host' })
    ...
    const editDialog = kinguPage.getByRole('dialog', { name: 'Edit SSH host' })
    ...
    await kinguPage.locator('[data-slot="dialog-overlay"]').click({ position: { x: 8, y: 8 } })
    await expect(editDialog).toBeVisible()
    ...
    await createdCard.getByRole('button', { name: 'Edit target' }).click()
    const reopened = kinguPage.getByRole('dialog', { name: 'Edit SSH host' })
  ...
  test('add-ssh-host settings intent opens the same modal dialog', async ({ kinguPage }) => {
    ...
    await expect(kinguPage.getByPlaceholder('Search settings')).toBeVisible({ timeout: 10_000 })
    await dismissTransientAnnouncement(kinguPage)

    const dialog = kinguPage.getByRole('dialog', { name: 'Add SSH host' })
