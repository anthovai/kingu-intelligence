 ...
 *   KINGU_LARGE_FILE_COUNT=9500 npx playwright test \
...
async function addAndActivateRepo(kinguPage: Page, repoPath: string): Promise<string> {
  const repoId = await kinguPage.evaluate(async (pathToRepo: string) => {
  ...
  await expect
    .poll(
      () =>
        kinguPage.evaluate(async (targetRepoId: string) => {
          const store = window.__store
          if (!store) {
            return 0
          }
  ...
  const sourceControlButton = kinguPage.getByRole('button', { name: /^Source Control/ })
  await expect(sourceControlButton).toBeVisible()
  await expect
    .poll(() => kinguPage.evaluate(() => window.__store?.getState().rightSidebarTab))
    .toBe('source-control')
  await expect(kinguPage.getByRole('button', { name: 'Filter files by name' })).toBeVisible()
...
async function unregisterLargeFileCountRepos(
  ...
  for (const repoPath of repoPaths) {
    await kinguPage.evaluate(async (pathToRepo) => {
      const store = window.__store
...
async function measureSourceControlLoad(
  ...
  return await kinguPage.evaluate(async ({ worktreeId, repoPath, expectedRows, pollCycles }) => {
    const store = window.__store
    if (!store) {
      throw new Error('window.__store is not available')
    }
...
test.describe('Source Control large file count (#8013)', () => {
  ...
  test('a large untracked set under the status cap stays responsive', async ({
    ...
    const untrackedFiles = Number(process.env.KINGU_LARGE_FILE_COUNT ?? '950')
    // Why: KINGU_LARGE_FILE_BYTES gives untracked files realistic sizes so the
    ...
    const untrackedFileBytes = Number(process.env.KINGU_LARGE_FILE_BYTES ?? '0')
    ...
    try {
      ...
      const measurement = await measureSourceControlLoad(kinguPage, {
        worktreeId,
      ...
      await unregisterLargeFileCountRepos(kinguPage, [fixture.repoPath])
    }
  })
  ...
  test('a large modified set under the status cap stays responsive', async ({
    ...
    test.setTimeout(600_000)
    const modifiedFiles = Number(process.env.KINGU_LARGE_FILE_COUNT ?? '750')
  ...
  test('a change set over the status cap degrades to the too-many-changes state', async ({
    ...
    try {
      ...
      const activationMaxLagMs = await kinguPage.evaluate(() => {
      ...
      const hugeState = await kinguPage.evaluate(
  ...
  test('untracked line-stat cache stays effective up to the status cap', async ({
    ...
    try {
      ...
      const warmRescanPerFileMs = async (repoPath: string, files: number): Promise<number> => {
        const worktreeId = await addAndActivateRepo(kinguPage, repoPath)
        const measurement = await measureSourceControlLoad(kinguPage, {
          worktreeId,
          repoPath,
          expectedRows: files,
      ...
      await unregisterLargeFileCountRepos(kinguPage, [
        smallRepo.repoPath,
  ...
  test('a large clean repo (tracked files only) loads instantly', async ({
    ...
    test.setTimeout(600_000)
    const trackedFiles = Number(process.env.KINGU_LARGE_FILE_COUNT ?? '15000')
