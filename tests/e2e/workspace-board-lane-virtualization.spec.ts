...
test.describe('Workspace board lane virtualization', () => {
  ...
  test('mounts a window of cards for a large lane and keeps lane indexes', async ({
    ...
    await kinguPage.evaluate((count) => {
    ...
    await kinguPage.getByRole('button', { name: 'Workspace board' }).click()

    const cards = kinguPage.locator('[data-workspace-board-card-id]')
  ...
  test('renders later lane indexes after the lane scrolls', async ({ kinguPage }) => {
    await kinguPage.evaluate((count) => {
      const store = window.__store
      if (!store) {
        throw new Error('window.__store is not available')
  ...
  test('bounds mounted lanes and cards while preserving a 21-status workflow', async ({
    ...
    const board = kinguPage.locator('[data-workspace-board-selection-surface]')
    ...
    await expect
      .poll(() => kinguPage.evaluate(() => window.__store?.getState().workspaceBoardColumnWidth))
      .toBe(328)
  ...
  test('selects the full lane across a single large marquee scroll jump', async ({ kinguPage }) => {
    ...
    const lane = kinguPage.locator(`[data-workspace-status="${statusId}"]`)
    ...
    const emptyLaneScroll = kinguPage.locator(
    ...
    await kinguPage.mouse.move(startPoint.x, startPoint.y)
    await kinguPage.mouse.down()
    await kinguPage.mouse.move(box.x + box.width - 18, box.y + 80, { steps: 4 })
    ...
    await expect(kinguPage.locator('[data-workspace-board-selection-rect]')).toBeVisible()
    await expect
    ...
    await kinguPage.mouse.move(box.x + box.width - 18, box.y + box.height - 12)
    await kinguPage.mouse.up()
    ...
    await expect(kinguPage.getByText(/^\d+ selected$/)).toHaveText(
