/**
 * Invariant: the documented hello-kingu plugin stays inert before visible
...
async function openDemoPanel(page: Page): Promise<void> {
  ...
  const panelButton = page.getByRole('button', { name: 'Hello Kingu', exact: true })
  await expect(panelButton).toBeVisible({ timeout: 15_000 })
  await panelButton.click()
  const frame = page.frameLocator('iframe[title="Hello Kingu"]')
  await expect(frame.getByRole('heading', { name: 'Hello Kingu 👋' })).toBeVisible()
...
test('runs hello-kingu panel, command, and event behind visible consent', async ({ kinguPage }) => {
  const tempRoot = await mkdtemp(join(tmpdir(), 'kingu-hello-plugin-e2e-'))
  const pluginRoot = join(tempRoot, 'hello-kingu')
  let createdWorktreeId: string | null = null
  await cp(join(process.cwd(), 'examples', 'plugins', 'hello-kingu'), pluginRoot, {
    recursive: true
  })

  try {
    const installed = await kinguPage.evaluate(async (sourcePath) => {
    ...
    await openPluginSettings(kinguPage)
    await kinguPage.getByRole('tab', { name: /^Installed/ }).click()
    const row = kinguPage.locator(`[data-plugin-key="${installed.pluginKey}"]`)
    await expect(row).toContainText('Needs review')
    await row.getByRole('button', { name: 'Review & enable' }).click()
    const consent = kinguPage.getByRole('dialog', { name: 'Review permissions' })
    ...
    const commandResults = await kinguPage.evaluate(async (pluginKey) => {
    ...
    await kinguPage.evaluate(async (sourcePath) => {
      const settings = await window.api.settings.set({ devPluginPaths: [sourcePath] })
      window.__store?.setState({ settings })
      await window.api.plugins.refresh()
    }, pluginRoot)

    await openDemoPanel(kinguPage)

    const panelPath = join(pluginRoot, 'panel.html')
    const panelHtml = await readFile(panelPath, 'utf8')
    await writeFile(panelPath, panelHtml.replace('Hello Kingu 👋', 'Hello Kingu reloaded'))
    await expect(
      kinguPage.frameLocator('iframe[title="Hello Kingu"]').getByRole('heading', {
        name: 'Hello Kingu reloaded'
      })
    ).toBeVisible({ timeout: 15_000 })

    createdWorktreeId = await createWorktree(kinguPage, `plugin-event-${Date.now()}`)
    await expect
      .poll(
        () =>
          kinguPage.evaluate(
