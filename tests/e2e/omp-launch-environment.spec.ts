...
const test = base.extend({
  launchEnv: async ({ seedTestRepo }, run, testInfo) => {
    void seedTestRepo
    if (!process.env.KINGU_OMP_PROOF_BINARY || process.platform !== 'darwin') {
      await run({})
      return
    }
...
`
    ...
    await writeFile(
      shell,
      `#!/bin/sh
case "$KINGU_E2E_HOME_DIR" in
  */kingu-e2e-userdata-*/home) ;;
  *) echo 'Refusing profile proof outside isolated E2E home' >&2; exit 73 ;;
esac
export HOME="$KINGU_E2E_HOME_DIR"
...
`
    ...
    await run({
      PATH: [dirname(process.env.KINGU_OMP_PROOF_BINARY ?? '/usr/bin/omp'), process.env.PATH]
        .filter(Boolean)
        .join(delimiter),
      SHELL: shell,
...
test.skip(
  !process.env.KINGU_OMP_PROOF_BINARY || process.platform !== 'darwin',
  'Opt-in macOS OMP runtime proof'
)

test('OMP launched by Kingu uses login-profile data and config roots', async ({
  kinguPage,
  electronApp
}, testInfo) => {
...
export default function (api) {
 api.on('session_start', (_event, ctx) => {
  writeFileSync(process.env.KINGU_PATH_PROBE_OUTPUT || ${JSON.stringify(result)}, JSON.stringify({
   data: process.env.XDG_DATA_HOME, config: process.env.PI_CONFIG_DIR,
   source: process.env.KINGU_OMP_SOURCE_AGENT_DIR,
   status: process.env.KINGU_OMP_STATUS_EXTENSION,
  ...
  await kinguPage.screenshot({ path: testInfo.outputPath('omp-profile-root.png') })
  await expect(async () => {
  ...
  const overridePty = await kinguPage.evaluate(
    async ({ command, worktreeId, home, overrideData, overrideResult }) => {
      const pane = await window.api.pty.spawn({
        ...
        env: {
          ...
          KINGU_PATH_PROBE_OUTPUT: overrideResult
        }
      })
      return pane.id
    },
  ...
  try {
    ...
    await kinguPage.evaluate((id) => window.api.pty.kill(id), overridePty)
  }
  ...
  const baselineCommand = buildShellCommandFromArgv(
    [
      ...
      ...[
        ...
        'XDG_CACHE_HOME',
        'PI_CONFIG_DIR',
        'KINGU_OMP_SOURCE_AGENT_DIR',
        'KINGU_OMP_STATUS_EXTENSION',
        'KINGU_PI_STATUS_OWNED',
        'ZDOTDIR',
        'KINGU_ORIG_ZDOTDIR'
      ].flatMap((key) => ['-u', key]),
      `KINGU_PATH_PROBE_OUTPUT=${baselineResult}`,
      `HOME=${home}`,
      `ZDOTDIR=${testInfo.outputPath('profile')}`,
      '/bin/zsh',
      '-ilc',
      buildShellCommandFromArgv(
        [process.env.KINGU_OMP_PROOF_BINARY ?? '', '--no-extensions', '--extension', probe],
        'posix'
      )
    ],
    'posix'
  )
  const baselinePty = await kinguPage.evaluate(
  ...
  try {
    ...
    await kinguPage.evaluate((id) => window.api.pty.kill(id), baselinePty)
  }
})
