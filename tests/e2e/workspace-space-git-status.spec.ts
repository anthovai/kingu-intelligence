...
test.describe('Workspace Space git status checks', () => {
  test('checks every scanned deletable row, including rows after the first 50', async ({
    ...
    // Why: on symlinked tmpdirs (/var→/private/var on macOS, /tmp→… on CI) Kingu
    ...
    const worktreeParent = realpathSync(
      mkdtempSync(path.join(os.tmpdir(), 'kingu-space-git-status-'))
    )
    ...
    try {
      ...
      const repoId = await kinguPage.evaluate((testRepoPath) => {
      ...
      await loadWorktreesUntilPathsPresent(kinguPage, repoId, registeredWorktreePaths)

      const rowDisplayNames = await kinguPage.evaluate(
      ...
      // anything.
      const rowCheckboxes = kinguPage.getByRole('checkbox', {
        name: new RegExp(
      ...
      await expect(kinguPage.getByText('Keep: git not checked')).toHaveCount(0, { timeout: 30_000 })
    } finally {
      for (const worktreePath of worktreePaths) {
        try {
