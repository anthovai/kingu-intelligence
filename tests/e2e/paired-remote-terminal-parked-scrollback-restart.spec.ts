...
const scratch = mkdtempSync(path.join(os.tmpdir(), 'kingu-parked-restart-'))
...
function readOnDiskPartitions(userDataDir: string, webTabId: string): OnDiskPartitionReading[] {
  ...
  for (const file of globSync(path.join(userDataDir, '**', 'kingu-data.json'))) {
    try {
...
test.describe('host retains nothing', () => {
  ...
  test('the debounced write alone lands the parked scrollback in the remote host’s partition, so a hard kill loses nothing', async ({
    ...
    const extraEnv = { KINGU_E2E_TERMINAL_PARKING_DELAY_MS: String(PARK_DELAY_MS) }
    ...
    try {
      const parked = await parkRemoteTerminalWithToken(kinguPage, first, createdTerminals)
