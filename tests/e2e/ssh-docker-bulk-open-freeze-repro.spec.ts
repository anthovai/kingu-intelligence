 ...
 *
 * Requires: KINGU_E2E_SSH_DOCKER=1 and Docker available.
 *
 * Run:
 *   KINGU_E2E_SSH_DOCKER=1 pnpm run test:e2e:ssh-docker-bulk-open-freeze
 */
...
test.describe('R2 Docker SSH bulk-open freeze', () => {
  test.skip(!RUN_DOCKER_SSH, 'Set KINGU_E2E_SSH_DOCKER=1 to run Docker SSH freeze repro')
  ...
  test('bulk-open many flooding SSH terminals and measure renderer lag @freeze-repro @headful', async ({
    ...
    try {
      ...
      const firstPtyId = await waitForActivePanePtyId(kinguPage, 60_000)
      await execInTerminal(kinguPage, firstPtyId, continuousFloodCommand(runId, 0))
      ...
      await waitForTerminalOutput(kinguPage, `BG:SSH_BULK_${runId}_0:`, 60_000, FLOOD_READ_CHARS)

      for (let i = 1; i < SESSION_SPLITS; i += 1) {
        await splitActiveTerminalPane(kinguPage, 'vertical')
        await focusLastTerminalPane(kinguPage)
        const panePtyId = await waitForActivePanePtyId(kinguPage, 30_000)
        await execInTerminal(kinguPage, panePtyId, continuousFloodCommand(runId, i))
      ...
      await kinguPage.waitForTimeout(4_000)

      const hiddenProbe = await startRendererLagProbe(kinguPage)
      ...
      const openProbe = await startRendererLagProbe(kinguPage)
      await kinguPage.evaluate(() => window.__store?.getState().setActiveView('terminal'))
      for (let pass = 0; pass < 3; pass += 1) {
        for (let i = 0; i < SESSION_SPLITS; i += 1) {
          await kinguPage.keyboard.press(process.platform === 'darwin' ? 'Meta+]' : 'Control+]')
      ...
      const interactionProbeMs = await kinguPage.evaluate(async () => {
