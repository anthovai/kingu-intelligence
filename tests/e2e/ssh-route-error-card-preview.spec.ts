...
// Run: KINGU_SSH_CARD_PREVIEW=1 pnpm exec playwright test --config tests/playwright.config.ts \
//   --project electron-headless --workers=1 tests/e2e/ssh-route-error-card-preview.spec.ts
import { expect, test } from './helpers/kingu-app'

test.skip(
  process.env.KINGU_SSH_CARD_PREVIEW !== '1',
  'Preview only; run with KINGU_SSH_CARD_PREVIEW=1'
)
...
test('shows the SSH routing error cards and holds for review', async ({
  ...
  const targetId = await kinguPage.evaluate(async () => {
  ...
  await expect(kinguPage.getByText('SSH connection unavailable')).toBeVisible({
    timeout: 30_000
  })
