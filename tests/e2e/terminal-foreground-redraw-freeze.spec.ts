...
test.describe('Terminal foreground redraw freeze repro', () => {
  test('@headful Codex-style line rewrites request a visible row refresh', async ({
    ...
    const webglAttached = await forceActivePaneWebglRenderer(kinguPage)
    ...
    await installActivePaneRefreshProbe(kinguPage)
    try {
      const refreshBaseline = await readRefreshProbe(kinguPage)
      await resetSchedulerDebug(kinguPage)
      const measurement = await measureRendererDuringRewriteBurst(kinguPage, paneKey)
      const scheduler = await readSchedulerDebug(kinguPage)
      ...
      await expect
        .poll(
          async () => {
            const refresh = await readRefreshProbe(kinguPage)
      ...
      await disposeActivePaneRefreshProbe(kinguPage)
    }
  })
  ...
  test('active OpenTUI-style redraw bursts do not monopolize the renderer', async ({
    ...
    await waitForTerminalPtyDataInjector(kinguPage, paneKey)
    await resetSchedulerDebug(kinguPage)
    const measurement = await measureRendererDuringBurst(kinguPage, paneKey)
    const scheduler = await readSchedulerDebug(kinguPage)
  ...
  test('captured OpenCode/OpenTUI redraw bytes do not monopolize foreground writes', async ({
    ...
    await waitForTerminalPtyDataInjector(kinguPage, paneKey)
    await resetSchedulerDebug(kinguPage)
    const measurement = await measureRendererDuringFrames(kinguPage, paneKey, frames)
    const scheduler = await readSchedulerDebug(kinguPage)
    annotateMeasurement(testInfo, measurement, scheduler)
