...
test.describe('Diff note delete', () => {
  ...
  test('clicking the trash button removes the saved note', async ({ kinguPage }) => {
    ...
    const deleteButton = kinguPage.getByTitle('Delete note').first()
    ...
    await expect(kinguPage.getByTitle('Delete note')).toHaveCount(0, {
      timeout: 5_000
    })
  })
})
