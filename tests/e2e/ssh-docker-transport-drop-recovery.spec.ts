...
test.describe('SSH transport drop recovery', () => {
  test.skip(!RUN_DOCKER_SSH, 'Set KINGU_E2E_SSH_DOCKER=1 to run the dockerized SSH relay tests')
  ...
  test('recovers a live pane after the transport dies under it', async ({
    ...
    try {
      ...
      await execInTerminal(kinguPage, ptyId, `printf 'DROP_MARKER_%s\\n' ${markerSuffix}`)
      await waitForTerminalOutput(kinguPage, marker, 30_000)

      await recoverDockerSshRelayAfterFault(kinguPage, remote.targetId, () => {
      ...
      await waitForTerminalOutput(kinguPage, marker, 60_000)
      ...
      await waitForTerminalOutput(kinguPage, afterMarker, 60_000)
    } finally {
      if (target) {
  ...
  test('stays bounded when a disconnected shell floods its pty', async ({
    ...
    try {
      ...
      await ensureTerminalVisible(kinguPage, 45_000)
      await waitForActiveTerminalManager(kinguPage, 240_000)
      const ptyId = await waitForActivePanePtyId(kinguPage, 240_000)
      ...
      await execInTerminal(
        kinguPage,
        ptyId,
        `yes "$(printf 'KINGU_%s' FLOOD_LINE)" | head -c 48000000; printf 'FLOO%s\\n' DED`
      )
      await waitForTerminalOutput(kinguPage, 'KINGU_FLOOD_LINE', 30_000, 20_000)
      await recoverDockerSshRelayAfterFault(kinguPage, remote.targetId, () => {
        expect(dropDockerSshRelayTransport(target!)).toBeGreaterThan(0)
      })
      await waitForActiveTerminalManager(kinguPage, 240_000)
      ...
      await waitForTerminalOutput(kinguPage, 'FLOODED', 120_000, 20_000)
      ...
      await execInTerminal(
        kinguPage,
        await waitForActivePanePtyId(kinguPage, 240_000),
        `printf 'FLOOD_AFTER_%s\\n' ${markerSuffix}`
      )
      await waitForTerminalOutput(kinguPage, marker, 60_000, 20_000)
    } finally {
      if (target) {
        clearDockerSshRelayFaults(target)
        cleanupDockerSshRelayTarget(target)
  ...
  test('replaces the pane only when the host proves the session is gone', async ({
    ...
    try {
      ...
      await execInTerminal(kinguPage, ptyId, `printf 'KILL_MARKER_%s\\n' ${markerSuffix}`)
      await waitForTerminalOutput(kinguPage, marker, 30_000)
      ...
      await expect
        .poll(() => waitForActivePanePtyId(kinguPage, 60_000).catch(() => ptyId), {
          timeout: 120_000,
  ...
  test('keeps one reattachable lease per pane across repeated relay restarts', async ({
    ...
    try {
      ...
      for (let generation = 1; generation <= 5; generation++) {
        const previousPtyId = await waitForActivePanePtyId(kinguPage, 60_000)
        await recoverDockerSshRelayAfterFault(kinguPage, remote.targetId, () => {
          expect(
   ...
   */
  test('keeps the session while a frozen host goes silent', async ({ kinguPage }, testInfo) => {
    test.slow()
    let target: DockerSshRelayTarget | null = null
    try {
      target = startDockerSshRelayTarget(testInfo)
      ...
      await waitForSessionReady(kinguPage)
      await waitForActiveWorktree(kinguPage)
      await connectDockerSshRelayTarget(kinguPage, target, { relayGracePeriodSeconds: 0 })
      await ensureTerminalVisible(kinguPage, 45_000)
      await waitForActiveTerminalManager(kinguPage, 60_000)
      const ptyId = await waitForActivePanePtyId(kinguPage, 60_000)
      ...
      await execInTerminal(kinguPage, ptyId, `printf 'STALL_MARKER_%s\\n' ${markerSuffix}`)
      await waitForTerminalOutput(kinguPage, marker, 30_000)
      ...
      await withStalledDockerSshRelayTarget(target, async () => {
        await kinguPage.waitForTimeout(30_000)
      })
  ...
  test('accepts input again after a frozen host resumes', async ({ kinguPage }, testInfo) => {
    test.slow()
    let target: DockerSshRelayTarget | null = null
    ...
    try {
      ...
      await execInTerminal(kinguPage, ptyId, `printf 'STALL_BEFORE_%s\\n' ${beforeSuffix}`)
      await waitForTerminalOutput(kinguPage, `STALL_BEFORE_${beforeSuffix}`, 60_000)
      ...
      await recoverDockerSshRelayAfterFault(kinguPage, remote.targetId, async () => {
        await withStalledDockerSshRelayTarget(target!, async () => {
          await kinguPage.waitForTimeout(30_000)
        })
      })
      ...
      await execInTerminal(kinguPage, ptyId, `printf 'STALL_AFTER_%s\\n' ${afterSuffix}`)
      await attachSshRecoveryInputObservation(
        kinguPage,
        testInfo,
        remote.targetId,
