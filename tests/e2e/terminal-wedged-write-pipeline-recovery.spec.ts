...
test.describe('Wedged terminal write pipeline recovery', () => {
  test('pane recovers rendering and input after its write pipeline wedges', async ({
    ...
    await kinguPage.keyboard.type(`echo ${beforeMarker}`, { delay: 20 })
    await kinguPage.keyboard.press('Enter')
    await expect
      .poll(async () => (await getTerminalContent(kinguPage)).includes(beforeMarker), {
        timeout: 15_000,
        message: 'Baseline echo did not render — pane unhealthy before wedge'
      })
      .toBe(true)

    await wedgeActivePaneWritePipeline(kinguPage)
    ...
    await focusActiveTerminalInput(kinguPage)
    await kinguPage.keyboard.type(`echo ${afterMarker}`, { delay: 20 })
    await kinguPage.keyboard.press('Enter')

    await expect
      .poll(async () => (await getTerminalContent(kinguPage)).includes(afterMarker), {
  ...
  test('pane recovers after its xterm is disposed under live bindings (zombie pane)', async ({
    ...
    await sendToTerminal(kinguPage, ptyId, `echo ${outputMarker}\r`)

    await expect
      .poll(async () => (await getTerminalContent(kinguPage)).includes(outputMarker), {
        timeout: 45_000,
        message:
    ...
    await kinguPage.keyboard.type(`echo ${typedMarker}`, { delay: 20 })
    await kinguPage.keyboard.press('Enter')
    await expect
      .poll(async () => (await getTerminalContent(kinguPage)).includes(typedMarker), {
        timeout: 15_000,
