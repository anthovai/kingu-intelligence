...
test('sidebar keeps a Cursor pane visible and an OpenCode pane out of Claude Code hands', async ({
  ...
  await useFullAgentActivityRows(kinguPage)

  const openCode = await openAgentTab(kinguPage, worktreeId, 'opencode')
  ...
  await waitForTerminalOutput(kinguPage, PANE_HOLD_MARKER, 15_000)
  ...
  await expect
    .poll(() => paneTitles(kinguPage, openCode.tabId), {
      timeout: 15_000,
      message: 'the OpenCode task title never reached the renderer'
    })
    .toContain(OPENCODE_TASK_OSC_TITLE)

  const cursor = await openAgentTab(kinguPage, worktreeId, 'cursor')
  ...
  const agentList = worktreeRow(kinguPage, worktreeId).locator('[aria-label="Agents"]')
  ...
  expect(await settledSidebarAgentRowIdentities(kinguPage, agentListSelector)).toEqual([
    'Cursor',
    'OpenCode'
  ])
