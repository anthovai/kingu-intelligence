...
async function startRemoteMonitor(page: Page, ptyId: string): Promise<void> {
  const marker = `KINGU_SSH_WAKE_READY_${Date.now()}`
...
test.describe('SSH terminal window-wake stale PTY grid repro', () => {
  ...
  test('window focus heals a remote PTY whose applied grid drifted from xterm', async ({
    ...
    try {
      ...
      kinguPage.on('pageerror', (error) => pageErrors.push(error.message))
      installIdleGridMonitor(target)
      await kinguPage.setViewportSize(BASE_VIEWPORT)
      await waitForSessionReady(kinguPage)
      await waitForActiveWorktree(kinguPage)
      const identity = await kinguPage.evaluate(() => window.api.app.getIdentity())
      expect(identity.isDev).toBe(true)
      expect(identity.devWorktreeName).toBe(path.basename(process.cwd()))

      await connectDockerSshRelayTarget(kinguPage, target, { relayGracePeriodSeconds: 300 })
      await ensureTerminalVisible(kinguPage, 60_000)
      await waitForActiveTerminalManager(kinguPage, 60_000)
      const ptyId = await waitForActivePanePtyId(kinguPage, 60_000)
      await startRemoteMonitor(kinguPage, ptyId)
      await expect
        .poll(
          () => {
            try {
      ...
      const drifted = await readRendererGrid(kinguPage, ptyId)
      expect(drifted.xterm).toEqual(baseline.xterm)
      expect(drifted.applied).toEqual(staleGrid)

      await kinguPage.evaluate(() => window.dispatchEvent(new Event('focus')))
      ...
      if (!actualGridMatchesXterm(wakeResult.last.remote, wakeResult.last.renderer)) {
        ...
        await kinguPage.setViewportSize({
      ...
      await kinguPage.screenshot({ path: healedScreenshot, fullPage: true })
