...
async function assertCardClearsFollowingLine(page: Page): Promise<void> {
  const card = page.locator('.kingu-diff-comment-card').first()
...
test.describe('Diff note layout', () => {
  ...
  test('saved notes reserve their rendered height in both diff layouts', async ({
    ...
    const added = await kinguPage.evaluate(
    ...
    await expect(kinguPage.locator('button:has(svg.lucide-rows-2)')).toBeVisible()
    await assertCardClearsFollowingLine(kinguPage)
    await attachDiffScreenshot(kinguPage, testInfo, 'side-by-side-diff-note-layout')

    await kinguPage.evaluate(() =>
      window.__store?.getState().updateSettings({ diffDefaultView: 'inline' })
    )
    await expect(kinguPage.locator('button:has(svg.lucide-columns-2)')).toBeVisible()
    await assertCardClearsFollowingLine(kinguPage)
    await attachDiffScreenshot(kinguPage, testInfo, 'inline-diff-note-layout')
  })
})
