...
test.describe('Diff note edit', () => {
  ...
  test('editing a saved inline note updates the open diff card', async ({ kinguPage }) => {
    ...
    const addResult = await kinguPage.evaluate(
      async ({ wId, rel, body }) => {
        const store = window.__store
        if (!store) {
    ...
    await expect(card.locator('.kingu-diff-comment-body')).toHaveText(seededBody)

    await card.getByTitle('Edit note').click()

    const textarea = card.locator('.kingu-diff-comment-popover-textarea')
    await expect(textarea).toBeVisible()
    await expect(textarea).toHaveValue(seededBody)

    const saveButton = card
      .locator('.kingu-diff-comment-popover-footer button')
      .filter({ hasText: 'Save' })
    ...
    const updatedCard = kinguPage
      .locator('.kingu-diff-comment-card')
      .filter({ has: kinguPage.locator('.kingu-diff-comment-body', { hasText: editedBody }) })
      .first()
    await expect(updatedCard, 'inline card did not update in the open diff').toBeVisible()
    await expect(updatedCard.locator('.kingu-diff-comment-body')).toHaveText(editedBody)
    await expect(updatedCard.locator('.kingu-diff-comment-body')).not.toHaveText(seededBody)
