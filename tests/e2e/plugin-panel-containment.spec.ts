...
async function materializeHostilePlugin(origin: string): Promise<string> {
  const tempRoot = await mkdtemp(join(tmpdir(), 'kingu-hostile-panel-e2e-'))
...
test('contains hostile panel network and navigation probes', async ({
  ...
  kinguPage.on('console', (message) => {
    browserEvents.push(`console:${message.type()}:${message.text()}`)
  })
  kinguPage.on('pageerror', (error) => {
    browserEvents.push(`pageerror:${error.message}`)
  })
  kinguPage.on('framenavigated', (frame) => {
    browserEvents.push(`framenavigated:${frame.url()}`)
  })
  try {
    const panel = await installApprovedPanel(kinguPage, pluginRoot)
    await openPanel(kinguPage, panel)

    const iframe = kinguPage.locator(`iframe[title="${panel.title}"]`)
    await expect(iframe).toHaveAttribute('sandbox', 'allow-scripts')
    const frame = kinguPage.frameLocator(`iframe[title="${panel.title}"]`)
        ...
        new Promise<string>((resolve, reject) => {
          ...
          const onMessage = (event: MessageEvent): void => {
            ...
            if (
              ...
              !data ||
              data.type !== 'kingu-panel-action-result' ||
    ...
    expect(kinguPage.url()).toBe(appUrl)
...
test('detects and suspends a busy-looping panel in an isolated renderer', async ({
  ...
  try {
    ...
    await iframe.evaluate((element) => {
      const panelWindow = (element as HTMLIFrameElement).contentWindow
      panelWindow?.postMessage({ type: 'kingu-hostile-busy-probe' }, '*')
    })

    await expect(
      kinguPage.getByText('This plugin panel stopped responding and was suspended.')
    ).toBeVisible({ timeout: 20_000 })
    await expect(
      kinguPage.getByRole('button', { name: new RegExp(`${panel.title}.*Error`) })
    ).toBeVisible()
    expect(kinguPage.url()).toBe(appUrl)
    expect(server.requests).toEqual([])
  } finally {
