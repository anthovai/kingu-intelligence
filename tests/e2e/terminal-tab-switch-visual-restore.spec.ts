...
async function startHiddenPtyOutputBurst(page: Page, ptyId: string, runId: string): Promise<void> {
  ...
  await runNodeScriptInTerminal(page, ptyId, script, { prefix: 'kingu-tab-switch-burst' })
}
...
test.describe('Terminal tab switch visual restore', () => {
  ...
  test('keeps full-width geometry after switching away and back', async ({
    ...
    const { firstTabId, secondTabId } = await ensureTwoTerminalTabs(kinguPage)
    await forceWebglOnActiveTab(kinguPage)

    const runId = `${Date.now()}`
    const marker = `${TAB_SWITCH_MARKER_PREFIX}_${runId}`
    const firstPtyId = await waitForPanePtyIdOnTab(kinguPage, firstTabId)
    await writeStaticTabContent(kinguPage, firstTabId, marker, TAB_A_GLYPH_ROW)

    const baseline = await readTabTerminalGeometry(kinguPage, firstTabId, runId)
    ...
    await resetTerminalOutputSchedulerDebug(kinguPage)
    await startHiddenPtyOutputBurst(kinguPage, firstPtyId, runId)

    for (let cycle = 0; cycle < 12; cycle += 1) {
      await activateTerminalTab(kinguPage, secondTabId)
      await injectHiddenStreamingBurst(kinguPage, firstTabId, runId)
      // Why: rapid back-to-back switches mirror the user's leave/return pattern
      // and race the overlay's rAF/50ms refit retries.
      await activateTerminalTab(kinguPage, firstTabId)
      if (cycle % 3 === 0) {
        await activateTerminalTab(kinguPage, secondTabId)
        await activateTerminalTab(kinguPage, firstTabId)
      }
      ...
      const immediate = await readTabTerminalGeometry(kinguPage, firstTabId, runId)
      ...
      await kinguPage.waitForTimeout(60)
      const settled = await readTabTerminalGeometry(kinguPage, firstTabId, runId)
    ...
    const schedulerActivity = await waitForHiddenOutputSchedulerActivity(kinguPage)
  ...
  test('keeps geometry after hidden alt-screen TUI redraws during tab switches', async ({
    ...
    const { firstTabId, secondTabId } = await ensureTwoTerminalTabs(kinguPage)
    await forceWebglOnActiveTab(kinguPage)
    await waitForPanePtyIdOnTab(kinguPage, firstTabId)
    ...
    for (let cycle = 0; cycle < 6; cycle += 1) {
      ...
      const { ptyId, cols, rows } = await readPaneIdentityOnTab(kinguPage, firstTabId)
      await setHiddenSnapshotOverride(kinguPage, ptyId, {
        data: buildAltScreenFrame(finalMarker, restoreFrame),
        cols,
        rows
      })
      await activateTerminalTab(kinguPage, secondTabId)
      await writeToPaneTerminal(kinguPage, firstTabId, redraw)
      await activateTerminalTab(kinguPage, firstTabId)

      const geometry = await readTabTerminalGeometry(kinguPage, firstTabId, `${runId}_ALT`)
      const renderedFrame = await readRenderedAltScreenFrame(kinguPage, firstTabId, finalMarker)
      renderPaths.push(
  ...
  test('restores skipped hidden agent output on light tab resume', async ({ kinguPage }) => {
    await waitForSessionReady(kinguPage)
    await waitForActiveWorktree(kinguPage)
    await ensureTerminalVisible(kinguPage)
    await waitForActiveTerminalManager(kinguPage, 30_000)
    ...
    const agentTabId = await createCodexMarkedTerminalTab(kinguPage)
    await waitForActiveTerminalManager(kinguPage, 30_000)
    await waitForPanePtyIdOnTab(kinguPage, agentTabId)
    const paneIdentity = await readPaneIdentityOnTab(kinguPage, agentTabId)
    ...
    await resetHiddenOutputDebug(kinguPage)
    await injectPaneData(kinguPage, paneKey, hiddenFrame, {
      seq: hiddenFrame.length,
      rawLength: hiddenFrame.length
    })

    await expect
      .poll(async () => (await readHiddenOutputDebug(kinguPage))?.hiddenRendererSkipCount ?? 0, {
        timeout: 5_000,
        message: 'Codex-marked hidden output did not take the skipped renderer path'
      })
      .toBeGreaterThan(0)
    await setHiddenSnapshotOverride(kinguPage, paneIdentity.ptyId, {
    ...
    await activateTerminalTab(kinguPage, agentTabId)

    await expect
      .poll(() => getTerminalContent(kinguPage, 8_000), {
        timeout: 10_000,
  ...
  test('restores skipped hidden Grok output on light tab resume', async ({ kinguPage }) => {
    await waitForSessionReady(kinguPage)
    await waitForActiveWorktree(kinguPage)
    await ensureTerminalVisible(kinguPage)
    await waitForActiveTerminalManager(kinguPage, 30_000)
    ...
    const shellTabId = (await getActiveTabId(kinguPage))!
    const grokTabId = await createGrokMarkedTerminalTab(kinguPage)
    await waitForActiveTerminalManager(kinguPage, 30_000)
    await waitForPanePtyIdOnTab(kinguPage, grokTabId)
    const paneIdentity = await readPaneIdentityOnTab(kinguPage, grokTabId)
    ...
    await activateTerminalTab(kinguPage, grokTabId)
  ...
  test('@headful keeps returned tab glyphs intact across tab switches', async ({
    ...
    await forceWebglOnActiveTab(kinguPage)
    await activateTerminalTab(kinguPage, firstTabId)
    const firstWebgl = await waitForWebglOnTab(kinguPage, firstTabId)
    await activateTerminalTab(kinguPage, secondTabId)
    await kinguPage.evaluate((id) => {
      window.__paneManagers?.get(id)?.setTerminalGpuAcceleration?.('on')
    }, secondTabId)
    const secondWebgl = await waitForWebglOnTab(kinguPage, secondTabId)
    ...
    const firstPtyId = await waitForPanePtyIdOnTab(kinguPage, firstTabId)
    const secondPtyId = await waitForPanePtyIdOnTab(kinguPage, secondTabId)
    await sendToTerminal(kinguPage, firstPtyId, SILENT_FOREGROUND_COMMAND)
    await sendToTerminal(kinguPage, secondPtyId, SILENT_FOREGROUND_COMMAND)
    await kinguPage.waitForTimeout(1_000)

    const runId = `${Date.now()}`
    const markerA = `${TAB_SWITCH_MARKER_PREFIX}_A_${runId}`
    const markerB = `${TAB_SWITCH_MARKER_PREFIX}_B_${runId}`
    await writeStaticTabContent(kinguPage, firstTabId, markerA, TAB_A_GLYPH_ROW)
    await activateTerminalTab(kinguPage, secondTabId)
    await writeStaticTabContent(kinguPage, secondTabId, markerB, TAB_B_GLYPH_ROW)

    await activateTerminalTab(kinguPage, firstTabId)
    await resetAtlasOnTab(kinguPage, firstTabId)
    await kinguPage.waitForTimeout(800)
    const baseline = await captureStableTabScreenshot(kinguPage, firstTabId)
    ...
    for (let cycle = 0; cycle < 8; cycle += 1) {
      ...
      const afterReturn = await captureStableTabScreenshot(kinguPage, firstTabId)
