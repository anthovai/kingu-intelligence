 ...
 * Kingu's file-path provider resolves link candidates asynchronously.
 */
...
test.describe('Terminal link hover after worktree return', () => {
  ...
  test('re-establishes a URL link on hover after the pointer leaves the terminal', async ({
    ...
    const url = `https://example.com/kingu-link-${randomUUID()}`
    await sendToTerminal(kinguPage, ptyId, `echo ${url}\r`)
    await expect
      .poll(() => getTerminalContent(kinguPage, 4000), {
        timeout: 10_000,
    ...
    const probe = await locateHoverProbe(kinguPage, url)
    await expect
      .poll(() => hoverAndReadActiveLinkText(kinguPage, probe), {
        timeout: 5_000,
        message: 'baseline hover never established the URL link'
      })
      .toContain(url)

    await dispatchScreenMouseLeave(kinguPage, probe.tabId)
    await expect.poll(() => readActiveLinkText(kinguPage, probe.tabId)).toBeNull()
    await expect.poll(() => readTerminalCursor(kinguPage, probe.tabId)).not.toBe('pointer')
    ...
    await expect
      ...
      .toContain(url)
    await expect.poll(() => readTerminalCursor(kinguPage, probe.tabId)).toBe('pointer')
  })
  ...
  test('re-establishes a file-path link on hover after switching worktrees and back', async ({
    ...
    const worktreePath = await activeWorktreePath(kinguPage)
    const fileName = `kingu-linkfile-${randomUUID().slice(0, 8)}.txt`
    const filePath = path.join(worktreePath, fileName)
    writeFileSync(filePath, 'kingu file link target\n')
    const needle = `./${fileName}`

    try {
      await sendToTerminal(kinguPage, ptyId, `echo ${needle}\r`)
      await expect
        .poll(() => getTerminalContent(kinguPage, 4000), {
          timeout: 10_000,
      ...
      const probe = await assertLinkRecoversAfterReturn(kinguPage, {
        firstWorktreeId,
        secondWorktreeId,
        needle,
        expectContains: fileName
      })
      await activateHoveredLink(kinguPage, probe)
      ...
      await expect(kinguPage.locator('.editor-header-path').first()).toContainText(fileName, {
        timeout: 20_000
      })
    } finally {
      await kinguPage.evaluate((filePath) => {
        const state = window.__store?.getState()
  ...
  test('re-establishes a URL link on hover after switching worktrees and back', async ({
    ...
    await assertLinkRecoversAfterReturn(kinguPage, {
      firstWorktreeId,
      secondWorktreeId,
      needle: url,
      expectContains: url
