/**
 * TOPOLOGY (a): real Kingu DESKTOP app as the remote server, isolated profile,
 * window ATTACHED, paired to a separate real desktop client.
 *
 * A terminal created by `kingu terminal create` (RPC `terminal.create` ->
 * KinguRuntimeService.createTerminal) must survive the renderer graph syncs that
...
async function prepareRetentionFixture(
  ...
  const { worktreeId, unrelatedWorktreeId } = await kinguPage.evaluate(() => {
  ...
  const rendererTabId = await createHostRendererTerminalTab(kinguPage, worktreeId)
...
/**
 * One `kingu terminal create` in the measured workspace, one renderer graph sync,
...
async function runCliTerminalRetentionJourney(
  ...
  kinguPage.on('pageerror', (error) => hostPageErrors.push(String(error)))
  ...
  try {
    const fixture = await prepareRetentionFixture(kinguPage, client)
    call = fixture.call
    ...
    const secondRendererTabId = await createHostRendererTerminalTab(kinguPage, worktreeId)
...
test('keeps a host-created CLI terminal when an earlier host-created terminal exists', async ({
  ...
  await runCliTerminalRetentionJourney(kinguPage, testInfo, 'cli-terminal-graph-sync', true)
})

// The reported incident, and the shape the real CLI takes: `kingu terminal
...
test('keeps a host-created CLI terminal that is the first one on the host', async ({
  ...
  test.setTimeout(600_000)
  await runCliTerminalRetentionJourney(kinguPage, testInfo, 'cli-terminal-graph-sync-first', false)
})
