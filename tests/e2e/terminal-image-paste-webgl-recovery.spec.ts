...
test.describe('terminal image paste WebGL recovery @headful', () => {
  test('clears the WebGL atlas after a real image clipboard paste', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.kingu-image-paste-redraw-${marker}.mjs`)
    ...
    try {
      ...
      await waitForTerminalOutput(kinguPage, `READY_${marker}`, 10_000)

      await forceWebgl(kinguPage)
      ...
      expect(await patchAtlasCounter(kinguPage)).toBe(true)

      await kinguPage.locator('.xterm-helper-textarea').first().focus()
      await kinguPage.evaluate(
        (dataUrl) => window.api.ui.writeClipboardImage(dataUrl),
        CLIPBOARD_IMAGE_DATA_URL
      )
      await kinguPage.keyboard.press(process.platform === 'darwin' ? 'Meta+V' : 'Control+V')
      await waitForTerminalOutput(kinguPage, `DONE_${marker}`, 10_000)

      await expect.poll(() => readAtlasResetCount(kinguPage), { timeout: 2_000 }).toBeGreaterThan(0)
    } finally {
