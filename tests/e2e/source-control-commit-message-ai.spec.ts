...
test.describe('Source Control AI commit messages', () => {
  ...
  for (const { label, linkedIssue, expected } of [
    ...
    test(`${label} the commit-message recipe`, async ({ kinguPage, testRepoPath }) => {
      ...
      try {
        ...
        await openSourceControlForWorktree(kinguPage, testRepoPath, worktreePath)
            ...
            await store.getState().updateSettings({
              ...
              sourceControlAi: {
                ...
                actions: {
                  commitMessage: {
                    agentId: 'custom' as const,
                    commandInputTemplate: 'KINGU_E2E_ISSUE={linkedIssue}\n\n{basePrompt}'
                  }
                }
              }
            })
        ...
        const textarea = kinguPage.getByRole('textbox', { name: 'Commit message' })
        await expect(textarea).toBeVisible({ timeout: 10_000 })

        const generate = kinguPage.getByRole('button', { name: 'Generate commit message with AI' })
