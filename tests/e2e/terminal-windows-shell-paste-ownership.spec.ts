...
test.describe('Windows terminal shell paste ownership', () => {
  ...
  test('PowerShell default terminal keyboard paste preserves exact content with one PTY owner', async ({
    ...
    await createWindowsDefaultShellTerminalTab(kinguPage, 'powershell.exe')
    await waitForActiveTerminalManager(kinguPage, 30_000)
    await installTerminalPtyWriteSpy(electronApp)

    const ptyId = await waitForActivePanePtyId(kinguPage)
    const runId = randomUUID()
    const sentinel = `KINGU_E2E_POWERSHELL_DONE_${runId}`
    const powershellEscape = '`'
    const payload = [
      `KINGU_E2E_POWERSHELL_PASTE_${runId}`,
      `PowerShell metacharacters: ${powershellEscape} $ " ' ; | & < > @ { } ( )`,
      'quoted Windows path: C:\\Program Files\\Kingu Test\\file name.txt',
      'cmd metacharacters preserved as text: %PATH% !PROMPT! ^ & | < >',
      'Unicode: café 你好 مرحبا 😀',
      `mixed-newline-before\r\nlf-line\ncrlf-line\r\n${sentinel}`
    ].join('\n')
    const scriptPath = path.join(testRepoPath, `.kingu-paste-powershell-shell-${runId}.mjs`)
    ...
    try {
      ...
      scriptStarted = true
      await waitForTerminalOutput(kinguPage, `PASTE_READY_${runId}`, 10_000)

      await clearTerminalPtyWriteLog(electronApp)
      await kinguPage.evaluate((text) => window.api.ui.writeClipboardText(text), payload)
      await focusActiveTerminalInput(kinguPage)

      await kinguPage.keyboard.press('Control+V')
      await waitForTerminalOutput(kinguPage, `PASTE_COMPLETE_${runId}:MATCH`, 10_000, 12_000)
  ...
  test('cmd.exe default terminal keyboard paste preserves exact content with one PTY owner', async ({
    ...
    await waitForActiveWorktree(kinguPage)
    await ensureTerminalVisible(kinguPage)
    await createWindowsDefaultShellTerminalTab(kinguPage, 'cmd.exe')
    await waitForActiveTerminalManager(kinguPage, 30_000)
    await installTerminalPtyWriteSpy(electronApp)

    const ptyId = await waitForActivePanePtyId(kinguPage)
    const runId = randomUUID()
    const sentinel = `KINGU_E2E_CMD_DONE_${runId}`
    const payload = [
      `KINGU_E2E_CMD_PASTE_${runId}`,
      'cmd metacharacters: %PATH% !PROMPT! ^ & | < >',
      'quoted Windows path: C:\\Program Files\\Kingu Test\\file name.txt',
      'PowerShell metacharacters: ` $ " \' ; @ { } ( )',
      `mixed-newline-before\r\nlf-line\ncrlf-line\r\n${sentinel}`
    ].join('\n')
    const scriptPath = path.join(testRepoPath, `.kingu-paste-cmd-shell-${runId}.mjs`)
    const expectedText = payload.replace(/\r?\n/g, '\r')
    writeFileSync(scriptPath, pasteCollectScript(runId, sentinel, expectedText))
    let scriptStarted = false
  ...
  test('Git Bash default terminal keyboard paste preserves POSIX-shaped content with one PTY owner', async ({
    ...
    await skipWhenGitBashUnavailable(kinguPage)

    await waitForSessionReady(kinguPage)
    await waitForActiveWorktree(kinguPage)
    await ensureTerminalVisible(kinguPage)
    await createWindowsDefaultShellTerminalTab(kinguPage, WINDOWS_GIT_BASH_SHELL)
    await waitForActiveTerminalManager(kinguPage, 30_000)
    await installTerminalPtyWriteSpy(electronApp)

    const ptyId = await waitForActivePanePtyId(kinguPage)
    const runId = randomUUID()
    const sentinel = `KINGU_E2E_GIT_BASH_DONE_${runId}`
    const payload = [
      `KINGU_E2E_GIT_BASH_PASTE_${runId}`,
    ...
    const scriptPath = path.join(testRepoPath, `.kingu-paste-git-bash-shell-${runId}.mjs`)
    const expectedText = payload.replace(/\r?\n/g, '\r')
    writeFileSync(scriptPath, pasteCollectScript(runId, sentinel, expectedText))
    let scriptStarted = false
  ...
  test('WSL terminal keyboard paste preserves Linux shell content with one PTY owner', async ({
    ...
    await waitForActiveWorktree(kinguPage)
    await ensureTerminalVisible(kinguPage)
    const wslDistro = await configureActiveProjectWslRuntime(kinguPage)
    test.skip(!wslDistro, 'No WSL distro is available on this Windows host')
    await createWindowsProjectRuntimeTerminalTab(kinguPage, 'wsl.exe')
    await waitForActiveTerminalManager(kinguPage, 30_000)
    await installTerminalPtyWriteSpy(electronApp)

    const ptyId = await waitForActivePanePtyId(kinguPage)
    const runId = randomUUID()
    const sentinel = `KINGU_E2E_WSL_DONE_${runId}`
    const payload = [
      `KINGU_E2E_WSL_PASTE_${runId}`,
      'POSIX shell metacharacters: $ ` " \' ; | & < > * ? [ ] ( )',
    ...
    const scriptPath = path.join(testRepoPath, `.kingu-paste-wsl-shell-${runId}.mjs`)
    const expectedText = payload.replace(/\r?\n/g, '\r')
    writeFileSync(scriptPath, pasteCollectScript(runId, sentinel, expectedText))
    let scriptStarted = false
  ...
  test('existing WSL terminal keeps paste runtime after default shell changes', async ({
    ...
    const wslDistro = await configureActiveProjectWslRuntime(kinguPage)
    test.skip(!wslDistro, 'No WSL distro is available on this Windows host')
    const tabId = await createWindowsProjectRuntimeTerminalTab(kinguPage, 'wsl.exe')
    await waitForActiveTerminalManager(kinguPage, 30_000)
    await installTerminalPtyWriteSpy(electronApp)

    const ptyId = await waitForActivePanePtyId(kinguPage)
    const runId = randomUUID()
    const sentinel = `KINGU_E2E_WSL_RETENTION_DONE_${runId}`
    const payload = [
      `KINGU_E2E_WSL_RETENTION_PASTE_${runId}`,
    ...
    const scriptPath = path.join(testRepoPath, `.kingu-paste-wsl-retention-${runId}.mjs`)
    const expectedText = payload.replace(/\r?\n/g, '\r')
    writeFileSync(scriptPath, pasteCollectScript(runId, sentinel, expectedText))
    let scriptStarted = false
    ...
    try {
      ...
      await updateWindowsDefaultShellSetting(kinguPage, 'cmd.exe')
      await expect(
        kinguPage.locator(`[data-testid="sortable-tab"][data-tab-id="${tabId}"] [data-shell-icon]`)
      ).toHaveAttribute('data-shell-icon', 'wsl.exe')
      expect(await waitForActivePanePtyId(kinguPage)).toBe(ptyId)
