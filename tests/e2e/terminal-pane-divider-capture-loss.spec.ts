...
test('@headful keeps resizing after the divider loses pointer capture', async ({
  ...
  await kinguPage.setViewportSize({ width: 1600, height: 1000 })
  await addTestRepo(kinguPage, testRepoPath)
  ...
  await waitForPaneCount(kinguPage, 2, 30_000)

  await expect
    .poll(async () => (await readDividerGeometry(kinguPage)).second.width)
    .toBeGreaterThan(400)
  ...
  const before = await readDividerGeometry(kinguPage)
  const startX = box.x + box.width / 2
  const startY = box.y + box.height / 2
  await kinguPage.mouse.move(startX, startY)
  await kinguPage.mouse.down()
  await kinguPage.mouse.move(startX + 140, startY, { steps: 10 })
  await expect
    .poll(async () =>
      Math.abs((await readDividerGeometry(kinguPage)).first.width - before.first.width)
    )
    .toBeGreaterThan(80)
  ...
  await kinguPage.mouse.move(startX + 260, startY, { steps: 10 })
  await expect
    .poll(() => divider.evaluate((element) => Number(element.dataset.captureLossCount ?? '0')))
    .toBe(1)
  await kinguPage.mouse.up()
  await expect.poll(async () => gridsMatch(await readDividerGeometry(kinguPage))).toBe(true)
  const after = await readDividerGeometry(kinguPage)
