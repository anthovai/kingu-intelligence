...
test('keeps the visible Agent Dashboard interactive during a 100-pane status replay', async ({
  ...
  const panes = await kinguPage.evaluate(
  ...
  const dashboardButton = kinguPage.getByRole('button', { name: /Agent Dashboard/ })
  ...
  await kinguPage.locator('[data-agent-dashboard-sheet]').waitFor({ state: 'visible' })
  const interactionElapsedMs = performance.now() - interactionStartedAt
  const statusPublicationsAtVisible = await kinguPage.evaluate(() => {
    const probe = (
  ...
  const evidence = await kinguPage.evaluate(
