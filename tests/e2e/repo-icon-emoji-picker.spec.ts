...
async function openRepoSettings(page: Page, repoId: string): Promise<void> {
  // Why: the host OS locale (e.g. ko-KR) drives Kingu's default UI language.
...
test.describe('Repository icon emoji picker', () => {
  test('search selects a native emoji and persists it as the repo icon', async ({
    ...
    const repos = await getStoreState<Repo[]>(kinguPage, 'repos')
    expect(repos.length).toBeGreaterThan(0)
    const repo = repos[0]

    await openRepoSettings(kinguPage, repo.id)

    const repoSection = kinguPage.locator(`[data-settings-section="repo-${repo.id}"]`)
    ...
    await attachScreenshot(kinguPage, testInfo, 'repo-icon-emoji-picker-search')
    ...
    await expect
      .poll(
        async () => {
          const current = await getStoreState<Repo[]>(kinguPage, 'repos')
    ...
    await attachScreenshot(kinguPage, testInfo, 'repo-icon-emoji-picker-selected')
  })
})
