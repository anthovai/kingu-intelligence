 ...
 * - PR #677 (`return to Kingu landing screen after closing last terminal`)
...
test.describe('Tab Close Navigation', () => {
   ...
   */
  test('closing the active editor tab activates its visual neighbor', async ({ kinguPage }) => {
    const worktreeId = await waitForActiveWorktree(kinguPage)

    const fileIds = await openSeededEditorTabs(kinguPage, worktreeId, [
      'package.json',
      'README.md',
      'tsconfig.json'
    ])
    ...
    await setActiveFile(kinguPage, fileIds[1])
    await expect.poll(async () => getActiveFileId(kinguPage), { timeout: 3_000 }).toBe(fileIds[1])

    await closeFile(kinguPage, fileIds[1])
    ...
    await expect
      .poll(async () => getActiveFileId(kinguPage), {
        timeout: 5_000,
   ...
   */
  test('closing the active diff tab activates a still-open neighbor', async ({ kinguPage }) => {
    const worktreeId = await waitForActiveWorktree(kinguPage)

    // Seed two editor tabs + one diff tab in the same worktree.
    const editorIds = await openSeededEditorTabs(kinguPage, worktreeId, [
      'package.json',
      'README.md'
    ])
    expect(editorIds.length).toBe(2)

    const diffId = await kinguPage.evaluate((wId) => {
      const store = window.__store
      if (!store) {
        return null
      }
    ...
    await expect.poll(async () => getActiveFileId(kinguPage), { timeout: 3_000 }).toBe(diffId)

    await closeFile(kinguPage, diffId!)
   ...
   */
  test('closing the last visible surface returns the app to Landing', async ({ kinguPage }) => {
    const worktreeId = await waitForActiveWorktree(kinguPage)
    ...
    const editorIds = await openSeededEditorTabs(kinguPage, worktreeId, ['package.json'])
    expect(editorIds.length).toBe(1)

    await setActiveFile(kinguPage, editorIds[0])
    await expect.poll(async () => getActiveFileId(kinguPage), { timeout: 3_000 }).toBe(editorIds[0])
    ...
    await closeFile(kinguPage, editorIds[0])
