...
test.describe('Worktree Recent Sort', () => {
  ...
  function createFolderFixture(): string {
    const dir = mkdtempSync(path.join(os.tmpdir(), 'kingu-e2e-folder-'))
  ...
  test.afterEach(() => {
    ...
    // matches the mkdtempSync/rmSync pairing used in helpers/kingu-app.ts
    // and helpers/kingu-restart.ts.
  ...
  test('stamps lastActivityAt on a newly-added folder repo so it sorts to the top of Recent', async ({
    ...
    const repoId = await addFolderRepo(kinguPage, folderPath)
    const lastActivityAt = await readFolderWorktreeLastActivity(kinguPage, repoId)
  ...
  test('leaves lastActivityAt stable across repeated list refreshes', async ({ kinguPage }) => {
    ...
    const first = await readFolderWorktreeLastActivity(kinguPage, repoId)

    await kinguPage.evaluate(async (id) => {
      await window.__store?.getState().fetchWorktrees(id)
      await window.__store?.getState().fetchWorktrees(id)
    }, repoId)

    const second = await readFolderWorktreeLastActivity(kinguPage, repoId)
