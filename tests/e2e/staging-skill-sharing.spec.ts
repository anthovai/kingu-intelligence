...
const RUN_STAGING = process.env.KINGU_E2E_SKILL_STAGING === '1'
const AUTH_TOKEN = process.env.KINGU_CLOUD_AUTH_TOKEN?.trim()
const PHYSICAL_HOST_PAIRING_URL = process.env.KINGU_E2E_SKILL_PHYSICAL_PAIRING_URL?.trim()
const RUN_HEADLESS_PAIRED = process.env.KINGU_E2E_SKILL_PAIRED_HEADLESS === '1'
const PHYSICAL_WSL_DISTRO = 'Ubuntu-24.04'
const SKILL_NAME = `kingu-staging-${randomUUID().slice(0, 8)}`
const SSH_TARGET = RUN_STAGING ? stagingSkillSshTargetFromEnvironment() : null

if (RUN_STAGING && !AUTH_TOKEN) {
  throw new Error('KINGU_CLOUD_AUTH_TOKEN is required for the noninteractive staging journey.')
}
...
test.use({
  kinguAppExtraEnv: {
    KINGU_ARTIFACTS_API_URL: 'https://cloud-api-staging.onkingu.dev',
    KINGU_CLOUD_API_URL: 'https://auth-staging.onkingu.dev',
    KINGU_CLOUD_AUTH_URL: 'https://auth-staging.onkingu.dev',
    KINGU_CLOUD_CLIENT_ID: 'kingu-desktop',
    ...(AUTH_TOKEN ? { KINGU_CLOUD_AUTH_TOKEN: AUTH_TOKEN } : {})
  }
})

test.skip(!RUN_STAGING, 'Set KINGU_E2E_SKILL_STAGING=1 to run the live staging journey.')
...
test('publishes, updates, revokes, and deletes without losing local state', async ({
  ...
  const sourceRoot = mkdtempSync(join(tmpdir(), 'kingu-staging-skill-source-'))
  ...
  try {
    if (RUN_HEADLESS_PAIRED) {
      ...
      physicalEnvironmentId = await addPhysicalHost(kinguPage, pairedHost.offer.pairingUrl)
    } else if (PHYSICAL_HOST_PAIRING_URL) {
      physicalEnvironmentId = await addPhysicalHost(kinguPage, PHYSICAL_HOST_PAIRING_URL)
    }
    if (SSH_TARGET) {
      sshTargetId = await connectStagingSkillSshTarget(kinguPage, SSH_TARGET)
    }
    ...
    const firstInstall = await installVersion(kinguPage, first.published, { scope: 'global' })
    ...
    for (const target of externalTargets(physicalEnvironmentId, sshTargetId)) {
      ...
      await expectManagedRemoteVersion(kinguPage, target, second.published.version.versionId)
    }
    const conflict = await installVersion(kinguPage, second.published, { scope: 'global' })
    ...
    for (const target of externalTargets(physicalEnvironmentId, sshTargetId)) {
      ...
      await expectManagedRemoteVersion(kinguPage, target, first.published.version.versionId)
    }
    ...
    const revokedInstall = await kinguPage.evaluate(
    ...
    const removed = await kinguPage.evaluate(
    ...
    expect(existsSync(globalSkill)).toBe(false)
    expect(
      await kinguPage.evaluate((id) => window.api.skills.getPackage(id), packageId)
    ).toMatchObject({ status: 'ok' })
    expect(
      await kinguPage.evaluate((id) => window.api.skills.deletePackage(id), packageId)
    ).toMatchObject({ status: 'ok' })
    packageId = null
  } finally {
    for (const target of externalTargets(physicalEnvironmentId, sshTargetId)) {
    ...
    if (sshTargetId) {
      await removeStagingSkillSshTarget(kinguPage, sshTargetId).catch(() => undefined)
    }
...
function writeSkill(directory: string, version: string): void {
  writeFileSync(
    ...
    `---\nname: ${SKILL_NAME}\ndescription: Kingu staging installation journey\n---\n\n# Staging journey\n\nversion: ${version}\n`
  )
}
