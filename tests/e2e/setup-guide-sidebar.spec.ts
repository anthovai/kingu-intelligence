...
test.describe('Setup guide sidebar entry', () => {
  ...
  test('does not flash while completed setup waits for capability readiness', async ({
    ...
    await waitForSessionReady(kinguPage)
    await seedCompletedSetupExceptCapabilityReadiness(kinguPage)

    await expect
      .poll(async () => getStoreState<boolean>(kinguPage, 'setupGuideSidebarDismissed'), {
        timeout: 5_000
      })
      .toBe(false)
    await expect(kinguPage.getByText(CHECKLIST_TEXT)).toHaveCount(0)

    await startSetupGuideFlashMonitor(kinguPage)

    await setActiveViewForFlashProbe(kinguPage, 'tasks')
    await expect
      .poll(async () => getStoreState<string>(kinguPage, 'activeView'), { timeout: 5_000 })
      .toBe('tasks')
    await kinguPage.waitForTimeout(500)

    await setActiveViewForFlashProbe(kinguPage, 'automations')
    await expect
      .poll(async () => getStoreState<string>(kinguPage, 'activeView'), { timeout: 5_000 })
      .toBe('automations')
    await kinguPage.waitForTimeout(500)

    await setActiveViewForFlashProbe(kinguPage, 'mobile')
    await expect
      .poll(async () => getStoreState<string>(kinguPage, 'activeView'), { timeout: 5_000 })
      .toBe('mobile')
    await kinguPage.waitForTimeout(500)

    const flashSamples = await stopSetupGuideFlashMonitor(kinguPage)
    ...
    await kinguPage.evaluate(() => {
      window.dispatchEvent(new CustomEvent('kingu:installed-agent-skills-changed'))
    })
  })
})
...
async function installBlockedCompletedCapabilityFakes(
  ...
  await evaluateInElectronMainWithNavigationRetry(electronApp, ({ ipcMain }) => {
    ...
    const makeSkill = (name: string, id: string): SkillDiscoveryResult['skills'][number] => ({
      ...
      rootPath: '/tmp/kingu-e2e-skills',
      directoryPath: `/tmp/kingu-e2e-skills/${name}`,
      skillFilePath: `/tmp/kingu-e2e-skills/${name}/SKILL.md`,
      installed: true,
      updatedAt: 1
    })
    ...
    ipcMain.handle('skills:discover', async (): Promise<SkillDiscoveryResult> => {
      ...
      return {
        skills: [
          makeSkill('kingu-cli', 'e2e-kingu-cli'),
