...
test('deleting the active scrolled worktree preserves position and closes the row gap', async ({
  ...
  await kinguPage.setViewportSize({ width: 1_200, height: 800 })
  const { belowId, successorId, targetId } = await seedActiveDeletionRows(kinguPage)
  await prepareScrolledActiveRow(kinguPage, targetId)
  const target = kinguPage.locator(
    `[data-worktree-sidebar] [data-worktree-id=${JSON.stringify(targetId)}]`
  )
  const below = kinguPage.locator(
    `[data-worktree-sidebar] [data-worktree-id=${JSON.stringify(belowId)}]`
  )
  await pauseForVisualProof(kinguPage)
  ...
  const deleteItem = kinguPage.getByRole('menuitem', { name: /^Delete(?:\s|$)/ })
  await expect(deleteItem).toBeVisible()
  await expect(deleteItem).toBeInViewport()
  await pauseForVisualProof(kinguPage)
  await startRowRemovalSampling(kinguPage, targetId, belowId)
  await deleteItem.click()

  await expect(target).toHaveCount(0)
  await expect(below).toBeVisible()
  await expect
    .poll(() => kinguPage.evaluate(() => window.__store?.getState().activeWorktreeId ?? null))
    .toBe(successorId)
  const frames = await finishRowRemovalSampling(kinguPage)
  await pauseForVisualProof(kinguPage)
  ...
  await expect(
    kinguPage.locator(`[data-worktree-sidebar] [data-worktree-id=${JSON.stringify(successorId)}]`)
  ).toHaveCount(0)
})
...
test('reduced motion removes the active row without animating its neighbor', async ({
  ...
  await waitForSessionReady(kinguPage)
  const { belowId, targetId } = await seedActiveDeletionRows(kinguPage)
  await prepareScrolledActiveRow(kinguPage, targetId)

  const animationCount = await kinguPage.evaluate(
