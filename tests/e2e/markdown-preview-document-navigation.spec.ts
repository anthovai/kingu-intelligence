...
const FIXTURE_DIRECTORY = 'kingu-e2e-preview-document-navigation'
  ...
  for (const anchored of [false, true]) {
    test(`${kind} ${anchored ? 'anchored' : 'plain'} links stay in Markdown preview`, async ({
      ...
      try {
        ...
        const linkElement = kinguPage.getByRole('link', { name: 'Open target', exact: true })
        ...
        await expect(kinguPage.getByRole('heading', { name: 'Target', exact: true })).toBeVisible()
