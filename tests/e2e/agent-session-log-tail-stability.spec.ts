...
test.describe('Agent Session History live log', () => {
  test.use({
    kinguAppExtraArgs: ['--js-flags=--expose-gc', '--enable-precise-memory-info']
  })
  ...
  test('keeps a long View Log viewport stable while the transcript grows', async ({
    ...
    await kinguPage.setViewportSize({ width: 520, height: 720 })
    await openSessionHistory(kinguPage)
    const title = kinguPage.getByText(fixture.title, { exact: true }).first()
    await expect(title).toBeVisible({ timeout: 30_000 })
    await title.click()
    await kinguPage.getByText('View Log', { exact: true }).click()
    await expect
      .poll(() => readProbeOrNull(kinguPage), { timeout: 120_000 })
      .toMatchObject({ filePath: fixture.filePath, valueLength: fixture.initialLength })
    await kinguPage.setViewportSize({ width: 900, height: 720 })
    ...
    let baseline = await restoreAnchorState(kinguPage, true)
    ...
    for (let batch = 0; batch < 3; batch++) {
      ...
      replacementMemory.push(await measureLegacyControl(electronApp, kinguPage, baseline, batch))
      baseline = await restoreAnchorState(kinguPage, baseline.find.open, equivalentState.scrollTop)
      ...
      await forceGcAndSettle(kinguPage)
      const before = await readMemory(electronApp, kinguPage)
      appendFileSync(fixture.filePath, suffix)
      fixture.initialLength += suffix.length
      await kinguPage.waitForTimeout(APPEND_CADENCE_MS)
      await expect
        .poll(() => readProbe(kinguPage), { timeout: 30_000 })
        .toMatchObject({ valueLength: fixture.initialLength })
      const after = await readMemory(electronApp, kinguPage)
      await forceGcAndSettle(kinguPage)
      const settled = await readMemory(electronApp, kinguPage)
      suffixMemory.push({ before, after, settled })
      const current = await readProbe(kinguPage)
      ...
      expect(await kinguPage.evaluate(() => 2 + 2)).toBe(4)
      expect((await readMainProcessCrashProbe(electronApp)).processGone).toBeNull()
      baseline = current
      if (batch === 0) {
        await kinguPage.keyboard.press('Escape')
        await expect(kinguPage.locator('.monaco-editor .find-widget')).toHaveAttribute(
          'aria-hidden',
          'true'
        )
        baseline = await readProbe(kinguPage)
