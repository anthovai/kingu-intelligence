...
test.describe('Terminal output scheduler', () => {
  test('background tab output bursts use the shared drain while the active tab renders', async ({
    ...
    const ptyIdsByTabId: Record<string, string> = {
      [firstTabId]: await waitForTabPtyId(kinguPage, firstTabId)
    }

    while (tabIds.length < TAB_COUNT) {
      const tabId = await createTerminalTab(kinguPage)
      await waitForActiveTerminalManager(kinguPage, 30_000)
      tabIds.push(tabId)
      ptyIdsByTabId[tabId] = await waitForTabPtyId(kinguPage, tabId)
    }

    await tabLocator(kinguPage, firstTabId).click()
    await expect
    ...
    await resetSchedulerDebug(kinguPage)
    ...
    await sendPtyCommands(kinguPage, [
      {
    ...
    await expect
      .poll(async () => (await getTerminalContent(kinguPage)).includes(foregroundMarker), {
        timeout: 5_000,
    ...
    await tabLocator(kinguPage, firstBackgroundTabId).click()
    await expect
      .poll(() => getDomActiveTabId(kinguPage), {
        timeout: 5_000,
    ...
    await expect
      .poll(async () => (await getTerminalContent(kinguPage)).includes(firstBackground.marker), {
        timeout: 5_000,
  ...
  test('visible bulk output uses the high-priority drain instead of synchronous xterm writes', async ({
    ...
    const activeTabId = await createTerminalTab(kinguPage)
    if (!activeTabId) {
      throw new Error('Expected a fresh terminal tab')
    }
    const ptyId = await waitForTabPtyId(kinguPage, activeTabId)
    await resetSchedulerDebug(kinguPage)
    ...
    await sendPtyCommands(kinguPage, [{ ptyId, command: floodCommand }])

    await expect
      .poll(async () => (await getTerminalContent(kinguPage, 12_000)).includes(marker), {
        timeout: 30_000,
  ...
  test('hidden overflow restores from main-owned terminal state when the tab becomes visible', async ({
    ...
    const foregroundTabId = await getActiveTabId(kinguPage)
    if (!foregroundTabId) {
      throw new Error('Expected an initial terminal tab')
    }
    const hiddenTabId = await createTerminalTab(kinguPage)
    await waitForActiveTerminalManager(kinguPage, 30_000)
    const hiddenPtyId = await waitForTabPtyId(kinguPage, hiddenTabId)

    await tabLocator(kinguPage, foregroundTabId).click()
    await expect
      .poll(() => getDomActiveTabId(kinguPage), {
        timeout: 5_000,
    ...
    await sendPtyCommands(kinguPage, [{ ptyId: hiddenPtyId, command: floodCommand }])

    await expect
      .poll(async () => mainSnapshotContains(kinguPage, hiddenPtyId, marker), {
        timeout: 30_000,
        message: 'Main-owned terminal snapshot did not capture the hidden flood marker'
      })
      .toBe(true)

    await tabLocator(kinguPage, hiddenTabId).click()
    await expect
      .poll(() => getDomActiveTabId(kinguPage), {
        timeout: 5_000,
    ...
    expect(await getTerminalContent(kinguPage)).not.toContain(
      'Kingu skipped hidden terminal output'
    )
  })
})
