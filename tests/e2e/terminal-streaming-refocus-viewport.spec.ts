...
test.describe('terminal streaming refocus viewport', () => {
  test('keeps follow-output at the bottom through a queued-write refocus wobble', async ({
    ...
    await execInTerminal(kinguPage, ptyId, nodeTerminalCommand([STREAMING_FIXTURE_PATH]))
    await waitForPhaseOneAtBottom(kinguPage, tabId)

    const framesPromise = sampleRevealFrames(kinguPage, tabId)
    await injectQueuedWriteAndRefocus(kinguPage, tabId, paneKey)
    ...
    await expect
      .poll(() => getTerminalContent(kinguPage), { timeout: 15_000 })
      .toContain('REFOCUS_STREAM_DONE')
    const visibleScrollbar = kinguPage.locator('.xterm-scrollbar.xterm-vertical:visible').first()
