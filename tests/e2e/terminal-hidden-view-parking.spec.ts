...
test.describe('Terminal hidden view parking', () => {
  test('parks a hidden terminal tab and restores rich TUI output on reveal', async ({
    ...
    const setup = await setUpParkableTabA(kinguPage)
    const { worktreeId, tabAId, tabAPtyId } = setup

    const runId = randomUUID()
    const finalMarker = `PARKED_RESTORE_FINAL_${runId}_${PARKED_FRAME_COUNT - 1}`
    const scriptPath = path.join(testRepoPath, `.kingu-parked-rich-tui-${runId}.mjs`)
    writeParkedFrameScript(scriptPath, runId)
    try {
      await sendToTerminal(kinguPage, tabAPtyId, `node ${JSON.stringify(scriptPath)}\r`)
      await expect
        .poll(() => getTerminalContent(kinguPage, 12_000), {
          timeout: 15_000,
          message: 'rich TUI final frame did not render while tab A was visible'
        })
        .toContain(finalMarker)

      const tabBId = await createActiveTerminalTab(kinguPage, worktreeId)
      const parkDetectedAfterMs = await parkHiddenTabBehindDecoy(kinguPage, worktreeId, tabAId, {
        parkDelayMs: PARKING_DELAY_MS
      })
      const wiring = await readParkingWiring(kinguPage)
      ...
      const tabBState = await readTerminalTabViewState(kinguPage, tabBId)
      expect(tabBState.hasManager).toBe(true)
      expect(tabBState.paneCount).toBeGreaterThan(0)

      await activateTerminalTab(kinguPage, tabAId)
      await waitForActiveTerminalManager(kinguPage, 30_000)
      const revealedSnapshot = await waitForPaneIdentitySnapshot(kinguPage, 1)
      ...
      const content = await getTerminalContent(kinguPage, 12_000)
      expect(content).toContain(`Frame ${String(PARKED_FRAME_COUNT - 1).padStart(3, '0')}`)
      expect(content).toContain('╭')
      expect(content).toContain('├')
      expect(content).toContain('█')
      expect(content).not.toContain('Kingu skipped hidden terminal output')
      ...
      await sendToTerminal(kinguPage, tabAPtyId, '\x03')
      ...
      await waitForPtyShellEcho(kinguPage, tabAPtyId, 15_000).catch((error: unknown) => {
        throw new Error(
      ...
      await runNodeScriptInTerminal(kinguPage, tabAPtyId, typedProbeScript, {
        prefix: 'kingu-parked-typed-probe'
      })
      await expect
        .poll(() => getTerminalContent(kinguPage, 12_000), {
          timeout: 10_000,
      ...
      await kinguPage.screenshot({ path: screenshotPath, fullPage: true })
  ...
  test('keeps bell and title side effects live while parked', async ({ kinguPage }) => {
    await waitForSessionReady(kinguPage)
    const setup = await setUpParkableTabA(kinguPage)
    const { worktreeId, tabAId, tabAPtyId } = setup

    await createActiveTerminalTab(kinguPage, worktreeId)
    await parkHiddenTabBehindDecoy(kinguPage, worktreeId, tabAId, {
      parkDelayMs: PARKING_DELAY_MS
    })
    ...
    // Why: delivered via a temp file — `node -e` quoting is not PowerShell-safe (#8521).
    await runNodeScriptInTerminal(kinguPage, tabAPtyId, sideEffectScript, {
      prefix: 'kingu-parked-side-effect'
    })

    await expect
      .poll(() => getTerminalTabTitle(kinguPage, worktreeId, tabAId), {
        timeout: 10_000,
        message: 'parked OSC 0 title did not update the tab title in the store'
      })
      .toBe(parkedTitle)
    await expect
      .poll(async () => (await getUnreadTerminalTabIds(kinguPage)).includes(tabAId), {
        timeout: 10_000,
        message: 'parked BEL did not mark the terminal tab unread'
      })
      .toBe(true)
    await expect
      .poll(() => isWorktreeUnread(kinguPage, worktreeId), {
        timeout: 10_000,
    ...
    expect((await readTerminalTabViewState(kinguPage, tabAId)).hasManager).toBe(false)
  ...
  test('does not park excluded tabs', async ({ kinguPage }) => {
    await waitForSessionReady(kinguPage)
    const setup = await setUpParkableTabA(kinguPage)
    ...
    const tabCId = await createActiveTerminalTab(kinguPage, worktreeId)
    ...
    expect(await hasPendingStartupCommand(kinguPage, tabCId)).toBe(true)

    // Tab B on top hides both A and C.
    const tabBId = await createActiveTerminalTab(kinguPage, worktreeId)
    await expect
      .poll(() => getActiveTabId(kinguPage), {
        timeout: 5_000,
    ...
    expect(await hasPendingStartupCommand(kinguPage, tabCId)).toBe(true)
    const tabCState = await readTerminalTabViewState(kinguPage, tabCId)
  ...
  test('reproduces a static frame byte-for-byte across 25 park/reveal cycles', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.kingu-cycle-reference-${runId}.mjs`)
    ...
    try {
      ...
      const tabBId = await createActiveTerminalTab(kinguPage, worktreeId)
      const decoyTabId = await createActiveTerminalTab(kinguPage, worktreeId)
      ...
      const runOneParkRevealCycle = async (cycle: number): Promise<string[]> => {
        await activateTerminalTab(kinguPage, tabBId)
        // Hide tab B behind the decoy so B (not A) holds the #8262 exemption.
        await activateTerminalTab(kinguPage, decoyTabId)
        await waitForTabParked(kinguPage, tabAId, { parkDelayMs: PARKING_DELAY_MS })
        await activateTerminalTab(kinguPage, tabAId)
        await waitForActiveTerminalManager(kinguPage, 30_000)
        const revealed = await waitForPaneIdentitySnapshot(kinguPage, 1)
        ...
        await expect
          ...
          .toContain(marker)
        const rows = terminalContentRows(await getTerminalContent(kinguPage, 12_000))
        // Garble sentinel: the hidden-skip banner must never appear.
        expect(rows.join('\n')).not.toContain('Kingu skipped hidden terminal output')
        return rows
      }
      ...
      for (let cycle = 1; cycle < CYCLES; cycle++) {
        ...
        await kinguPage.waitForTimeout(PARK_VERDICT_BURST_SETTLE_MS)
