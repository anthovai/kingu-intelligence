 ...
 * split pane carries into its PTY binding, KINGU_PANE_KEY, and context menu.
 *
 * User Prompt:
...
test.describe('Terminal Panes', () => {
   ...
   * - terminal panes can be split
   */
  test('can split terminal pane right', async ({ kinguPage }) => {
    const paneCountBefore = await countVisibleTerminalPanes(kinguPage)

    await splitActiveTerminalPane(kinguPage, 'vertical')
    await waitForPaneCount(kinguPage, paneCountBefore + 1)

    const paneCountAfter = await countVisibleTerminalPanes(kinguPage)
   ...
   * - terminal panes can be split
   */
  test('can split terminal pane down', async ({ kinguPage }) => {
    const paneCountBefore = await countVisibleTerminalPanes(kinguPage)
  ...
  test('split panes persist PTY bindings by stable UUID leaf id', async ({ kinguPage }) => {
    const paneCountBefore = await countVisibleTerminalPanes(kinguPage)

    await splitActiveTerminalPane(kinguPage, 'vertical')
    await waitForPaneCount(kinguPage, paneCountBefore + 1)

    const snapshot = await waitForPaneIdentitySnapshot(kinguPage, paneCountBefore + 1)
  ...
  test('terminal process receives KINGU_PANE_KEY with the active UUID leaf id', async ({
    kinguPage
  }) => {
    ...
    const marker = `KINGU_PANE_KEY_E2E_${Date.now()}`

    await execInTerminal(kinguPage, ptyId, `printf '${marker}=%s\\n' "$KINGU_PANE_KEY"`)
    await waitForTerminalOutput(kinguPage, `${marker}=${expectedPaneKey}`)

    expect(activeLeafId).toMatch(UUID_RE)
  })

  test('terminal context menu copies the stable pane ID', async ({ kinguPage }) => {
    const snapshot = await waitForPaneIdentitySnapshot(kinguPage, 1)
    ...
    await kinguPage.getByText('Copy Pane ID', { exact: true }).click()

    await expect
      .poll(() => kinguPage.evaluate(() => window.api.ui.readClipboardText()), { timeout: 3_000 })
      .toBe(expectedPaneKey)
    await expect(kinguPage.getByText('Pane ID copied', { exact: true })).toBeVisible()
