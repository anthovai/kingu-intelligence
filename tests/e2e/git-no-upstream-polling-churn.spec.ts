...
test.use({
  kinguAppExtraEnv: {
    ...
    JENKINS_URL: '',
    TEAMCITY_VERSION: '',
    KINGU_DIAGNOSTICS_DISABLED: '',
    DO_NOT_TRACK: '1',
    KINGU_TELEMETRY_DISABLED: '1'
  }
})
...
test.describe('Git no-upstream polling churn repro', () => {
  test('active worktree polling does not repeatedly retry stable no-upstream probes', async ({
    ...
    await selectRepoForActivePolling(kinguPage, testRepoPath, repoPath)

    const diagnostics = await readDiagnosticsStatus(kinguPage)
    expect(diagnostics.localFileEnabled).toBe(true)
    expect(diagnostics.bundleEnabled).toBe(false)

    clearTraceFile(diagnostics)
    const measurement = await measureRendererDuringPolling(kinguPage)
    await flushTraceFile(kinguPage, diagnostics)
