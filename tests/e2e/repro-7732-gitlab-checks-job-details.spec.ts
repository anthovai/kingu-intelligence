...
const FIXTURE = {
  ...
  webUrl: 'https://gitlab.example.test/acme/kingu/-/jobs/987654',
  mrUrl: 'https://gitlab.example.test/acme/kingu/-/merge_requests/4242',
...
const SCREENSHOT_DIR =
  process.env.KINGU_GITLAB_CHECKS_JOB_DETAILS_SCREENSHOT_DIR ??
...
test.describe('#7732 GitLab Checks panel job details', () => {
  test('expanding a failed pipeline job shows its log, not "No inline details"', async ({
    ...
    await kinguPage.waitForTimeout(8_000)

    const jobRow = kinguPage.getByText(`${FIXTURE.stage}: ${FIXTURE.jobName}`, { exact: true })
    for (let attempt = 0; attempt < 40 && (await jobRow.count()) === 0; attempt++) {
      await linkGitLabMRToWorktree(kinguPage, worktreeId)
    ...
    await jobRow.click()
    const noDetails = kinguPage.getByText('No inline details are available for this check.')
    const viewFullLogs = kinguPage.getByRole('button', { name: 'View full logs' })
