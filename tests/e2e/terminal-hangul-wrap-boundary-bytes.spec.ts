  ...
  for (const scenario of SCENARIOS) {
    for (const policy of scenario.policies) {
      test(`sends ${scenario.name} unaltered to the pty (${policy})`, async ({
        ...
        await expectImePlatformPolicy(kinguPage, policy)
        await narrowPaneBySplitting(kinguPage, 2)
        ...
        try {
          ...
          const observedCols = await waitForSettledCols(kinguPage, arena.ptyId)
          ...
          const startRow = (await readPaneGrid(kinguPage, arena.ptyId)).cursorRow
          if (scenario.offsetByOneCell) {
            await dispatchImeRewrittenPrintableKey(arena.session, OFFSET_KEY)
          }
          await scenario.drive(arena.session, kinguPage, run)
          ...
          await expect
            .poll(async () => (await readPaneGrid(kinguPage, arena.ptyId)).cursorRow, {
              timeout: 10_000,
          ...
          const received = await waitForTerminalImeBytes(kinguPage, reader, 20_000)
