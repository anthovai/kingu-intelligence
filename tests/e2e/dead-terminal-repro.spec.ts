...
test.describe('Dead Terminal Reproduction @headful', () => {
  ...
  test('@headful setup-split flow does not produce dead terminals', async ({ kinguPage }) => {
    test.setTimeout(120_000)
    const homeWorktreeId = await waitForActiveWorktree(kinguPage)
    await waitForActiveTerminalManager(kinguPage, 30_000)
    await checkWebglState(kinguPage, 'home-initial')

    for (let i = 0; i < STRESS_ITERATIONS; i++) {
      const direction = i % 2 === 0 ? 'vertical' : 'horizontal'
      const newId = await createAndActivateWorktreeWithSetup(kinguPage, `setup-${i}`, direction)
      ...
      await checkWebglState(kinguPage, `setup-${i}`)
      await waitForAllPanesToHaveContent(kinguPage, `setup-${i} both panes`)
  ...
  test('@headful setup-split then switch-back does not leave panes dead', async ({ kinguPage }) => {
    test.setTimeout(120_000)
    const homeWorktreeId = await waitForActiveWorktree(kinguPage)
    await waitForActiveTerminalManager(kinguPage, 30_000)
    ...
    for (let i = 0; i < STRESS_ITERATIONS; i++) {
      ...
      await waitForActiveTerminalManager(kinguPage, 30_000)
      await waitForPaneCount(kinguPage, 2, 15_000)
      await waitForAllPanesToHaveContent(kinguPage, `switchback-${i} initial`)
      ...
      await expect
        ...
        .toBe(homeWorktreeId)
      await ensureTerminalVisible(kinguPage)
      await waitForActiveTerminalManager(kinguPage, 15_000)

      await switchToWorktree(kinguPage, newId)
      await expect.poll(async () => getActiveWorktreeId(kinguPage), { timeout: 10_000 }).toBe(newId)
      await ensureTerminalVisible(kinguPage)
      await waitForActiveTerminalManager(kinguPage, 15_000)
      await waitForAllPanesToHaveContent(kinguPage, `switchback-${i} after return`)
  ...
  test('@headful rapid switching between many setup-split worktrees', async ({ kinguPage }) => {
    test.setTimeout(120_000)
    const homeWorktreeId = await waitForActiveWorktree(kinguPage)
    await waitForActiveTerminalManager(kinguPage, 30_000)
    ...
    for (let i = 0; i < 4; i++) {
      const newId = await createAndActivateWorktreeWithSetup(kinguPage, `multi-${i}`, 'vertical')
      createdWorktreeIds.push(newId)
      ...
      await waitForActiveTerminalManager(kinguPage, 30_000)
      await waitForPaneCount(kinguPage, 2, 15_000)
      await waitForAllPanesToHaveContent(kinguPage, `multi-create-${i}`)
    }

    for (let round = 0; round < 3; round++) {
      for (const wId of worktreeIds) {
        await switchToWorktree(kinguPage, wId)
        await expect.poll(async () => getActiveWorktreeId(kinguPage), { timeout: 10_000 }).toBe(wId)
        await ensureTerminalVisible(kinguPage)
        await waitForActiveTerminalManager(kinguPage, 15_000)
        await waitForAllPanesToHaveContent(kinguPage, `multi-r${round}-${wId.slice(0, 8)}`)
      }
    }
  })
})
