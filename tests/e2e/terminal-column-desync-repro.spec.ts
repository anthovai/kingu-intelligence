...
test.describe('Terminal column desync repro', () => {
  test('PTY columns stay in sync with xterm across a visible resize', async ({ kinguPage }) => {
    test.setTimeout(120_000)
    await waitForSessionReady(kinguPage)
    await waitForActiveWorktree(kinguPage)
    await closeRightSidebarAndFeatureTips(kinguPage)
    await ensureTerminalVisible(kinguPage)
    const ptyId = await settleTerminal(kinguPage)
    ...
    const expectColumnsInSync = async (label: string): Promise<void> => {
      await expect
        .poll(
          async () => {
            const snap = await readColumnSnapshot(kinguPage, ptyId)
    ...
    await kinguPage.setViewportSize({ width: 760, height: 800 })
    await expectColumnsInSync('after shrink')

    await kinguPage.setViewportSize({ width: 1280, height: 800 })
  ...
  test('pty:getSize reports the size the PTY actually applied', async ({ kinguPage }) => {
    test.setTimeout(120_000)
    await waitForSessionReady(kinguPage)
    await waitForActiveWorktree(kinguPage)
    await closeRightSidebarAndFeatureTips(kinguPage)
    ...
    await kinguPage.setViewportSize({ width: 900, height: 800 })
    ...
    await expect
      .poll(
        async () => {
          const ptyCols = await readPtyCols(kinguPage, ptyId)
          const reportedCols = await readReportedPtyCols(kinguPage, ptyId)
  ...
  test('PTY columns re-sync after the terminal is resized while hidden', async ({ kinguPage }) => {
    test.setTimeout(120_000)
    await waitForSessionReady(kinguPage)
    const homeWorktreeId = await waitForActiveWorktree(kinguPage)
    const otherWorktreeId = (await getAllWorktreeIds(kinguPage)).find((id) => id !== homeWorktreeId)
    ...
    const ptyId = await settleTerminal(kinguPage)
    await kinguPage.setViewportSize({ width: 1280, height: 800 })
    await kinguPage.waitForTimeout(400)

    const baseline = await readColumnSnapshot(kinguPage, ptyId)
    ...
    await switchToWorktree(kinguPage, otherWorktreeId)
    await waitForActiveTerminalManager(kinguPage, 30_000)
    await kinguPage.setViewportSize({ width: 720, height: 800 })
    ...
    const afterReturn = await readColumnSnapshot(kinguPage, ptyId)
    expect(
      afterReturn.ptyCols,
  ...
  test('PTY columns re-sync after repeated background resizes', async ({ kinguPage }) => {
    ...
    for (const [index, width] of widths.entries()) {
      await switchToWorktree(kinguPage, otherWorktreeId)
      await waitForActiveTerminalManager(kinguPage, 30_000)
      await kinguPage.setViewportSize({ width, height: 800 })
      await kinguPage.waitForTimeout(350)
      await switchToWorktree(kinguPage, homeWorktreeId)
      await ensureTerminalVisible(kinguPage)
      await waitForActiveTerminalManager(kinguPage, 30_000)
      await kinguPage.waitForTimeout(500)

      const snapshot = await readColumnSnapshot(kinguPage, ptyId)
      expect(
        snapshot.ptyCols,
  ...
  test('both panes keep PTY columns synced after a vertical split reparent', async ({
    ...
    const firstPtyId = await settleTerminal(kinguPage)

    const baseline = await readColumnSnapshot(kinguPage, firstPtyId)
    expect(baseline.ptyCols).toBe(baseline.xtermCols)
    ...
    for (const pane of snapshot.panes) {
      ...
      const ptyCols = await readPtyCols(kinguPage, ptyId)
      const xtermCols = await readRenderedColsForPty(kinguPage, ptyId)
      expect(
        ptyCols,
  ...
  test('both panes stay PTY-synced when a tab MOUNTS with a split layout present', async ({
    ...
    await kinguPage.waitForTimeout(300)
    await settleTerminal(kinguPage)
    ...
    for (let attempt = 0; attempt < MOUNT_ATTEMPTS; attempt += 1) {
      ...
      await kinguPage.waitForFunction(() => Boolean(window.__store), null, { timeout: 30_000 })
      await waitForSessionReady(kinguPage)
      await waitForActiveWorktree(kinguPage)
      await closeRightSidebarAndFeatureTips(kinguPage)
      await ensureTerminalVisible(kinguPage)
      ...
      await kinguPage.setViewportSize({ width: 1180, height: 800 })
      await kinguPage.waitForTimeout(300)
