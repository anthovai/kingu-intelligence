...
test.describe('Terminal Panes', () => {
  ...
  test('Set Title strip activates its pane and accepts file-path drops', async ({ kinguPage }) => {
    ...
    const initialSnapshot = await waitForPaneIdentitySnapshot(kinguPage, 1)
    ...
    const splitSnapshot = await waitForPaneIdentitySnapshot(kinguPage, 2)
    ...
    await expect
      .poll(async () => (await readPaneIdentitySnapshot(kinguPage))?.activeLeafId ?? null)
      .toBe(otherPane.leafId)

    const titleBar = kinguPage.locator('.pane-title-bar', { hasText: title }).first()
    ...
    await titleBar.evaluate((element, path) => {
      ...
      dataTransfer.setData('text/x-kingu-file-path', path)
    ...
    await expect
      ...
      .toBe(titledLeafId)
    await expect
      .poll(async () => (await getTerminalContent(kinguPage)).includes(droppedPath), {
        timeout: 5_000,
  ...
  test('Set Title overlay follows its pane after same-count pane move', async ({ kinguPage }) => {
    ...
    await splitActiveTerminalPane(kinguPage, 'vertical')
    await waitForPaneCount(kinguPage, 2)
    const beforeMove = await waitForPaneIdentitySnapshot(kinguPage, 2)
    ...
    const beforeOrder = await readTerminalPaneDomLeafOrder(kinguPage)

    await expectPaneTitleAttachedToLeaf(kinguPage, title, titledLeafId)
    await moveTerminalPaneByLeafId(kinguPage, titledLeafId, target.leafId, 'right')

    await expect
      .poll(async () => readTerminalPaneDomLeafOrder(kinguPage), {
        timeout: 10_000,
  ...
  test('Set Title keeps the pane drag handle available over the title strip', async ({
    ...
    const titleTopHit = await kinguPage.evaluate(
      ({ title, titledLeafId }) => {
    ...
    await kinguPage.locator('.pane-title-bar', { hasText: title }).click({
      position: { x: 20, y: 18 }
    })
    await expect(kinguPage.locator('.pane-title-input')).toBeVisible()
  })

  test('@headful Set Title pane can be dragged from the title strip', async ({ kinguPage }) => {
    ...
    await splitActiveTerminalPane(kinguPage, 'vertical')
    await waitForPaneCount(kinguPage, 2)
    const beforeDrag = await waitForPaneIdentitySnapshot(kinguPage, 2)
    ...
    const titleDragHandle = kinguPage
    ...
    const targetBox = await kinguPage
    ...
    await kinguPage.mouse.move(sourceBox!.x + sourceBox!.width / 2, sourceBox!.y + 4)
    await kinguPage.mouse.down()
    await kinguPage.mouse.move(targetDropX, targetBox!.y + targetBox!.height / 2, {
      steps: 20
    })
    ...
    await expect
      ...
      .not.toEqual(beforeOrder)
    const afterDrag = await waitForPaneIdentitySnapshot(kinguPage, 2)
