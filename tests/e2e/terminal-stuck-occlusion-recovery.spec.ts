...
test.describe('terminal stuck-occlusion recovery', () => {
  ...
  test('a keystroke unlatches the hidden-delivery gate wedged by stale visibilityState', async ({
    ...
    await execInTerminal(kinguPage, ptyId, 'echo live-before-$((41+1))')
    await expect
      .poll(async () => getTerminalContent(kinguPage), { timeout: 15_000 })
    ...
    await expect
      .poll(async () => (await getDeliverySnapshot(kinguPage)).hiddenDeliveryGatedPtyCount, {
        timeout: 15_000
      })
      .toBeGreaterThan(0)
    expect(
      (await getDeliverySnapshot(kinguPage)).hiddenDeliveryGatedVisiblePtyCount
    ).toBeGreaterThan(0)

    // The freeze repro: output produced now is dropped by main, not painted.
    const droppedBefore = (await getDeliverySnapshot(kinguPage)).hiddenDeliveryDroppedChars
    await execInTerminal(kinguPage, ptyId, 'echo occluded-$((70+8))')
    await expect
      .poll(async () => (await getDeliverySnapshot(kinguPage)).hiddenDeliveryDroppedChars, {
        timeout: 15_000
      })
      .toBeGreaterThan(droppedBefore)
    expect(await getTerminalContent(kinguPage)).not.toContain('occluded-78')
    ...
    await kinguPage.keyboard.press('Shift')

    // Gate unlatches and the missed output repaints from the main-owned
    // snapshot — no reload, visibilityState still reads 'hidden'.
    await expect
      .poll(async () => getTerminalContent(kinguPage), { timeout: 30_000 })
      .toContain('occluded-78')
    await expect
      .poll(async () => (await getDeliverySnapshot(kinguPage)).hiddenDeliveryGatedVisiblePtyCount, {
        timeout: 15_000
      })
      .toBe(0)

    // Live delivery continues under the override.
    await execInTerminal(kinguPage, ptyId, 'echo live-after-$((200+56))')
    await expect
      .poll(async () => getTerminalContent(kinguPage), { timeout: 15_000 })
    ...
    const report = await kinguPage.evaluate(() =>
      (
        window as Window & {
          __kinguTerminalFreezeReport?: () => Promise<{
