...
test('keeps long repository names inside the cross-project confirmation dialog', async ({
  ...
  const tempRoot = mkdtempSync(path.join(os.tmpdir(), 'kingu-e2e-cross-project-dialog-'))
  const secondRepositoryPath = path.join(tempRoot, LONG_REPOSITORY_NAME)
  createGitRepository(secondRepositoryPath)

  try {
    const secondRepositoryId = await kinguPage.evaluate(async (repositoryPath) => {
    ...
    const currentRepositoryId = await kinguPage.evaluate(() => {
      const store = window.__store
    ...
    await kinguPage.setViewportSize({ width: 640, height: 720 })
    ...
    const confirmation = kinguPage.getByRole('dialog', { name: 'Switch project?' })
    await expect(confirmation).toBeVisible()
    await expect(confirmation).toContainText(LONG_REPOSITORY_NAME)

    if (process.env.KINGU_VISUAL_PROOF === '1') {
