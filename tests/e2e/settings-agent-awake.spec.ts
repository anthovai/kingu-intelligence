...
async function installPowerSaveBlockerProbe(electronApp: ElectronApplication): Promise<void> {
  await electronApp.evaluate(({ powerSaveBlocker }) => {
    ...
    if (root.__kinguAwakePowerProbe) {
      root.__kinguAwakePowerProbe.starts = []
      root.__kinguAwakePowerProbe.stops = []
      return
    }
    ...
    powerSaveBlocker.start = ((type) => {
      ...
      root.__kinguAwakePowerProbe?.starts.push({ type, id })
      return id
    }) as typeof powerSaveBlocker.start

    powerSaveBlocker.stop = ((id) => {
      root.__kinguAwakePowerProbe?.stops.push({ id })
      originalStop(id)
...
test.describe('Agent awake setting', () => {
  ...
  test('can be changed from Agents settings and persists through IPC', async ({ kinguPage }) => {
    await openSettings(kinguPage)
    await dismissTransientAnnouncement(kinguPage)
    await kinguPage.getByPlaceholder('Search settings').fill('awake')

    await expect(kinguPage.getByText('Keep computer awake').first()).toBeVisible()

    const keepAwakeModes = kinguPage.getByRole('radiogroup', {
    ...
    await expect
      .poll(async () => (await getSettings(kinguPage)).computerAwakeMode, {
        timeout: 5_000,
  ...
  test('keeps the OS awake only while a hook-reported agent is working', async ({
    ...
    await setKeepAwake(kinguPage, true)
    ...
    await expect(
      kinguPage.getByRole('button', { name: 'Keep computer awake, Agent · Active' })
    ).toBeVisible()
    ...
    await expect(
      kinguPage.getByRole('button', { name: 'Keep computer awake, Agent · Inactive' })
    ).toBeVisible()
    if (process.platform === 'darwin') {
      await expect
        .poll(() => readMacosSleepAssertionPids(electronApp), { timeout: 5_000 })
