...
const LINEAR_ISSUE_VIEW_STORAGE_KEY = 'kingu.linear.issue-view.v1'
...
test.describe('Linear issue view persistence', () => {
  test('preserves view mode, grouping, ordering, and filters across a tasks remount', async ({
    ...
    await waitForLinearIssuesChrome(kinguPage, ISSUE_A.title)

    await setLinearViewPreferences(kinguPage, {
      viewMode: 'Board',
      groupBy: 'Status',
      orderBy: 'Updated'
    })
    await applyStatusFilter(kinguPage, STATE_A.name)

    await waitForLinearIssueViewPersisted(kinguPage, (view) => {
      if (
        !view ||
        view.viewMode !== 'board' ||
        view.groupBy !== 'status' ||
    ...
    await expectRestoredLinearView(kinguPage)
    ...
    await closeTasksPage(kinguPage)
    await openLinearTasks(kinguPage)
    await waitForLinearIssuesChrome(kinguPage, ISSUE_A.title)

    await expectRestoredLinearView(kinguPage)
    await expect(
      kinguPage.getByRole('button', { name: 'Remove Status filter' }).locator('..')
    ).toContainText(STATE_A.name)
    // Board surface, not the flat list column header.
    await expect(kinguPage.getByText(STATE_A.name, { exact: true }).first()).toBeVisible()
  })
  ...
  test('keeps attribute filters scoped per Linear workspace', async ({
    ...
    await applyPriorityFilter(kinguPage, 'High')
    await expect(
      kinguPage.getByRole('button', { name: 'Remove Priority filter' }).locator('..')
    ).toContainText('High')
    ...
    await switchLinearWorkspace(kinguPage, WORKSPACE_B.organizationName)
    await waitForLinearIssuesChrome(kinguPage, ISSUE_B.title)
    // Workspace B starts unfiltered — Alpha's High must not leak.
    await expect(kinguPage.getByRole('button', { name: 'Remove Priority filter' })).toHaveCount(0)

    await applyPriorityFilter(kinguPage, 'Low')
    await expect(
      kinguPage.getByRole('button', { name: 'Remove Priority filter' }).locator('..')
    ).toContainText('Low')
    ...
    await switchLinearWorkspace(kinguPage, WORKSPACE_A.organizationName)
    await waitForLinearIssuesChrome(kinguPage, ISSUE_A.title)
    await expect(
      kinguPage.getByRole('button', { name: 'Remove Priority filter' }).locator('..')
    ).toContainText('High')
