...
for (const theme of ['dark', 'light'] as const) {
  test(`tab drag stays cancelled after blur and a later drag still splits (${theme})`, async ({
    ...
    const tabs = kinguPage.locator('[data-testid="sortable-tab"]:visible')
    const panels = kinguPage.locator('[data-tab-group-body-id]:visible')
    const preview = kinguPage.getByText('New split', { exact: true })
    ...
    const startDrag = async (): Promise<void> => {
      ...
      await kinguPage.mouse.move(tab.x + tab.width / 2, tab.y + tab.height / 2)
      await kinguPage.mouse.down()
      await kinguPage.mouse.move(target.x, target.y, { steps: 12 })
    ...
    await kinguPage.mouse.move(target.x - 10, target.y + 10, { steps: 3 })
