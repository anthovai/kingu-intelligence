...
const fakeGhDir = mkdtempSync(path.join(os.tmpdir(), 'kingu-e2e-fake-gh-'))
...
test('GitHub Tasks drawer recovers when gh stalls on issue details', async ({
  ...
  const { repoId } = await kinguPage.evaluate((repoPath) => {
  ...
  const detailHeading = kinguPage.getByRole('heading', {
  ...
  await expect(kinguPage.getByText('Unable to load details for this GitHub item.')).toBeVisible({
    timeout: 5_000
  })
