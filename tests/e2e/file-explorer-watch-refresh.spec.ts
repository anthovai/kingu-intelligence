...
test('refreshes the visible tree after external Windows file changes', async ({ kinguPage }) => {
  await waitForSessionReady(kinguPage)
  await waitForActiveWorktree(kinguPage)
  await kinguPage.evaluate(() => window.__store?.getState().setRightSidebarOpen(false))
  await expect
    .poll(() => kinguPage.evaluate(() => window.__store?.getState().rightSidebarOpen))
    .toBe(false)
  await openFileExplorer(kinguPage)

  const worktreePath = await kinguPage.evaluate(() => {
  ...
  const row = (name: string) =>
    kinguPage
      ...
      .filter({ has: kinguPage.getByText(name, { exact: true }) })
  ...
  try {
    ...
    writeFileSync(originalPath, 'created outside Kingu\n')
