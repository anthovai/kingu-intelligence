...
test.describe('Droid notifications', () => {
  test('Codex hook completion dispatches while its worktree is inactive', async ({
    ...
    await sendToTerminal(kinguPage, ptyId, `printf '${readyMarker}\\n'\r`)
    await waitForTerminalOutput(kinguPage, readyMarker)

    const { paneKey, worktreeId } = await waitForActivePaneHookDescriptor(kinguPage)
    ...
    await expect
      .poll(
        async () =>
          (await getRendererOrCachedAgentStatuses(kinguPage)).some(
            (status) =>
    ...
    await switchToOtherExistingWorktree(kinguPage)
    ...
    await expect
      .poll(
        async () =>
          (await getAgentStatuses(kinguPage)).some(
            (status) =>
    ...
    await expect
      .poll(async () => isWorktreeUnread(kinguPage, worktreeId), {
        timeout: 10_000,
  ...
  test('recognized agent title completion dispatches one task-complete notification', async ({
    ...
    await emitOscTitle(kinguPage, ptyId, 'Codex working')
    await expect
      .poll(async () => (await getRendererTitleLog(kinguPage)).includes('Codex working'), {
        timeout: 10_000,
        message: 'Codex working title did not reach the renderer before completion'
      })
      .toBe(true)

    await emitOscTitle(kinguPage, ptyId, 'Codex done')
    await expect
      .poll(async () => (await getRendererTitleLog(kinguPage)).includes('Codex done'), {
        timeout: 10_000,
  ...
  test('Factory Droid needs-input native title does not dispatch a task-complete notification', async ({
    ...
    await emitOscTitle(kinguPage, ptyId, '⠋ Droid')
    await emitOscTitle(kinguPage, ptyId, 'Factory Droid needs input')

    await expect
      .poll(
        async () => (await getRendererTitleLog(kinguPage)).includes('Factory Droid needs input'),
        {
          timeout: 10_000,
