  ...
  test('survives an SSH reconnect while the replacement shell is not ready @headful', async ({
    ...
    try {
      ...
      await expect
        ...
        .toBe('ready')
      await expectStartupCommandQueuedByCompatibilityFallback(kinguPage, created)
      expect(
        execDockerSshRelayTargetControlCommand(
          target,
      ...
      await expectStartupExecRecovery(kinguPage, created, runId)
      ...
      await closeStartupExecTerminal(kinguPage, terminal)
