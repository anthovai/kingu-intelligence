...
const SAME_WORKSPACE_PANES = readPositiveInt(
  'KINGU_E2E_OPENCODE_SAME_WORKSPACE_PANES',
  DEFAULT_SAME_WORKSPACE_PANES
)
const CROSS_WORKSPACE_PANES_PER_WORKTREE = readPositiveInt(
  'KINGU_E2E_OPENCODE_CROSS_WORKSPACE_PANES',
  DEFAULT_CROSS_WORKSPACE_PANES_PER_WORKTREE
)
const PRESSURE_BACKGROUND_PANES = readPositiveInt(
  'KINGU_E2E_OPENCODE_PRESSURE_BACKGROUND_PANES',
  DEFAULT_PRESSURE_BACKGROUND_PANES
)
const PRESSURE_OUTPUT_CHARS = readPositiveInt(
  'KINGU_E2E_OPENCODE_PRESSURE_OUTPUT_CHARS',
  DEFAULT_PRESSURE_OUTPUT_CHARS
)
const HIDDEN_PRESSURE_PANES = readPositiveInt(
  'KINGU_E2E_OPENCODE_HIDDEN_PRESSURE_PANES',
  DEFAULT_HIDDEN_PRESSURE_PANES
)
const FRAME_COUNT = readPositiveInt('KINGU_E2E_OPENCODE_FRAME_COUNT', DEFAULT_FRAME_COUNT)
const FRAME_INTERVAL_MS = readPositiveInt(
  'KINGU_E2E_OPENCODE_FRAME_INTERVAL_MS',
  DEFAULT_FRAME_INTERVAL_MS
)
const SCALE_SAME_WORKSPACE_PANES = readPositiveIntList('KINGU_E2E_OPENCODE_SCALE_PANES')
const SCALE_CROSS_WORKSPACE_PANES = readPositiveIntList(
  'KINGU_E2E_OPENCODE_SCALE_CROSS_WORKSPACE_PANES'
)
const SCALE_PRESSURE_PANES = readPositiveIntList('KINGU_E2E_OPENCODE_SCALE_PRESSURE_PANES')
const SCALE_HIDDEN_PRESSURE_PANES = readPositiveIntList(
  'KINGU_E2E_OPENCODE_SCALE_HIDDEN_PRESSURE_PANES'
)
...
async function measureCrossWorkspaceTypingDuringHiddenLoad({
  ...
  const hiddenPanes = await ensureActiveWorktreePaneLoad(kinguPage, hiddenPaneCount)

  await switchToWorktree(kinguPage, firstWorktreeId)
  await expect.poll(() => getActiveWorktreeId(kinguPage), { timeout: 10_000 }).toBe(firstWorktreeId)
  ...
  const scriptPath = path.join(
    testRepoPath,
    `.kingu-opencode-cross-${hiddenPaneCount}-${runId}.mjs`
  )
  writeInteractivePromptScript(scriptPath, runId)
  await resetTerminalPtyOutputDebug(kinguPage)
  ...
  try {
    const measurement = await measureTypingDuringLoad(kinguPage, scriptPath, typingPtyId, runId)
    const debug = await readTerminalPtyOutputDebug(kinguPage)
    const scheduler = await readTerminalOutputSchedulerDebug(kinguPage)
    const mainPressure = await readMainPtyPressureDebug(kinguPage)
...
test.describe('Artificial OpenCode terminal load', () => {
  ...
  test('measures baseline typing responsiveness with one active terminal', async ({
    ...
    const runId = randomUUID()
    const scriptPath = path.join(testRepoPath, `.kingu-opencode-baseline-typing-${runId}.mjs`)
    writeInteractivePromptScript(scriptPath, runId)
    await resetTerminalPtyOutputDebug(kinguPage)
    try {
      const measurement = await measureTypingDuringLoad(kinguPage, scriptPath, typingPtyId, runId)
  ...
  test('keeps typing responsive while same-workspace panes redraw simultaneously', async ({
    ...
    await waitForSessionReady(kinguPage)
    await waitForActiveWorktree(kinguPage)
    const panes = await ensureActiveWorktreePaneLoad(kinguPage, SAME_WORKSPACE_PANES)
    const [typingPane, ...loadPanes] = panes
    await focusPane(kinguPage, typingPane.paneKey)

    const runId = randomUUID()
    const scriptPath = path.join(testRepoPath, `.kingu-opencode-typing-${runId}.mjs`)
    writeInteractivePromptScript(scriptPath, runId)
    await resetTerminalPtyOutputDebug(kinguPage)
    const load = await startSyntheticOpenCodeInjection({
      frameCount: FRAME_COUNT,
    ...
    try {
      ...
      annotateTypingMeasurement(
        ...
        panes.length,
        measurement,
        await readTerminalPtyOutputDebug(kinguPage),
        await readTerminalOutputSchedulerDebug(kinguPage),
        await readMainPtyPressureDebug(kinguPage)
      )
      expect(measurement.medianLatencyMs).toBeLessThan(MAX_MEDIAN_KEY_LATENCY_MS)
      expect(measurement.worstLatencyMs).toBeLessThan(MAX_WORST_KEY_LATENCY_UNDER_LOAD_MS)
      expect(measurement.maxTimerDriftMs).toBeLessThan(MAX_TIMER_DRIFT_UNDER_LOAD_MS)
      ...
      await load.stop()
      await sendToTerminal(kinguPage, typingPane.ptyId, '\x03').catch(() => undefined)
      rmSync(scriptPath, { force: true })
    }
  })
  ...
  for (const paneCount of SCALE_SAME_WORKSPACE_PANES) {
    test(`keeps typing responsive at ${paneCount} same-workspace OpenCode panes`, async ({
      ...
      await waitForSessionReady(kinguPage)
      await waitForActiveWorktree(kinguPage)
      const panes = await ensureActiveWorktreePaneLoad(kinguPage, paneCount)
      const [typingPane, ...loadPanes] = panes
      await focusPane(kinguPage, typingPane.paneKey)

      const runId = randomUUID()
      const scriptPath = path.join(testRepoPath, `.kingu-opencode-scale-${paneCount}-${runId}.mjs`)
      writeInteractivePromptScript(scriptPath, runId)
      await resetTerminalPtyOutputDebug(kinguPage)
      const load = await startSyntheticOpenCodeInjection({
        frameCount: FRAME_COUNT,
  ...
  for (const hiddenPressureCase of hiddenPressureCases) {
    test(hiddenPressureCase.title, async ({ kinguPage, testRepoPath }, testInfo) => {
