...
// scoped to this spec's app launches via kinguAppExtraEnv (same pattern as
...
test.use({
  ...
  kinguAppExtraArgs: ['--enable-precise-memory-info']
})

// Why: 8 hidden tabs is below the 12-tab hot-retain limit, but that limit
// never retains anything here — the KINGU_E2E_TERMINAL_PARKING_DELAY_MS
...
test.describe('Terminal parked memory', () => {
  test('releases renderer terminal memory when hidden tabs park', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.kingu-parked-memory-${runId}.mjs`)
    writeScrollbackFillScript(scriptPath, runId)
    try {
      const { worktreeId, scrollbackTabs } = await setUpScrollbackTabs(kinguPage, scriptPath, runId)
      ...
      // the other 7 park.
      const visibleTab = await createActiveTerminalTab(kinguPage, worktreeId)
      ...
      const metrics = await sampleParkedMemoryMetrics(kinguPage)
      ...
      for (const tab of parkableTabs) {
        expect((await readTerminalTabViewState(kinguPage, tab.tabId)).hasManager).toBe(false)
      }
      expect((await readTerminalTabViewState(kinguPage, lastActiveTab.tabId)).hasManager).toBe(true)
      const visibleState = await readTerminalTabViewState(kinguPage, visibleTab.tabId)
  ...
  test('retains terminal views when parking is disabled', async ({
    ...
    await expect
      .poll(
        () =>
          kinguPage.evaluate(() => window.__store?.getState().settings?.terminalHiddenViewParking),
    ...
    try {
      ...
      await kinguPage.waitForTimeout(PARKING_DELAY_MS * 3)
      expect(await countMountedPaneManagers(kinguPage, scrollbackTabIds)).toBe(SCROLLBACK_TAB_COUNT)
      ...
      for (const tab of scrollbackTabs) {
        const state = await readTerminalTabViewState(kinguPage, tab.tabId)
        expect(state.hasManager).toBe(true)
        expect(state.paneCount).toBeGreaterThan(0)
      }
      expect((await readTerminalTabViewState(kinguPage, visibleTab.tabId)).hasManager).toBe(true)
...
test.describe('Terminal hidden worktree retention budget', () => {
  test.use({
    kinguAppExtraEnv: {
      KINGU_E2E_TERMINAL_PARKING_DELAY_MS: String(PARKING_DELAY_MS),
      ...
      KINGU_E2E_TERMINAL_RETENTION_LIMIT: '1'
    },
    kinguAppExtraArgs: ['--enable-precise-memory-info']
  })
  ...
  test('releases un-parkable hidden worktree buffers only once the retention budget engages', async ({
    ...
    await waitForSessionReady(kinguPage)
    const victimWorktreeId = await waitForActiveWorktree(kinguPage)
    await skipUnlessParkingWired(kinguPage)

    const decoyWorktreeId = (await getAllWorktreeIds(kinguPage)).find(
    ...
    await updateTerminalSettings(kinguPage, {
      terminalHiddenWorktreeRetentionBudget: false,
      terminalScrollbackRows: RETENTION_SCROLLBACK_ROWS
    })
    await waitForRetentionBudgetSetting(kinguPage, false)

    const runId = randomUUID()
    const scriptPath = path.join(testRepoPath, `.kingu-retention-memory-${runId}.mjs`)
    ...
    try {
      ...
      const baselineSnapshot = await waitForPaneIdentitySnapshot(kinguPage, 1)
      const baselinePtyId = baselineSnapshot.panes[0]?.ptyId
      if (!baselinePtyId) {
      ...
      for (let tabIndex = 0; tabIndex < RETENTION_TAB_COUNT; tabIndex += 1) {
        if (tabIndex > 0) {
          victimTabs.push(await createActiveTerminalTab(kinguPage, victimWorktreeId))
        }
        ...
        await stageUnparkableWorktreeTabs(kinguPage, victimWorktreeId)
      }
      ...
      await switchToWorktree(kinguPage, decoyWorktreeId)
      await expect
        .poll(() => kinguPage.evaluate(() => window.__store?.getState().activeWorktreeId), {
          timeout: 5_000,
      ...
      const decoySnapshot = await waitForPaneIdentitySnapshot(kinguPage, 1)
      const decoyTabIds = [decoySnapshot.tabId]
      expect(await countMountedPaneManagers(kinguPage, decoyTabIds)).toBe(1)
      ...
      await stageUnparkableWorktreeTabs(kinguPage, victimWorktreeId)
      await stageUnparkableWorktreeTabs(kinguPage, decoyWorktreeId)
      await waitForUnparkableWorktreeTabs(kinguPage, victimWorktreeId)
      await waitForUnparkableWorktreeTabs(kinguPage, decoyWorktreeId)
      ...
      await expect
        .poll(
          async () => {
            await stageUnparkableWorktreeTabs(kinguPage, victimWorktreeId)
            await stageUnparkableWorktreeTabs(kinguPage, decoyWorktreeId)
            const victimMounted = await countMountedPaneManagers(kinguPage, victimTabIds)
            const decoyMounted = await countMountedPaneManagers(kinguPage, decoyTabIds)
      ...
      const before = await readRetentionMemorySample(kinguPage)
      expect(before.bufferMb).toBeGreaterThan(MIN_STAGED_BUFFER_MB)

      const flipStartedAt = Date.now()
      await updateTerminalSettings(kinguPage, { terminalHiddenWorktreeRetentionBudget: true })
      await waitForRetentionBudgetSetting(kinguPage, true)
      await expect
        .poll(() => countMountedPaneManagers(kinguPage, victimTabIds), {
          timeout: 30_000,
      ...
      const after = await readRetentionMemorySample(kinguPage)
      testInfo.annotations.push({
      ...
      expect(await countMountedPaneManagers(kinguPage, decoyTabIds)).toBe(decoyTabIds.length)
