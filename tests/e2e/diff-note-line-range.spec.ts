...
const BAND = '.kingu-diff-comment-range-highlight'
const COMPOSER = '.kingu-diff-comment-popover'
const COMPOSER_LABEL = '.kingu-diff-comment-popover-label'
const COMPOSER_TEXTAREA = '.kingu-diff-comment-popover-textarea'
...
async function revealAddButton(
  ...
  const button = page.locator('.kingu-diff-comment-add-btn')
...
test.describe('Diff note line range', () => {
  ...
  test('dragging the gutter selects a range, keeps it lit, and saves one ranged note', async ({
    ...
    await seedDiffFile(kinguPage, worktreeId, 'src/diff-note-range-drag.ts')

    const from = await gutterPoint(kinguPage, 4)
    const to = await gutterPoint(kinguPage, 9)

    await kinguPage.mouse.move(from.x, from.y)
    await kinguPage.mouse.down()
    // Anchor alone is lit before the pointer travels.
    await expect(kinguPage.locator(BAND)).toHaveCount(1)

    await kinguPage.mouse.move(to.x, (from.y + to.y) / 2)
    await kinguPage.mouse.move(to.x, to.y)
    await expect(
      kinguPage.locator(BAND),
      'the band did not follow the pointer while the button was held'
    ).toHaveCount(6)

    await kinguPage.mouse.up()

    await expect(kinguPage.locator(COMPOSER_LABEL)).toHaveText('Lines 4-9')
    await expect(
      kinguPage.locator(BAND),
    ...
    expect(await kinguPage.evaluate(() => window.getSelection()?.toString() ?? '')).toBe('')

    await submitNote(kinguPage, 'Collapse these six lines into a loop.')

    expect(await readNotes(kinguPage, worktreeId)).toEqual([
      { startLine: 4, lineNumber: 9, body: 'Collapse these six lines into a loop.' }
    ])
    const card = kinguPage.locator('.kingu-diff-comment-card').first()
    ...
    await expect(kinguPage.locator(BAND)).toHaveCount(0)
  })
  ...
  test('dragging the gutter upward commits the same range as dragging down', async ({
    ...
    const worktreeId = await waitForActiveWorktree(kinguPage)
    await seedDiffFile(kinguPage, worktreeId, 'src/diff-note-range-drag-up.ts')

    const from = await gutterPoint(kinguPage, 9)
    const to = await gutterPoint(kinguPage, 4)
    ...
    await submitNote(kinguPage, 'Dragged bottom to top.')

    expect(await readNotes(kinguPage, worktreeId)).toEqual([
      { startLine: 4, lineNumber: 9, body: 'Dragged bottom to top.' }
    ])
    await expect(kinguPage.locator('.kingu-diff-comment-card').first()).toContainText('lines 4-9')
  })
  ...
  test('dragging from the "+" itself selects a range and the button rides the selection', async ({
    ...
    const worktreeId = await waitForActiveWorktree(kinguPage)
    await seedDiffFile(kinguPage, worktreeId, 'src/diff-note-range-button-drag.ts')

    const button = await revealAddButton(kinguPage, 4)
    const to = await gutterPoint(kinguPage, 9)

    await kinguPage.mouse.move(button.x, button.y)
    await kinguPage.mouse.down()
    await kinguPage.mouse.move(button.x, (button.y + to.y) / 2)
    await kinguPage.mouse.move(button.x, to.y)
    ...
    await expect
      .poll(
        async () =>
          kinguPage.locator('.kingu-diff-comment-add-btn').evaluate((node) => {
            if (!(node instanceof HTMLElement)) {
              throw new Error('the add button is not an HTMLElement')
            }
            return node.style.top
    ...
    await kinguPage.mouse.up()
    await expect(kinguPage.locator(COMPOSER_LABEL)).toHaveText('Lines 4-9')
    await submitNote(kinguPage, 'Dragged straight off the plus button.')
  ...
  test('a press with no drag still saves the single-line note it always did', async ({
    ...
    const worktreeId = await waitForActiveWorktree(kinguPage)
    await seedDiffFile(kinguPage, worktreeId, 'src/diff-note-range-click.ts')

    const point = await gutterPoint(kinguPage, 6)
    await kinguPage.mouse.move(point.x, point.y)
    await kinguPage.mouse.down()
    await kinguPage.mouse.up()

    await expect(kinguPage.locator(COMPOSER_LABEL)).toHaveText('Line 6')
    await expect(kinguPage.locator(BAND)).toHaveCount(1)

    await submitNote(kinguPage, 'Single line still works.')
    ...
    await expect(kinguPage.locator('.kingu-diff-comment-card').first()).toContainText('line 6')
  })

  test('Escape abandons a drag without opening a composer', async ({ kinguPage }) => {
    const worktreeId = await waitForActiveWorktree(kinguPage)
    await seedDiffFile(kinguPage, worktreeId, 'src/diff-note-range-escape.ts')

    const from = await gutterPoint(kinguPage, 3)
    const to = await gutterPoint(kinguPage, 8)
    await kinguPage.mouse.move(from.x, from.y)
    await kinguPage.mouse.down()
    await kinguPage.mouse.move(to.x, to.y)
    await expect(kinguPage.locator(BAND)).toHaveCount(6)

    await kinguPage.keyboard.press('Escape')
    await expect(kinguPage.locator(BAND), 'Escape left the band behind').toHaveCount(0)

    await kinguPage.mouse.up()
    await expect(
      kinguPage.locator(COMPOSER),
  ...
  test('the Add Review Note chord turns an editor selection into a ranged note', async ({
    ...
    const worktreeId = await waitForActiveWorktree(kinguPage)
    await seedDiffFile(kinguPage, worktreeId, 'src/diff-note-range-chord.ts')

    // Click the code column, not the gutter: Monaco still owns that side.
    const line = await gutterPoint(kinguPage, 5)
    await kinguPage.mouse.click(line.x + 220, line.y)
    await kinguPage.keyboard.press('Shift+ArrowDown')
    await kinguPage.keyboard.press('Shift+ArrowDown')

    await pressShortcut(kinguPage, 'KeyA', { shift: true })

    await expect(kinguPage.locator(COMPOSER_LABEL)).toHaveText('Lines 5-7')
    await submitNote(kinguPage, 'Range from the keyboard.')
