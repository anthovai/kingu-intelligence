...
async function createProjectHeaderSortFixture(): Promise<string[]> {
  ...
  const root = realpathSync(await mkdtemp(path.join(os.tmpdir(), 'kingu-e2e-project-sort-')))
  tempRoots.push(root)
...
test.describe('Project Group manual sorting', () => {
  test('dragging a project header body reorders the visible project headers', async ({
    ...
    const projects = await seedProjectHeaderSortScenario(kinguPage, repoPaths)

    await expect
      .poll(() => getProjectHeaderOrder(kinguPage, projects), {
        timeout: 12_000,
  ...
  test('dragging a duplicate-ranked Project Group header reorders the visible headers', async ({
    ...
    await waitForSessionReady(kinguPage)
    const groups = await seedDuplicateTabOrderProjectGroups(kinguPage)

    await expect
      .poll(() => getProjectGroupHeaderOrder(kinguPage, groups), {
        timeout: 12_000,
