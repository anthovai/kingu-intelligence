...
const PARKING_DELAY_MS = Number(process.env.KINGU_E2E_TERMINAL_PARKING_DELAY_MS) || 2_000
...
test.describe('terminal pane close vs hidden/park lifecycle keeps layout consistent', () => {
  test('control: close while visible', async ({ kinguPage }) => {
    const { tabId } = await setUpSplitTab(kinguPage)
    await closeLastPaneOnTab(kinguPage, tabId)
    await expectLayoutConsistent(kinguPage, tabId, 1, 'close-visible')
  })

  test('close and hide the tab in the same tick', async ({ kinguPage }) => {
    const { worktreeId, tabId } = await setUpSplitTab(kinguPage)
    ...
    await waitForTabRemounted(kinguPage, tabId)
    await expectLayoutConsistent(kinguPage, tabId, 1, 'close-then-hide-same-tick')
  })

  test('close while hidden but still mounted (hot-retain window)', async ({ kinguPage }) => {
    const { worktreeId, tabId } = await setUpSplitTab(kinguPage)
    ...
    await activateTerminalTab(kinguPage, tabId)
    await waitForTabRemounted(kinguPage, tabId)
    await expectLayoutConsistent(kinguPage, tabId, 1, 'close-while-hidden-mounted')
  })

  test('close immediately after reveal remount, before panes settle', async ({ kinguPage }) => {
    const { worktreeId, tabId } = await setUpSplitTab(kinguPage)
    await createActiveTerminalTab(kinguPage, worktreeId)
    await parkHiddenTabBehindDecoy(kinguPage, worktreeId, tabId, {
      parkDelayMs: PARKING_DELAY_MS
    ...
    await expectLayoutConsistent(kinguPage, tabId, 1, 'close-mid-reveal')
  })

  test('clean visible close survives a later park/reveal cycle', async ({ kinguPage }) => {
    const { worktreeId, tabId } = await setUpSplitTab(kinguPage)
    await closeLastPaneOnTab(kinguPage, tabId)
    await expectLayoutConsistent(kinguPage, tabId, 1, 'pre-park close')
    await createActiveTerminalTab(kinguPage, worktreeId)
    await parkHiddenTabBehindDecoy(kinguPage, worktreeId, tabId, {
      parkDelayMs: PARKING_DELAY_MS
    })
    await activateTerminalTab(kinguPage, tabId)
    await waitForTabRemounted(kinguPage, tabId)
    await expectLayoutConsistent(kinguPage, tabId, 1, 'post-park-reveal')
  })

  test('split pane shell exits while hidden but still mounted', async ({ kinguPage }) => {
    const { worktreeId, tabId, splitPtyId } = await setUpSplitTab(kinguPage)
    await createActiveTerminalTab(kinguPage, worktreeId)
    // The setup-script analog: the split's shell ends on its own while the
    // tab is hidden-but-mounted.
    await sendToTerminal(kinguPage, splitPtyId, 'exit\r')
    await kinguPage.waitForTimeout(PARKING_DELAY_MS / 2)
    await activateTerminalTab(kinguPage, tabId)
    await waitForTabRemounted(kinguPage, tabId)
    await expectLayoutConsistent(kinguPage, tabId, 1, 'shell-exit-while-hidden-mounted', splitPtyId)
  })

  test('split pane shell exits while the tab is parked', async ({ kinguPage }) => {
    const { worktreeId, tabId, splitPtyId } = await setUpSplitTab(kinguPage)
    await createActiveTerminalTab(kinguPage, worktreeId)
    await parkHiddenTabBehindDecoy(kinguPage, worktreeId, tabId, {
      parkDelayMs: PARKING_DELAY_MS
    ...
    await sendToTerminal(kinguPage, splitPtyId, 'exit\r')
    await kinguPage.waitForTimeout(PARKING_DELAY_MS)
    await activateTerminalTab(kinguPage, tabId)
    await waitForTabRemounted(kinguPage, tabId)
    ...
    await expectLayoutConsistent(kinguPage, tabId, 1, 'shell-exit-while-parked', splitPtyId)
  })
})
