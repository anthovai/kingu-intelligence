 ...
 *
 * Scenarios are gated behind KINGU_TYPING_BENCH=1 (they are benchmarks that
 * may legitimately "fail" while the bug reproduces, not CI regression gates).
 * Set KINGU_TYPING_BENCH_INSTRUMENTATION=0 for a probe-off observer control.
...
const BENCH_ENABLED = process.env.KINGU_TYPING_BENCH === '1'
...
const LOAD_WORKSPACES = readPositiveInt('KINGU_TYPING_BENCH_LOAD_WORKSPACES', 1)
const LOAD_PANES = readPositiveInt('KINGU_TYPING_BENCH_LOAD_PANES', 4)
const LOAD_RATE_KBPS = readPositiveInt('KINGU_TYPING_BENCH_RATE_KBPS', 256)
const KEY_COUNT = readPositiveInt('KINGU_TYPING_BENCH_KEYS', 32)
const KEY_CADENCE_MS = readPositiveInt('KINGU_TYPING_BENCH_KEY_CADENCE_MS', 250)
const CPU_WORKERS = readPositiveInt('KINGU_TYPING_BENCH_CPU_WORKERS', 0)
const PTY_METADATA = process.env.KINGU_TYPING_BENCH_PTY_METADATA === '1'
const BENCH_LABEL = process.env.KINGU_TYPING_BENCH_LABEL ?? 'dev'
// Request optional probes by default; the report records when the build does not install them.
const BENCH_INSTRUMENTATION_REQUESTED = process.env.KINGU_TYPING_BENCH_INSTRUMENTATION !== '0'
// Diagnostic only: patching main's invoke handler is observer overhead, so keep it out of acceptance runs.
const GRAPH_PROBE_REQUESTED = process.env.KINGU_TYPING_BENCH_GRAPH_PROBE === '1'
const GRAPH_PROBE_SELF_TEST_MS = readPositiveInt('KINGU_TYPING_BENCH_GRAPH_PROBE_SELFTEST_MS', 0)
...
const CPU_THROTTLE_RATE = readPositiveInt('KINGU_TYPING_BENCH_CPU_THROTTLE', 1)
...
async function measureTypingWindow(
  ...
  const { result, appliedRate } = await withRendererCpuThrottle(page, CPU_THROTTLE_RATE, () =>
    withTypingRendererCpuProfile(page, process.env.KINGU_TYPING_BENCH_CPU_PROFILE, () =>
...
function writeBenchReport(
  ...
  const report = {
    ...
    config: {
      ...
      visitedWorkspaces: readPositiveInt('KINGU_TYPING_BENCH_VISITED_WORKSPACES', LOAD_WORKSPACES),
      ...
      cpuProfile: process.env.KINGU_TYPING_BENCH_CPU_PROFILE ?? null,
      titleChangeMs: readPositiveInt('KINGU_TYPING_BENCH_TITLE_CHANGE_MS', 0),
      lifecycleMs: readPositiveInt('KINGU_TYPING_BENCH_LIFECYCLE_MS', 0),
      agentRows: process.env.KINGU_TYPING_BENCH_AGENT_ROWS ?? 'default',
      metadataWorktrees: readPositiveInt('KINGU_TYPING_BENCH_METADATA_WORKTREES', 870),
      metadataRepositories: readPositiveInt('KINGU_TYPING_BENCH_METADATA_REPOSITORIES', 27),
      metadataTerminalTabs: readPositiveInt('KINGU_TYPING_BENCH_METADATA_TERMINAL_TABS', 1410),
      metadataUnifiedTabs: readPositiveInt('KINGU_TYPING_BENCH_METADATA_UNIFIED_TABS', 2000),
      metadataPanesPerTab: readPositiveInt('KINGU_TYPING_BENCH_METADATA_PANES', 1),
      metadataSleepingRecords: readPositiveInt('KINGU_TYPING_BENCH_METADATA_SLEEPERS', 857),
      metadataLiveStatuses: readPositiveInt('KINGU_TYPING_BENCH_METADATA_LIVE_STATUSES', 177),
      metadataStatusHistory: readPositiveInt('KINGU_TYPING_BENCH_METADATA_STATUS_HISTORY', 3),
      metadataStatusIntervalMs: readPositiveInt(
        'KINGU_TYPING_BENCH_METADATA_STATUS_INTERVAL_MS',
        100
      ),
...
async function startSustainedLoadInPanes(
  ...
  for (const [index, pane] of panes.entries()) {
    await sendToTerminal(
      page,
      pane.ptyId,
      `node ${JSON.stringify(scriptPath)} ${index} ${LOAD_RATE_KBPS} ${LOAD_DURATION_S} ${PTY_METADATA ? 1 : 0} ${readPositiveInt('KINGU_TYPING_BENCH_TITLE_CHANGE_MS', 0)} ${readPositiveInt('KINGU_TYPING_BENCH_LIFECYCLE_MS', 0)}\r`
    )
  }
...
function removeLoadReadyFiles(directory: string, runId: string, paneCount: number): void {
  for (let index = 0; index < paneCount; index++) {
    ...
    rmSync(path.join(directory, `.kingu-mwt-load-stats-${runId}-${index}`), { force: true })
  }
}
...
test.describe('Multi-workspace sustained typing latency bench', () => {
  ...
  test('baseline: paced typing with no agent load', async ({
    ...
    const probePath = path.join(testRepoPath, `.kingu-mwt-probe-${runId}.mjs`)
    const sidecarPath = path.join(testRepoPath, `.kingu-mwt-arrivals-${runId}.jsonl`)
    writeTypingEchoProbeScript(probePath, runId, sidecarPath)
    try {
      await resetDeliveryDebug(kinguPage)
      await startTypingProbe(kinguPage, typingPtyId, probePath, runId)
      const measured = await measureTypingWindow(kinguPage, runId, sidecarPath)
      const { measurement } = measured
      writeBenchReport(
        testInfo,
        'baseline',
        measured,
        await readSchedulerDebug(kinguPage),
        await readMainDeliveryDebug(kinguPage)
      )
      ...
      await stopPtysQuietly(kinguPage, [typingPtyId])
  ...
  test('typing under sustained hidden multi-workspace agent load', async ({
    ...
    await waitForSessionReady(kinguPage)
    const typingWorktreeId = await waitForActiveWorktree(kinguPage)
    const loadWorktreeId = (await getAllWorktreeIds(kinguPage)).find(
    ...
    const scratch = mkdtempSync(path.join(tmpdir(), 'kingu-typing-load-'))
    const runId = randomUUID()
    const loadPath = path.join(scratch, `.kingu-mwt-load-${runId}.mjs`)
    const probePath = path.join(scratch, `.kingu-mwt-probe-${runId}.mjs`)
    const sidecarPath = path.join(scratch, `.kingu-mwt-arrivals-${runId}.jsonl`)
    ...
    try {
      await switchToWorktree(kinguPage, loadWorktreeId)
      loadPanes = await createTypingLoadWorkspaces(
        kinguPage,
        loadWorktreeId,
        LOAD_PANES,
        LOAD_WORKSPACES,
        readPositiveInt('KINGU_TYPING_BENCH_VISITED_WORKSPACES', LOAD_WORKSPACES),
        createdWorktreeIds
      )
      await startSustainedLoadInPanes(kinguPage, loadPanes, loadPath, runId, scratch)

      await switchToWorktree(kinguPage, typingWorktreeId)
      await expect
        .poll(() => getActiveWorktreeId(kinguPage), { timeout: 10_000 })
        .toBe(typingWorktreeId)
      await ensureTerminalVisible(kinguPage)
      await waitForActiveTerminalManager(kinguPage, 30_000)
      const typingPtyId = await waitForActivePanePtyId(kinguPage)

      const fixtureSummary = await seedAccumulatedWorkspaceFixture(kinguPage, {
        worktrees: readPositiveInt('KINGU_TYPING_BENCH_METADATA_WORKTREES', 870),
        repositories: readPositiveInt('KINGU_TYPING_BENCH_METADATA_REPOSITORIES', 27),
        terminalTabs: readPositiveInt('KINGU_TYPING_BENCH_METADATA_TERMINAL_TABS', 1410),
        unifiedTabs: readPositiveInt('KINGU_TYPING_BENCH_METADATA_UNIFIED_TABS', 2000),
        panesPerTab: readPositiveInt('KINGU_TYPING_BENCH_METADATA_PANES', 1),
        sleepingRecords: readPositiveInt('KINGU_TYPING_BENCH_METADATA_SLEEPERS', 857),
        liveStatuses: readPositiveInt('KINGU_TYPING_BENCH_METADATA_LIVE_STATUSES', 177),
        statusHistoryEntries: readPositiveInt('KINGU_TYPING_BENCH_METADATA_STATUS_HISTORY', 3)
      })
      if (process.env.KINGU_TYPING_BENCH_AGENT_ROWS === 'full') {
      ...
      const statusTrafficEnabled =
        !PTY_METADATA && process.env.KINGU_TYPING_BENCH_METADATA_STATUS !== '0'
      if (statusTrafficEnabled) {
        statusIngressValidation = await validateAccumulatedStatusIpcIngress(electronApp, kinguPage)
      ...
      if (BENCH_INSTRUMENTATION_REQUESTED) {
        instrumentationAvailable = await startAccumulatedBenchmarkInstrumentation(kinguPage)
      }
      if (GRAPH_PROBE_REQUESTED) {
        graphProbeStart = await startRuntimeGraphPublicationProbe(electronApp, kinguPage)
      ...
      if (statusTrafficEnabled) {
        const statusTraffic = await startAccumulatedStatusTraffic(
          electronApp,
          kinguPage,
          readPositiveInt('KINGU_TYPING_BENCH_METADATA_STATUS_INTERVAL_MS', 100)
        )
        expect(statusTraffic.trackedStatuses).toBe(fixtureSummary.liveStatuses)
        statusTrafficStarted = true
      }
      if (!PTY_METADATA && process.env.KINGU_TYPING_BENCH_METADATA_TITLES === '1') {
        titleWorkload = await startAccumulatedTitleTraffic(kinguPage, 100)
        console.log(
      ...
      await expect
        .poll(
          async () => (await readMainDeliveryDebug(kinguPage))?.hiddenDeliveryDroppedChars ?? 0,
      ...
      const statusWorkload = statusTrafficStarted
        ? await stopAccumulatedStatusTraffic(electronApp, kinguPage)
        : null
      ...
      const instrumentation = BENCH_INSTRUMENTATION_REQUESTED
        ? await stopAccumulatedBenchmarkInstrumentation(kinguPage)
      ...
      writeBenchReport(
        ...
        await readTypingScaleCensus(kinguPage),
        fixtureSummary,
        {
          producers: loadPanes.map((_, index) =>
            JSON.parse(
              readFileSync(path.join(scratch, `.kingu-mwt-load-stats-${runId}-${index}`), 'utf8')
            )
          ),
          receipts: await kinguPage.evaluate(() => {
      ...
      await kinguPage.screenshot({ path: path.join(screenDirectory, `${BENCH_LABEL}-screen.png`) })
      ...
      if (instrumentationAvailable) {
        await stopAccumulatedBenchmarkInstrumentation(kinguPage).catch(() => undefined)
      }
      if (graphProbeStart) {
        await stopRuntimeGraphPublicationProbe(
          electronApp,
      ...
      await stopAccumulatedTitleTraffic(kinguPage)
      await cleanupAccumulatedWorkspaceFixture(kinguPage)
      for (const worker of cpuWorkers) {
        worker.kill('SIGKILL')
      }
      await switchToWorktree(kinguPage, loadWorktreeId).catch(() => undefined)
      await stopPtysQuietly(
        kinguPage,
        loadPanes.map((pane) => pane.ptyId)
      )
      await switchToWorktree(kinguPage, typingWorktreeId).catch(() => undefined)
      rmSync(loadPath, { force: true })
      rmSync(probePath, { force: true })
      rmSync(sidecarPath, { force: true })
      removeLoadReadyFiles(scratch, runId, LOAD_PANES)
      await removeTypingLoadWorkspaces(kinguPage, createdWorktreeIds)
  ...
  test('typing under sustained visible split agent load', async ({
    ...
    const runId = randomUUID()
    const loadPath = path.join(testRepoPath, `.kingu-mwt-load-${runId}.mjs`)
    const probePath = path.join(testRepoPath, `.kingu-mwt-probe-${runId}.mjs`)
    const sidecarPath = path.join(testRepoPath, `.kingu-mwt-arrivals-${runId}.jsonl`)
    ...
    try {
      ...
      panes = await ensureActiveWorktreePaneLoad(kinguPage, 2)
      const [typingPane, ...loadPanes] = panes
      await startSustainedLoadInPanes(kinguPage, loadPanes, loadPath, runId, testRepoPath)
      await focusPane(kinguPage, typingPane.paneKey)

      await resetDeliveryDebug(kinguPage)
      await startTypingProbe(kinguPage, typingPane.ptyId, probePath, runId)
      const measured = await measureTypingWindow(kinguPage, runId, sidecarPath)
      const { measurement } = measured
      writeBenchReport(
        testInfo,
