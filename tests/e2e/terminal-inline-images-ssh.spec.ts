...
test('inline images stay bounded and recover through SSH drop and stall', async ({
  ...
  test.skip(process.env.KINGU_E2E_SSH_DOCKER !== '1', 'Requires isolated Docker SSH target')
  ...
  try {
    ...
    await enableInlineImages(kinguPage)
    const emit = async (stage: string) => {
      await execInTerminal(kinguPage, ptyId, `node /tmp/image-producer.cjs ${stage}`)
      await waitForTerminalOutput(kinguPage, `IMAGE_PROOF_${stage}`, 30_000)
      await assertInlineImagePixels(kinguPage, testInfo.outputPath(`${stage}.png`))
    }
    await emit('before-drop')
    await execInTerminal(kinguPage, ptyId, 'export IMAGE_SHELL_ID=$$')
    ...
    await waitForTerminalOutput(kinguPage, 'SHELL_SURVIVED', 30_000)
    await emit('after-drop')
    await withStalledDockerSshRelayTarget(target, async () => {
      await execInTerminal(kinguPage, ptyId, 'node /tmp/image-producer.cjs after-stall')
      // A timed fault models a silent established TCP connection, not a render wait.
      await new Promise((resolve) => setTimeout(resolve, 1500))
    })
    await waitForTerminalOutput(kinguPage, 'IMAGE_PROOF_after-stall', 60_000)
    expect(await waitForActivePanePtyId(kinguPage)).toBe(ptyId)
    await assertInlineImagePixels(kinguPage, testInfo.outputPath('after-stall.png'))
    ...
    await execInTerminal(kinguPage, ptyId, 'node /tmp/image-flood.cjs')
    await waitForTerminalOutput(kinguPage, 'FLOOD_DONE', 30_000)
    const flooded = await readInlineImageState(kinguPage)
    ...
    expect((await readInlineImageState(kinguPage))?.pending).toBe(0)
    const beforeZoom = await readInlineImageState(kinguPage)
    if (!beforeZoom) {
    ...
    await expect
      ...
      .toBe(28)
    await assertInlineImagePixels(kinguPage, testInfo.outputPath('after-font-zoom.png'))
    const afterZoom = await readInlineImageState(kinguPage)
    if (!afterZoom) {
