...
test.describe('browser split navigation shortcuts', () => {
  ...
  test('Back and Forward typed in one guest move only that split', async ({ kinguPage }) => {
    const fixture = await createBrowserSplitWithHistory(kinguPage, server)

    await pressInGuest(kinguPage, fixture, backChord)
    await waitForGuestUrl(kinguPage, fixture.firstBrowserTabId, server.pageUrl('a', 1))
    await expectSecondSplitUntouched(kinguPage, fixture, server)

    await pressInGuest(kinguPage, fixture, forwardChord)
    await waitForGuestUrl(kinguPage, fixture.firstBrowserTabId, server.pageUrl('a', 2))
    await expectSecondSplitUntouched(kinguPage, fixture, server)
  })
  ...
  test('Reload and Hard Reload typed in one guest reload only that split', async ({
    ...
    await pressInGuest(kinguPage, fixture, reloadChord)
    await expectFirstSplitReloaded(kinguPage, fixture)
    await expectSecondSplitUntouched(kinguPage, fixture, server)

    await recordGuestLoadStarts(kinguPage, [fixture.firstBrowserTabId])
    await pressInGuest(kinguPage, fixture, hardReloadChord)
    await expectFirstSplitReloaded(kinguPage, fixture)
    await expectSecondSplitUntouched(kinguPage, fixture, server)
    expect(await guestUrl(kinguPage, fixture.firstBrowserTabId)).toBe(server.pageUrl('a', 2))
  })

  // Ctrl+wheel is not covered: before-mouse-event only fires for OS input, not sendInputEvent.
  test('page zoom typed in one guest zooms only that split', async ({ kinguPage }) => {
    const fixture = await createBrowserSplitWithHistory(kinguPage, server)
    const firstZoom = (): Promise<number | null> =>
      guestZoomLevel(kinguPage, fixture.firstBrowserTabId)
    const secondZoom = (): Promise<number | null> =>
      guestZoomLevel(kinguPage, fixture.secondBrowserTabId)
    const initialFirstZoom = await firstZoom()
    const initialSecondZoom = await secondZoom()
    expect(initialFirstZoom).not.toBeNull()

    await pressInGuest(kinguPage, fixture, { guestKeyCode: '=', guestModifiers: [guestModifier] })
  ...
  test('Focus Address Bar typed in one guest focuses only that split', async ({ kinguPage }) => {
    const fixture = await createBrowserSplitWithHistory(kinguPage, server)

    await pressInGuest(kinguPage, fixture, { guestKeyCode: 'L', guestModifiers: [guestModifier] })

    await expect(browserAddressBar(kinguPage, fixture.firstBrowserTabId)).toBeFocused()
    await expect(browserAddressBar(kinguPage, fixture.secondBrowserTabId)).not.toBeFocused()
  })
  ...
  test('Back and Reload pressed from one split toolbar act on only that split', async ({
    ...
    const fixture = await createBrowserSplitWithHistory(kinguPage, server)
    const reloadButton = browserOverlay(kinguPage, fixture.firstBrowserTabId).getByRole('button', {
      name: 'Reload',
      exact: true
    })

    await reloadButton.focus()
    await waitForFocusedGroup(kinguPage, fixture.firstBrowserGroupId)
    await kinguPage.keyboard.press(backChord.renderer)
    await waitForGuestUrl(kinguPage, fixture.firstBrowserTabId, server.pageUrl('a', 1))
    await expectSecondSplitUntouched(kinguPage, fixture, server)

    await recordGuestLoadStarts(kinguPage, [fixture.firstBrowserTabId])
    await reloadButton.focus()
    await kinguPage.keyboard.press(reloadChord.renderer)
    await expectFirstSplitReloaded(kinguPage, fixture)
    await expectSecondSplitUntouched(kinguPage, fixture, server)
  })
  ...
  test('Back and Reload pressed in a focused terminal leave the browser split alone', async ({
    ...
    const fixture = await createTerminalBrowserSplit(kinguPage, server.pageUrl('a', 1))
    await waitForGuestUrl(kinguPage, fixture.browserTabId, server.pageUrl('a', 1))
    await navigateGuest(kinguPage, fixture.browserTabId, server.pageUrl('a', 2))
    await waitForGuestIdle(kinguPage, fixture.browserTabId)
    await recordGuestLoadStarts(kinguPage, [fixture.browserTabId])

    await focusBrowserGroup(kinguPage, fixture.terminalGroupId)
    await focusActiveTerminalInput(kinguPage)
    await waitForFocusedGroup(kinguPage, fixture.terminalGroupId)
    await kinguPage.keyboard.press(backChord.renderer)
    await kinguPage.keyboard.press(reloadChord.renderer)
    await kinguPage.keyboard.press(hardReloadChord.renderer)
    // Why: the next toolbar press proves these chords reach the pane in this app; it only has to lose here.
    const reloadButton = browserOverlay(kinguPage, fixture.browserTabId).getByRole('button', {
      name: 'Reload',
      exact: true
    })
    await waitForGuestIdle(kinguPage, fixture.browserTabId)
    expect(await guestLoadStarts(kinguPage, fixture.browserTabId)).toBe(0)
    expect(await guestUrl(kinguPage, fixture.browserTabId)).toBe(server.pageUrl('a', 2))

    await reloadButton.focus()
    await waitForFocusedGroup(kinguPage, fixture.browserGroupId)
    await kinguPage.keyboard.press(backChord.renderer)
    await waitForGuestUrl(kinguPage, fixture.browserTabId, server.pageUrl('a', 1))
  })
})
