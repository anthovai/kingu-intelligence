...
test('shows interrupted hidden SSH cleanup as retryable', async ({ electronApp, kinguPage }) => {
  const userDataPath = await electronApp.evaluate(({ app }) => app.getPath('userData'))
  writeFileSync(
    path.join(userDataPath, 'kingu-ephemeral-vm-runtimes.json'),
    JSON.stringify({
      version: 1,
      runtimes: [
        {
  ...
  await expect(kinguPage.getByPlaceholder('Search settings')).toBeVisible()
  await kinguPage
    .getByRole('group', { name: 'Remote server workflow' })
    .getByRole('button', { name: /^Cloud VM/ })
    .click()

  const runtimes = kinguPage.locator('[data-settings-section="temporary-vm-runtimes"]')
...
test('stops long-running cleanup and keeps it retryable', async ({ electronApp, kinguPage }) => {
  const repoPath = mkdtempSync(path.join(tmpdir(), 'kingu-cleanup-stop-'))
  ...
  try {
    ...
    const repoId = await kinguPage.evaluate(async (repo) => {
    ...
    await openCloudVmRuntimes(kinguPage)
    const runtimes = kinguPage.locator('[data-settings-section="temporary-vm-runtimes"]')
    ...
    const dialog = kinguPage.getByRole('dialog', { name: 'Stop cleanup?' })
    ...
    await expect(runtimes.getByRole('button', { name: 'Retry cleanup' })).toBeVisible()
    await expect(kinguPage.getByText('Cleanup stopped by user.')).toBeVisible()
