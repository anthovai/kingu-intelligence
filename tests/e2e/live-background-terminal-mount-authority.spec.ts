...
import { test as base, expect } from './helpers/kingu-app'
...
const fakeCliDir = mkdtempSync(path.join(os.tmpdir(), 'kingu-live-mount-cli-'))
...
appendFileSync(process.env.KINGU_E2E_CODEX_SPAWN_LEDGER, JSON.stringify({ args, pid: process.pid }) + '\\n')
...
for (const signal of ['SIGINT', 'SIGHUP', 'SIGTERM']) process.on(signal, () => appendFileSync(process.env.KINGU_E2E_SIGNAL_LEDGER, JSON.stringify({ kind: 'agent', pid: process.pid, signal }) + '\\n'))
  ...
  launchEnv: [
    {
      ...
      KINGU_E2E_CODEX_SPAWN_LEDGER: spawnLedgerPath,
      KINGU_E2E_SETUP_LEDGER: setupLedgerPath,
      KINGU_E2E_CANARY_LEDGER: canaryLedgerPath,
      KINGU_E2E_SIGNAL_LEDGER: signalLedgerPath
    },
    { option: true }
  ]
})
...
function createSourceRepo(): string {
  const repoPath = mkdtempSync(path.join(os.tmpdir(), 'kingu-live-mount-repo-'))
  writeFileSync(
    path.join(repoPath, 'setup-live.js'),
    `const { appendFileSync } = require('node:fs')\nappendFileSync(process.env.KINGU_E2E_SETUP_LEDGER, JSON.stringify({ pid: process.pid }) + '\\n')\nconsole.log('SETUP_READY:' + process.pid)\nlet inputBuffer = ''\nprocess.stdin.on('data', chunk => {\n  inputBuffer += chunk.toString()\n  const lines = inputBuffer.split(/[\\r\\n]+/)\n  inputBuffer = lines.pop() || ''\n  for (const line of lines) if (line) console.log('SETUP_INPUT:' + process.pid + ':' + line)\n})\nfor (const signal of ['SIGINT', 'SIGHUP', 'SIGTERM']) process.on(signal, () => appendFileSync(process.env.KINGU_E2E_SIGNAL_LEDGER, JSON.stringify({ kind: 'setup', pid: process.pid, signal }) + '\\n'))\nprocess.stdin.resume()\nsetInterval(() => {}, 60000)\n`
  )
  writeFileSync(
    path.join(repoPath, 'canary-live.js'),
    `const { appendFileSync } = require('node:fs')\nappendFileSync(process.env.KINGU_E2E_CANARY_LEDGER, JSON.stringify({ pid: process.pid }) + '\\n')\nconsole.log('CANARY_READY:' + process.pid)\nlet inputBuffer = ''\nprocess.stdin.on('data', chunk => {\n  inputBuffer += chunk.toString()\n  const lines = inputBuffer.split(/[\\r\\n]+/)\n  inputBuffer = lines.pop() || ''\n  for (const line of lines) if (line) console.log('CANARY_INPUT:' + process.pid + ':' + line)\n})\nfor (const signal of ['SIGINT', 'SIGHUP', 'SIGTERM']) process.on(signal, () => appendFileSync(process.env.KINGU_E2E_SIGNAL_LEDGER, JSON.stringify({ kind: 'canary', pid: process.pid, signal }) + '\\n'))\nprocess.stdin.resume()\nsetInterval(() => {}, 60000)\n`
  )
  ...
  execFileSync(
    'git',
    ['-c', 'user.name=Kingu E2E', '-c', 'user.email=kingu-e2e@example.com', 'commit', '-m', 'seed'],
    { cwd: repoPath }
  )
  return repoPath
}
...
test('adopts runtime-owned agent and Setup PTYs on first mount', async ({
  ...
  await assertTargetBindings(kinguPage, worktreeId, allIdentities)
  await seedAgentRecoveryMetadata(kinguPage, worktreeId, terminalIdentity(agent!))

  await faultProjectionAndActivate(kinguPage, worktreeId, [agent!, setup!], agent!.tabId)
  const mountedAgentPtyId = await waitForActivePanePtyId(kinguPage)
  await enableTerminalAccessibility(kinguPage, agent!.tabId)
  await expect
    .poll(
      async () => ({
        ...
        visibleOriginalReady: (
          await terminalAccessibility(kinguPage, agent!.tabId).innerText()
  ...
  await typeIntoTerminal(kinguPage, agent!.tabId, agentMarker)
  await assertExactPtyReceivedMarker(electronApp, agent!.ptyId, agentMarker)
  await expect(terminalAccessibility(kinguPage, agent!.tabId)).toContainText(
    `AGENT_INPUT:${agentPid}:${agentMarker}`
  )
  await expect(terminalAccessibility(kinguPage, agent!.tabId)).not.toContainText(
    'Conversation interrupted'
  )

  await activateTerminal(kinguPage, worktreeId, setup!.tabId)
  const mountedSetupPtyId = await waitForActivePanePtyId(kinguPage)
  await enableTerminalAccessibility(kinguPage, setup!.tabId)
  expect(mountedSetupPtyId).toBe(setup!.ptyId)
  await expect(terminalAccessibility(kinguPage, setup!.tabId)).toContainText(
    `SETUP_READY:${setupPid}`
  )
  const setupMarker = `SETUP_KB_${randomUUID().slice(0, 8)}`
  await clearTerminalPtyWriteLog(electronApp)
  await typeIntoTerminal(kinguPage, setup!.tabId, setupMarker)
  await assertExactPtyReceivedMarker(electronApp, setup!.ptyId, setupMarker)
  await expect(terminalAccessibility(kinguPage, setup!.tabId)).toContainText(
    `SETUP_INPUT:${setupPid}:${setupMarker}`
  )
  await expect(terminalAccessibility(kinguPage, setup!.tabId)).not.toContainText(
    'Conversation interrupted'
  )
  ...
  const beforeReloadDelivery = await kinguPage.evaluate(() =>
  ...
  await expect
    .poll(() => kinguPage.evaluate(() => window.api.pty.getRendererDeliveryDebugSnapshot()))
    .toMatchObject(postReloadDelivery)
  await activateTerminal(kinguPage, worktreeId, agent!.tabId)
  const remountedAgentPtyId = await waitForActivePanePtyId(kinguPage)
  ...
  await expect
    .poll(() => terminalViewportText(kinguPage, agent!.tabId))
    .toContain(remountAgentLiveOutput)
  expect(
    await kinguPage.evaluate(() => window.api.pty.getRendererDeliveryDebugSnapshot())
  ).toMatchObject(postReloadDelivery)
  ...
  await clearTerminalPtyWriteLog(electronApp)
  await typeIntoTerminal(kinguPage, agent!.tabId, remountAgentMarker)
  ...
  await activateTerminal(kinguPage, worktreeId, setup!.tabId)
  const remountedSetupPtyId = await waitForActivePanePtyId(kinguPage)
  ...
  await expect
    .poll(() => terminalViewportText(kinguPage, setup!.tabId))
  ...
  await clearTerminalPtyWriteLog(electronApp)
  await typeIntoTerminal(kinguPage, setup!.tabId, remountSetupMarker)
