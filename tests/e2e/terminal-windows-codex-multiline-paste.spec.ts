...
import { attachRepoAndOpenTerminal } from './helpers/kingu-restart'
import {
...
const DRAFT = 'KINGU_CODEX_PASTE_DRAFT_SHOULD_STAY_UNSENT'
...
test.describe('Windows Codex multiline paste', () => {
  ...
  test('multiline Ctrl+V keeps the existing Codex draft unsent @local-real-codex', async ({
    ...
    test.skip(
      process.env.KINGU_E2E_REAL_CODEX !== '1',
      'Set KINGU_E2E_REAL_CODEX=1 to exercise the locally installed Codex TUI'
    )
    test.slow()

    await waitForSessionReady(kinguPage)
    await activateTestRepository(kinguPage, testRepoPath)
    ...
    await sendToTerminal(kinguPage, ptyId, 'codex -m kingu-e2e-invalid-model\r')
    await expect
      .poll(() => getTerminalContent(kinguPage, 12_000), { timeout: 20_000 })
      .toMatch(/Do[\s\S]*you[\s\S]*trust[\s\S]*contents|OpenAI Codex/i)
    if (CODEX_TRUST_PROMPT_RE.test(await getTerminalContent(kinguPage, 12_000))) {
      await sendToTerminal(kinguPage, ptyId, '\r')
    }
    await waitForTerminalOutput(kinguPage, 'OpenAI Codex', 20_000, 30_000)
    await waitForCodexComposerReady(kinguPage)
    await enableTerminalAccessibilityDom(kinguPage, ptyId)
    await focusActiveTerminalInput(kinguPage)
    await kinguPage.keyboard.type(DRAFT)
    const terminalDom = kinguPage.locator(
      `[data-pty-id=${JSON.stringify(ptyId)}] .xterm-accessibility-tree`
    )
    await expect(terminalDom).toContainText(DRAFT, { timeout: 10_000 })
    await kinguPage.evaluate((text) => window.api.ui.writeClipboardText(text), pastePayload())
  ...
  test('delivers a normalized large paste through native ConPTY', async ({
    ...
    const marker = `KINGU_LARGE_PASTE_${randomUUID().replaceAll('-', '')}`
    ...
    try {
      ...
      await waitForTerminalOutput(kinguPage, `${marker}_READY`, 10_000, 12_000)
      await enableTerminalAccessibilityDom(kinguPage, ptyId)
      await focusActiveTerminalInput(kinguPage)
