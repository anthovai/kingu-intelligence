...
test.describe('SSH config host picker', () => {
  ...
  test.afterEach(async ({ kinguPage }) => {
    await closeOpenDialogs(kinguPage).catch(() => undefined)
    await removeSshTargetsByPrefix(kinguPage, HOST_PREFIX).catch(() => undefined)
  })

  // ── P1 ─────────────────────────────────────────────────────────────
  test('P1: empty config shows empty state; Back returns to blank form', async ({ kinguPage }) => {
    ...
    await expect(picker.getByRole('button', { name: 'Add all to Kingu' })).toBeDisabled()

    await picker.getByRole('button', { name: 'Back' }).click()
    const form = kinguPage.getByRole('dialog', { name: 'Add SSH host' })
  ...
  test('P3: select host prefills form; Save persists; N3 identity hint', async ({
    ...
    await expect(
      kinguPage.getByText(new RegExp(`Filled from ${escapeRegExp(prod.alias)}`, 'i'))
  ...
  // ── P4 ─────────────────────────────────────────────────────────────
  test('P4: filter narrows host list', async ({ electronApp, kinguPage }) => {
    const hosts = await seedPairConfig(electronApp, HOST_PREFIX)
    const picker = await openSshConfigHostPicker(kinguPage)
  ...
  // ── P8 ─────────────────────────────────────────────────────────────
  test('P8: Back without select leaves form empty', async ({ electronApp, kinguPage }) => {
    const hosts = await seedPairConfig(electronApp, HOST_PREFIX)
    const picker = await openSshConfigHostPicker(kinguPage)
    await expect(configHostRow(picker, hosts.alpha)).toBeVisible()
    ...
    await expect(fields.username).toHaveValue('')
    await expect(fields.label).toHaveValue('')
    await expect(kinguPage.getByText(/Filled from /i)).toHaveCount(0)
  })
})
