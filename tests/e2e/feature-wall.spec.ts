...
async function openFeatureTourFromMenu(electronApp: ElectronApplication): Promise<void> {
  await electronApp.evaluate(({ BrowserWindow, Menu }) => {
    const featureTourItem = Menu.getApplicationMenu()
      ...
      ?.submenu?.items.find((item) => item.label === 'Explore Kingu')

    if (!featureTourItem) {
      throw new Error('Explore Kingu menu item was not registered')
    }
...
test.describe('Feature tour modal', () => {
  ...
  test('opens from the Help menu and renders the workflow rail', async ({
    ...
    await expect(kinguPage.getByRole('dialog', { name: 'Get to know Kingu' })).toBeVisible({
      timeout: 10_000
    })
    await expect(kinguPage.getByText('Reopen any time from Help > Explore Kingu.')).toBeVisible()

    // Five workflow rows in the rail.
    const rail = kinguPage.getByRole('navigation', { name: 'Workflows' })
    ...
    await expect(kinguPage.locator('[data-ws-id]')).toHaveCount(3)

    // ArrowDown moves selection through the rail.
    await rail.getByRole('tab', { name: /Workspaces/i }).focus()
    await kinguPage.keyboard.press('ArrowDown')
    ...
    await expect(
      kinguPage.getByText(
        "Run your app in Kingu's browser, send selected UI elements to agents, and let your agents interact with your webpage."
      )
    ).toBeVisible()
    await expect(kinguPage.getByRole('heading', { name: 'Browser Use skill' })).toBeVisible()
    await expect(
      kinguPage.getByText("Enables agents to navigate and verify pages in Kingu's browser.")
    ).toBeVisible()
    await expect(kinguPage.getByRole('heading', { name: 'CLI skill' })).toHaveCount(0)
    await expect(kinguPage.getByText('With the Kingu CLI skill', { exact: false })).toHaveCount(0)
  })

  test('shows unified task copy without leaving the walkthrough', async ({ kinguPage }) => {
    ...
    await kinguPage
      ...
      .click()
    await expect(kinguPage.getByText('Start work directly from GitHub or Linear.')).toBeVisible()
    await expect(kinguPage.getByText('Connect GitHub or Linear once')).toHaveCount(0)
    await expect(kinguPage.getByRole('dialog', { name: 'Get to know Kingu' })).toBeVisible()
    await expect
      .poll(async () => getStoreState<string>(kinguPage, 'activeView'))
      .not.toBe('settings')
  })
  ...
  test('continue advances through workflow substeps before the next workflow', async ({
    ...
    const rail = kinguPage.getByRole('navigation', { name: 'Workflows' })
    const continueButton = kinguPage.getByRole('button', { name: /^Continue/ })
  ...
  test('does not pre-check configured workflows until the user visits them', async ({
    ...
    await kinguPage.evaluate(async () => {
      for (const key of [
        'kingu.featureWall.visitedWorkflows.v1',
        'kingu.featureWall.visitedAgentSteps.v1',
        'kingu.featureWall.visitedWorkbenchSteps.v1',
        'kingu.featureWall.visitedReviewSteps.v1',
        'kingu.featureWall.completedWorkflows.v1',
        'kingu.featureWall.completedAgentSteps.v1',
        'kingu.featureWall.completedWorkbenchSteps.v1',
        'kingu.featureWall.completedReviewSteps.v1'
      ]) {
