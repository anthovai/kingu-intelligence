...
test('@golden stages and commits a file through Source Control', async ({
  ...
  await kinguPage.getByRole('textbox', { name: 'Commit message' }).fill('test: golden daily loop')
  await kinguPage.getByRole('button', { name: 'Commit', exact: true }).click()
  ...
  await expect(
    kinguPage.locator('[data-sonner-toast]').filter({ hasText: /node|command not found|cmd\.exe/i })
  ).toHaveCount(0)
})
