...
const SCRATCH_DIR =
  process.env.KINGU_TABLE_ROW_BACKSPACE_SCREENSHOT_DIR ??
...
test.describe('Markdown table keyboard', () => {
  ...
  test('Tab/Shift-Tab move between cells and empty-row Backspace deletes the row', async ({
    ...
    try {
      ...
      await kinguPage.keyboard.press('Tab')
      await expect
        .poll(async () => selectionCellText(kinguPage), {
          timeout: 5_000,
      ...
      await kinguPage.keyboard.press('Shift+Tab')
      await expect
        .poll(async () => selectionCellText(kinguPage), {
          timeout: 5_000,
      ...
      await expect
        .poll(async () => tableRowCount(kinguPage), {
          timeout: 5_000,
