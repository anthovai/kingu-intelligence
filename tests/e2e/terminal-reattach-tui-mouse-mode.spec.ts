/**
 * Regression proof for #8291: a real alt-screen TUI survives an Kingu quit/relaunch, and after the
...
function createIsolatedProofRepo(): string {
  // Why realpathSync: macOS tmpdir symlinks through /private and Kingu canonicalizes repo.path.
  const repoDir = realpathSync(mkdtempSync(path.join(os.tmpdir(), 'kingu-mouse-reattach-repo-')))
  ...
  writeFileSync(path.join(repoDir, 'README.md'), '# Kingu mouse-mode reattach proof repo\n')
  git('add', '-A')
...
// Why: this suite quits and relaunches Kingu against one userDataDir, and the
...
test.describe('terminal reattach mouse mode', () => {
  ...
  {}, testInfo) => {
    ...
    try {
      ...
      // Why a screenshot and not the video fixture: this spec quits and relaunches Kingu,
      ...
      const proofShot = process.env.KINGU_E2E_PROOF_SCREENSHOT
      if (proofShot) {
