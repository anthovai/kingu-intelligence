...
test.describe('terminal hidden-worktree retention budget', () => {
  ...
  test('force-parks the older hidden un-parkable worktree and spares the newest', async ({
    ...
    try {
      ...
      const older = await connectDockerSshRelayTarget(kinguPage, target)
      await expect
      ...
      const olderPtyId = await waitForActivePanePtyId(kinguPage, 60_000)
      const olderTabId = await getActiveTabId(kinguPage)
      if (!olderTabId) {
      ...
      await sendToTerminal(kinguPage, olderPtyId, `echo "${olderMarker}:"\r`)
      await expect
        .poll(() => getTerminalContent(kinguPage, 20_000), {
          timeout: 30_000,
      ...
      const newerTabId = await getActiveTabId(kinguPage)
      if (!newerTabId) {
      ...
      await waitForTabParked(kinguPage, olderTabId, { parkDelayMs: PARKING_DELAY_MS })
      // …while the newest hidden worktree keeps its mounted panes (last-active exemption).
      const newerStillMounted = await kinguPage.evaluate(
