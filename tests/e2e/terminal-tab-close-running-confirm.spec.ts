...
test('the tab X button applies the same running-process confirmation as Cmd+W', async ({
  ...
  await execInTerminal(kinguPage, ptyId, 'echo repro-10142-ready')
  await waitForTerminalOutput(kinguPage, 'repro-10142-ready', 20_000)
  await execInTerminal(kinguPage, ptyId, 'sleep 300')
  ...
  await expect
    .poll(
      async () =>
        (await kinguPage.evaluate((id) => window.api.pty.inspectProcess(id), ptyId))
          .foregroundProcess,
      { timeout: 20_000, message: 'sleep 300 never became the foreground process' }
    )
    .toBe('sleep')

  const busyTabId = (await getActiveTabId(kinguPage))!
  ...
  expect(
    {
      confirmDialogVisible: await closeDialogTitle(kinguPage).isVisible(),
      tabStillPresent: (await countRenderedTabs(kinguPage)) === tabsBefore
    },
