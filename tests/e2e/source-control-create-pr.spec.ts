...
async function seedCreatePREligibleBranch(
  ...
  return page.evaluate(async ({ createResult }) => {
    ...
    const pr = {
      ...
      state: 'open' as const,
      url: 'https://github.com/acme/kingu/pull/73',
...
test.describe('Source Control create pull request', () => {
  ...
  test('creates the pull request from the Source Control primary action', async ({ kinguPage }) => {
    const { branch, worktreeId } = await seedCreatePREligibleBranch(kinguPage)
    await openSourceControl(kinguPage, worktreeId)
    await forceCreatePREligibleStatus(kinguPage, worktreeId, branch)

    const titleInput = kinguPage.getByRole('textbox', { name: 'Pull request title' })
    const descriptionInput = kinguPage.getByRole('textbox', {
      name: 'Pull request description'
    })
    const createButton = getCreatePRComposerSubmitButton(kinguPage)
    await expect(createButton).toBeVisible({ timeout: 10_000 })
    await expect(createButton).toBeEnabled()
    await expect(titleInput).toHaveValue('E2e secondary')
    await expect(kinguPage.getByRole('combobox', { name: 'Pull request base branch' })).toHaveValue(
      'main'
    )
  ...
  test('surfaces create failures without clearing the pull request composer', async ({
    ...
    await expect(kinguPage.getByText(failureMessage)).toBeVisible()
