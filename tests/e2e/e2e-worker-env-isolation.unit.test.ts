 ...
 * Use `test.use({ kinguAppExtraEnv })`, which Playwright scopes to the file.
 */
...
describe('e2e worker env isolation', () => {
  ...
  it('detects the shape it is meant to catch', () => {
    ...
    expect(MODULE_SCOPE_ENV_WRITE.test("process.env.KINGU_E2E_X ??= '1'")).toBe(true)
    expect(MODULE_SCOPE_ENV_WRITE.test("process.env.KINGU_E2E_X = '1'")).toBe(true)
    expect(MODULE_SCOPE_ENV_WRITE.test('delete process.env.KINGU_E2E_X')).toBe(true)
    expect(MODULE_SCOPE_ENV_WRITE.test("Object.assign(process.env, { KINGU_E2E_X: '1' })")).toBe(
      true
    )
    // Reads, and writes nested in any body, stay legal.
    expect(MODULE_SCOPE_ENV_WRITE.test('const x = Number(process.env.KINGU_E2E_X) || 500')).toBe(
      false
    )
    expect(MODULE_SCOPE_ENV_WRITE.test("  process.env.KINGU_E2E_X = '1'")).toBe(false)
    expect(MODULE_SCOPE_ENV_WRITE.test("if (process.env.KINGU_E2E_X === '1') {")).toBe(false)
  })
})
