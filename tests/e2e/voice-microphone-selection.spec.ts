...
async function installFakeMicrophoneDevices(
  ...
  await page.addInitScript((initialDevices) => {
    ...
    ;(
      window as Window & { __kinguE2EFakeMicrophone?: FakeMicrophoneState }
    ).__kinguE2EFakeMicrophone = state
  }, devices)
}
...
test.describe('Voice microphone selection', () => {
  test('lists devices, persists a selected microphone, and restores it', async ({ kinguPage }) => {
    await waitForSessionReady(kinguPage)
    await installFakeMicrophoneDevices(kinguPage, [
      { deviceId: 'built-in', label: 'Built-in Microphone' },
      { deviceId: 'usb-mic', label: 'USB Microphone' }
    ])
    await kinguPage.reload({ waitUntil: 'domcontentloaded' })
    await waitForSessionReady(kinguPage)
    await prepareVoiceSettings(kinguPage, null, null)

    const microphone = kinguPage.getByRole('combobox', { name: 'Microphone' })
    ...
    await expect(kinguPage.getByRole('option', { name: 'USB Microphone' })).toBeVisible()
    // Keyboard selection bypasses the transient pointer stability gate in CI.
    await kinguPage.getByRole('option', { name: 'USB Microphone' }).press('Enter')

    await expect
      .poll(() => readMicrophoneSettings(kinguPage), {
    ...
    await kinguPage.reload({ waitUntil: 'domcontentloaded' })
    await waitForSessionReady(kinguPage)
    await prepareVoiceSettings(kinguPage, 'usb-mic', 'USB Microphone')
    await expect(kinguPage.getByRole('combobox', { name: 'Microphone' })).toHaveText(
      'USB Microphone'
    )
  })
  ...
  test('marks an unplugged device unavailable and follows a relabeled device', async ({
    ...
    await kinguPage.reload({ waitUntil: 'domcontentloaded' })
    await waitForSessionReady(kinguPage)
    await prepareVoiceSettings(kinguPage, 'stale-airpods-id', 'AirPods')

    const microphone = kinguPage.getByRole('combobox', { name: 'Microphone' })
    await microphone.press('Space')
    await expect(kinguPage.getByRole('option', { name: 'AirPods (unavailable)' })).toBeVisible()
    await kinguPage.keyboard.press('Escape')

    await kinguPage.evaluate(() => {
      const state = (window as Window & { __kinguE2EFakeMicrophone?: FakeMicrophoneState })
        .__kinguE2EFakeMicrophone
      if (!state) {
    ...
    const airpodsOption = kinguPage.getByRole('option', { name: 'AirPods', exact: true })
    await expect(async () => {
      if (!(await airpodsOption.isVisible().catch(() => false))) {
        const trigger = kinguPage.getByRole('combobox', { name: 'Microphone' })
        await expect(trigger).toHaveText('AirPods', { timeout: 1_000 })
        await trigger.press('Space')
      }
      await expect(airpodsOption).toBeVisible({ timeout: 1_000 })
    }).toPass({ timeout: 10_000 })
    await expect(kinguPage.getByRole('option', { name: 'AirPods (unavailable)' })).toHaveCount(0)
    await kinguPage.keyboard.press('Escape')
    await expect(readMicrophoneSettings(kinguPage)).resolves.toEqual({
