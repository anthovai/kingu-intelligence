...
const scratch = mkdtempSync(path.join(os.tmpdir(), 'kingu-paired-stalled-stream-'))
...
test('restarts one ACK-starved paired terminal stream without replacing its PTY @headful', async ({
  ...
  const worktree = await kinguPage.evaluate(() => {
  ...
  const client = await launchPairedWebClient(electronApp, offer, {
    disableRemoteTerminalStallRecovery:
      process.env.KINGU_E2E_DISABLE_REMOTE_TERMINAL_STALL_RECOVERY === '1'
  })
  ...
  try {
    await expect
      ...
      .toBe(true)
    const observerOffer = await createRuntimeDesktopPairingOffer(kinguPage)
    ...
    await minimizeHeadedHost(electronApp, kinguPage)
    ...
    await restoreHeadedHost(electronApp, kinguPage)
    await kinguPage.evaluate(
      (worktreeId) => window.__store?.getState().setActiveWorktree(worktreeId),
      worktree.id
    )
    const hostTab = kinguPage.locator(
      `[data-testid="sortable-tab"][data-tab-id="${created.tab.parentTabId}"]`
    )
    await expect(hostTab).toBeVisible({ timeout: 30_000 })
    await hostTab.click()
    const hostPane = await findHostPaneWithMarker(kinguPage, `LIVE:${liveMarker}`)
    expect(hostPane.tabId).toBe(created.tab.parentTabId)
    await kinguPage.evaluate(({ paneId, tabId }) => {
      const manager = window.__paneManagers?.get(tabId)
      manager?.setActivePane?.(paneId, { focus: true })
    }, hostPane)
    await expect
      .poll(() => getTerminalContent(kinguPage), { timeout: 30_000 })
      .toContain(`LIVE:${liveMarker}`)
    const restoredTerminalScreenshot = await kinguPage
      .locator(
    ...
    await restoreHeadedHost(electronApp, kinguPage)
    const restoredHostScreenshot = await kinguPage.screenshot({ fullPage: true })
    expect(
