  ...
  for (const chord of COMMITTING_ENTER_CHORDS) {
    for (const redispatchAfterKeyup of [false, true]) {
      ...
      test(`${chord.name} sends once with ${order}`, async ({
        ...
        const scriptPath = path.join(testRepoPath, `.kingu-korean-ime-harness-${runId}.cjs`)
        ...
        try {
          ...
          await installImeKeyEventLog(kinguPage)
          ...
          await composeHangulSyllable(session, kinguPage)
          await commitSyllableAndSpace(session, kinguPage)
          await composeHangulSyllable(session, kinguPage)
          await commitSyllableAndSpace(session, kinguPage)
          ...
          await chord.assertOutcome(kinguPage)
          await dispatchPlainEnter(session)
          await expect
            .poll(() => readReceived(kinguPage), {
              timeout: 10_000,
              message: 'the next physical Enter must not be consumed by stale IME state'
            })
            .toBe(chord.expectedAfterPlainEnter.received)
          expect(await readSubmitted(kinguPage)).toEqual(chord.expectedAfterPlainEnter.submitted)
          await attachEvidence(kinguPage, testInfo, `korean-${chord.slug}-${order}-commit`)
        } finally {
          await attachEvidence(kinguPage, testInfo, `korean-${chord.slug}-${order}-final`).catch(
            () => undefined
          )
