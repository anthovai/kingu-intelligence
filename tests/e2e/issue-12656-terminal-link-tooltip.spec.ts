...
test.describe('Issue #12656 terminal link tooltip', () => {
  test('clears hover state without permanently shrinking the terminal', async ({
    ...
    const url = `https://example.com/kingu-issue-12656-${randomUUID().slice(0, 8)}`
    await sendToTerminal(
      kinguPage,
      ptyId,
      `printf 'issue-12656-output-%02d\\n' $(seq 1 64); printf '${url}\\n'\r`
    )
    await waitForTerminalOutput(kinguPage, url)

    let probe: LinkProbe | null = null
    await expect
      .poll(
        async () => {
          probe = await locateUrl(kinguPage, url)
          return probe
        },
    ...
    const idle = await readTooltipState(kinguPage, probe.tabId)
    expect(Math.abs(idle.paneBottom - idle.terminalBottom)).toBeLessThanOrEqual(1)
    await expect
      .poll(async () => {
        const currentProbe = await locateUrl(kinguPage, url)
        if (!currentProbe) {
          return { display: 'none', text: '' }
        }
        probe = currentProbe
        await moveToLink(kinguPage, currentProbe)
        return readTooltipState(kinguPage, currentProbe.tabId)
      })
      .toMatchObject({ display: '', text: expect.stringContaining(url) })

    const hovered = await readTooltipState(kinguPage, probe.tabId)
    ...
    await captureProof(kinguPage, testInfo, 'issue-12656-fixed-hover.png')

    await kinguPage.evaluate(() => window.dispatchEvent(new Event('blur')))
    await expect
      .poll(() => readTooltipState(kinguPage, probe.tabId))
      .toMatchObject({ display: 'none', cursor: 'text' })
    const cleared = await readTooltipState(kinguPage, probe.tabId)
    expect(Math.abs(cleared.paneBottom - cleared.terminalBottom)).toBeLessThanOrEqual(1)
    await captureProof(kinguPage, testInfo, 'issue-12656-fixed-after-blur.png')

    await expect.poll(() => getTerminalContent(kinguPage)).toContain(url)
  })
})
