...
test.describe('Existing-user golden core flow', () => {
  test('adds project, creates workspace, opens a terminal tab, and splits a pane', async ({
    ...
    const repoPath = await createGitRepo('kingu-e2e-golden-existing-', 'golden-existing-project')

    await addProjectFromSidebar(kinguPage, electronApp, repoPath)
    const workspaceName = `golden-existing-${Date.now()}`
    await createWorkspace(kinguPage, workspaceName)
    await expectActiveWorkspaceBelongsToRepo(kinguPage, workspaceName, repoPath)
    await ensureTerminalVisible(kinguPage)
    await expectTerminalSurface(kinguPage)
    await waitForTerminalPaneManager(kinguPage)

    await createTerminalTabThroughMenu(kinguPage)
    await splitTerminalPaneAndAssertIdentity(kinguPage)
  })
})
...
test.describe('New-user golden core flow', () => {
  ...
  test('completes onboarding, adds a project, and follows the workspace tour handoff', async ({
    ...
    await selectCodexAgent(kinguPage)
    await continueOnboarding(kinguPage)
    await expect(kinguPage.getByRole('heading', { name: /Make it feel like home/i })).toBeVisible()
    await chooseOppositeTheme(kinguPage)
    await continueOnboarding(kinguPage)
    await continueThroughOptionalSetupToNotifications(kinguPage)
    await expect(kinguPage.getByRole('button', { name: /Send Test Notification/i })).toBeVisible()
    await chooseNotificationSound(kinguPage)
    await continueFromNotificationsToAddProject(kinguPage)

    const repoPath = await createGitRepo('kingu-e2e-golden-new-', 'golden-new-project')
    await chooseFolderInNativeDialog(electronApp, repoPath)
    await kinguPage
      .getByRole('button', { name: /Browse for a folder|Open a folder|Browse folder/i })
      .click()
    await waitForRepoLoaded(kinguPage, repoPath)
    await expectProjectVisible(kinguPage, repoPath)
    await waitForActiveWorktree(kinguPage)
    await ensureTerminalVisible(kinguPage)
    await expectTerminalSurface(kinguPage)
    await waitForTerminalPaneManager(kinguPage)

    await requestAgentSessionsTour(kinguPage)
    const paneCountBeforeTourSplit = await countVisibleTerminalPanes(kinguPage)
    await kinguPage.getByRole('button', { name: /^Split terminal$/ }).click()
    await waitForPaneCount(kinguPage, paneCountBeforeTourSplit + 1)
    await waitForPaneIdentitySnapshot(kinguPage, paneCountBeforeTourSplit + 1)

    await expect(
      kinguPage.getByRole('dialog', { name: /Start another task in parallel/i })
    ).toBeVisible()
    const createControl = kinguPage
    ...
    await completeWorkspaceCreationTour(kinguPage, workspaceName)
    await expectActiveWorkspaceBelongsToRepo(kinguPage, workspaceName, repoPath)
  })
})
