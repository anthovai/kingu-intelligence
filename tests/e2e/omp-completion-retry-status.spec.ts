...
test('OMP completion retry clears the rendered working indicator', async ({
  ...
  const proxy = createServer(async (request, response) => {
    ...
    const forwarded = await fetch(`http://127.0.0.1:${endpoint.port}/hook/omp`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'X-Kingu-Agent-Hook-Token': endpoint.token },
      body
    })
  ...
  try {
    ...
    runInNewContext(code, {
      ...
      process: {
        ...
        env: {
          ...
          KINGU_TAB_ID: paneKey.split(':')[0],
          KINGU_WORKTREE_ID: worktreeId,
          KINGU_AGENT_HOOK_PORT: String(address.port),
          KINGU_AGENT_HOOK_TOKEN: endpoint.token,
          KINGU_AGENT_HOOK_ENV: endpoint.env,
          KINGU_AGENT_HOOK_VERSION: endpoint.version
        }
      },
      fetch,
      AbortController,
    ...
    await kinguPage.screenshot({ path: testInfo.outputPath('before-working.png') })
    ...
    await expect
      ...
      .toBe('done')
    await kinguPage.screenshot({ path: testInfo.outputPath('after-completed.png') })
  } finally {
