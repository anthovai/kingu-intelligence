...
function forgetPersistedClientHostedPages(userDataDir: string): number {
  return listKinguDataFiles(userDataDir).reduce(
...
/**
 * Every kingu-data.json under a user-data dir.
 *
 * The live one is `profiles/<id>/kingu-data.json`; the root file is only the harness's onboarding
 ...
 */
function listKinguDataFiles(userDataDir: string): string[] {
  ...
  try {
    profileFiles = readdirSync(profilesDir, { withFileTypes: true })
      ...
      .map((entry) => path.join(profilesDir, entry.name, 'kingu-data.json'))
  } catch {
    // No profile directory yet; only the harness seed exists.
  }
  return [path.join(userDataDir, 'kingu-data.json'), ...profileFiles].filter((file) => {
    try {
 ...
 * retirement is covered by kingu-runtime-browser-ghost-session-row-close.test.ts, and neither
 * covers the other.
 */
