...
async function createBrowserFixture(
  ...
  const fixtureDir = mkdtempSync(path.join(os.tmpdir(), 'kingu-browser-recovery-'))
...
test('browser chrome recovers a live registered file guest after renderer loss', async ({
  ...
  await expect
    .poll(() => readBrowserGuestState(kinguPage, browserTab.id), { timeout: 10_000 })
    .toMatchObject({
      chromePresent: true,
      marker: 'painted-file-guest',
      url: fixtureUrl
    })
  const before = await readBrowserGuestState(kinguPage, browserTab.id)
  expect(before.webContentsId).not.toBeNull()
  const beforeProcessId = await readGuestProcessId(electronApp, before.webContentsId!)
  await expect
    .poll(
      async () =>
        (await listRegisteredBrowserPages(kinguPage, worktreeId)).result?.tabs?.find(
  ...
  const recovered = await readBrowserGuestState(kinguPage, browserTab.id)
  ...
  await expect
    .poll(() => listRegisteredBrowserPages(kinguPage, worktreeId), { timeout: 10_000 })
    .toMatchObject({
      ok: true,
  ...
  await kinguPage.evaluate((browserPageId) => {
    return window.api.browser.unregisterGuest({ browserPageId })
  }, browserTab.activePageId)
  await expect
    .poll(
      async () =>
        (await listRegisteredBrowserPages(kinguPage, worktreeId)).result?.tabs?.some(
          (tab) => tab.browserPageId === browserTab.activePageId
        ) ?? false
    )
    .toBe(false)
  ...
  const resumeRecovered = await readBrowserGuestState(kinguPage, browserTab.id)
  ...
  const backgroundTab = await kinguPage.evaluate(
    ({ targetWorktreeId }) =>
  ...
  await expect
    .poll(() => readBrowserGuestState(kinguPage, backgroundTab!.id), { timeout: 10_000 })
  ...
  await expect
    .poll(() => readBrowserGuestState(kinguPage, browserTab.id), { timeout: 10_000 })
    .toMatchObject({ chromePresent: true, marker: 'painted-file-guest', url: fixtureUrl })
  const rendererReloaded = await readBrowserGuestState(kinguPage, browserTab.id)
  expect(rendererReloaded.webContentsId).not.toBe(beforeRendererReloadId)

  await kinguPage.evaluate((targetBrowserTabId) => {
    window.__store?.getState().setActiveBrowserTab(targetBrowserTabId)
  }, backgroundTab!.id)
  const hiddenBefore = await readBrowserGuestState(kinguPage, browserTab.id)
...
test('dom-ready ID loss waits for validation without reloading the guest', async ({
  ...
  await expect
    .poll(() => readBrowserGuestState(kinguPage, browserTab.id))
  ...
  await expect
    .poll(() => listRegisteredBrowserPages(kinguPage, worktreeId))
    .toMatchObject({
      ok: true,
      result: { tabs: [{ browserPageId: browserTab.activePageId, url: fixtureUrl }] }
    })
...
test('explicit navigation repairs a recovery error without dom-ready churn', async ({
  ...
  await expect
    .poll(() => readBrowserPageRecoveryState(kinguPage, browserTab.id, browserTab.activePageId))
  ...
  const addressBar = kinguPage.locator(
    `[data-browser-overlay-tab-id="${browserTab.id}"] [data-kingu-browser-address-bar="true"]`
  )
  ...
  await expect(kinguPage.getByText('Recovery fixture error', { exact: true })).toBeVisible()
  expect(
    await electronApp.evaluate(() => {
      const testState = globalThis as typeof globalThis & {
        browserRecoveryValidationCalls?: number
...
test('attachment keeps recovery error until document readiness', async ({
  ...
  await kinguPage
    ...
    .nth(2)
    .click()
  const recoveryErrorCode = await kinguPage.evaluate(
    ({ workspaceId, browserPageId }) =>
...
test('minimized browser guest stays painted and registered after restore @headful', async ({
  ...
  await occludeHeadedWindow(electronApp, kinguPage)
  await revealHeadedWindow(electronApp, kinguPage)

  await expect
    .poll(() => readBrowserGuestState(kinguPage, browserTab.id))
    .toMatchObject({ chromePresent: true, marker: 'painted-file-guest', url: fixtureUrl })
  const restored = await readBrowserGuestState(kinguPage, browserTab.id)
