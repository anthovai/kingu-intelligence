...
test.describe('PTY input write queue over SSH', () => {
  test.skip(!RUN_DOCKER_SSH, 'Set KINGU_E2E_SSH_DOCKER=1 to run Docker-backed SSH E2E.')
  ...
  test('returns an xterm OSC query reply through the live SSH PTY', async ({
    ...
    try {
      ...
      await execInTerminal(kinguPage, ptyId, `node -e ${shellQuote(remoteOscQueryScript(runId))}`)
      await waitForTerminalOutput(kinguPage, `REMOTE_OSC_READY_${runId}`, 30_000, 80_000)
      await waitForTerminalOutput(kinguPage, `REMOTE_OSC_REPLY_${runId}`, 30_000, 80_000)
    } finally {
  ...
  test('keeps fish query replies out of the next child stdin on an upstream relay pty', async ({
    ...
    try {
      ...
      const runId = String(Date.now())
      const home = `/tmp/kingu-fish-${runId}`
      const prompt = `KINGU_SSH_FISH_${runId}> `
      ...
      const relayExports = execDockerSshRelayTargetCommand(
        target,
        'module=$(find /root/.kingu-remote -type d -path \'*/node_modules/node-pty\' | head -n 1); node -e "const p=require(process.argv[1]); console.log(Object.keys(p.native || {}).join(\',\'))" "$module"'
      )
      ...
      await waitForTerminalOutput(kinguPage, prompt, 30_000, 80_000)

      const blocker = `node -e ${shellQuote(`console.log('BLOCKER_STARTED_${runId}'); setTimeout(() => process.exit(0), 5000)`)}`
      await execInTerminal(kinguPage, ptyId, blocker)
      await waitForTerminalOutput(kinguPage, `BLOCKER_STARTED_${runId}`, 30_000, 80_000)
      await execInTerminal(kinguPage, ptyId, `node ${shellQuote(childScript)}`)
      await waitForTerminalOutput(kinguPage, childReady, 30_000, 80_000)
      await sendToTerminal(kinguPage, ptyId, 'hello\r')
      await waitForTerminalOutput(kinguPage, `${childRead}:"hello\\n"`, 30_000, 80_000)
