...
test('seeded project terminal runs a typed shell command @golden', async ({ kinguPage }) => {
  ...
  expect(await getTerminalContent(kinguPage)).not.toMatch(CSI_REPLY_RE)

  const marker = `kingu-e2e-alive-${Date.now()}`
  ...
  if (process.platform === 'win32') {
    ...
    if (shell === 'cmd') {
      const pwshAvailable = await kinguPage.evaluate(() => window.api.pwsh.isAvailable())
      expect(pwshAvailable, 'cmd.exe must not replace an available PowerShell default').toBe(false)
    }
    const begin = 'KINGU_E2E_PATH_BEGIN'
    const end = 'KINGU_E2E_PATH_END'
    const pathCommand =
      shell === 'pwsh' || shell === 'powershell'
        ...
        : shell === 'cmd'
          ...
          : `printf '${begin}\\n%s\\n${end}\\n' "$LOCALAPPDATA"`
    await focusActiveTerminalInput(kinguPage)
    await kinguPage.keyboard.type(pathCommand)
    await kinguPage.keyboard.press('Enter')
    let expandedPath = ''
    await expect
      .poll(async () => {
        ...
        const lines = stripAnsiEscapeSequences(await getTerminalContent(kinguPage, 8_000))
          .split(/\r?\n/)
          .map((line) => line.trim())
  ...
  const finalBuffer = await getTerminalContent(kinguPage, 8_000)
