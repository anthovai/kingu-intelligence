...
test.describe('Terminal 2-Set Korean preedit visibility', () => {
  test('shows every assembling jamo at non-zero size and commits the syllable ahead of the newline', async ({
    ...
    try {
      ...
      await expectPreeditHidden(kinguPage, 'before composing')

      for (const frame of HAN_FRAMES) {
        await composeHangulSyllable(arena.session, kinguPage, [frame])
        await expectPreeditRendered(kinguPage, frame.preedit, `composing ${frame.preedit}`)
      }

      await commitImeText(arena.session, '한')
      await expectPreeditHidden(kinguPage, 'after committing 한')

      await dispatchPlainEnter(arena.session)

      const received = await waitForTerminalImeBytes(kinguPage, reader)
  ...
  test('keeps a preedit the IME resumes without a compositionstart visible', async ({
    ...
    try {
      ...
      await dispatchResumedCompositionUpdate(kinguPage, '한')

      const sample = await samplePreeditOverlay(kinguPage)
  ...
  test('renders the preedit at every update of a recorded Windows/WSL Hangul session', async ({
    ...
    await applyImePlatformPolicy(kinguPage, 'windows')
    const arena = await openTerminalImePaneArena(kinguPage)
    let completed = false
    try {
      const replay = await replayRecordedImeDomTrace(kinguPage, RECORDED_TRACE)
  ...
  test('loses and duplicates nothing when back-to-back syllables commit at full speed', async ({
    ...
    try {
      ...
      for (let repetition = 0; repetition < 4; repetition += 1) {
        await composeHangulSyllable(arena.session, kinguPage, HAN_FRAMES, 0)
        await commitImeText(arena.session, '한')
        await composeHangulSyllable(arena.session, kinguPage, GEUL_FRAMES, 0)
