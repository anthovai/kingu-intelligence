...
test('keeps a collapsed group visible when an idle card below it grows', async ({
  ...
  const root = realpathSync(mkdtempSync(path.join(os.tmpdir(), 'kingu-sidebar-measured-row-')))
  ...
  const fixture = await kinguPage.evaluate(async (repoPaths): Promise<SidebarFixture> => {
  ...
  const sidebar = kinguPage.locator('[data-worktree-sidebar]')
  ...
  const before = await settledGeometry(kinguPage, fixture)
  ...
  await expect
    .poll(async () => (await readGeometry(kinguPage, fixture)).cardHeight)
    .toBeCloseTo(before.cardHeight + 50, 0)
  const after = await settledGeometry(kinguPage, fixture)
