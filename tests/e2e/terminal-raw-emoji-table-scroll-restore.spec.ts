...
test.describe('Terminal raw emoji table scroll restore repro', () => {
  ...
  test('uses WebGL by default for ordinary terminal output when available @terminal-rendering-golden', async ({
    ...
    const marker = `KINGU_AUTO_WEBGL_SMOKE_${randomUUID()}`

    await sendToTerminal(kinguPage, ptyId, `printf ${JSON.stringify(`${marker}\\n`)}\r`)
    await waitForTerminalOutput(kinguPage, marker, 10_000)

    const expectedWebgl = await expectAutoWebgl(kinguPage)
    ...
    let diagnostics = await readTerminalRenderDiagnostics(kinguPage)
    await expect
      .poll(
        async () => {
          diagnostics = await readTerminalRenderDiagnostics(kinguPage)
  ...
  test('keeps raw emoji box table aligned after restore and scroll @terminal-rendering-golden', async ({
    ...
    await ensureTerminalVisible(kinguPage)
    await waitForActiveTerminalManager(kinguPage, 30_000)
    await setWideRenderedTableViewport(kinguPage)
    await waitForActiveTerminalColumns(kinguPage, RAW_EMOJI_BOX_TABLE_WIDTH)
    const ptyId = await waitForActivePanePtyId(kinguPage)
    const runId = randomUUID()
    const scriptPath = path.join(testRepoPath, `.kingu-raw-emoji-fixture-table-${runId}.mjs`)
    ...
    try {
      ...
      await waitForTerminalOutput(kinguPage, completionMarker, 20_000, 30_000)
      await expect
      ...
      await scrollActiveTerminalToText(kinguPage, 'Singer')
      await closeFeatureTips(kinguPage)
      const expectedWebgl = await expectAutoWebgl(kinguPage)
      ...
      const singerGeometry = await readVisibleSingerRowGeometry(kinguPage)
