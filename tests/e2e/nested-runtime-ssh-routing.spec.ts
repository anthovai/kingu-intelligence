...
import { expect, test as base } from './helpers/kingu-app'
import {
...
const isDockerNestedRuntimeRun =
  process.env.KINGU_E2E_NESTED_RUNTIME_SSH === '1' && process.env.KINGU_E2E_WEB_CLIENT === '1'
...
const test = base.extend<{ proxyJumpFixture: NestedRuntimeProxyJumpFixture | null }>({
  ...
  kinguAppExtraEnv: async ({ proxyJumpFixture }, provideFixture) => {
    await provideFixture(
      proxyJumpFixture ? { KINGU_SYSTEM_SSH_PATH: proxyJumpFixture.wrapperPath } : {}
    )
  }
})
...
test.skip(
  ...
  'Run with KINGU_E2E_NESTED_RUNTIME_SSH=1 and KINGU_E2E_WEB_CLIENT=1'
)
...
test('routes HUB desktop, web, and two paired desktops through HUB-owned SSH', async ({
  ...
  try {
    ...
    const remote = await connectDockerSshRelayTarget(kinguPage, sshTarget)
    await installProxyJumpFixture(proxyJumpFixture, proxyJumpDestination, proxyJumpHost)
    const proxyJumpRemote = await connectDockerSshRelayTarget(kinguPage, proxyJumpDestination, {
      viaProxyJump: true
    })
    const localRepoId = await kinguPage.evaluate(() => {
    ...
    const webOffer = await createRuntimeDesktopPairingOffer(kinguPage)
    ...
    const offerA = await createRuntimeDesktopPairingOffer(kinguPage)
    ...
    const offerB = await createRuntimeDesktopPairingOffer(kinguPage)
    ...
    await disconnectDockerSshRelayTarget(kinguPage, remote.targetId)
    await assertRuntimeSshStatus(clientA, remote.targetId, 'disconnected')
    await reconnectDisconnectedDockerSshRelayTarget(kinguPage, remote.targetId)
    ...
    const rePairOffer = await createRuntimeDesktopPairingOffer(kinguPage)
