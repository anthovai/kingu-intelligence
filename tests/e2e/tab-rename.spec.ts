...
test.describe('Tab Rename (Inline)', () => {
  ...
  test('double-clicking a tab opens an inline rename input and Enter commits', async ({
    ...
    const originalTitle = await getActiveTabTitle(kinguPage, worktreeId)
    expect(originalTitle.length).toBeGreaterThan(0)

    const tabLocator = tabLocatorByTitle(kinguPage, originalTitle)
    await tabLocator.dblclick()

    const renameInput = kinguPage.getByRole('textbox', {
    ...
    await expect
      .poll(async () => getActiveCustomTitle(kinguPage, worktreeId), { timeout: 3_000 })
      .toBe('My Custom Title')
    await expect(renameInput).toBeHidden()
    await expect(tabLocatorByTitle(kinguPage, 'My Custom Title')).toBeVisible()
  })
  ...
  test('context-menu Change Title opens a focused select-all rename input', async ({
    ...
    await tabLocatorByTitle(kinguPage, originalTitle).click({ button: 'right' })
    await kinguPage.getByRole('menuitem', { name: /^Change Title(?:\s|$)/ }).click()
    ...
    await expect(tabLocatorByTitle(kinguPage, 'Context Menu Title')).toBeVisible()
  })

  test('Escape during inline rename discards the edit', async ({ kinguPage }) => {
    const worktreeId = (await getActiveWorktreeId(kinguPage))!
    const originalTitle = await getActiveTabTitle(kinguPage, worktreeId)
    ...
    await expect(tabLocatorByTitle(kinguPage, originalTitle)).toBeVisible()
    await expect
      .poll(async () => getActiveCustomTitle(kinguPage, worktreeId), { timeout: 3_000 })
      .toBe(null)
  })

  test('renaming to an empty string resets the tab to its default title', async ({ kinguPage }) => {
    const worktreeId = (await getActiveWorktreeId(kinguPage))!
    ...
    const defaultTitle = await getActiveTabTitle(kinguPage, worktreeId)
    ...
    const tabLocator = tabLocatorByTitle(kinguPage, 'Seeded Custom')
    await tabLocator.dblclick()
    ...
    await expect(tabLocatorByTitle(kinguPage, defaultTitle)).toBeVisible()
    await expect
      .poll(async () => getActiveCustomTitle(kinguPage, worktreeId), { timeout: 3_000 })
      .toBe(null)
  })

  test('clicking away (blur) commits the rename', async ({ kinguPage }) => {
    const worktreeId = (await getActiveWorktreeId(kinguPage))!
    ...
    await expect
      .poll(async () => (await getWorktreeTabs(kinguPage, worktreeId)).length, { timeout: 3_000 })
      .toBeGreaterThanOrEqual(2)

    const tabs = await getWorktreeTabs(kinguPage, worktreeId)
    const activeId = await getActiveTabId(kinguPage)
    const activeTab = tabs.find((t) => t.id === activeId)!
    const otherTab = tabs.find((t) => t.id !== activeId)!

    const tabLocator = tabLocatorByTitle(kinguPage, activeTab.title!)
    await tabLocator.dblclick()
    ...
    await tabLocatorByTitle(kinguPage, otherTab.title!).click()

    await expect(renameInput).toBeHidden()
    await expect(tabLocatorByTitle(kinguPage, 'Committed By Blur')).toBeVisible()
    expect(
      await kinguPage.evaluate(
  ...
  test('terminal title updates do not resize neighboring tabs', async ({ kinguPage }) => {
    const worktreeId = (await getActiveWorktreeId(kinguPage))!
    const tabIds = await kinguPage.evaluate((targetWorktreeId) => {
    ...
    const tabs = tabIds.map((id) =>
      kinguPage.locator(`[data-testid="sortable-tab"][data-tab-id="${id}"]`)
    )
  ...
  test('rename input stays at a usable width when many tabs are open', async ({ kinguPage }) => {
    const worktreeId = (await getActiveWorktreeId(kinguPage))!
    const targetTabId = await getActiveTabId(kinguPage)
    ...
    const tabLocator = tabLocatorByTitle(kinguPage, targetTitle)
  ...
  test('middle-clicking inside the rename input does not close the tab', async ({ kinguPage }) => {
    const worktreeId = (await getActiveWorktreeId(kinguPage))!
    const tabsBefore = (await getWorktreeTabs(kinguPage, worktreeId)).length
    const originalTitle = await getActiveTabTitle(kinguPage, worktreeId)
    ...
    await expect(renameInput).toBeVisible()
    await expect(tabLocatorByTitle(kinguPage, originalTitle)).toBeVisible()
    expect((await getWorktreeTabs(kinguPage, worktreeId)).length).toBe(tabsBefore)
  })
})
