...
test.describe('Terminal long table scroll restore repro', () => {
  test('reproduces long markdown table artifacts after workspace switch and scroll', async ({
    ...
    await kinguPage.evaluate(() => {
      window.__store
        ?.getState()
        .markFeatureTipsSeen(['kingu-cli', 'cmd-j-palette', 'voice-dictation'])
    })
    ...
    const scriptPath = path.join(testRepoPath, `.kingu-long-table-${runId}.mjs`)
    ...
    try {
      ...
      await scrollActiveTerminalLikeUser(kinguPage)
      await closeFeatureTips(kinguPage)
      const diagnostics = await readTerminalRenderDiagnostics(kinguPage)
  ...
  test('keeps narrow wrapped signer markdown table coherent after restore and scroll', async ({
    ...
    await setRenderedTableViewport(kinguPage)
    await forceDarkTerminalRendererPath(kinguPage)
    await ensureTerminalVisible(kinguPage)
    await waitForActiveTerminalManager(kinguPage, 30_000)
    const ptyId = await waitForActivePanePtyId(kinguPage)
    await waitForPtyShellEcho(kinguPage, ptyId, 15_000)
    ...
    const scriptPath = path.join(testRepoPath, `.kingu-narrow-signer-table-${runId}.mjs`)
    ...
    try {
      ...
      const content = await getTerminalContent(kinguPage, 30_000)
  ...
  test('keeps real emoji markdown table right edge clean after restore and scroll', async ({
    ...
    await ensureTerminalVisible(kinguPage)
    await waitForActiveTerminalManager(kinguPage, 30_000)
    await setNarrowTerminalViewport(kinguPage)
    const renderedTableTerminalCols = await waitForRenderedTerminalColumnsAtMost(
      kinguPage,
      NARROW_TERMINAL_MAX_COLS
    )
    const ptyId = await waitForActivePanePtyId(kinguPage)
    await waitForPtyColumnsAtMost(kinguPage, ptyId, renderedTableTerminalCols)
    const runId = randomUUID()
    const marker = `EMOJI_FIXTURE_TABLE_RESTORE_${runId}`
    const scriptPath = path.join(testRepoPath, `.kingu-emoji-fixture-table-${runId}.mjs`)
    ...
    try {
      ...
      await waitForActiveTerminalManager(kinguPage, 30_000)
      await setNarrowTerminalViewport(kinguPage)
      await waitForRenderedTerminalColumnsAtMost(kinguPage, NARROW_TERMINAL_MAX_COLS)
      await expect
        .poll(() => getTerminalContent(kinguPage, 30_000), {
          timeout: 10_000,
        ...
        .toContain(marker)
      const generatedWidthContent = await getTerminalContent(kinguPage, 30_000)
      ...
      await scrollActiveTerminalToText(kinguPage, retainedEmojiCell)
      await closeFeatureTips(kinguPage)
      await expect
        .poll(() => readActiveTerminalVisibleText(kinguPage), {
          timeout: 5_000,
          message: `${retainedEmojiCell} row fragment should be visible before screenshot`
        })
        .toContain(retainedEmojiCell)
      const diagnostics = await readTerminalRenderDiagnostics(kinguPage)
      const overpaint = await readTerminalRightEdgeOverpaint(kinguPage)
      const wrapDiagnostics = await readTerminalBoxTableWrapDiagnostics(kinguPage)
