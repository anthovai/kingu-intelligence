...
test.describe('Terminal typing latency vs scrollback history size', () => {
  test('typing stays responsive as terminal history grows', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.kingu-history-benchmark-${runId}.mjs`)
    ...
    try {
      ...
      commandSent = true
      await waitForRecentTerminalMarker(kinguPage, `HIST_READY_${runId}`, 10_000)
      await focusActiveTerminalInput(kinguPage)

      const phases: PhaseLatency[] = []
      let seq = 0

      const baseline = await measureTypingLatency(kinguPage, runId, 'empty history', seq)
