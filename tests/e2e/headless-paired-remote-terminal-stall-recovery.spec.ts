...
const scratch = mkdtempSync(path.join(os.tmpdir(), 'kingu-headless-stalled-stream-'))
...
test('recovers an ACK-starved stream from an isolated headless Kingu host @headful', async ({
  testRepoPath
}) => {
