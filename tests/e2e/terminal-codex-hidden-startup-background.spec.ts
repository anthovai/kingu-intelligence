...
function codexLikeStartupCommand(marker: string): string {
  ...
  return stageNodeScriptForTerminal(script, { prefix: 'kingu-codex-startup-bg' }).command
}
  ...
  test('restores the input background when a Codex worktree first becomes visible', async ({
    ...
    await expect
      .poll(
        async () => {
          try {
            const nextTarget = await readCodexStartupBackgroundTarget(kinguPage, marker)
            target = nextTarget
    ...
    const visibleBackgroundPixels = await countVisibleBackgroundPixels(kinguPage, target)
