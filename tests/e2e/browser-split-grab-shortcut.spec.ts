...
async function openNativeChatTranscript(
  ...
  const transcriptPath = path.join(
    mkdtempSync(path.join(os.tmpdir(), 'kingu-e2e-split-grab-')),
    `${sessionId}.jsonl`
  )
...
test.describe('browser split grab shortcut', () => {
  ...
  test.beforeEach(async ({ electronApp, kinguPage }) => {
  ...
  test('copy chord from one split toolbar arms grab only in that split', async ({ kinguPage }) => {
    ...
    await reloadButton(kinguPage, fixture.firstBrowserTabId).focus()
    await waitForFocusedGroup(kinguPage, fixture.firstBrowserGroupId)
    await kinguPage.keyboard.press(copyChord)

    await expect(grabBanner(kinguPage, fixture.firstBrowserTabId)).toBeVisible()
    await expect(grabBanner(kinguPage, fixture.secondBrowserTabId)).toBeHidden()
  })
  ...
  test('copy chord with native chat text selected copies instead of arming grab', async ({
    ...
    await openNativeChatTranscript(kinguPage, {
    ...
    const replyText = kinguPage.locator('[data-native-chat-window]').getByText(reply)
    await replyText.click({ clickCount: 3 })
    await expect
      .poll(() => kinguPage.evaluate(() => window.getSelection()?.toString()))
      .toContain(reply)
    await kinguPage.keyboard.press(copyChord)

    await expect.poll(() => readClipboard(electronApp)).toContain(reply)
    await expect(grabBanner(kinguPage, fixture.browserTabId)).toBeHidden()

    // Why: with the browser split focused, only the live-selection check can tell this is a copy.
    await writeClipboard(electronApp, 'clipboard before copy')
    await reloadButton(kinguPage, fixture.browserTabId).focus()
    await waitForFocusedGroup(kinguPage, fixture.browserGroupId)
    expect(await kinguPage.evaluate(() => window.getSelection()?.toString())).toContain(reply)
    await kinguPage.keyboard.press(copyChord)
    await expect.poll(() => readClipboard(electronApp)).toContain(reply)
    await expect(grabBanner(kinguPage, fixture.browserTabId)).toBeHidden()

    await expectToolbarCopyChordArmsGrab(kinguPage, fixture.browserTabId)
  })
})
