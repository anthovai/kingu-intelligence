...
test.describe('Worktree Lifecycle', () => {
  ...
  test('removing a worktree clears its tabs, open files, and browser tabs', async ({
    ...
    createdWorktreeId = await createIsolatedWorktree(kinguPage)
    const newWorktreeId = createdWorktreeId
    await switchToWorktree(kinguPage, newWorktreeId)
    await expect
    ...
    await kinguPage.evaluate((worktreeId) => {
      const store = window.__store
      if (!store) {
        return
      }
    ...
    await clickFileInExplorer(kinguPage, ['README.md', 'package.json'])

    // Baseline: the new worktree now has tabs/browser tabs/open files.
    expect((await getWorktreeTabs(kinguPage, newWorktreeId)).length).toBeGreaterThan(0)
    expect((await getBrowserTabs(kinguPage, newWorktreeId)).length).toBeGreaterThan(0)
    expect((await getOpenFiles(kinguPage, newWorktreeId)).length).toBeGreaterThan(0)
    ...
    // is protecting.
    await switchToWorktree(kinguPage, originalWorktreeId)
    await expect
      .poll(async () => getActiveWorktreeId(kinguPage), { timeout: 10_000 })
      .toBe(originalWorktreeId)

    const result = await removeWorktreeViaStore(kinguPage, newWorktreeId)
    ...
    await expect
      .poll(async () => (await getWorktreeTabs(kinguPage, newWorktreeId)).length, {
        timeout: 10_000,
        message: 'tabsByWorktree still holds entries for the removed worktree'
      })
      .toBe(0)
    await expect
      .poll(async () => (await getBrowserTabs(kinguPage, newWorktreeId)).length, { timeout: 5_000 })
      .toBe(0)
    await expect
      .poll(async () => (await getOpenFiles(kinguPage, newWorktreeId)).length, { timeout: 5_000 })
      .toBe(0)

    const allIds = await getAllWorktreeIds(kinguPage)
  ...
  test('switching worktrees preserves per-worktree state across a round-trip', async ({
    ...
    const originalState = await kinguPage.evaluate((wId) => {
      const store = window.__store
      if (!store) {
    ...
    const afterRoundTrip = await kinguPage.evaluate((wId) => {
      const store = window.__store
      if (!store) {
   ...
   */
  test('terminal tabs stay scoped to the worktree that created them', async ({ kinguPage }) => {
    const allIds = await getAllWorktreeIds(kinguPage)
    expect(
      allIds.length,
      'fixture should provide primary + e2e-secondary worktrees'
    ...
    const worktreeA = await waitForActiveWorktree(kinguPage)
    ...
    await expect
      .poll(async () => (await getWorktreeTabs(kinguPage, worktreeA)).length, { timeout: 5_000 })
      .toBeGreaterThanOrEqual(2)

    // Switch to B and create a tab there too.
    await switchToWorktree(kinguPage, worktreeB)
    await expect
      .poll(async () => getActiveWorktreeId(kinguPage), { timeout: 10_000 })
      .toBe(worktreeB)
    await ensureTerminalVisible(kinguPage)
    ...
    await expect
      .poll(async () => (await getWorktreeTabs(kinguPage, worktreeB)).length, { timeout: 5_000 })
      .toBeGreaterThanOrEqual(2)

    const tabsA = await getWorktreeTabs(kinguPage, worktreeA)
    const tabsB = await getWorktreeTabs(kinguPage, worktreeB)
