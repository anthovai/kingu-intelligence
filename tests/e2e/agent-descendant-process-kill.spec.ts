...
test('killing an agent PTY terminates its detached-pgid descendants', async ({ kinguPage }) => {
  test.skip(process.platform === 'win32', 'descendant tree-kill is POSIX-only for now')

  const stage = mkdtempSync(join(tmpdir(), 'kingu-agent-descendant-'))
  ...
  try {
    ...
    const ptyId = await kinguPage.evaluate(
    ...
    await kinguPage.evaluate((id) => window.api.pty.kill(id), ptyId)
