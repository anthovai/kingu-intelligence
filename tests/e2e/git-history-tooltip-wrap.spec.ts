...
test('keeps conventional commit-message lines intact in the history tooltip', async ({
  ...
  await openSourceControlForWorktree(kinguPage, testRepoPath, fixture.worktreePath)

  const commitsToggle = kinguPage.getByRole('button', { name: /Commits/ })
  await expect(commitsToggle).toBeVisible()
  await commitsToggle.click()

  const row = kinguPage.getByTestId('git-history-row').filter({ hasText: subject })
