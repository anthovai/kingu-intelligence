...
const RUN_REPRO = process.env.KINGU_E2E_CODEX_SKILL_PREVIEW_REPRO === '1'
const EXPECT_NO_ARTIFACTS = process.env.KINGU_E2E_EXPECT_NO_CODEX_SKILL_PREVIEW_ARTIFACTS === '1'
const FULLSCREEN_MIN_SIZE = { width: 1200, height: 760 }
const MIN_REPRO_DIFF_RATIO = 0.006
const MAX_CLEAN_DIFF_RATIO = 0.0015
const KINGU_REPO_PATH = realpathSync(process.cwd())
...
const CODEX_SKILL_PREVIEW_RE =
  /Press enter to insert|esc to close|electron|kingu-cli|kingu-emulator/i
const SETUP_PANE_ACTIVITY_RE = /install-kingu-skills|pnpm|Progress:|Packages:|Lockfile/i
...
async function addRealKinguRepo(page: Page, repoPath: string): Promise<string> {
  return page.evaluate(async (repoPath) => {
    ...
    if (!repo) {
      throw new Error(`Real Kingu repo did not load: ${repoPath}`)
    }
    ...
    if (!worktree) {
      throw new Error(`Real Kingu worktree did not load: ${repoPath}`)
    }
...
async function createWorkspaceThroughComposer(page: Page, workspaceName: string): Promise<string> {
  ...
  await expect
    .poll(
      ...
      {
        timeout: 60_000,
        message: `Workspace ${workspaceName} did not appear in the real Kingu repo`
      }
    )
    .not.toBeNull()
  ...
  await expect
    .poll(() => getActiveWorktreeId(page), {
      timeout: 30_000,
      message: 'Created real Kingu workspace did not become active'
    })
    .toBe(createdId)
...
test.describe('Codex skill preview terminal artifact repro @headful', () => {
  ...
  test.skip(!RUN_REPRO, 'Set KINGU_E2E_CODEX_SKILL_PREVIEW_REPRO=1 to run this repro.')
  ...
  test('captures the real Kingu repo setup-split Codex skill preview overpaint before any click', async ({
    electronApp,
    kinguPage
  }, testInfo) => {
    ...
    await setStableFullscreenWindow(electronApp, kinguPage)
    await waitForSessionReady(kinguPage)
    await addRealKinguRepo(kinguPage, KINGU_REPO_PATH)
    ...
    const worktreeId = await createWorkspaceThroughComposer(kinguPage, workspaceName)
    createdWorktreeIds.push(worktreeId)

    await ensureTerminalVisible(kinguPage, 30_000)
    await waitForActiveTerminalManager(kinguPage, 30_000)
    await waitForPaneCount(kinguPage, 2, 60_000)
    await waitForPaneIdentitySnapshot(kinguPage, 2)
    const webglActive = await forceTerminalWebgl(kinguPage)
    test.skip(!webglActive, 'Codex skill preview artifact repro needs WebGL rendering')

    const leftPane = await focusLeftTerminalPane(kinguPage)
    const rightPane = await getRightTerminalPane(kinguPage)
    ...
    await dismissCodexPromptsIfPresent(kinguPage, leftPane)
    await waitForPaneContent(kinguPage, leftPane.tabId, leftPane.ptyId, CODEX_READY_RE, 60_000)
    await waitForPaneContent(
      kinguPage,
      leftPane.tabId,
      leftPane.ptyId,
    ...
    await kinguPage.waitForTimeout(CODEX_READY_SETTLE_MS)
    await waitForPaneVisibleContentChanges(kinguPage, rightPane, 1, 8_000)

    await focusLeftTerminalPane(kinguPage)
    await kinguPage.keyboard.type('test $e', { delay: 70 })
    await waitForPaneContent(
      kinguPage,
      leftPane.tabId,
      leftPane.ptyId,
    ...
    const evidence = await captureClickEvidence(kinguPage, leftPane, testInfo)
