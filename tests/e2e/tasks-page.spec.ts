...
async function openInstrumentedGitHubTasksPage(
  ...
  await page.evaluate(() => {
    ...
    const existingIssue = {
      ...
      state: 'open' as const,
      url: 'https://github.com/kingu/e2e/issues/999',
      labels: [],
    ...
    store.setState({
      repos: state.repos.map((candidate) =>
        candidate.id === repo.id
          ? { ...candidate, upstream: { owner: 'kingu', repo: 'e2e' } }
          : candidate
      ),
      settings: {
        ...state.settings,
...
test.describe('Tasks page', () => {
  ...
  test('opening the tasks view renders the tasks UI', async ({ kinguPage }) => {
    await openTasksPage(kinguPage)

    await expect
      .poll(async () => getStoreState<string>(kinguPage, 'activeView'), { timeout: 5_000 })
      .toBe('tasks')

    await expect(kinguPage.getByRole('button', { name: 'Close tasks' })).toBeVisible({
      timeout: 10_000
    })
    ...
    await expect
      .poll(
        async () => {
          renderedSources = await getRenderedTaskSources(kinguPage)
    ...
    if (renderedSources.some((source) => source.source === 'github' && source.active)) {
      await expect(kinguPage.getByRole('button', { name: 'Issues', exact: true })).toBeVisible()
      await expect(kinguPage.getByRole('button', { name: 'PRs', exact: true })).toBeVisible()
      await expect(kinguPage.getByRole('button', { name: 'Projects', exact: true })).toBeVisible()
      await expect(kinguPage.getByPlaceholder(/Search GitHub (issues|PRs)/i)).toBeVisible()
    }
  })

  test('closing the tasks page returns to the previous view', async ({ kinguPage }) => {
    const previousView = await getStoreState<string>(kinguPage, 'activeView')
    ...
    await kinguPage.getByRole('button', { name: 'Close tasks' }).click()
    ...
    await expect(kinguPage.getByRole('button', { name: 'Close tasks' })).toHaveCount(0)
    if (previousView === 'terminal') {
      await expect(kinguPage.locator('.xterm').first()).toBeVisible({ timeout: 5_000 })
    }
  })

  test('reopening restores the GitHub page and scroll position', async ({ kinguPage }) => {
    await openMockedPaginatedGitHubTasks(kinguPage)

    await kinguPage.getByRole('button', { name: 'Page 28', exact: true }).click()
    await expect(kinguPage.getByText('Issue page 28 item 1', { exact: true })).toBeVisible()

    const list = kinguPage.locator('[data-task-list-scroll="github"]')
    ...
    const clampedRowsStyle = await kinguPage.addStyleTag({
      content:
        '[data-task-list-scroll="github"] > .divide-y { max-height: 0 !important; overflow: hidden !important; }'
    })
    await openTasksPage(kinguPage)

    await expect(kinguPage.getByRole('button', { name: 'Page 28', exact: true })).toHaveAttribute(
      'aria-current',
      'page'
    )
    const restoredList = kinguPage.locator('[data-task-list-scroll="github"]')
    ...
    await kinguPage.getByText('Issue page 28 item 12', { exact: true }).click()
    await expect(restoredList).toHaveCount(0)
    await expect
      .poll(async () => {
        const position = await getStoreState<{ scrollTop: number }>(kinguPage, 'taskListPosition')
        return position.scrollTop
      })
      .toBeGreaterThan(300)
    await kinguPage.getByRole('button', { name: 'GitHub list', exact: true }).click()
    await expect(kinguPage.getByText('Issue page 28 item 1', { exact: true })).toBeVisible()
    await expect
      .poll(() => restoredList.evaluate((element) => element.scrollTop))
      .toBeGreaterThan(300)
    ...
    await kinguPage.getByRole('button', { name: 'Close tasks' }).click()
    const pendingRestoreStyle = await kinguPage.addStyleTag({
      content:
        '[data-task-list-scroll="github"] > .divide-y { max-height: 0 !important; overflow: hidden !important; }'
    })
    await openTasksPage(kinguPage)
    ...
    await kinguPage.getByRole('button', { name: 'Page 1', exact: true }).click()
    await pendingRestoreStyle.evaluate((element) => element.remove())
    await expect(kinguPage.getByRole('button', { name: 'Page 1', exact: true })).toHaveAttribute(
      'aria-current',
      'page'
    )
    await expect
    ...
    const permanentlyClampedRowsStyle = await kinguPage.addStyleTag({
      content:
        '[data-task-list-scroll="github"] > .divide-y { max-height: 0 !important; overflow: hidden !important; }'
    })
    await openTasksPage(kinguPage)
    ...
    await kinguPage.waitForTimeout(5_500)
    await kinguPage.getByRole('button', { name: 'Close tasks' }).click()
    await expect
      .poll(async () => {
        const position = await getStoreState<{ scrollTop: number }>(kinguPage, 'taskListPosition')
  ...
  test('GitHub search waits for idle, keeps rows visible, and Enter does not double-fetch', async ({
    ...
    await openInstrumentedGitHubTasksPage(kinguPage)

    const input = kinguPage.getByPlaceholder('Search GitHub issues...')
    const existingIssue = kinguPage.getByText('Existing GitHub issue', { exact: true })
    await expect(input).toBeVisible()
    await expect(existingIssue).toBeVisible()

    await input.fill('')
    await expect
      .poll(async () => readTaskSearchRequestProbe(kinguPage), {
        timeout: TASK_SEARCH_PROBE_TIMEOUT_MS
      })
      .toEqual({ countQueries: ['is:issue is:open'], fetchQueries: ['is:issue is:open'] })
    await resetTaskSearchRequestProbe(kinguPage)
    ...
    await kinguPage.waitForTimeout(TASK_SEARCH_SETTLE_MS)
    expect(await readTaskSearchRequestProbe(kinguPage)).toEqual({
