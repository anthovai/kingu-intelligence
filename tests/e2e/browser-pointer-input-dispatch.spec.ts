...
test('dispatches coordinate pointer input fast enough for real gestures', async ({ kinguPage }) => {
  ...
  try {
    ...
    const pageId = await createBrowserTab(kinguPage, worktreeId!, server.url)

    await expect
      .poll(() => evaluateInPage(kinguPage, pageId, 'document.title'), { timeout: 20_000 })
    ...
    const latency = await kinguPage.evaluate(async (targetPage) => {
    ...
    await evaluateInPage(kinguPage, pageId, 'window.__events = []; true')
    const clickPairStarted = Date.now()
    await driveRpcSequence(kinguPage, pageId, [...clickSteps(120, 60), ...clickSteps(120, 60)])
    ...
    await evaluateInPage(kinguPage, pageId, 'window.__events = []; true')
    await driveRpcSequence(kinguPage, pageId, [
    ...
    await expect
      .poll(() =>
        evaluateInPage(kinguPage, pageId, 'window.__events.some((e) => e.type === "contextmenu")')
      )
      .toBe('true')

    // ── Drag selection ──

    await evaluateInPage(kinguPage, pageId, 'window.getSelection().removeAllRanges(); true')
    await driveRpcSequence(kinguPage, pageId, [
    ...
    await expect
      .poll(() => evaluateInPage(kinguPage, pageId, 'String(window.getSelection())'))
      .toContain('alpha')
    ...
    await expect
      .poll(
        async () =>
          Number(
            await evaluateInPage(kinguPage, pageId, 'document.querySelector("#scroller").scrollTop')
          ),
        { timeout: 10_000 }
      )
      .toBeGreaterThan(0)
