 ...
 *
 * Topology: headed Kingu desktop host (remote server) + a separate paired Kingu
...
const scratch = mkdtempSync(path.join(os.tmpdir(), 'kingu-host-park-viewer-'))
...
test.use({ kinguAppExtraEnv: { KINGU_E2E_TERMINAL_PARKING_DELAY_MS: String(PARK_DELAY_MS) } })
...
test('a cold-parked host pane keeps serving its paired remote viewer', async ({
  ...
  try {
    ...
    await expect
      .poll(() => kinguPage.evaluate((id) => window.__paneManagers?.has(id) ?? false, hostTabId), {
        timeout: 60_000,
    ...
    await waitForTabParked(kinguPage, hostTabId, { parkDelayMs: PARK_DELAY_MS })
    ...
    while (Date.now() < timelineDeadline) {
      const hostMounted = await kinguPage.evaluate(
