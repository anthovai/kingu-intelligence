...
test.skip(process.env.KINGU_TYPING_BENCH !== '1', 'Opt-in benchmark calibration')
...
test('typing measurement charges known renderer stalls to the planned schedule', async ({
  ...
  for (const injectStall of [false, true]) {
    ...
    await sendToTerminal(kinguPage, ptyId, `node ${JSON.stringify(script)}\r`)
    await waitForTerminalOutputForPtyId(kinguPage, ptyId, typingProbeReadyMarker(runId), 15_000)
    const control = await kinguPage.evaluateHandle((enabled) => {
      let keys = 0
      let stalls = 0
    ...
    try {
      const measurement = await measurePacedTyping(kinguPage, runId, sidecar, {
        keyCount: 32,
        keyCadenceMs: 25
      })
      ...
      await sendToTerminal(kinguPage, ptyId, '\x03')
