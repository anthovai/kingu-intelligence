...
test('name filter narrows to the matching file', async ({ kinguPage }) => {
  ...
  const input = kinguPage.getByPlaceholder('Find files')
  await expect(input).toBeVisible({ timeout: 10_000 })

  await input.fill('package.')
  await expect(rowByName(explorer, kinguPage, 'package.json').first()).toBeVisible({
    timeout: 10_000
  })
...
test('name filter shows the empty message only for a true no-match', async ({ kinguPage }) => {
  await waitForSessionReady(kinguPage)
  await waitForActiveWorktree(kinguPage)
  await openFileExplorer(kinguPage)
  ...
  await input.fill('')
  await expect(rowByName(explorer, kinguPage, 'README.md').first()).toBeVisible({
    timeout: 10_000
  })
})
