...
import { KinguRuntimeService } from '../../src/main/runtime/kingu-runtime'
...
function internals(runtime: KinguRuntimeService): RuntimeInternals {
...
function createHostProvider() {
  ...
  let runtime: KinguRuntimeService | null = null
  const controller = {
  ...
  return {
    ...
    writes,
    session,
    bind(target: KinguRuntimeService) {
      runtime = target
    },
...
function startMultiplex(runtime: KinguRuntimeService, connectionId: string) {
