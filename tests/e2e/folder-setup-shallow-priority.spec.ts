...
async function createShallowPriorityTruncationFixture(): Promise<{
  ...
  const parentPath = realpathSync(
    await mkdtemp(path.join(os.tmpdir(), 'kingu-e2e-shallow-priority-'))
  )
...
async function createCancellableScanFixture(): Promise<{
  ...
  const parentPath = realpathSync(
    await mkdtemp(path.join(os.tmpdir(), 'kingu-e2e-cancellable-scan-'))
  )
  tempRoots.push(parentPath)
...
test('prioritizes shallow sibling repositories in a bounded nested scan', async ({
  ...
  const dialog = kinguPage.getByRole('dialog', { name: /Add a project/i })
  await expect(dialog).toBeVisible()
  await dialog.getByRole('button', { name: /Browse folder/i }).click()

  const importDialog = kinguPage.getByRole('dialog', {
  ...
  await expect(kinguPage.getByText(fixture.groupName)).toBeVisible()
  await expect(kinguPage.getByText('z-web-client').first()).toBeVisible()
})
