...
const FORCE_SYSTEM_SSH = process.env.KINGU_SSH_FORCE_SYSTEM_TRANSPORT === '1'
...
test.describe('Docker SSH port-forward lifecycle', () => {
  ...
  test('keeps a user-forwarded listener live across scan refresh @headful', async ({
    ...
    const marker = `KINGU_FORWARD_${Date.now()}`
    ...
    try {
      ...
      await openPortsPanel(kinguPage)
      ...
      await expect
        ...
        .toBe(remotePid)
      await expect(kinguPage.getByText(`:${REMOTE_PORT}`, { exact: true })).toBeVisible()
      await expect
        .poll(
          () =>
            kinguPage.evaluate(
        ...
        .toBe(true)
      await expect(kinguPage.getByText(`:${REFRESH_BARRIER_PORT}`, { exact: true })).toBeVisible()
      ...
      await installRendererForwardCapture(kinguPage)
      await openPortsPanel(kinguPage)
      await localPortReservation.release()
      await forwardPortFromPanel(kinguPage, localPort, REMOTE_PORT)
      await expect(kinguPage.getByText('Forwarded', { exact: true })).toBeVisible()
      await expect(
        kinguPage.getByText(`:${localPort} → :${REMOTE_PORT}`, { exact: true })
      ).toBeVisible()
      await expect.poll(() => requestForward(localPort)).toContain(marker)
      await expectForwardEvidence(kinguPage, remote.targetId, [
      ...
      const postHydrationRoundTripForwards = await kinguPage.evaluate(
      ...
      await expect(kinguPage.getByText(`:${REFRESH_BARRIER_PORT}`, { exact: true })).toBeVisible()
      const staleSnapshotEvidence = await readPortForwardEvidence(kinguPage, remote.targetId)
      ...
      await expect
        ...
        .toBe(true)
      await expect(kinguPage.getByText(`:${SCAN_REFRESH_PORT}`, { exact: true })).toBeVisible()

      await unrelatedLocalPortReservation.release()
      const unrelatedForward = await addPortForward(kinguPage, {
      ...
      await forceDockerSshRelayChannelReconnect(kinguPage, target, remote.targetId)
      await expectForwardEvidence(kinguPage, remote.targetId, [
        { localPort, remotePort: REMOTE_PORT },
        { localPort: unrelatedLocalPort, remotePort: REFRESH_BARRIER_PORT }
      ])
      ...
      const authorityBeforeTransportReconnect = await kinguPage.evaluate(
      ...
      await expect
        .poll(
          async () => {
            const state = await kinguPage.evaluate(
              (targetId) => window.__store?.getState().sshConnectionStates.get(targetId),
              remote.targetId
            )
            return (
      ...
      await expect(
        kinguPage.getByText(`:${unrelatedLocalPort} → :${REFRESH_BARRIER_PORT}`, { exact: true })
      ).toBeVisible()
      ...
      try {
        const collisionResult = await kinguPage.evaluate(
      ...
      const evidence = await readPortForwardEvidence(kinguPage, remote.targetId)
      ...
      const relayReconnectStates = await readSshStateCapture(kinguPage)
