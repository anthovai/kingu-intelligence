/**
 * Regression: a worktree created via the CLI (`kingu worktree
...
test.describe('worktree visibility with a remote runtime active', () => {
  test('a CLI-created worktree appears in the sidebar while a remote runtime is active', async ({
    ...
    const repoId = await kinguPage.evaluate(() => {
    ...
    // dir — exactly what `kingu worktree create` does from a terminal.
    ...
    const worktreeRow = (worktreeId: string) =>
      kinguPage.locator(`[data-worktree-id=${JSON.stringify(worktreeId)}]`).first()
