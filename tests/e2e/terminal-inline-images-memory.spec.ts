...
test.use({ kinguAppExtraArgs: ['--enable-precise-memory-info'] })
...
test('twelve image terminals release decoder and image storage across reset and close cycles', async ({
  ...
  const baselineTabId = (await waitForPaneIdentitySnapshot(kinguPage, 1)).tabId
  ...
  try {
    for (let cycle = 0; cycle < CYCLES; cycle++) {
      ...
      for (let index = 0; index < TAB_COUNT; index++) {
        const id = await kinguPage.evaluate((worktree) => {
        ...
        await activateTab(kinguPage, id)
        const identity = await waitForPaneIdentitySnapshot(kinguPage, 1)
        ...
        tabs.push({ id, ptyId })
        await expect.poll(() => readInlineImageState(kinguPage), { timeout: 30_000 }).not.toBeNull()
        const marker = `${cycle}_${index}`
        await execInTerminal(kinguPage, ptyId, nodeTerminalCommand([producerPath, marker]))
        await waitForTerminalOutput(kinguPage, `RETENTION_DONE_${marker}`, 30_000)
        await expect.poll(async () => (await readInlineImageState(kinguPage))?.pending).toBe(2)
        if (index === TAB_COUNT - 1) {
          await assertInlineImagePixels(kinguPage, testInfo.outputPath(`cycle-${cycle}-images.png`))
        }
      }
      ...
      for (const tab of tabs) {
        await activateTab(kinguPage, tab.id)
        ...
        await waitForTerminalOutput(kinguPage, 'RESET_DONE', 30_000)
        await expect
          .poll(async () => {
            const [resource] = await readInlineImageResources(kinguPage, [tab.id])
            return {
      ...
      await activateTab(kinguPage, baselineTabId)
      for (const tab of tabs) {
        await kinguPage.evaluate((id) => window.__store!.getState().closeTab(id), tab.id)
      }
      await expect
        .poll(
          async () =>
