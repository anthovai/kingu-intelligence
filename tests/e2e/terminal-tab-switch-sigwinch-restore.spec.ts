...
function buildSigwinchResetProbeCommand(): string {
  ...
  return stageNodeScriptForTerminal(script, { prefix: 'kingu-sigwinch-probe' }).command
}
...
test.describe('Terminal tab switch SIGWINCH restore', () => {
  test('keeps an alternate-screen Codex viewport after hidden snapshot replay', async ({
    ...
    await sendToTerminal(kinguPage, paneIdentity.ptyId, 'ARM_SIGWINCH_PROBE\n')
    await expect
      .poll(() => getTerminalContent(kinguPage, 8_000), {
        timeout: 10_000,
    ...
    const armedContentAfterSettle = await getTerminalContent(kinguPage, 8_000)
    ...
    await expect
      .poll(
        async () => {
          const content = await getTerminalContent(kinguPage, 8_000)
      ...
      .toBe('snapshot')
    await kinguPage.waitForTimeout(1_200)
    const contentAfterSettle = await getTerminalContent(kinguPage, 8_000)
