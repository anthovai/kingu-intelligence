...
test.describe('Docker SSH relay perf', () => {
  test.skip(!RUN_DOCKER_SSH, 'Set KINGU_E2E_SSH_DOCKER=1 to run Docker-backed SSH relay perf.')
  ...
  test('keeps remote typing responsive while the Linux relay streams TUI output', async ({
    ...
    try {
      ...
      await execInTerminal(kinguPage, ptyId, `node -e ${shellQuote(remoteTypingLoadScript(runId))}`)
      await waitForTerminalOutput(kinguPage, `REMOTE_TUI_READY_${runId}`, 30_000, 80_000)
      const measurement = await measureRemoteTyping(kinguPage, ptyId, runId)
      ...
      await stopRemoteLoad(kinguPage, ptyId)
    } finally {
  ...
  test('keeps active remote typing responsive while a background SSH PTY stream is ACK-stalled', async ({
    ...
    try {
      ...
      await ensureTerminalVisible(kinguPage, 45_000)
      await waitForActiveTerminalManager(kinguPage, 60_000)
      backgroundPtyId = await waitForActivePanePtyId(kinguPage, 60_000)
      ...
      await waitForTerminalOutput(kinguPage, `REMOTE_ACK_FLOOD_READY_${runId}`, 30_000, 80_000)
      await holdSshPtyAckGate(kinguPage, [backgroundPtyId])
      await kinguPage.evaluate((ptyId) => window.api.pty.write(ptyId, 'g'), backgroundPtyId)

      await splitActiveTerminalPane(kinguPage, 'vertical')
      await focusLastTerminalPane(kinguPage)
      activePtyId = await waitForActivePanePtyId(kinguPage, 60_000)
      ...
      await waitForTerminalOutput(kinguPage, `REMOTE_TUI_READY_${activeRunId}`, 30_000, 80_000)
      const heldAckPressure = expect.poll(
        async () => (await readSshPtyAckGate(kinguPage))?.heldAckChars ?? 0,
        {
          timeout: 30_000,
      ...
      const measurement = await measureRemoteTyping(kinguPage, activePtyId, activeRunId)
      const ackGate = await readSshPtyAckGate(kinguPage)
      const summary = `median=${measurement.medianLatencyMs.toFixed(
        1
      ...
      await releaseSshPtyAckGate(kinguPage)
      const releasedAckGate = await readSshPtyAckGate(kinguPage)
      expect(releasedAckGate?.heldAckChars ?? 0).toBe(0)
    } finally {
      await releaseSshPtyAckGate(kinguPage).catch(() => undefined)
      if (activePtyId) {
        await stopRemoteLoad(kinguPage, activePtyId).catch(() => undefined)
      }
      if (backgroundPtyId) {
        await stopRemoteLoad(kinguPage, backgroundPtyId).catch(() => undefined)
      }
      cleanupDockerSshRelayTarget(target)
    }
  })
  ...
  test('keeps remote typing responsive while relay file streams and git churn are active', async ({
    ...
    try {
      ...
      const loadFile = `/tmp/kingu-relay-load-${runId}.png`
      ...
      await waitForTerminalOutput(kinguPage, `LOAD_FILES_READY_${runId}`, 60_000, 80_000)
      ...
      const measurement = await measureRemoteTyping(kinguPage, ptyId, runId)
      const load = await kinguPage.evaluate(() => {
        const state = (
  ...
  test('keeps an SSH workspace terminal usable after disconnect and reconnect', async ({
    ...
    try {
      ...
      await ensureTerminalVisible(kinguPage, 45_000)
      await waitForActiveTerminalManager(kinguPage, 60_000)
      const beforePtyId = await waitForActivePanePtyId(kinguPage, 60_000)
      ...
      await execInTerminal(kinguPage, beforePtyId, beforeCommand)
      await waitForTerminalOutput(kinguPage, beforeMarker, 20_000, 60_000)
      ...
      await execInTerminal(kinguPage, beforePtyId, recoveryCommand)
      await waitForTerminalOutput(kinguPage, recoveryStartedMarker, 30_000, 80_000)

      await reconnectDockerSshRelayTarget(kinguPage, remote.targetId)
      await ensureTerminalVisible(kinguPage, 45_000)
      await waitForActiveTerminalManager(kinguPage, 60_000)
      const afterPtyId = await waitForActivePanePtyId(kinguPage, 60_000)
      await waitForTerminalOutput(kinguPage, recoveryMarker, 30_000, 80_000)
      ...
      await execInTerminal(kinguPage, afterPtyId, afterCommand)
      await waitForTerminalOutput(kinguPage, afterMarker, 20_000, 60_000)
