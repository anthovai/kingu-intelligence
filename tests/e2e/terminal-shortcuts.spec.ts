...
async function dispatchCtrlCToActiveTerminalTextarea(
  ...
  return page.evaluate((dispatchOptions) => {
    ...
    // synthetic DOM events still exercise Kingu's installed xterm boundary.
...
test.describe('Terminal Shortcuts', () => {
  ...
  test('Shift+Enter follows the pane Kitty keyboard state', async ({ kinguPage, electronApp }) => {
    await installMainProcessPtyWriteSpy(electronApp)
    const ptyId = await waitForActivePanePtyId(kinguPage)

    await pressAndExpectWrite(kinguPage, electronApp, 'Shift+Enter', '\x1b\r')
    ...
    await execInTerminal(kinguPage, ptyId, "printf '\\033[>1u'")
    await expect.poll(() => getKittyKeyboardFlags(kinguPage)).toBe(1)
    await pressAndExpectWrite(kinguPage, electronApp, 'Shift+Enter', '\x1b[13;2u')
    ...
    await sendToTerminal(kinguPage, ptyId, '\x15\x03')
    await execInTerminal(kinguPage, ptyId, "printf '\\033[=0u'")
    await expect.poll(() => getKittyKeyboardFlags(kinguPage)).toBe(0)
    await pressAndExpectWrite(kinguPage, electronApp, 'Shift+Enter', '\x1b\r')
  })
  ...
  test('Droid gets CSI-u Shift+Enter on Windows without changing Antigravity', async ({
    ...
    const paneKey = await setActivePaneForegroundAgent(kinguPage, 'droid')
    try {
      await pressAndExpectWrite(kinguPage, electronApp, 'Shift+Enter', '\x1b[13;2u', 2)
      await setActivePaneForegroundAgent(kinguPage, 'antigravity')
      await pressAndExpectWrite(kinguPage, electronApp, 'Shift+Enter', '\x1b\r')
    } finally {
  ...
  test('Windows forwards genuine Ctrl+Alt text chords to the PTY', async ({
    ...
    await pressAndExpectWrite(kinguPage, electronApp, 'Control+Alt+u', '\x1b\x15')
    await pressAndExpectWrite(kinguPage, electronApp, 'Control+Alt+2', '\x1b2')
    await pressAndExpectWrite(kinguPage, electronApp, 'Control+Alt+;', '\x1b;')
  })
  ...
  test('Ctrl+Enter protects local ConPTY shells without breaking trusted TUI chords', async ({
    ...
    if (process.platform === 'win32') {
      await pressAndExpectWrite(kinguPage, electronApp, 'Control+Enter', '\r')
      const paneKey = await setActivePaneForegroundAgent(kinguPage, 'droid')
      try {
        // Droid queries CSI-u without activating live flags; trusted process evidence preserves cue/queue.
        await pressAndExpectWrite(kinguPage, electronApp, 'Control+Enter', '\x1b[13;5u')
      } finally {
        await kinguPage.evaluate(
          (key) => window.__store?.getState().clearPaneForegroundAgent(key),
          paneKey
  ...
  test('plain Ctrl+C sends ETX under kitty keyboard reporting', async ({
    ...
    await installMainProcessPtyWriteSpy(electronApp)
    await waitForActivePanePtyId(kinguPage)
    await enableKittyKeyboardReporting(kinguPage, 31)
    await clearPtyWriteLog(electronApp)
    await focusActiveTerminalInput(kinguPage)
    await kinguPage.keyboard.down('Control')
    await kinguPage.keyboard.up('Control')
    expect((await getPtyWrites(electronApp)).join('')).toBe('')
    await clearPtyWriteLog(electronApp)

    expect(await dispatchCtrlCToActiveTerminalTextarea(kinguPage, { keyupCtrlKey: false })).toEqual(
      {
        keydownDefaultPrevented: false,
    ...
    await expect
      .poll(async () => await getKittyKeyboardFlags(kinguPage), {
        timeout: 5_000,
    ...
    await clearPtyWriteLog(electronApp)
    await focusActiveTerminalInput(kinguPage)
    await kinguPage.keyboard.type('x')
    await expect
  ...
  test('@headful Codex-like background output stays visible without disabling WebGL in auto mode', async ({
    ...
    const hasPane = await kinguPage.evaluate(() => {
      const state = window.__store?.getState()
      const worktreeId = state?.activeWorktreeId
      const tabId =
        state?.activeTabType === 'terminal'
    ...
    const webglActive = await kinguPage
      .waitForFunction(
        () => {
          const state = window.__store?.getState()
          const worktreeId = state?.activeWorktreeId
    ...
    await execInTerminal(kinguPage, ptyId, `printf '\\033[48;2;52;52;52m  ${marker}  \\033[0m\\n'`)
    await waitForTerminalOutput(kinguPage, marker)

    await expect
      .poll(
        () =>
          kinguPage.evaluate((expectedMarker) => {
            const state = window.__store?.getState()
            const worktreeId = state?.activeWorktreeId
            const tabId =
              state?.activeTabType === 'terminal'
  ...
  test('floating terminal owns tab switch shortcuts while focused', async ({ kinguPage }) => {
    const scenario = await seedFloatingTerminalTabSwitchScenario(kinguPage)
    await kinguPage.evaluate(async () => {
      ...
      if (!document.querySelector('[data-floating-terminal-panel][aria-hidden="false"]')) {
        window.dispatchEvent(new CustomEvent('kingu-toggle-floating-terminal'))
      }
    })
    await expect(
      kinguPage.locator('[data-floating-terminal-panel][aria-hidden="false"]')
    ).toBeVisible()
    await focusFloatingTerminal(kinguPage)

    await kinguPage.keyboard.press(`${mod}+Shift+BracketRight`)
    await expect
      .poll(() => getActiveFloatingTerminalTabId(kinguPage), {
        timeout: 5_000,
        message: 'floating terminal did not switch to the next tab'
      })
      .toBe(scenario.floatingSecondTabId)
    await expect
      .poll(() => getActiveBackgroundTerminalTabId(kinguPage), {
        timeout: 1_000,
    ...
    await focusFloatingTerminal(kinguPage)
    await kinguPage.keyboard.press(`${mod}+Shift+BracketLeft`)
    await expect
      .poll(() => getActiveFloatingTerminalTabId(kinguPage), {
        timeout: 5_000,
    ...
    await expect(getActiveBackgroundTerminalTabId(kinguPage)).resolves.toBe(
  ...
  test('all terminal chords reach the PTY or fire their action', async ({
    ...
    await pressAndExpectWrite(kinguPage, electronApp, 'Alt+ArrowLeft', '\x1bb')
    await pressAndExpectWrite(kinguPage, electronApp, 'Alt+ArrowRight', '\x1bf')
    ...
    if (!isMac && process.platform !== 'win32') {
      await pressAndExpectWrite(kinguPage, electronApp, 'Control+ArrowLeft', '\x1bb')
      await pressAndExpectWrite(kinguPage, electronApp, 'Control+ArrowRight', '\x1bf')
    }

    // Alt+Backspace → Esc+DEL (readline backward-kill-word).
    await pressAndExpectWrite(kinguPage, electronApp, 'Alt+Backspace', '\x1b\x7f')

    // Ctrl+Backspace → \x17 (unix-word-rubout).
    await pressAndExpectWrite(kinguPage, electronApp, 'Control+Backspace', '\x17')
    ...
    if (isMac) {
      ...
      await pressAndExpectWrite(kinguPage, electronApp, 'Meta+ArrowLeft', '\x01')
      await pressAndExpectWrite(kinguPage, electronApp, 'Meta+ArrowRight', '\x05')

      // Cmd+Backspace → Ctrl+U (kill line). Cmd+Delete → Ctrl+K (kill to EOL).
      await pressAndExpectWrite(kinguPage, electronApp, 'Meta+Backspace', '\x15')
      await pressAndExpectWrite(kinguPage, electronApp, 'Meta+Delete', '\x0b')
    }
    ...
    await focusActiveTerminalInput(kinguPage)
    await kinguPage.keyboard.press(`${mod}+k`)
    await expect
    ...
    const panesBeforeSplit = await countVisibleTerminalPanes(kinguPage)
    await focusActiveTerminalInput(kinguPage)
    await kinguPage.keyboard.press(splitVerticalChord)
    await waitForPaneCount(kinguPage, panesBeforeSplit + 1)
    ...
    await focusActiveTerminalInput(kinguPage)
    await kinguPage.keyboard.press(`${mod}+BracketRight`)
    await focusActiveTerminalInput(kinguPage)
    await kinguPage.keyboard.press(`${mod}+BracketLeft`)
    expect(await countVisibleTerminalPanes(kinguPage)).toBe(panesBeforeSplit + 1)
    ...
    await focusActiveTerminalInput(kinguPage)
    await kinguPage.keyboard.press(`${mod}+Shift+Enter`)
    await expect
    ...
    await closeActivePaneAndSettle(kinguPage, panesBeforeSplit)

    // Split horizontally (chord varies by platform — see splitHorizontalChord).
    const panesBeforeHSplit = await countVisibleTerminalPanes(kinguPage)
    await focusActiveTerminalInput(kinguPage)
    await kinguPage.keyboard.press(splitHorizontalChord)
    await waitForPaneCount(kinguPage, panesBeforeHSplit + 1)
    await waitForActivePanePtyId(kinguPage)
    await closeActivePaneAndSettle(kinguPage, panesBeforeHSplit)

    // Cmd/Ctrl+F toggles the search overlay.
    await focusActiveTerminalInput(kinguPage)
    await kinguPage.keyboard.press(`${mod}+f`)
    const searchInput = kinguPage.locator('[data-terminal-search-root] input').first()
    ...
    await expect(kinguPage.locator('[data-terminal-search-root]').first()).toBeHidden({
      timeout: 3_000
    })
  })
  ...
  test('Cmd+Up/Down scrolls terminal viewport without writing to the PTY on macOS', async ({
    ...
    await execInTerminal(kinguPage, ptyId, `for i in {1..120}; do echo ${marker}_$i; done`)
    await waitForTerminalOutput(kinguPage, `${marker}_120`)

    await expect
      .poll(
        async () => {
          const viewport = await getActiveTerminalViewport(kinguPage)
    ...
    await clearPtyWriteLog(electronApp)
    await focusActiveTerminalInput(kinguPage)
    await kinguPage.keyboard.press('Meta+ArrowUp')
    await expect
      .poll(async () => getActiveTerminalViewport(kinguPage), {
        timeout: 5_000,
    ...
    await focusActiveTerminalInput(kinguPage)
    await kinguPage.keyboard.press('Meta+ArrowDown')
    await expect
      .poll(
        async () => {
          const viewport = await getActiveTerminalViewport(kinguPage)
  ...
  test('Shift with Russian layout text reaches the PTY as Cyrillic under kitty keyboard reporting', async ({
    ...
    const dispatch = await pressShiftedRussianLayoutKey(kinguPage)
