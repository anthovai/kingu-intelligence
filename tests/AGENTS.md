...
whenever `KINGU_E2E_HEADLESS=1`, `KINGU_E2E_HEADFUL=1`, or `KINGU_BACKGROUND_LAUNCH=1`:
...
- Launch through `tests/e2e/helpers/kingu-app.ts` (or `kingu-restart.ts`) — they already set the env.
- A raw `electron.launch()` outside those helpers must pass `KINGU_BACKGROUND_LAUNCH: '1'`.
...
- Native-focus tests belong on an isolated display or CI. Do not set `KINGU_E2E_FOREGROUND=1`
...
- `KINGU_E2E_TERMINAL_PERF_XVFB=1`, set inside `xvfb-run` by that workflow;
...
Keep `KINGU_BACKGROUND_LAUNCH=1`: the application must still suppress automatic reveals and focus.
